...
> is the orchestrator's job, per D6). **Migrated from plan-optimization 2026-07-22** (successor epic;
> plan id kept).
>
...
**Disjointness:** phase-1-init / refine surface — disjoint from `manage-architecture` (PLAN-43),
finalize steps (PLAN-44), the CI/await seam (PLAN-42), and plan-optimization's running
`_markers_search.py` (PLAN-23) / `manage-execution-manifest.py` (PLAN-35). Emittable now.

## D5 — the interim `spec_body` inline must be verbatim-by-construction

`orchestrate.md` (68-77, 122, 128) requires the emit to inline the spec body **verbatim** beneath the
pointer *until this plan lands* (the `spec_body` field of the `emitted[]` contract). Corroborated
2026-07-22 by a cross-epic incident: an orchestrator **condensed** the spec into a blockquote despite
the 3× "verbatim" contract — the exact named-deliverable-loss the pointer form exists to prevent.
Make the mitigation drift-proof: `spec_body` is produced by a **deterministic file read** (cat the
spec), never by the orchestrator retyping/paraphrasing. When D2 lands (lifecycle ingests the file),
`orchestrate.md` retires the inline (its own contract says `spec_body` "empties once the lifecycle
ingests a referenced spec's contents"). Confirm the emit step reads-not-renders, and that the
retirement path is clean.
...
- The `spec_body`-verbatim angle (D5) touches `orchestrate.md`, which PLAN-49 renames — PLAN-41
  ships well before the drain-gated PLAN-49, so no collision; just re-ground line numbers at outline.

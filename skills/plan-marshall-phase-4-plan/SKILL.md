---
name: plan-marshall-phase-4-plan
...
When a task step target lives under a skill test directory (any path matching `test/**/`) and represents a test helper (shared fixtures, sys.path shims, or other non-test Python module), the filename MUST NOT be `conftest.py`. Rename the target to `_fixtures.py` (or another descriptive `_*.py` name that is clearly not a pytest collection file) during task creation — before composing the JSON array passed to `manage-tasks batch-add`. The allow-list holds a single entry — only the root `test/conftest.py` is permitted; any additional `conftest.py` under `test/{bundle}/{skill}/` changes pytest's global collection semantics for that bundle and causes hidden coupling or spurious collection failures.

If a deliverable's `Affected files` list names any `conftest.py` other than that single allowed root file, phase-4-plan MUST rewrite the target to `_fixtures.py` (preserving the parent directory) before persisting the step. Cross-reference: phase-3-outline owns the outline-time rule and rationale in [outline-workflow-detail.md §10d "Test Helper File Naming"](../phase-3-outline/standards/outline-workflow-detail.md#10d-test-helper-file-naming); this subsection enforces the same constraint at task-creation time so that any late-surviving `conftest.py` target is corrected before tasks reach phase-5-execute.
...
- `affected_files_count` — `manage-references get --field affected_files`, count entries. The key is **derived** from the outline's structured per-deliverable declarations by `manage-references sync-affected-files`, which reads all three declaration headings — so a survey-scope deliverable's `Files expected to mutate:` paths ARE carried. The read below is ordered immediately after a refresh of that derivation, so this figure is the declared surface as of now rather than the phase-3 snapshot. Step 8b § B2 still defines the cardinality its own predicate needs and says how to derive it; do not assume the two are the same number.
...
Refresh the declared footprint BEFORE reading it. Scope moves between phase-3-outline and here — a deliverable gains a path, an outline iteration adds one — and a faithful read of a stale value cannot detect its own staleness: re-reading `affected_files` returns exactly what was written, so nothing at compose time can tell the current declared surface from the phase-3 snapshot. Only a re-derivation can, and it MUST be ordered before the read that produces `affected_files_count`, or the manifest is composed from the snapshot:
...
The `affected_files` write is a set union, so the refresh only ever adds to the key read below — that key is never narrowed, and an outline that did not move yields `added_count: 0`. Scope it to `affected_files` deliberately: `sync-affected-files` also writes the sibling `read_intent_files` key, whose STORED half the persisted mutation-wins subtraction can narrow (reported as `stored_read_reclassified_count`), so a claim about the verb's writes in general would be false. Nothing here reads that key. Branch on **any** non-`success` status, not on a locally-remembered subset: every error status this verb can return means nothing was written, so log the failure and proceed with the unrefreshed value rather than aborting the compose, since the union write cannot have damaged it. The error set is not restated here — its single source of truth is [`../manage-references/SKILL.md`](../manage-references/SKILL.md) § "Error Responses" (the rows tagged `sync-affected-files`), the same source the sibling call site in [`plan-marshall/workflow/q-gate-validation.md`](../plan-marshall/workflow/q-gate-validation.md) § Step 7 branches against, so the two cannot drift into disagreeing about which errors exist. A hand-mirrored enumeration goes stale the moment the verb gains a code, and the status it then fails to recognise is exactly the one nobody is watching for. See `manage-references` Canonical invocations → `sync-affected-files`.
...
**Marshal-authoritative candidate source**: the `--phase-5-steps` / `--phase-6-steps` CSVs on `compose` are fallback-only. With a readable `marshal.json` the composer reads its candidate lists authoritatively from `plan.phase-5-execute.verification_steps` / `plan.phase-6-finalize.steps` and ignores the CSVs entirely — the fallback exists solely for callers without a `marshal.json` (tests / no-marshal contexts). Passing the CSVs here keeps the invocation deterministic but cannot inject a plan-scoped candidate in an inited project; their live consumer is the `validate` call below.
...
**Validate allow-list contract (separate surface)**: on `validate` the `--phase-5-steps` / `--phase-6-steps` flags are NOT a candidate source — the CSV is `validate`'s own caller-supplied allow-list of permitted step IDs (prefix-agnostic on `default:`; `project:` / `bundle:skill` prefixes compare verbatim). Every manifest step ID must appear in the allow-list or validation fails with `invalid_manifest`. Because compose's `execution_tier` routing can append `verify:{canonical}` step IDs that were never in the configured candidate list (orchestrator-tier task-verification verbs routed to phase-5 steps), the original `{p5_csv}` — read from `marshal.json` **before** compose ran — does NOT cover those routed IDs, and passing it verbatim produces a spurious `invalid_manifest` failure whenever routing appended a verb. Read back the composed manifest's `phase_5.verification_steps` (which already contains any routed `verify:{verb}` IDs) and merge it into the allow-list **before** the `validate` call:
...
Parse `phase_5.verification_steps` from the returned manifest and union it with `{p5_csv}` to produce the expanded phase-5 allow-list `{p5_allow}` (de-duped; the union preserves the configured candidates while admitting the compose-routed verbs). Then validate against the expanded allow-list:

```bash
python3 .plan/execute-script.py plan-marshall:manage-execution-manifest:manage-execution-manifest \
  validate \
  --plan-id {plan_id} \
  --phase-5-steps "{p5_allow}" \
  --phase-6-steps "{p6_csv}"
```
...
**Activation guard**: Runs after every successful phase-4-plan invocation, regardless of `plan_source`, EXCEPT when the operator's `plan.phase-4-plan.q_gate_validation` knob resolves to `off` (B1 below) or when the surgical-scope bypass predicate (B2) below fires. Both validators apply to every plan (lesson-derived, issue-derived, recipe-derived, free-form). The phase sets `qgate_validation_required: true` in its return TOON on every successful completion where neither B1 nor B2 suppresses it; the orchestrator's existing `max_iterations` budget gates re-entry on its side. Skipping the signal is reserved for the unrecoverable error path (the phase has aborted with `status: error` before reaching the return-results step), the B1 `q_gate_validation == off` opt-out, AND the B2 surgical-scope bypass below.

**B1 — `q_gate_validation` knob consult (deterministic)**: Before evaluating B2, read the operator's planning-time Q-Gate knob:
...
**B2 — Surgical q-gate-validation bypass (deterministic)**: When `q_gate_validation != off`, before signalling `qgate_validation_required: true`, FIRST derive `{N}` with the command block below, THEN evaluate the surgical-scope predicate against it. A stated guard requires an implementing mechanism: the predicate MUST NOT be evaluated against any count this block did not produce.

**Derivation (deterministic)** — read the deliverables' own structured records:
...
Parse `deliverables[]` from the returned TOON. Each record carries three declared-path fields — `affected_files`, `survey_scope`, `mutation_scope` — and every entry of all three is a record whose `path` key holds the declared path. Union the `path` of every entry of all three fields across ALL records into ONE set, deduplicate it, and bind `{N}` to its cardinality:

> `N = |{ entry.path : d ∈ deliverables, f ∈ {affected_files, survey_scope, mutation_scope}, entry ∈ d[f] }|`

A deliverable that declares none of the three contributes nothing — the union is over whatever each record actually declares, so a survey-scope deliverable contributes through `survey_scope` / `mutation_scope` exactly as a flat-field deliverable contributes through `affected_files`. The bound `{N}` is the `affected_files_count` the predicate reads and the `{N}` both bypass log lines below emit; no other count may be substituted for either use.

Then evaluate the predicate:

> `scope_estimate == surgical AND affected_files_count <= 2`

where `affected_files_count` is `{N}` as derived above — the cardinality of the deduplicated union of every deliverable's **declared file surface** — `Affected files:`, plus the `Files to survey:` / `Files expected to mutate:` pair a survey-scope deliverable declares INSTEAD of it. ⚠ **Never substitute the Step 7b input `manage-references get --field affected_files`** — that key is the plan's **expected-modification** surface, partitioned by declared intent, so a `Files to survey:` path is recorded outside it. B2 needs the widest declared surface, read-intent paths included; substituting the narrower key would silently re-narrow the count this derivation exists to widen. ⛔ **Reading `Affected files:` alone would make the bypass fire MORE readily on exactly the deliverables whose scope is least knowable**: a survey-scope deliverable contributes zero to a flat-field count, so a plan built entirely from them would satisfy `<= 2` no matter how wide its declared scope. That is the self-reinforcing direction named above, reached through the other declaration form. When the predicate holds, override `qgate_validation_required` to `false` in the Step 10 return TOON instead of the unconditional `true` — the orchestrator's q-gate-validation dispatch site reads the flag and skips the validator envelope when it is `false`. When the predicate is false (non-surgical, or 3+ affected files), the unconditional `true` value documented above is preserved.

...
**Tree state:** `61a43e5` on `claude/code-intelligence-substrate-analysis-kah884`; re-derived under adversarial review at `9ae90b4` (no change to any audited surface between the two)
...
- **Verdict:** CONFIRMED as specified, with two completeness gaps:
  1. **The ordinary write path manufactures the contradictory state the report dismissed as "nonsensical input" — proven by execution, not by reading.** Two supported CLI verbs in sequence, against the repository's own `setup_test_project` fixture:
     1. `enrich_skills_by_profile('module-a', {'module_testing': {'defaults': [], 'optionals': [], 'minimal': True}})` → `status: success`, **`warnings: None`**. The validator accepts the declaration; this is the exact state `architecture-persistence.md:455-459` documents as "Declared minimal", and the state `test_cmd_enrich.py:469` pins as correct.
     2. `enrich_add_domain('module-a', 'general-dev')` → persisted `module_testing` block is
        `{'defaults': [4 skill entries incl. plan-marshall:persona-module-tester], 'minimal': True, 'optionals': []}`.
     Measured `CONTRADICTION_REACHABLE = True`; `detect_stale_skills_by_profile` on that persisted map returned **`[]`**. Mechanism: `_cmd_enrich.py:337` `merged = dict(existing)` carries `minimal` across, `:344` `merged['defaults'].append(entry)` populates it, and `enrich_add_domain` never calls `_validate_skills_by_profile_structure` (it runs only in `enrich_skills_by_profile`, `:469`). phase-4-plan then reads `minimal` **before** emptiness (`SKILL.md:314-316`) and sets `task.skills = []`. **The report's "nonsensical input" rejection is refuted: no hand-editing is involved at any step, and neither step warns.** → G2.
...
- **Verdict:** PARTIAL — the condition exists, is named, and is non-fatal, but three narrowings mean it does not fire for the situations most likely to occur:
  1. **Fail-open emitter.** `_cmd_client_query.py:543-549` returns as soon as the bundles root cannot be resolved, discarding the unresolved-profile signal — while the module comment at `:428-431` states the opposite: *"the missing/empty and unresolved-profile checks still fire because they need no registry."* Re-measured under adversarial review with a four-row truth table (`resolve_bundles_root` patched to raise, `log_entry` captured): non-empty map + root OK → both messages; non-empty map + root raises → `[]`; **empty map + root raises → the missing/empty message still fires**; absent `skills_by_profile` key + root raises → likewise. So exactly **one** of the two promised signals is lost — the unresolved-profile one, because it is reachable only through a non-empty map, which is the branch that early-returns. The comment is half-false, not wholly false. → G1.
...
- **Checks run:**
  ...
  - **Independently re-run under adversarial review**, same three mutations, same file, fresh snapshot (`md5sum` before and after: `7af6ab64430af50ff6d71cf847a141cc`, `git status --porcelain` clean for that path). Baseline **9 passed**. M1 (`is True` → `bool(...)`): **48 passed** across the guard file *and* `test_cmd_enrich.py` together — so not even the enrich-validator tests catch the weakening, which widens G4 slightly beyond the guard file. M2 (`return False`): **2 failed, 7 passed** — `test_no_warning_on_declared_minimal_profile`, `test_declared_minimal_and_unmarked_empty_are_distinguishable`. M3 (`return True`): **2 failed, 7 passed** — `test_warns_on_unresolved_undeclared_empty_profile`, `test_declared_minimal_and_unmarked_empty_are_distinguishable`, reproducing the run report's pre-fix signature test-for-test. Both directions confirmed load-bearing a second time, by a reviewer who did not write the first sweep.
...
1. **Fail-open emitter contradicting its own comment** — `_cmd_client_query.py:543-549`. Condition: `skills_by_profile` non-empty **and** `resolve_bundles_root` raises (any deployment layout the walker does not recognise; `marketplace_bundles.py:200-231` raises `RuntimeError` when no `plan-marshall` bundle ancestor is found). Consequence: the unresolved-profile signal is suppressed, which the comment at `:428-431` promises needs no registry. The missing/empty signal is **not** affected — an empty or absent map takes the `else` branch and never reaches the early return (re-measured, § D3). Proven by execution. The pre-fix code had the same early return, but the *claim* that the new signal is registry-independent was added by this plan and is false for that signal.
2. **A contradictory populated+`minimal` block is reachable from the ordinary write path** — `_cmd_enrich.py:332-348`. Condition: a profile declared `{"defaults": [], "optionals": [], "minimal": true}` is later populated by `enrich add-domain` / `enrich all`. Consequence: `phase-4-plan/SKILL.md:314-316` tests `minimal` **before** emptiness, so the task is created with `skills: []` although the inventory resolved real skills — a silent skill loss, with the guard silent too (it treats the block as populated). **Reproduced end-to-end under adversarial review** (§ D2): both steps are supported CLI verbs, both return `status: success`, neither emits a warning, and the guard returns `[]` on the resulting map. The report dismissed this as "nonsensical input"; the enrichment path makes it ordinary input, and this plan is what introduced the state — `minimal` did not exist before it.
...
5. **A fourth instance of the two-signal restatement defect survived the run's own sweep** — `test/plan-marshall/manage-architecture/test_skills_by_profile_staleness_guard.py:4-11`. The module docstring still says the guard *"flags a module whose `skills_by_profile` references skill notations absent from the live registry (retired / renamed IDs) or is missing entirely"* — the pre-fix two-signal model, in the very file whose new tests cover the third signal. The run report recorded and fixed three instances of this defect kind (findings 1–3, all in the production file) and missed this one. → G6
6. **The read-surface contract does not carry the new field** — `manage-architecture/standards/client-api.md`. `grep -n minimal client-api.md` → no hits; the `architecture module` examples show only `defaults`/`optionals`, and the `[STALENESS]` unresolved-profile condition is unmentioned. The three-state model lives only in the persistence standard (`architecture-persistence.md:437-474`), so a consumer reading the command contract cannot learn that a `0`-skill profile now has two distinct meanings. → G15
7. No fail-open, off-by-one, unguarded `None`, or order-dependency found in `detect_stale_skills_by_profile` itself: the map-type guard, the whole-map guard, the per-profile loop over a sorted key list, and the stale-notation set are each total over the inputs the tests and the schema admit.
...
| Fail-closed `is True` | — | **Vacuous by omission**: mutation M1 (`is True` → truthy) leaves the guard file's 9 tests green — and, re-measured under adversarial review, leaves `test_cmd_enrich.py` green too (**48 passed** across both files). So a change that lets `"minimal": "true"` launder the signal ships undetected anywhere in the suite — the exact laundering the plan's design caveat exists to prevent. The enrich validator *warns* on a non-boolean `minimal` but still persists it (`_cmd_enrich.py:482-483`), so the value is reachable on disk and only the guard's `is True` stops it. → G4 |
...
- **Refuted — finding #6's rejection rationale.** *"Rejected (out of scope) — nonsensical input; neither the plan nor the closed-vocabulary posture asks to reconcile a `minimal` flag on a populated profile."* Refuted by execution, not by inspection: two supported CLI verbs in sequence (`enrich skills-by-profile` declaring the profile minimal, then `enrich add-domain`) produce the state, both returning `status: success` with no warning (§ D2). No hand-written input is involved anywhere in the sequence, so "nonsensical input" is factually wrong about how the state arises. → G12, G2

...
`code-intelligence-substrate`, it has no counterpart ledger under `.plan/orchestrator/`. It was
...
| `105` | `090` | `marketplace/bundles/**` — `090`'s, exclusively — at **one file for D3 and ~250 for D7** | `105` § D3 widens `analyze_test_module_line_budget` so it measures non-collected modules, in the same file `090` § D4 amends. § D7 then removes dead `noqa` directives across the whole bundle tree — mechanical and tool-reproducible, but the widest overlap any plan in this epic has with `090`. `105` takes D7 last and re-checks this row immediately before it, so a collision costs one commit rather than the run |

...
- User review gates must be respected. The forward-direction transition gates (`plan_without_asking`, `execute_without_asking`) gate phase transitions (3→4, 4→5); the two auto-continuation knobs `finalize_without_asking` and `loop_back_without_asking` live under the top-level `ceremony_policy.automation` block (read at runtime via `manage-config ceremony-policy get --field automation.<knob>`). `ceremony_policy.automation.finalize_without_asking` gates the forward 5→6 transition; `ceremony_policy.automation.loop_back_without_asking` is the symmetric **reverse-direction** gate that controls whether a phase-6-finalize step's `outcome: loop_back` re-dispatches the execute pipeline inline (capped by `phase-6-finalize.max_iterations`, default 3) or halts and prompts the user. Defaults are asymmetric — `finalize_without_asking` defaults to `true` (forward auto-continue), `loop_back_without_asking` defaults to `false` (reverse halt); full unattended execution requires opting into both for the phase-5-execute ↔ phase-6-finalize cycle. See [`manage-config/SKILL.md`](../../manage-config/SKILL.md) § "Section: ceremony_policy" for the automation-axis semantics and [`phase-6-finalize/SKILL.md`](../../phase-6-finalize/SKILL.md) Step 3 § "Loop-back continuation hook" for the dispatch shape.

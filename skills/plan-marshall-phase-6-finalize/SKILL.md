---
name: plan-marshall-phase-6-finalize
...
```text
Call the `view_file` tool on `plan-marshall-persona-plan-marshall-agent/SKILL.md` before continuing.
Call the `view_file` tool on `plan-marshall-workflow-integration-git/SKILL.md` before continuing.
Call the `view_file` tool on `plan-marshall-tools-integration-ci/SKILL.md` before continuing.
```
...
- Never invoke a CI, Sonar, or GitHub/GitLab script (`ci`, `sonar`, `workflow-integration-*`) without an explicit routing flag. Forward `--plan-id {plan_id}` (preferred — auto-resolves the worktree via `manage-status get-worktree-path`) or `--project-dir {worktree_path}` / `--project-dir {main_checkout}` (escape hatch / explicit override after worktree removal). The two flags are mutually exclusive. The executor is cwd-pass-through; routing must be explicit at every call site.
- A **build** invocation is governed differently and is deliberately NOT in the rule above: resolve it via `architecture resolve` and run the returned `executable` **verbatim**, per the canonical-command contract (`extension-api/standards/canonical-commands.md` § "Build Commands", the rule owner; the agent-instructions file merely persists the resolved commands). `architecture resolve` returns no routing flag, so a rule demanding one at every build call site would contradict the canonical form every gate arm uses. Routing for a build is the caller's pinned cwd (ADR-002 pins it to the worktree for phases 5-6), not a flag.
...
| `session_id` | string | No (optional) | Current host-platform session id — forwarded to `default:record-metrics` for `manage-metrics enrich`, which hands it to the platform-runtime `metrics normalized-tokens` op to capture main-context token usage. Absent on transcript-less targets (the transcript-gated resolver in `plan-marshall/workflow/execution.md` dispatches finalize without it); there `record-metrics` skips `enrich` carrying the population-carrying gap flag. On transcript-capable targets the value is required and an absent identity keeps the hard block. |
...
| `default:pre-submission-self-review` | `workflow/pre-submission-self-review.md` | Pre-submission structural self-review over deterministically-surfaced candidates. The check classes are enumerated once, in that document's own frontmatter `description` — read them there; a partial list here would read as the set while naming a fraction of it. When it may record `done` — a full-surface clean pass ALONE is not sufficient — is Step 4's own precondition set; read it there rather than restating it. Its verdict distinguishes an analysis that RAN and observed nothing from one that never ran at all (the zero-generator fallback, when no domain surfacer resolves in the executor) |
...
- **SETTLE (local HEAD-changing, `order < 11`)** — `finalize-step-sync-baseline` (3, rebase→HEAD), `finalize-step-simplify` (5, edits→HEAD) / `project:finalize-step-plugin-doctor` (6, lint gate) / `finalize-step-security-audit` (7, hardening→HEAD), `pre-submission-self-review` (8, review of the settled tree — ordered after the mutating steps so it examines the tree the push ships rather than a pre-simplification diff), `architecture-refresh` (9, derived-state — refreshes the descriptors after the code-mutating settle edits, including security-audit's hardening, so the descriptors describe the settled tree), and `pre-push-quality-gate` (10, gate — sorts LAST in the settle band so its `head_at_completion` anchors the exact tree the push barrier ships; every `mutates_source: true` step ordered below it commits before the ledger row its builds write, so those commits are covered by it and produce no re-stale). Each mutating step's edits are committed on the feature branch by the dispatcher's commit instrumentation (Step 3 item 5f) before the barrier runs.
- **PUSH (`order: 11`)** — the single `default:push` barrier ships the fully-settled HEAD. There is exactly ONE push.
- **WAIT (post-push, `order > 11`)** — `create-pr`, `ci-verify`, `automatic-review`, `sonar-roundtrip` are the D4 **concurrent WAIT barrier** off that one settled HEAD (see § the wait-region narrative for the barrier mechanics — this section does not restate it). Post-push HEAD mutations that structurally REQUIRE the remote PR (era-stamp-fill's PR-number resolution, sonar/review fix application) are NOT pulled before the push — they are absorbed by the D4 bounded re-settle mutation-fixpoint. `adr-propose` (62) then runs before `branch-cleanup` (70) merges.
...
**Ordering authority.** The settle-before-push-before-wait sequence is NOT a compose-layer `order` field and NOT a `run_at_all` boolean — it is governed entirely by each step doc's `order:` frontmatter, resolved through the single choke-point `_manifest_validation._sort_steps_by_frontmatter_order` (consumed by BOTH the plan-local manifest composer AND `manage-config steps-sort`). To move a step between the settle stage and the wait region, edit its `order:` frontmatter (settle `< 11`, wait `> 11`); the composer and `steps-sort` re-materialize the roster and the ascending-order validator asserts the barrier holds. See the roster block above and [`extension-api/standards/ext-point-finalize-step.md`](../extension-api/standards/ext-point-finalize-step.md) for the lane/order contract.
...
FOR each step_id in manifest.phase_6.steps:
     ...
     - PROJECT/SKILL: Branch on the dispatched-vs-inline classification from the
       ...
       **DISPATCHED project/skill step** — route through the generic
       ...
       (2) Dispatch via the Task tool. The workflow doc for a dispatched project/skill
           ...
           `producer` runtime input as workflow-specific prompt-body inputs, plus
  ...
  5c. Record dispatch-boundary row for the just-returned step (per-step, only when 5b also ran):
      ...
      | `blocked_user_review` | The dispatched step raised an `ask_question` review gate that halted dispatch (e.g., branch-cleanup confirmation, or a `plan-marshall:automatic-review` `escalate_ask{reason: re_review_timeout}` return whose `ask` policy made the dispatcher fire the re-review-timeout `ask_question` — see item 7a). |
      ...
      **Ordering note for the `escalate_ask` path** — classification at 5c reads the return TOON; it does NOT itself fire the `ask_question`. When the just-returned step carries `status: escalate_ask{reason: re_review_timeout}`, the three subsequent items run in a fixed order: **5c classifies** the return (this `blocked_user_review` row applies only once item 7a actually fires the `ask`-policy prompt) → **5d skips** the completion guard for the `escalate_ask` return (its dedicated carve-out — terminality is NOT asserted, see 5d) → **7a consumes** the escalation envelope (reads `re_review_on_timeout`, branches on `action`/`reason`, and fires the `ask_question` for the `ask` policy). 5d does NOT assert terminality on the `escalate_ask` path; the `step_record_missing` halt the guard would otherwise raise is precisely the bug the 5d carve-out removes, which is what keeps item 7a reachable.
  ...
  5e. Record per-step execution outcome to the manifest (mirror of the phase-5-execute Step 8c record-step call):
      ...
      - `--outcome` names WHICH SITUATION the row records, and the five values do not overlap:
        - `executed` — the step ran and completed.
        - `skipped` — the resumable re-entry check (item 1) skipped an already-`done` step, or a HEAD-comparison decided no re-run was needed.
        - `loop_back` — the step's `mark-step-done` recorded `outcome: loop_back`: a **productive non-completion**, where the step examined its surface, filed real findings and handed control back. This is the SAME event item 5c stamps `returned_with_findings` for, so the two ledgers now agree by construction rather than by coincidence.
        - `failed` — the step's `mark-step-done` recorded `outcome: failed`: it ran cleanly and self-assessed not-clean (a red gate). The dispatch did not raise.
        - `error` — the DISPATCH raised, timed out (item 5's timeout path), or was cut short, including the 5d post-dispatch-guard violation at item 5d.b — record the `error` row BEFORE halting the FOR loop so the failed attempt is on the execution log.

        ⚠ `loop_back` and `failed` were both recorded as `error` before this partition. Any archive-wide analysis counting `error` then mis-graded every multi-round self-review as a defect, so *the more thoroughly a gate worked, the worse its plan looked* — silently, cumulatively, and in the archive. See [`../manage-execution-manifest/standards/manifest-schema.md`](../manage-execution-manifest/standards/manifest-schema.md) § "Which situation each `outcome` value means".
      - The token-attribution triple is the SAME triple captured by 5b, and it is **three-state, not defaulted**. Forward the `<usage>` integers for a dispatched step. **OMIT the three flags entirely for an inline step that carries no `<usage>` tag** — nothing was measured there, and `record-step` records the omission as the `unmeasured` token rather than as a fabricated `0`. Pass an explicit `0` only where zero is what was actually measured: a step the item-1 re-entry check SKIPPED genuinely consumed nothing, so its `--outcome skipped` row carries `--total-tokens 0 --tool-uses 0 --duration-ms 0`. Passing `0` for an unmeasured column is prohibited — it is byte-identical to a measured zero and no reader can recover the difference, which is exactly the fabricated total the token exists to prevent. See [`../manage-execution-manifest/standards/manifest-schema.md`](../manage-execution-manifest/standards/manifest-schema.md) § "`execution_log[]` — the per-step execution log". 5b sums the measured integers into the per-phase accumulator that fills the `total_tokens` column; 5e records the per-step breakdown. The two are complementary, not redundant.
  ...
  5f. Commit instrumentation (after the step has recorded a terminal `done`/`skipped` outcome — see "Commit instrumentation contract" below):
      ...
      (0) **Post-run-band tracked-source guard** (fires ONLY when `post_run_review` is `true`; the two facts are mutually exclusive, so this arm is reached only on the `mutates_source: false` path): the `mutates_source: false` declaration is the ONLY thing standing between a post-run-review step and an unpushable source edit, and a declaration cannot detect a branch that violates it. A post-run-review step runs AFTER the merge gate, so a tracked-source write one of its branches makes lands as an uncommitted diff with no remaining push path. Observe the main checkout once per such step return:
          ...
          **The tree to observe is `{main_checkout}`, never `{worktree_path}`.** Every
          ...
          - **The path predicate is dirty AND tracked; `.plan/` is exempt ONLY when untracked.** Every finalize step legitimately writes plan STATE under `.plan/` (status, logs, findings) and those writes are untracked — but a number of files under `.plan/` ARE git-tracked (`marshal.json`, every `project-architecture/**/enriched.json` descriptor). `--untracked-files=no` drops the untracked plan state (git excludes it), and the shared exemption predicate (`_plan_state_exemption.partition_plan_state_exemption`, used by BOTH this guard and the layer-D main-checkout drift capture in `plan-marshall/scripts/_invariants.py`) is keyed on git TRACKEDNESS, not on the `.plan/` prefix: a *tracked* `.plan/` file left dirty is reported like any other tracked source, and only an untracked `.plan/` write is exempt. A bare `.plan/`-prefix exemption would hide a dirty tracked `.plan/` write (an architecture-enrich write is the case that matters), so trackedness is the discriminator. The TOON payload publishes the examined population — `considered_paths` (every dirty tracked path observed), `exempted_paths` (the untracked `.plan/` subset dropped), and `offending_paths` (what is reported) — so a `clean: true` that names what it examined is distinguishable from a looked-at-nothing pass.
          ...
          a. Log an attributed WARNING naming the writing step and the offending tracked paths:
             ...
             python3 .plan/execute-script.py plan-marshall:manage-logging:manage-logging \
               work --plan-id {plan_id} --level WARNING \
               --message "[STATUS] (plan-marshall:phase-6-finalize) Post-run-review step {step_id} declares mutates_source: false but left dirty TRACKED paths (source, or tracked .plan/ config/descriptors): {offending_paths} — the edit ran after the merge gate and has no push path; it is NOT committed and NOT blocking"
             ```
          ...
          b. Record the observation as a finding so it survives the run:
             ...
             python3 .plan/execute-script.py plan-marshall:manage-findings:manage-findings qgate add \
               ...
               --component "plan-marshall:phase-6-finalize" \
               --title "post_run_review step left a dirty tracked path after the merge gate" \
               --detail "{step_id} declares mutates_source: false yet left these dirty tracked paths: {offending_paths}. A post-run-review step runs after the merge gate, so the edit is unpushable. A path OUTSIDE .plan/ is a source write the step must stop making (or it does not belong in the post-run band). A tracked path UNDER .plan/ (marshal.json, a project-architecture descriptor) is the architecture-enrich case in the disposition below."
             ```

          **Disposition for a dirty tracked `.plan/` path (the architecture-enrich case).** A tracked `.plan/` descriptor / `marshal.json` that the guard reports is not a stray source write — it is an architecture-enrich write the run legitimately owed. The owed enrich is normally flushed in the PRE-merge band: `default:architecture-refresh` (order 9) writes and COMMITS the descriptors on the feature branch (`chore(architecture)`) before `default:branch-cleanup` (70) merges, so those writes are pushable and the main checkout is clean by the time the post-run band runs. `default:finalize-step-preference-emitter` (order 992, post-merge) deliberately NAMES the enrich calls it discovers rather than writing them, precisely because a post-merge write would be unpushable. A tracked `.plan/` path reported HERE therefore means an owed enrich write ESCAPED the pre-merge commit — a genuine gap worth surfacing, handled by (a)/(b) above (non-blocking, so a merged plan is never stranded). The durable remedy — giving a legitimately-owed post-merge enrich write a push path, versus flushing all owed hints in the pre-merge `architecture-refresh` band, versus classifying the descriptor finding as informational — is a finalize-lifecycle decision that changes when/where architecture-enrich writes, which is out of scope for the guard fix; it is recorded as an open proposal in the plan report for `doc/plans/truthful-signals/330-post-run-guard-exempts-every-tracked-plan-file`.
  ...
  7a. Escalate-ask continuation hook (consult the dispatched step's return status):
      ...
      **Resolve `{outcome_detail}` ONCE here.** Every branch below that names this escalation in a decision-log line or an operator prompt renders `{outcome_detail}` rather than the bare `{reason}`, because `reason` alone cannot tell a decline from a timeout:
      ...
      - **`action: defer`** (policy `defer`): skip the merge for this run — do NOT advance to `branch-cleanup`'s merge. Decision-log the deferral, leave the `plan-marshall:automatic-review` step record ABSENT (do NOT call `mark-step-done` — the absent record is what makes the resumable re-entry check re-issue the step on the next finalize entry), and HALT the FOR loop returning control for re-entry. This is the deliberate inverse of the "Merge anyway" branch below: merge-anyway records a terminal `done` because the operator resolved the step, whereas defer intentionally records nothing so the step re-runs:
           ...
           python3 .plan/execute-script.py plan-marshall:manage-logging:manage-logging \
             decision --plan-id {plan_id} --level INFO \
             --message "(plan-marshall:phase-6-finalize) plan-marshall:automatic-review returned escalate_ask{action: defer} — {outcome_detail}; skipping merge for unreviewed head_sha={head_sha}; re-enter finalize later"
        ...
        - **"Merge anyway — proceed unreviewed"** → decision-log a WARNING, then record the terminal step outcome on the `plan-marshall:automatic-review` REQUIRED step BEFORE advancing, then continue the FOR loop (advance to `branch-cleanup`). The terminal record is mandatory: `plan-marshall:automatic-review` is head-dependent (its doc declares `head_dependent: true`) and a REQUIRED step in the `phase_steps_complete` handshake — without an `--outcome done` record on this branch the handshake deadlocks at the 6-finalize phase transition with a `step_record_missing` gap. `plan-marshall:automatic-review` requires a `--head-at-completion {sha}` on its terminal `done` record; resolve `{sha}` by reason:
           ...
           - `reason: rate_window_timeout` / `rate_window_not_awaitable` / `rate_window_exhausted` → the envelope carries no `head_sha`; resolve the live worktree HEAD via `git -C {worktree_path} rev-parse HEAD` and stamp that.
             ...
             python3 .plan/execute-script.py plan-marshall:manage-logging:manage-logging \
               ...
               --message "(plan-marshall:phase-6-finalize) plan-marshall:automatic-review {reason} ({outcome_detail}): user chose merge-anyway — advancing UNREVIEWED head_sha={head_sha}"
  ...
  7b. Loop-back continuation hook (consult the just-recorded outcome):
      ...
      **Placement is load-bearing — it is BEFORE the (ii) knob check, deliberately.** Both continuations re-read the key: the `value == true` branch re-enters the FOR loop in this same dispatch, and the `value == false` branch halts for an operator re-run that re-enters finalize fresh. Refreshing after the knob would leave the halting (`value == false`) configuration re-entering against the stale snapshot — the configuration that loops most.
      ...
      **(ii) Symmetric-knob check (AFTER the ceiling gate, BEFORE the granularity branch)** — the `loop_back_without_asking` knob applies uniformly to BOTH granularity tiers and gates whether the admitted loop-back executes inline or halts and prompts. The `loop_back_target` value selects the dispatch shape AFTER this gate passes.
      ...
      - IF `value == false`: halt the FOR loop, mark the finalize phase as needing a re-entry, and emit the user-facing prompt (named for the persisted `loop_back_target`). The iteration has already been admitted and persisted, so the operator's re-run resumes against the incremented count rather than a fresh zero:
        ...
        IF `loop_back_target == "5-execute"`:
          Display: "Review turned up problems, and tasks to fix them have been written. Nothing is merged yet. Run '/plan-marshall-plan-marshall action=execute plan={plan_id}' when you are ready to work through them."
        IF `loop_back_target == "6-finalize"`:
          Display: "Review turned up problems that can be settled without changing any code. Nothing is merged yet. Run '/plan-marshall-plan-marshall action=finalize plan={plan_id}' to pick the run back up at the step that raised them."
        STOP.
...
Note the mechanics differ from the same-suffixed merge knob: `loop_back_without_asking=false` halts the dispatcher and *instructs* the operator via a Display + STOP prompt (no `ask_question` is fired — see § "Loop-back continuation hook" item 7b), whereas `final_merge_without_asking=false` fires a genuine inline pre-merge `ask_question` gate (see [standards/branch-cleanup.md](standards/branch-cleanup.md) § "Pre-Merge Confirmation Gate") — same suffix, opposite mechanics.
...
5. **PR state + number** — via `ci pr view --plan-id {plan_id}` (preferred) or `ci pr view --project-dir {main_checkout}` (escape hatch). The disposition of a `status: error` return is owned by the Snapshot Procedure referenced below — do NOT collapse it here. A `pr view` error is an exit-0 `status: error`, which § "Exit-code convention for every script call" above already forbids reading as a usable value, and only one of its `error_cause` arms means *no PR exists*.
...
```text
Call the `view_file` tool on `plan-marshall-phase-6-finalize/SKILL.md` before continuing.

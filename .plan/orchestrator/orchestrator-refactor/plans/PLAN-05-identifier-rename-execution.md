...
- HYPOTHESIS — `epic-surface-partition` is cross-bundle (`pm-plugin-development`) and its
  ...
  - verdict: corroborated | checked_at: 14d8f3ccd74718e8258a674472e9f01b595bc2cc | by: orchestrator-refactor/cleanup | rescoped: n/a | evidence: Cross-bundle and all four artifacts intact (SKILL.md, _epic_partition.py, epic-surface-partition.py, epic-surface-derivation.md all tracked). Caveat re-confirmed live: supporting citation truthful-signals/epic.md:2160-2166 is stale -- that range now holds unrelated D-087-e content, not the third-plan-id-detector finding. PLAN-06's OBSERVED claims 4/5 cite the same dead range.
- HYPOTHESIS — an accepted-alias period is unnecessary because every caller is in-repo;
  ...
  - verdict: corroborated | checked_at: 14d8f3ccd74718e8258a674472e9f01b595bc2cc | by: orchestrator-refactor/cleanup | rescoped: n/a | evidence: Checklist exactly where cited: outline-workflow-detail.md:809 header, :815-822 four-condition table, :824-829 decision table. ADR-023 (a):136 already took the clean break -- --slug retired at every site, no alias or transition window recorded. Condition 2 holds by construction: only two --slug declaration sites are in-repo (orchestrator.py, platform_runtime.py:458); epic-surface-partition already on --epic. ADR-024 sets the matching no-shim precedent.
- Verify-first clause: ADR-007 records that the deleted-symbol / renamed-identifier
  ...
  - verdict: corroborated | checked_at: 14d8f3ccd74718e8258a674472e9f01b595bc2cc | by: orchestrator-refactor/cleanup | rescoped: n/a | evidence: Every ADR-007 citation resolves verbatim (299-line file). :39-41 deleted symbol is the class with no dedicated whole-surface detector; :49-51 a rename is a deletion plus an addition, stale references are exactly the deleted-symbol case; :161-172 a future detector must use source_fingerprint.list_tracked_files/hash_objects; :176-177 MUST NOT parse unified-diff text. ADR-023 Risks :269-274 independently restates the same constraint for this exact rename.

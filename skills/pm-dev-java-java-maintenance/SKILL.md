---
name: pm-dev-java-java-maintenance
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/maintenance-prioritization.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/refactoring-triggers.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/compliance-checklist.md" }` before continuing.
```
...
- `plan-marshall:ref-code-quality` - Language-agnostic refactoring triggers and prioritization

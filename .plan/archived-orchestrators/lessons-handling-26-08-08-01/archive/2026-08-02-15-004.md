...
rollouts. Note the recurring shape — the corpus already records a case of a **vacuous guard
introduced by a fix for vacuous guards**; this is the same family. A change that establishes a
rule is the single most likely place for the rule's next violation, because the author is
thinking about enforcement rather than about compliance.

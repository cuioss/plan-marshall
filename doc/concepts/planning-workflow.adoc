...
The full graph (every conditional, every shared workflow, every inline-script step) is canonical in link:../../marketplace/bundles/plan-marshall/skills/ref-workflow-architecture/standards/call-graph.md[`ref-workflow-architecture/standards/call-graph.md`]. The 8-step anatomy of one dispatch (resolve → construct → dispatch → load → execute → return → record) is in xref:execution-context.adoc[Execution Context].

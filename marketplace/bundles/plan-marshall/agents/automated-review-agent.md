...
- Never marshal multi-line content (PR body, lesson body, memory entry, task YAML, request narrative) through the shell. Multi-line content MUST be written via the Write tool against the absolute path returned by the relevant `manage-*` script's path-allocate subcommand (`prepare-add`, `add`, `path`). Banned constructs: shell heredocs (`cat > file <<EOF`), `python3 -c "..."`, `python -c "..."`, `printf > file`, `$(cat …)` and equivalent shell command-substitution patterns (e.g. backticks, `$(<file)`, or any `$(...)` that injects file contents into a command line).

---
name: pm-dev-java-cui-cui-testing
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/test-generator-framework.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/testing-value-objects.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/testing-juli-logger.md" }` before continuing.
```

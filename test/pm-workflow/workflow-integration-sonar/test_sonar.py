...
class TestSonarFetch(unittest.TestCase):
    ...
    def test_fetch_with_project_only(self):
        ...
        stdout, stderr, code = run_script(['fetch', '--project', 'my-project'])
    ...
    def test_fetch_with_pr(self):
        ...
        stdout, stderr, code = run_script(['fetch', '--project', 'my-project', '--pr', '123'])
        self.assertEqual(code, 0)
        result = json.loads(stdout)
        self.assertEqual(result['project_key'], 'my-project')
    ...
    def test_fetch_with_severities(self):
        ...
        stdout, stderr, code = run_script(['fetch', '--project', 'my-project', '--severities', 'BLOCKER,CRITICAL'])
        self.assertEqual(code, 0)
        result = json.loads(stdout)
    ...
    def test_fetch_mcp_instruction_structure(self):
        ...
        stdout, stderr, code = run_script(['fetch', '--project', 'test-proj', '--pr', '456'])
        self.assertEqual(code, 0)
        result = json.loads(stdout)
...
class TestSonarTriage(unittest.TestCase):
    ...
    def test_triage_bug_major_fix(self):
        ...
        stdout, stderr, code = run_script(['triage', '--issue', json.dumps(issue)])
        self.assertEqual(code, 0)
        result = json.loads(stdout)
    ...
    def test_triage_invalid_json(self):
        ...
        stdout, stderr, code = run_script(['triage', '--issue', 'not-valid-json'])

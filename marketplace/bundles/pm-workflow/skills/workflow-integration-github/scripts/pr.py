...
def get_current_pr_number() -> int | None:
    ...
    stdout, stderr, code = run_gh_command(['pr', 'view', '--json', 'number', '--jq', '.number'])
...
def fetch_comments(pr_number: int) -> dict[str, Any]:
    ...
    stdout, stderr, code = run_gh_command(
        ['pr', 'view', str(pr_number), '--json', 'reviewThreads', '--jq', '.reviewThreads']
    )

    if code != 0:
        return {'error': f'Failed to fetch PR: {stderr}', 'status': 'failure'}

    try:
        threads = json.loads(stdout) if stdout.strip() else []
    except json.JSONDecodeError:
        return {'error': f'Failed to parse gh output: {stdout}', 'status': 'failure'}
...
""",
    ...
    # fetch-comments subcommand
    fetch_parser = subparsers.add_parser('fetch-comments', help='Fetch PR review comments from GitHub')

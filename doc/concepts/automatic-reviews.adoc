...
* **Seven review surfaces**, all funneling into one store:
  ...
  - **CI verify** immediately after `create-pr` (phase-6-finalize). Classifies CI failures into a multi-failure-mode taxonomy.
  - **Automated review** of PR comments after CI completes (phase-6-finalize).
  - **Sonar roundtrip** after CI completes (phase-6-finalize).
  - **`/workflow-pr-doctor`** slash command (on-demand multi-source PR sweep).
* **One unified store** — `.plan/local/{plans,archived-plans}/{plan_id}/artifacts/findings/{type}.jsonl`, append-only, hash-id deduplicated.
* **One triage envelope** — the `verification-feedback` workflow, parameterised by a `producer` runtime input. The five base modes (`build-runner`, `sonar`, `pr-comment`, `plugin-doctor`, `pr-state`) plus the seven ci-verify subtypes (`ci-verify-build`, `-policy`, `-timeout`, `-cancelled`, `-action-required`, `-stale`, `-missing`) share Steps 2-6; Step 1 is the producer-specific fetch.
...
image::../resources/diagrams/findings-pipeline.svg[Findings pipeline — workflow bodies on the left (Q-Gate validation, phase-5 verify-steps, phase-6 pre-submission, phase-6 ci-verify, phase-6 automated-review, phase-6 sonar-roundtrip, /workflow-pr-doctor) drive producer scripts that write to the manage-findings store. Pure consumers on the right (phase-3/4 qgate-resolve for Q-Gate findings, verification-feedback envelope plus per-finding ext-triage-{domain} for all other types) query and resolve., align=center]
...
`finalize-step-pre-submission-self-review` — order 7 in the default phase-6 step list, runs **before** the PR is created. A structural review of the diff itself. Two-phase by design:

* **Deterministic phase** — the per-domain link:../../marketplace/bundles/plan-marshall/skills/extension-api/standards/ext-point-self-review-surfacing.md[`ext-self-review-{domain}`] implementer (today: link:../../marketplace/bundles/plan-marshall/skills/ext-self-review-plan-marshall/SKILL.md[`plan-marshall:ext-self-review-plan-marshall`]) is invoked via `self_review surface --plan-id {plan_id}`. It walks the diff and emits six candidate lists in a TOON envelope: regex literals, user-facing strings, markdown sections, symmetric-pair function names, contract source files, schema-bearing files.
* **LLM-judgement phase** — link:../../marketplace/bundles/plan-marshall/skills/phase-6-finalize/workflow/pre-submission-self-review.md[`pre-submission-self-review.md`] workflow body (dispatched under `--phase phase-6-finalize`, no `--role`, tracking `phase-6-finalize.default`) reads only the candidates the surface helper produced and applies five checks:
...
All three external roundtrips declare `requires: [ci-complete]` in their workflow-doc frontmatter; the phase-6-finalize dispatcher resolves the precondition before each body runs. The default phase-6 order around these consumers is `create-pr` → `ci-verify` → `architecture-refresh` → `automated-review` → `sonar-roundtrip`, with `ci-verify` placed immediately after `create-pr` so the CI verdict is already triaged before the reviewer-bot and Sonar consumers run. Two precondition modes are in use: `ci-verify` runs in **`consume-failures`** mode (failing checks become structured findings, no short-circuit); `automated-review` and `sonar-roundtrip` keep the default **`strict`** mode (a non-success CI short-circuits the step itself — the consumer body is skipped and the dispatcher records the precondition outcome before moving on). Triage budgets are independent of CI queue depth.

=== CI verify (`producer=ci-verify-*`)

Workflow: link:../../marketplace/bundles/plan-marshall/skills/phase-6-finalize/workflow/ci-verify.md[`phase-6-finalize/workflow/ci-verify.md`]. Standards: link:../../marketplace/bundles/plan-marshall/skills/phase-6-finalize/standards/ci-verify.md[`phase-6-finalize/standards/ci-verify.md`].

Placement: AFTER `create-pr`, BEFORE `architecture-refresh` / `automated-review` / `sonar-roundtrip`. The step is the *only* finalize consumer that runs `ci-complete` in `consume-failures` mode — it ingests `failing_checks`, `final_status`, and `wait_outcome` from the resolver without short-circuiting, then classifies each failing check.

The producer side reads CI run state via `tools-integration-ci:ci status`, persists per-job log slices via `manage-ci-artifacts persist` (runs even on green CI so retrospectives have full evidence), then classifies each failing check against this taxonomy:

[cols="1,2,2,3", options="header"]
|===
| Row | Failure mode | Producer string | Detection

| b | Build / test / lint / coverage failure | `ci-verify-build` | `conclusion=failure` AND `workflow_name` matches an architecture-resolved build profile (`verify`, `quality-gate`, `module-tests`, `coverage`)
| c | Policy-workflow failure | `ci-verify-policy` | `conclusion=failure` AND `workflow_name` does NOT match a build profile (license/cla, dep-review, codeql, …)
| e | Timeout | `ci-verify-timeout` | `conclusion=timed_out` OR `wait_outcome=deadline_exceeded`
| f | Cancelled | `ci-verify-cancelled` | `conclusion=cancelled`
| g | Action required | `ci-verify-action-required` | `conclusion=action_required`
| h | Stale | `ci-verify-stale` | `conclusion=stale`
| i | No checks reported | `ci-verify-missing` | `final_status=none` (zero checks across the PR)
|===

Sonar quality-gate failures (row a in the canonical taxonomy) are deliberately excluded from `ci-verify`'s scope — Sonar's quality gate is fetched by `sonar-roundtrip` from the Sonar API, not from the GitHub-checks surface. If a project wires Sonar as a GitHub status check, `ci-verify` still files a finding for it under row (c) `ci_policy_failure`, but `sonar-roundtrip` remains the authoritative consumer of the gate verdict — the two consumers coexist by design and dedup via `hash_id` in the findings store.

Each classified check produces ONE finding with `finding-type: triage`, severity `warning`, message tagged with the subtype (`[ci_build_failure]`, `[ci_policy_failure]`, …) for retrospective grep, and a `source-path` pointing at the persisted per-job log. The consumer side then dispatches `verification-feedback` **once per producer string** (not once per finding) so the triage envelope batches findings of the same kind. If any per-producer triage returns `loop_back`, the step marks `loop_back`; otherwise `done`.

==== CI-wait primitive and timeout

The `ci-complete` precondition resolver invokes the bounded `checks wait` subcommand on `tools-integration-ci:ci` to drive the actual CI poll loop. The subcommand is `checks wait` (not the older `ci wait` form) — the executor's top-level surface partitions verbs across `{pr, checks, issue, branch}`, and the CI-wait primitive lives at `checks wait`. The wait ceiling is sourced from `run-configuration.json` under the command key `ci:wait` (default 600 s when no value is persisted), and the observed CI duration is written back via `run_config timeout set --command ci:wait --duration {observed}` after every successful wait so the ceiling adapts the same way build-command timeouts do.

==== Two distinct artifact surfaces

CI-verify writes to two on-disk surfaces that should not be conflated:

* `work/ci-bodies/` — operation-body scratch allocated by `ci_base.prepare_body`. Holds the multi-line bodies (PR comments, issue updates, etc.) that the executor needs to pass to `gh` / `glab` via `--body-file`. Lifetime is a single dispatcher pass; contents are NOT part of the audit trail.
* `artifacts/ci-runs/{run_id}/` — per-run CI evidence written by `manage-ci-artifacts persist`. One directory per CI run with a `manifest.toon` enumerating every job plus one `.log` slice per job. The manifest is read back via `manage-ci-artifacts read --run-id {run_id}` for a specific run, or `manage-ci-artifacts read --plan-id {plan_id} --latest` for the most-recently-persisted run (recency is decided by the manifest's `fetched_at` timestamp, never by lexicographic `run_id` sorting). The manifest carries a `jobs_source: enumerated | empty` label so a zero-jobs manifest is never silently mistaken for "no CI ran" — `empty` means the `persist` call was not handed a `--jobs-file` (a defect signal on a green-CI path), whereas a genuine no-CI run surfaces as the `ci_no_checks` finding via the `final_status=none` taxonomy row.
...
* **SUPPRESS** — write an inline annotation (`// NOSONAR ...` for Java, `# NOSONAR ...` for Python, etc. — exact syntax per `ext-triage-{domain}/standards/suppression.md`). Suppressions require an inline rationale comment.
...
Workflow: link:../../marketplace/bundles/plan-marshall/skills/plan-marshall/workflow/verification-feedback.md[`plan-marshall/workflow/verification-feedback.md`]. **One workflow, five base producer modes** — `build-runner`, `sonar`, `pr-comment`, `plugin-doctor`, `pr-state` (canonical enum in the workflow's Step 1 branch table). The seven `ci-verify-{build,policy,timeout,cancelled,action-required,stale,missing}` dispatch keys ride the same envelope through the store-only-query branch shape `pr-comment`/`sonar` use, but are surfaced as distinct producer strings so retrospectives can grep by subtype.

The `producer` runtime input selects only Step 1's branch — the producer-specific fetch/parse work. Steps 2-6 are identical across producers and live in link:../../marketplace/bundles/plan-marshall/skills/plan-marshall/workflow/triage.md[`plan-marshall/workflow/triage.md`] (the shared triage core that `verification-feedback` hands off to):

[cols="1,3", options="header"]
|===
| Step | Action

| 1 (producer-mode branch) | `build-runner` / `sonar` / `pr-comment` / `ci-verify-*` — store-only query (pre-flight was already mechanical). `plugin-doctor` — inline marketplace analysis (LLM-heavy). `pr-state` — multi-source PR sweep (CI status + comments + Sonar).
...
* **Producers are scripts, not LLM judgements.** Build logs parsed by regex. Sonar issues fetched via REST through `workflow-integration-sonar`. PR comments fetched via the `workflow-integration-github` / `workflow-integration-gitlab` provider abstraction (the underlying `gh` / `glab` CLIs are wrapped — `dev-agent-behavior-rules` forbids direct invocation). The Q-Gate candidate surface produced by `q-gate-validation`'s deterministic validators. No "the LLM decided this comment doesn't need attention today" — every signal makes it to the store unless an explicit pre-filter pattern excludes it. See xref:tools-and-scripts.adoc[Tools and Scripts] for the script-based-integration rationale.
...
* link:../../marketplace/bundles/plan-marshall/skills/plan-marshall/workflow/verification-feedback.md[`verification-feedback.md`] — the unified triage envelope (five producer modes; producer-specific Step 1 only)
* link:../../marketplace/bundles/plan-marshall/skills/plan-marshall/workflow/triage.md[`triage.md`] — the shared triage core (Steps 2-6) that `verification-feedback` hands off to
...
* link:../../marketplace/bundles/plan-marshall/skills/phase-6-finalize/workflow/ci-verify.md[`ci-verify.md`] — CI failure-classification workflow (7 producer subtypes)
* link:../../marketplace/bundles/plan-marshall/skills/phase-6-finalize/standards/ci-verify.md[`ci-verify standards`] — multi-failure-mode taxonomy, precondition-mode contract, build-profile match rule

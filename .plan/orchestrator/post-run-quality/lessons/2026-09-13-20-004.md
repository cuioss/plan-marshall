...
**2. The status-summary carve-out cannot fire on the real record shape.** (Review-mechanics
half — notified to `review-apparatus`, not owned here; kept for the shared archetype.)
...
2. **Status-summary carve-out — match against the shape that exists.** (review-apparatus)
   Either promote a top-level `body` on the ingest path, or make `_BODY_FIELDS` reach
   `raw_input.body`. Whichever is chosen, add a positive test built from a **real** stored
   record rather than a hand-constructed fixture — a fixture that happens to carry a top-level
   `body` is what let this pass unnoticed.

...
....
 ...
 ~/.plan-marshall/  marshalld/{socket, daemon.pid, daemon.log, registry.json,
                              machine-config.json, registry-audit.log,
                              journal/, job-logs/}
                    build-queue.json  (the single machine-global slot substrate,
                                       + the top-level upper_limit_seconds reap
                                       threshold its own entries are reaped by)
....
...
**`--project-dir` is re-resolved by the child, not the caller — pass an absolute path.** The daemon child re-invokes the executor with the caller's original argv, so a RELATIVE `--project-dir` is resolved against the daemon child's own working directory, never the caller's. Empirically this is a silent-wrong-tree trap: `--project-dir .` invoked from a worktree built the *main* checkout (green, wrong tree — the run's own edits were not tested), and a relative worktree path from the main checkout double-nested and died with a `pw/pwx not found` fallback. Only an ABSOLUTE `--project-dir` is cwd-independent. Whenever a gate targets a different tree than the caller's, pass the absolute path and confirm the tree actually built via the gate result's `[EXEC]` line (it prints the resolved executor path).
...
The operator runs `manage-build-server register` (user-invocable only). It upserts the project — keyed by its canonical, symlink-resolved root — into the machine-global `registry.json` via the `_build_server_registry` module, recording the worktree containers and the notation allowlist, and appends one line to the append-only registration audit log. Registration is the ONLY enablement; no config knob turns the daemon on and nothing git-tracked does.

That claim is about the *enable signal* specifically. The build-slot cap the scheduler admits under (F4) *does* have a machine-wide setting, reached through `manage-build-server config set --max-slots N` — but it is host state, so still nothing git-tracked, and it governs how many builds run at once, never whether the daemon serves a project.
...
Admission is bounded by `max_slots` (default 5), and the cap has exactly one home: the machine-global `machine-config.json` in the daemon state dir, beside `registry.json`. It is *not* a `marshal.json` key. Both consumers that must agree — this scheduler and the F6 in-process fallback's `build_queue` admission — resolve it through one shared, cwd-independent resolver, because both admit against the ONE machine-global `build-queue.json`. Cwd independence is load-bearing rather than incidental: the daemon double-forks and `chdir`s to `/` (F8), so a cwd-relative walk-up would find no repository at all and degrade silently to the default — indistinguishable from a deliberately configured 5.

The cap is re-resolved *per submit*, not only at daemon start, so a `config set` takes effect on the next submitted build with no restart. Every resolution carries a `max_slots_source` — `machine_config`, `default`, `invalid`, or `unreadable` — plus a `max_slots_detail` for any non-nominal one, because the value alone cannot distinguish a configured 5 from a fallback 5; `ping` reports both so `status` renders the cap the *running* daemon is applying rather than re-resolving locally. Two processes can nonetheless admit against different caps — an entry admitted before a `config set` keeps the cap it was admitted under while a later caller resolves the new one, and a resolution that comes back `invalid` or `unreadable` admits under the default while its neighbour admits under the configured value — so every queue entry is stamped with the cap it was admitted under and each `acquire` reports a `cap_agreement` verdict with the population it compared. That disagreement is *reported, never reconciled* — choosing a winner is what a per-caller cap over a shared queue cannot do correctly.

When a slot is free the scheduler admits the next job under per-project round-robin fairness (FIFO within a project), and the supervisor spawns one `asyncio` child in the submitted tree with a clean server-side baseline env — the client's environment is never forwarded and no provider secret reaches the build. The child's combined output streams to a per-job log as it is produced (durable even if the wait is reaped), each chunk marks liveness, and on exit the supervisor classifies the terminal status and the journal records the result and updates the command's ETA history.
...
* **S1 — positional argv-template check.** The command is non-empty; `command[0]` (the interpreter) is accepted — against an explicit interpreter pin when the caller supplies one (exact path or basename), and with no pin, which is what the shipped daemon passes, only as a bare canonical `python3` / `python` name carrying no path separator, so the daemon resolves the binary from its own server-side `PATH` rather than following a client-supplied location (`/tmp/x/python3` is refused). No registration field carries an interpreter, so the check is daemon-wide argv shape rather than per-project registered state, and it is defence-in-depth: the containment boundary is the owner-only socket, not this check. `command[1]` is exactly `{exec_path}/.plan/execute-script.py` inside the submitted tree; `command[2]` (the executor notation) is on the project's allowlist; the remaining args are plain, NUL-free strings.
...
| Registration IS the enable signal (F1). An always-available knob would erode the operator-interactivity wall and make the daemon a silent default. This absence is about *enablement* only — the build-slot cap does have a machine-wide setting (`config set --max-slots N`, F4), which is host state rather than anything git-tracked and decides concurrency, not service.

| Reconciliation of two processes that admitted under different caps
| Reported, never reconciled (F4). The admitting caller applies its own cap and existing stamps are left as admitted, because picking a winner between two callers contending over one shared queue is exactly what a per-caller cap cannot do correctly — which is why the report names no remedy command.
...
* xref:../user/install-claude.adoc[User Guide › Installation (Claude Code)] — the POSIX / WSL2 platform requirement and the per-distro daemon lifecycle.
* link:../../marketplace/bundles/plan-marshall/skills/manage-build-server/SKILL.md[`manage-build-server/SKILL.md`] — the operator control-surface contract (register / lifecycle / the `config get|set|migrate` cap verbs), including every `config migrate` outcome and all six refusal reasons.
...
* link:../../marketplace/bundles/plan-marshall/skills/manage-locks/SKILL.md[`manage-locks/SKILL.md`] — the queue side of the shared cap: the `acquire` / `release` result fields (the cap reports and the disagreement verdict ride an admission, not a release), the `admitted_under_max_slots` stamp, and the `limit get|set` threshold verbs.
* xref:../user/parallelism-and-locking.adoc[User Guide › Parallelism and Locking] — the operator-facing view of the same two machine-global settings and the leftover-per-repo-key migration.

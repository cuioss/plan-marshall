---
name: pm-plugin-development-plugin-task-plan
...
The fence in that verification pattern is written as the regex escape `\x60`, not as a literal backtick, and the call therefore drops `--literal`. This is deliberate and MUST be preserved: the R1 shell-construct rule scans a substitution view that masks single-quoted spans ONLY — a backtick stays live inside double quotes and in unquoted context — so a literal backtick would make the command's R1 verdict depend on how the `--pattern` argument happens to be quoted (inside this command's single quotes it would be inert data; under any other quoting it would fire the backtick substitution check). The escaped form keeps the command string free of the backtick character in every quoting context, so the verdict cannot change with the quoting. Because `--literal` is dropped, the pattern stays in regex mode and the `^` anchor is per line (`re.MULTILINE`), so the pattern reaches a fence opening any line of the file. See [`manage-architecture/standards/client-api.md`](../../../plan-marshall/skills/manage-architecture/standards/client-api.md) § search.

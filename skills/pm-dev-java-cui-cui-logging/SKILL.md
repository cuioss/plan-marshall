---
name: pm-dev-java-cui-cui-logging
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/logging-standards.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/logmessages-documentation.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/logging-maintenance-reference.md" }` before continuing.
```

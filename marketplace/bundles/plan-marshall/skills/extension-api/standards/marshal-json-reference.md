...
| `plan.phase-6-finalize.steps.<step>.lane` (per-element override `off`\|`minimal`\|`auto`\|`full`\|`ask`; **not the sole declaration channel** — the composer resolves this field through a merged plan-local-over-marshal source whose plan-scoped half, `status.metadata.finalize_step_overrides`, is not a marshal.json path and so has no row of its own here) | User config | manage-execution-manifest (lane resolution, scope-gate immunity, finalize ceremony gates) | [ext-point-lane-element.md](ext-point-lane-element.md) |

...
| `default:finalize-step-security-audit` | `standards/finalize-step-security-audit.md` | Proactive security-audit sweep — runs the shared five-stage engine over the plan footprint with each affected domain's `skills_by_profile.security` skills layered on, self-committing hardening edits before the push barrier (`persona: persona-security-expert`) |

...
- **Exception — `next` is never dispatched.** Emitting a `/plan-marshall` command and rendering the surface-disjointness verdict are orchestration judgement reserved by the prime directive.

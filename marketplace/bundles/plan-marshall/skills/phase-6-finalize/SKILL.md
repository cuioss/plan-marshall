...
| `ceremony_policy.automation.loop_back_without_asking` | bool | Symmetric counterpart to `ceremony_policy.automation.finalize_without_asking`. When `true`, a `loop_back` outcome from any phase-6-finalize step (FIX disposition, `pr-comment-overflow`, sonar-roundtrip FIX) auto-dispatches the execute pipeline inline and re-enters the finalize loop, capped by `max_iterations`. When `false` (default), the dispatcher halts and returns control to the user. Read at runtime via `manage-config ceremony-policy get --field automation.loop_back_without_asking`. See Step 3 § "Loop-back continuation" for the dispatch shape. |
| `phase-5-execute.commit_strategy` | string | per_deliverable / per_plan / none |
| `ceremony_policy.automation.finalize_without_asking` | bool | Forward-direction auto-continuation: when `true`, after `5-execute → 6-finalize` transition the orchestrator dispatches `phase-6-finalize` inline rather than halting and prompting the user. Read at runtime via `manage-config ceremony-policy get --field automation.finalize_without_asking`. The reverse-direction symmetric counterpart is `ceremony_policy.automation.loop_back_without_asking`. |
...
| `default:finalize-step-whole-tree-gate` | `standards/finalize-step-whole-tree-gate.md` | Whole-tree completeness gate for clean-slate/breaking plans — greps the entire marketplace tree (not the diff) for surviving references to deleted symbols/contracts and flags request-mandate items absent from the diff; runs pre-commit so a survivor BLOCKS the push (dispatches under `--phase phase-6-finalize`, no `--role`) |
...
**Symmetric auto-continuation invariant**: The `loop_back_without_asking` flag is the structural counterpart to `ceremony_policy.automation.finalize_without_asking`. The two knobs together define the four corners of the unattended-vs-interactive matrix:

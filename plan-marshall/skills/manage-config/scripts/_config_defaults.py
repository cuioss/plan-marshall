...
#
# The slot CAP is deliberately NOT seeded here, and is not a marshal.json key at
# all. The build queue is machine-global — every checkout on the host contends
# for one `build-queue.json` — so a per-repo cap is a cap two callers can
# disagree on while admitting against the same slots. It therefore lives in the
# machine-global `machine-config.json`, is read through
# `_machine_config.resolve_max_slots` (cwd-independent, and it states its
# source), and is set with
# `manage_build_server config set --max-slots N`. A project whose marshal.json
# still carries a `build.queue.max_slots` gets it REPORTED as not in effect on
# every queued build, never honoured; `manage_build_server config migrate` is
# the one-step way to act on that warning. `sync-defaults` is non-destructive,
# so an existing project keeps its stale key and is warned rather than silently

...
| Verb | Parameters | Description |
|------|-----------|-------------|
| `get` | `--field` | Get a project field. Rejects any `--field` outside the known project field set with `error_type: unknown_field` **before** the live `project` block is consulted — the same guard, through the same seam, that `set` applies below, so the two verbs cannot disagree about what a project field is. The order matters: `marshal.json` is operator-editable and `sync-defaults` preserves an already-present key without inspecting it, so a retired or typo'd key sitting in the live block would otherwise read back as `status: success` while `set` refuses the same name. Falls back to the canonical default (from `DEFAULT_PROJECT`) when an admitted key is absent from the live `project` block. A `user_language` read from the live block is re-validated at this read boundary and refused with `error_type: invalid_value` when it is not a non-empty string — `marshal.json` is operator-editable and `sync-defaults` preserves an already-present key without inspecting its value, so the guard is what keeps a hand-edited or migrated non-string from reaching the language rule, which resolves anything other than `auto` as a pin. The `DEFAULT_PROJECT` fallback carries no such guard: the seed is self-validated by `get_default_config`. |
...
Step-owned params nest under their owning step in the id-keyed `steps` map (`pr_merge_strategy` / `final_merge_without_asking` / `auto_rebase_threshold` under `default:branch-cleanup`; `review_bot_buffer_seconds` under `plan-marshall:automatic-review`; the prefix-stripped sonar params under `default:sonar-roundtrip`). They are read/written via the one-stop `step` verb against the marshal.json keyed map (the compose-time default + wizard global-config write target), NOT via flat `get --field` / `set --field`.
...
- `Unknown domain: {name}`
- `unknown_field` — `project get` or `project set` was given a `--field` outside the known project field set, which is exactly the keys of `DEFAULT_PROJECT` in `scripts/_config_defaults.py` (see both verb rows above). Both verbs route the same seam, so neither admits a name the other refuses: `set` refuses before any key is persisted, and `get` refuses before the live block is consulted. An unknown field therefore never masks an operator/doc typo behind a green `status: success`, in either direction.

...
- **Exception — provisioning stamps**: `system.provisioned_version` and
  ...
  NOT part of `get_default_config()`). Unlike every other already-present key,
  neither follows the key-exists preservation rule above — but their override
  conditions differ: `config_seed_fingerprint` is re-stamped **unconditionally**
  on every `sync-defaults` run, while `provisioned_version` advances only when
  `read_provisioned_version()` returns a real (non-empty) version; on an empty
  read (an unstamped/absent executor) any pre-existing `provisioned_version` is
  left intact rather than blanked, so a transient read failure never erases a
  known-good stamp. A live `marshal.json` therefore always reflects the
  currently provisioned config-seed fingerprint, and the most recently confirmed
  executor version, after a steward reconcile. See `marshall-steward/SKILL.md`
  config-reconcile step.
  ...
  "plan": {
    ...
    "phase-6-finalize": {
      ...
      "steps": {
        "default:push": {},
        "default:create-pr": {},
        "plan-marshall:automatic-review": { "required_bots": "", "optional_bots": "", "review_bot_buffer_seconds": 180 },
...
The canonical argparse surface for `manage-config.py`. The plugin-doctor `missing-canonical-block` rule checks that this section is PRESENT,
...
Re-writes `marshal.json` with the canonical top-level key order (reuses the `save_config` writer). Idempotent — an already-canonical file is left byte-stable. The result reports `action: normalized` plus `unrecognized_keys`: the top-level keys absent from `CANONICAL_TOP_LEVEL_KEY_ORDER`, which `order_config_keys` can only append after the canonical block rather than drop. When that set is non-empty the `status` is `warning` (naming the offenders); otherwise `success`. So a caller can no longer read `action: normalized` and conclude the file is canonical when a stray block remains appended.

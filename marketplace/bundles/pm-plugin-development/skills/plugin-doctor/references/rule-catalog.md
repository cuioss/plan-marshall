...
**Deliberately not flagged**: a bare `#NNNN` in prose with no incident noun (ordinary provenance, a worked-example citation, an external-tracker issue), a back-ticked `#NNNN` (a code token, exempt as inline code — tested at the reference's offset inside the match, so forms 2 and 5, whose matches begin BEFORE the reference, are exempt on the same terms as the rest; this widened form 2's exemption, which previously fired on a back-ticked reference), a lowercase "observed on #NNNN" in an observation record, and a version CONSTRAINT with no temporal preposition ("requires Python 3.12", "Node 20.1.0 or newer", ">= 1.2.3") — those state a requirement that holds now.
...
**Fail-closed conditions**: a flag with no `choices=`, a `choices=` the resolver cannot evaluate, an unresolvable notation, an unparseable script, and an enum above the block's first invocation each emit NOTHING — there is no authority to compare against, and inventing one would manufacture a false finding. The unresolved share is published on the analyzer's output (`unresolved_notation_fraction`) with a per-cause census (`unresolved_notation_causes`) rather than left silent, so a clean sweep states what it could not check. The census reports every cause including the zeroes, and separates the ONE cause that is not a blind spot from the rest. `no_choices_declared` means the subcommand's parser WAS modelled and declares no `choices=` for that flag — nothing to contradict. Every other cause is a real gap, and the largest is `parser_surface_not_derived`: a script whose parser is built by an IMPORTED helper has no `ArgumentParser()` in the file the notation names, so the walk models nothing and the flag's `choices=` sit unread in another module. Filing that under "no choices declared" would report the largest gap as reassurance, so the analyzer also publishes `blind_spots` — the sum over every cause except `no_choices_declared` — rather than leaving a reader to know which causes count.

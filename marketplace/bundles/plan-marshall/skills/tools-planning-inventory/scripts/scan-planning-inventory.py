...
PLANNING_PATTERNS = [
    ...
    'workflow-*',  # Workflow-prefixed skills (workflow-pr-doctor, workflow-outline-change-type, etc.)
    'task-*',  # Task commands (task-standalone)
...
def categorize_components(inventory: dict) -> dict:
    ...
    Handles two formats:
    - TOON: bundles are top-level keys (e.g., inventory['plan-marshall'])
    - JSON: bundles are in 'bundles' dict (e.g., inventory['bundles']['plan-marshall'])

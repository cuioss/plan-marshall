...
- **Dated-slug epic.** Each run opens its own epic with slug `lessons-handling-{YY-MM-DD}` (e.g. `lessons-handling-26-07-16`). Every invocation is a fresh, distinct epic, so the mode is repeatable over time; prior dated epics remain as closed history and are never reopened.

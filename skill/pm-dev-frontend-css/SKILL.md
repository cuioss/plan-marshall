...
```text
Call the `read` tool with `{ filePath: "standards/css-essentials.md" }` before continuing.
```
...
```text
Call the `read` tool with `{ filePath: "standards/css-responsive.md" }` before continuing.
```
...
```text
Call the `read` tool with `{ filePath: "standards/css-quality-tooling.md" }` before continuing.
```

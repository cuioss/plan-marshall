...
>
> ⚠ **Known already-closed ids cited by this plan (1):** `160/G2`.
...
- `marketplace/bundles/plan-marshall/skills/script-shared/scripts/build/_build_shared.py` *(added 2026-08-23, DA)*
- `test/plan-marshall/finalize-step-review-retrospective/test_review_retrospective.py` *(added 2026-08-23, DB)*
...
**DA — the routed build arm substitutes its own `tests_run` for the routed job's.** *(folded 2026-08-23 from PR #1330 / L1 — CORROBORATED + RECURRENCE)*
On `mechanism=daemon_longpoll` the OUTER wrapper reports `tests_run: 0` and prints
`[EXEC] green build: 0 test(s) executed - test-failure findings retained (this run tested nothing)`
while the daemon job log for the SAME build records `tests_run: 17888` at a near-identical duration
(115s vs 114s). The in-process arm reports correctly (21,675 in the same plan), so the defect is
specific to the routed arm.
**Mechanism, confirmed first-party:** `script-shared/scripts/build/_build_shared.py:749-754` derives the
count by PARSING THE LOG FILE (`parser_fn(log_file)`); when the outer wrapper's log is not the routed
job's log the parse yields `test_summary is None` and `tests_run` falls to `0`. The quoted sentence is
verbatim from line 754.
⭐ **Orchestrator sharpening — the wrong zero is not merely reported, it CHANGES BEHAVIOUR.** The same
file gates finding-lifecycle on it: `_build_shared.py:467` runs `_reconcile_pending_build_findings`
only `if tests_run > 0`, so on the routed arm stale `test-failure` findings are **never auto-resolved**.
The message's claim about finding retention is therefore self-fulfilling, not merely inaccurate.
⛔⛔ **This is a RECURRENCE, not a new defect.** The epic already records the same wrapper-boundary
substitution on a DIFFERENT field — `duration_seconds: 0` / `exit_code: -1` on the outer TOON against a
330s inner TIMEOUT — routed to **PLAN-TRUTH-027**, which **SHIPPED as #1224**. The fix was
**field-scoped, not boundary-scoped**: `duration_seconds` was addressed and `tests_run` at the same
boundary was left with the identical defect. Fix the BOUNDARY here (propagate the routed job's result
fields), or the next field repeats it a third time.
**Remedy:** propagate `tests_run` and the `green build: N test(s) executed` line from the routed job's
result instead of re-deriving them outer-side. Until then the routed arm's `tests_run` is not a
population figure, and the daemon job log under `~/.plan-marshall/marshalld/job-logs/` is the only
honest source.

**DB — one un-migrated code-scoped `type: ignore` remains, and it is the same CI-vs-local tripwire.** *(2026-08-23, FIRST-PARTY - not in the source report)*
PR #1330 fixed `test/plan-marshall/automatic-review/test_counting_rule_parity.py:52` to the bare
`# type: ignore` form after it blocked that run's `test-compile` gate: `review_retrospective` is a
project-local script under `.claude/skills/`, so mypy resolves it as `import-untyped` or
`import-not-found` depending on which roots are on its search path, and a CODE-SCOPED ignore is
therefore "unused" in whichever environment raises the other code — which `warn_unused_ignores` makes a
hard error. The line could not be green in both CI and local as written.
⛔ **A third import site was left un-migrated and still carries the code-scoped form:**
`test/plan-marshall/finalize-step-review-retrospective/test_review_retrospective.py:54` reads
`import review_retrospective as rr  # type: ignore[import-untyped]`. The other two sites are already on
the bare form (`test_counting_rule_parity.py:52` via #1330, and
`test_pr_agent_contentless_guide_interaction.py:104`, which carries the explanatory comment and is the
convention's documented home). Verified by enumerating ALL THREE importers 2026-08-23, so this is a
complete population, not a sample.
**Remedy:** migrate the third site to the bare form. Cheap, and it removes a latent environment-dependent
gate failure of exactly the kind that cost the source run a blocked gate and a wrong first diagnosis.

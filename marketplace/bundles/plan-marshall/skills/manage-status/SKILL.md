...
A SUPPLIED anchor is resolved against the local object store before anything is persisted: when `--head-at-completion` resolves to no commit (`git rev-parse --verify {sha}^{commit}` fails — a fabricated string, a non-commit object, or a leading-`-` option-injection shape), the command returns `status: error`, `error: unknown_head_at_completion` and **writes nothing**. The check is object presence, not ref reachability: a commit object present in the local store passes even when no branch or tag reaches it — reachability from a ref is neither required nor checked. The persisted value is the resolved full-hex commit ID, never the supplied spelling: a symbolic anchor (`HEAD`, a branch name, `HEAD~1`) would otherwise move under the record and break every later delta the record scopes. Presence without resolvability is a fabricated anchor, and a fabricated anchor records a verdict nobody can locate in history — so it is refused rather than persisted, on every `done` outcome, whether the step declares `head_dependent: true`.
...
For a `6-finalize` step, the terminal write emits a `[STEP] (plan-marshall:phase-6-finalize) Completed step: {step} (outcome={done|skipped|loop_back|failed})` work-log line as a side effect of the write (`_cmd_mark_step.py::_emit_completion_marker`). The line carries the outcome because the writer already holds it — a marker naming only the step said that a step finished without saying how, so a reader counting completions had to re-open `status.metadata.phase_steps` to tell a `done` from a `failed` or a `loop_back`. Its shape comes from the shared `_step_completion_marker` module, which binds the three surfaces two different ways. The `--no-completion-log` help text interpolates the same `COMPLETION_MARKER_TEMPLATE` constant the producer formats from, so it cannot describe a line that is not emitted. The audit's read pattern is a *separate* literal in that module (`COMPLETION_MARKER_RE`, whose `(outcome=…)` suffix is deliberately optional so historical lines still count) — co-location alone would not couple it, so a round-trip test (`test/plan-marshall/manage-status/test_step_completion_marker.py`) requires the pattern to recover the same `step` and `outcome` from a line the template actually formats. An edit that moves the emitted shape out from under the audit fails that test. This fuses the operational-log completion marker to the handshake: recording a finalize step's outcome and emitting its completion line are ONE action, so the step-completion invariant and the log line cannot diverge. The emission is scoped to the `6-finalize` phase — every other phase's `mark-step-done` writes no such line, so the marker's population (and the dispatch audit's `completion_count`) is unchanged for them. It rides only a real write: an idempotent no-change re-call emits nothing. `--no-completion-log` suppresses it on the one call that re-stamps an already-emitted outcome (the phase-6-finalize item-5f `head_at_completion` re-stamp), so the line is emitted exactly once per step; a first terminal recording never passes the flag.
...
**`open_phase_count` counts PHASES, not plans.** A plan holding two `in_progress` phases — the state a loop-back from `6-finalize` to `5-execute` leaves behind — contributes **one** `open_phase_records[]` row naming both, and **two** to its cohort's `open_phase_count`. Counting the rows instead would under-report by exactly the multi-open-phase plans this verb exists to surface.
...
**Phase closure and the unexaminable record.** Archive closes every phase recorded `in_progress`, leaves every `pending` phase alone, and writes `current_phase: complete` once the phase structure was read IN FULL and nothing remains open — see [status-lifecycle.md](standards/status-lifecycle.md) § "Phase-closure post-condition". `phase_closure` publishes which of those two paths ran on **every archive response that performs the move**, in the same shape the `census` cohort row uses for its two enumerated verdicts: `complete`, or `partial` with a `phase_closure_reason` naming the shortfall. A response that moves nothing carries no `phase_closure` field at all — both `--dry-run` (which returns `status: success` with `dry_run: true` and `would_archive_to`) and the no-reason unexaminable refusal below (which returns `status: error`) return before the phase-close write, so neither has a closure to report. A record whose `phases` could not be read in full keeps the `current_phase` it already had rather than being written `complete` — an unknown open-phase set is not an empty one.
...
**prose-only corroboration** — a prose warning (S7) that is the SOLE fired signal does not carry the lane when a **measurement** contradicts it. All three conditions must hold: `fired == [S7:risk_prose]`, `scope_estimate` is the non-committal middle band (`single_module` — resolved, yet neither deep-biasing nor `surgical`), **AND** the band was measured — `band_rule == path_count_middle_band`, a counted set of distinct paths that landed in the middle. Only then is the warning uncorroborated and the lane stays `light`, with the fired-but-denied signal reported under `suppressed_signals` (`signals.risk_prose` still reads `true`, so the record shows the signal fired and was suppressed). A `single_module` band that counted nothing (`pathless_non_empty_body`) is not a measurement, so it does not suppress: the author's warning keeps the lane. S7 still outranks a POSITIVELY-earned narrow band (`surgical`). A large change that fires any second signal is unaffected (broad/unknown scope → S2, generative change → S3, breaking compat → S4, no anchors → S5), and so is one whose middle band was never measured; the S6 override and the escalation ratchet remain available in every case. This is corroboration, not a provenance exemption: the sensor scores semantic scale vocabulary, not markup, so a genuine warning in a spec body is never blanket-suppressed.

**signal-resolution confidence** — the route reports how much of the signal vector actually resolved, under `confidence` (`signals_total` / `signals_resolved` / `signals_null` / `null_signals` / `low_confidence`). The split is taken over the six **read** signals; `planning_lane_override` is excluded, because its absence is the normal state (no operator override was set) rather than a read that failed. `low_confidence` keys on the four **discriminating** reads — `plan_source`, `scope_estimate`, `change_type`, `compatibility` — and is `true` when two or more of them are null. Keying on those four rather than on a bare majority of the vector matters because `request_concrete` and `risk_prose` are body-derived booleans that are never null: under a majority rule they always padded the resolved side and could outvote a genuine gap.
...
| `list-orphans` | _(none)_ | Discover orphan plan directories — entries under `plans/` with no `status.json` file. Inverse of `list`; the archived-plans directory is excluded |
...
| `planning-lane route` | `--plan-id [--lane-override deep\|light] [--persist]` | Deterministic planning-lane router. Resolves `planning_lane ∈ {light, deep}` from the DQ1 signal set (S1–S7) plus a `request.md` regex with zero discovery; `plan.phase-1-init.deep_lane` (`always`/`never`/`auto`) short-circuits the signals. Default is light; any deep signal forces deep. Also projects the recommended `execution_profile` posture (`minimal`/`standard`/`full`) over the same signals, independent of the deep-lane gate. A prose-only warning (S7) does not carry the lane alone against a MEASURED non-committal `single_module` scope (`band_rule == path_count_middle_band`) — it moves to `suppressed_signals`; a `single_module` band that counted no paths does not suppress it. The route reports the signal-resolution `confidence` (resolved-vs-null counts over the six read signals, with `low_confidence` keyed on two or more null discriminating reads). With `--persist`, writes `status.metadata.planning_lane` + `status.metadata.execution_profile`. Emits one decision-log line naming every signal value, the winning predicate, the projected posture, the `scope_provenance` block that explains which band rule fired, the `confidence` counts, and any `suppressed_signals`. |
...
**The monolithic layout is refused.** Every orchestrator-store verb first probes the epic header. A `status.json` that still carries `plans` or `resume_anchor` returns `error: legacy_layout` with a `remedy` naming `orchestrator migrate-layout --slug {slug}`, and writes nothing — there is no read-fallback. `create` refuses it too, `--force` included, because overwriting the header would drop the queue and the anchor it still carries. A header that exists but does not read as a JSON object returns `error: header_unreadable`, never `file_not_found`, from every verb except `create`, which reports it as `already_exists` (or overwrites it under `--force`). The per-concern layout itself is the contract in `persona-plan-orchestrator/standards/orchestration-model.md` § Directory Layout.

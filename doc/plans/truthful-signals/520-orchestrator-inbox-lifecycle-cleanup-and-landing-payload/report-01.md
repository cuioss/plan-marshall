...
Round 3's most useful output is not a finding. Across three rounds the defect **count** fell (18 → 14 → 10) while the defect **class** stayed perfectly stable: *each round's own corrections carried a fresh unverified mechanism.* Round 1's was the `--default` tier claim; round 2's was the O(queue) cost bound; round 3's were the `loadgroup` causal sentence and the binding-`max` omission. Three for three — a generator, not a tail.
...
**Two pre-existing defects surfaced and deliberately NOT fixed** (out of this plan's scope, recorded as residue): `cleanup.md`'s `## Output` block declares no field for a deferred relocation or for a migration having fired, and `applied[]`'s `{spec,finding_class,…}` shape cannot carry a relocated `epic.md` **section**, which has neither. Both predate this branch. Also pre-existing: `build.py` cites a `real_marshal_json` xdist group that exists nowhere in the tree.

### Survivors — left open, each characterised

| # | Finding | Why it may be left open |
|---|---|---|
| S1 | The `16287` module-tests figure in the 250 report is not cleanly re-derivable. | **(b) bounded, on a mechanism now verified rather than asserted.** Round 2 refuted the bound this row first offered — see finding R2-5 — so it is restated: the two runs sweep the **same** path, and the real difference is `--dist=loadgroup` (which pins `xdist_group` markers to one worker) versus the default `load` (which scatters them), read from `build.py cmd_module_tests`. That is what makes the *passed* count order-dependent, which the report line itself already records. The one-item collected delta carries **no assigned cause**. The bound: neither site now states the figure without its command and base, so a reader cannot mistake a sample for a stable count. Condition A does not reach the figure itself — it is an unreproducible historical observation, not a false statement — and the provenance note that *was* false has been corrected. |
...
| 2 | `cleanup.md` § Step 8 after D6(ii)–(iii) | the migration performed **before** the script; marker insertion **only** when the pair is absent, and the between-marker evacuation **only** when it is present | ❌ **round 1 exposed two real defects** — see below. Rewritten, re-read, and round 2 took the intended reading on all three scenarios. | ✅ intended, after a rewrite |

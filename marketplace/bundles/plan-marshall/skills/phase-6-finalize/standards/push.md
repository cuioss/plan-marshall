...
- **Finalize-internal re-stale (known-safe)** — a finalize-internal step committed DURING finalize, advancing the working-tree `worktree_sha` past the last `kind=build` ledger entry. The source a `verify` DID observe is unchanged; only a finalize-owned commit moved the currency hash. Overriding this silently with `--force` discards the distinction and the audit trail.
  ...
  **Membership is a discriminator, not a list**: a step whose authoritative doc declares `mutates_source: true` **and** whose `order` is greater than **or equal to** `default:pre-push-quality-gate`'s. Both operands are load-bearing. `mutates_source: true` is what makes the step capable of moving the hash at all; the order bound is what makes the move happen *after* the ledger entry this route is measured against, since that entry is written by the pre-push quality gate's **own** build calls, during its run. A `mutates_source: true` step ordered **strictly below** that gate commits before the ledger row exists, so its commit is already covered by it and produces no re-stale — and the gate ITSELF is a member for the same reason read the other way round: its item-5f commit lands chronologically *after* the row its own build calls just wrote, so nothing covers it. That self-referential case is why the bound is `>=` and not `>`. Resolve both operands from the step docs' own frontmatter rather than from any list here — this bullet deliberately states no membership count, because a count here would go stale the moment a step is added. `default:pre-push-quality-gate` and `plan-marshall:automatic-review` are among the members today, named as examples only. (`default:finalize-step-simplify` and `default:finalize-step-security-audit` are NOT members: ordered below the gate, their commits land before the ledger row its builds write, so that row already covers them.)
...
The same ceiling binds the halt-route details composed in the `stale` and `undecidable` rows of § "Freshness precondition", where the polarity inverts: `{reason}` is protected there by § "The `stale` `reason` MUST be carried into `display_detail`".

...
FOR each step_id in manifest.phase_6.steps:
  ...
  7a. Escalate-ask continuation hook (consult the dispatched step's return status):
      ...
      **Resolve `{outcome_detail}` ONCE here.** Every branch below that names this escalation in a decision-log line or an operator prompt renders `{outcome_detail}` rather than the bare `{reason}`, because `reason` alone cannot tell a decline from a timeout:
      ...
      | `reason: re_review_timeout`, `outcome: declined` | `re-review DECLINED by {declined_bots} — the bot answered without naming a reviewed commit; no budget expired` |
      ...
      **(ii) Symmetric-knob check (AFTER the ceiling gate, BEFORE the granularity branch)** — the `loop_back_without_asking` knob applies uniformly to BOTH granularity tiers and gates whether the admitted loop-back executes inline or halts and prompts. The `loop_back_target` value selects the dispatch shape AFTER this gate passes.
      ...
      - IF `value == false` (default): halt the FOR loop, mark the finalize phase as needing a re-entry, and emit the user-facing prompt (named for the persisted `loop_back_target`). The iteration has already been admitted and persisted, so the operator's re-run resumes against the incremented count rather than a fresh zero:
        ...
        IF `loop_back_target == "5-execute"`:
          Display: "Loop-back signalled. Run '/plan-marshall-plan-marshall action=execute plan={plan_id}' when ready to dispatch the fix tasks."
        IF `loop_back_target == "6-finalize"`:
          Display: "Loop-back signalled (inline replay). Run '/plan-marshall-plan-marshall action=finalize plan={plan_id}' to replay the finalize step."
        STOP.
...
| `scripts/foreign_pr_gate.py` | `plan-marshall:phase-6-finalize:foreign_pr_gate` | Pre-archive foreign-PR landing gate — refuses to archive while any *foreign* deliverable's change is `pushed_no_pr` (committed and pushed to a branch no PR carries). Consumes two deterministic signals, never artifact prose: the `foreign` column of `manage-solution-outline list-deliverables` for its population, and `ci pr landing-state` for per-repository done-ness. Returns `clear` / `blocked` / `error`; fails CLOSED, so unreadable evidence stops the archive exactly as `blocked` does |

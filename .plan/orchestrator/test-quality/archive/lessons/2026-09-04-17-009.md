# Relocated lesson 2026-09-04-17-009 — The suite was never order-independent and nothing measured it

> Moved from the global lessons corpus (`.plan/local/lessons-learned/`) into this
> epic's archive by the test-quality orchestrator, 2026-09-17. Defects landed fixed;
> the order-independence practice is carried by campaign deliverable D4 and the
> host-limitation watch (strict serial reverse-order not runnable on this host).
> Corpus entry removed as completely covered. Original body preserved verbatim below.

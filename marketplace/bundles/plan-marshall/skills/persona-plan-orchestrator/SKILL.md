...
3. **Direct-file-write carve-out.** Write/Edit are permitted ONLY within the epic's own `.plan/orchestrator/{slug}/` tree; logging and status transitions stay script-mediated (`manage-logging` / `manage-status` with the orchestrator store). Writes outside that tree follow the ordinary access rules.

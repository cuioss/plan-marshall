...
| `refused` | populations equal, but `execution_log_rows_in_population` is **0** — the ledger holds no row for those phases at all | `empty_population` | absent |
| `refused` | populations equal and non-empty, but an in-population row carries an unmeasured or unrecognised token column, so the sum is a FLOOR rather than a total | `incomplete_measurement` | absent |
| `computed` | populations equal **and** non-empty **and** every in-population row's token column is readable | absent | present |

**`computed` requires all three gates, not just the population-name one.** The scope check, the population-size check and the coverage check are independent, and a reader who treats the first as the whole gate will misattribute the others. Read `comparison_reason` to tell them apart: `population_mismatch` says the two figures measure different phase sets; `empty_population` says they name the same set and the ledger holds none of it; `incomplete_measurement` says they measure the same set but the recorded sum could not read all of it.

⛔ **`empty_population` is what stops the coverage gate from passing VACUOUSLY, and it is not a sub-case of it.** Over zero in-population rows the sum is `0`, `execution_log_rows_unmeasured` and `execution_log_rows_unrecognised` are both `0`, and "every in-population row was readable" is vacuously **true** — so a composed-but-unrun manifest passed the coverage gate and emitted `delta_tokens = 0 − predicted` as a `computed` comparison. That is a confident delta against a fabricated zero, and it fed the recalibration loop. The gate is therefore keyed on `execution_log_rows_in_population`, the quantity an empty population actually moves, and that count rides the refusal so the `0` beside it reads as an unpopulated sum rather than as a measured total. The general rule: **a set-guarding predicate must be population-derived — a check that can return `0` from an empty population MUST publish that population's size and refuse over it.**

⛔ **A subtraction across mismatched populations is one defect; a subtraction against an empty population is the second and against a floor the third, and all three refuse.** Each produces a *plausible* figure — two token counts, one subtraction, a percentage — which is exactly why none would look wrong to the `cost_size_token_table` recalibration loop that consumes `delta_pct`. Withholding the delta and stating `comparison_reason` turns a silent choice into a legible one.

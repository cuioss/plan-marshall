...
def test_request_create(plan_context):
    ...
    assert (plan_context.plan_dir_for('request-create') / 'request.md').exists()


def test_request_create_with_source_id(plan_context):
    ...
    content = (plan_context.plan_dir_for('request-source-id') / 'request.md').read_text()
    assert 'Test Feature' in content
    assert 'source_id: https://github.com/org/repo/issues/123' in content


def test_request_create_invalid_source(plan_context):
...
def test_request_read(plan_context):
    """Test reading a request document."""
    # Create file first
    (plan_context.plan_dir_for('request-read') / 'request.md').write_text("""# Request: Test
...
def test_request_read_raw(plan_context, capsys):
    """Test reading a request document in raw mode."""
    content = '# Request: Test\n\nRaw content here'
    (plan_context.plan_dir_for('request-raw') / 'request.md').write_text(content)
...
def test_request_read_not_found(plan_context):
...
def test_request_exists_present(plan_context):
    """Test checking if request exists (present)."""
    (plan_context.plan_dir_for('request-exists') / 'request.md').write_text('# Request')
...
def test_request_exists_absent(plan_context):
...
def test_request_remove(plan_context):
    """Test removing a request document."""
    plan_dir = plan_context.plan_dir_for('request-remove')
...
def test_request_create_metadata_only_emits_stub(plan_context):
    ...
    # File shape
    target = plan_context.plan_dir_for('meta-only-stub') / 'request.md'
...
def test_request_create_with_body_file_substitutes_contents(plan_context):
    """--body-file splices a pre-written body into the stub under Original Input."""
    body_path = plan_context.fixture_dir / 'external-body.md'
    body_content = (
    ...
    assert result['status'] == 'success'
    target = plan_context.plan_dir_for('body-file-splice') / 'request.md'
...
def test_request_create_rejects_removed_inline_args(plan_context, removed_arg, removed_value):
...
def test_request_create_rejects_missing_body_file(plan_context):
    """--body-file pointing at a nonexistent path returns body_file_not_found."""
    missing = plan_context.fixture_dir / 'definitely-does-not-exist.md'
    ...
    assert not (plan_context.plan_dir_for('body-file-missing') / 'request.md').exists()


def test_request_create_returns_path_in_output(plan_context):
    ...
    expected = (plan_context.plan_dir_for('returns-path') / 'request.md').resolve()
...
def test_request_path_returns_canonical_path(plan_context):
    """`request path` returns the absolute path to the existing request.md."""
    plan_dir = plan_context.plan_dir_for('path-ok')
    (plan_dir / 'request.md').write_text("""# Request: Test
...
""")
    ...
    assert returned == (plan_dir / 'request.md').resolve()
...
def test_request_path_missing_document(plan_context):
...
def test_mark_clarified_succeeds_when_section_present(plan_context):
    """mark-clarified returns success when Clarified Request section exists."""
    (plan_context.plan_dir_for('mc-ok') / 'request.md').write_text("""# Request: Test
...
def test_mark_clarified_succeeds_without_clarifications_section(plan_context):
    """mark-clarified still succeeds when only Clarified Request is present."""
    (plan_context.plan_dir_for('mc-no-clar') / 'request.md').write_text("""# Request: Test
...
def test_mark_clarified_fails_without_clarified_section(plan_context):
    """mark-clarified errors when Clarified Request section is absent."""
    (plan_context.plan_dir_for('mc-missing') / 'request.md').write_text("""# Request: Test
...
def test_mark_clarified_fails_when_document_missing(plan_context):
...
def test_three_step_edit_roundtrip(plan_context):
    ...
    assert plan_context.plan_dir_for('three-step').exists()
...
def test_invalid_plan_id_uppercase(plan_context, capsys):
...
def test_create_existing_fails(plan_context):
    """Test that creating over existing document fails without --force."""
    (plan_context.plan_dir_for('exists-fail') / 'request.md').write_text('# Existing')
...
def test_create_existing_with_force(plan_context):
    """Test that --force overwrites existing document."""
    plan_dir = plan_context.plan_dir_for('exists-force')
    (plan_dir / 'request.md').write_text('# Old Content')
    ...
    assert result['status'] == 'success'
    content = (plan_dir / 'request.md').read_text()
...
def test_read_section_clarified_request_fallback(plan_context):
    ...
    target = plan_context.plan_dir_for('fallback-test') / 'request.md'
    target.write_text(
...
def test_read_section_returns_clarified_when_present_after_direct_edit(plan_context):
    ...
    assert plan_context.plan_dir_for('clarified-present').exists()
...
def _write_phase1_request(ctx, plan_id: str) -> str:
    ...
    (ctx.plan_dir_for(plan_id) / 'request.md').write_text(content)
    return content


def test_read_section_source_from_header(plan_context):
    """Regression: `--section source` must resolve against header key:value lines."""
    _write_phase1_request(plan_context, 'header-source')
...
def test_read_section_source_id_from_header(plan_context):
    """Regression: `--section source_id` must resolve against header key:value lines."""
    _write_phase1_request(plan_context, 'header-source-id')
...
def test_read_section_plan_id_from_header(plan_context):
    """Regression: `--section plan_id` must resolve against header key:value lines."""
    _write_phase1_request(plan_context, 'header-plan-id')
...
def test_read_section_created_from_header(plan_context):
    """Regression: `--section created` must resolve against header key:value lines."""
    _write_phase1_request(plan_context, 'header-created')
...
def test_read_section_header_preserves_original_block(plan_context):
    """The _header virtual section must return the full original header block unchanged."""
    _write_phase1_request(plan_context, 'header-preserved')
...
def test_read_unknown_section_lists_header_fields_as_available(plan_context):
    """Unknown sections still error, but available_sections now advertises header virtuals."""
    _write_phase1_request(plan_context, 'header-unknown')
...
def test_cli_archive_plan_source_section_read(plan_context):
    """End-to-end: exact CLI shape used by archive-plan.md (`request read --section source`)."""
    _write_phase1_request(plan_context, 'archive-plan-source')
...
def test_cli_unknown_document_type(plan_context):
...
def test_cli_missing_required_field(plan_context):
...
def test_cli_request_create_roundtrip(plan_context):
...
def test_cli_request_path_subcommand(plan_context):
...
def test_cli_request_mark_clarified_subcommand(plan_context):
    ...
    target = plan_context.plan_dir_for('cli-mc') / 'request.md'

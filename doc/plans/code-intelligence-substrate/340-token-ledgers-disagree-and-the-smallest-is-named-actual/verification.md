...
- **Verdict:** PARTIAL. Every field the report claims is present, and the forward direction — each key
  written by `write_metrics` reaches the store, and each figure the Total row prints resolves to a key
  written in that same call — is verified. One observation on that direction, not a defect: the Total row
  reads the in-memory `data` dict that `write_metrics` was just handed rather than re-reading the file,
  so "the render reads the store back" is true of the values but not literally of a second read.

  **What is not verified is the direction the *Done when* actually states.** "No rendered figure lacks a
  persisted counterpart" is a claim about the rendered report read top-down, and this audit never
  enumerated one. It reasoned outward from the persist-then-render block instead, which establishes that
  the *Total row's* figures are persisted and says nothing about the rest of the report. The annotation
  blockquotes (`:2014-2097`) are the untested surface: they render formatted and derived figures —
  `f'{name} → {field} {value:,}'` in the reconciliation clause (`:2015-2018`), the two boundary-fold
  clauses (`:2067`, `:2084`), the assembled phase-name lists in the population clause (`:2039-2059`) —
  none of which was traced back into the store. The check cannot be run here: it needs a rendered report,
  and `.plan/local/` holds only `logs/` and `marshall-state.toon`, with no `plans/`
  tree to generate metrics from (the same blockage D1 records). The likely finding, if it were run, is a
  derived or reformatted figure in an annotation with no direct counterpart rather than a missing one —
  but "likely" is not the *Done when*. → **G11**.
...
- **D3's reverse round-trip.** Rendering a report and checking every figure in it — Total row *and*
  annotation blockquotes — back into the store needs a metrics corpus, and `.plan/local/` holds no
  `plans/` tree here. Only the forward direction was verified, which is why D3 is PARTIAL. → **G11**.
...
| A2 | False negatives | No CONFIRMED verdict was found unwarranted. Five load-bearing guards were re-driven red independently of the first audit: the population-mismatch refusal (`if predicted_population != EXECUTION_LOG_POPULATION:` → `if False:` ⇒ the two refusal tests fail), aggregate invalidation (`if not preserve_totals:` → `if False:` ⇒ red), the D7(c) faithful mutant (render the qualifier, withhold `*_population_count` from the file write ⇒ **4 red**, including `test_a_rendered_total_without_a_persisted_population_count_is_caught`), the D5 `end_time` guard (deleted ⇒ `test_a_closed_phase_takes_no_floor_marker` red), and nearest-first greedy (⇒ red). D3's persist-then-render block (`:1908-1980`) and `write_metrics`' scoping (`:948-957`) were read directly rather than through the audit's reasoning; the **Total row's** figures each resolve to a key written in the same call — which is the forward direction only, and is why D3 is now PARTIAL (G11). | None needed on the verdicts. One coverage addition (A7). |
...
| A8 | Internal consistency | The overall verdict follows from its rows (4 CONFIRMED + 3 PARTIAL ⇒ CONFIRMED WITH GAPS). Every gap traces back to a finding here (G1←Correctness 1, G2←Test adequacy, G3←Correctness 2, G4←D4/F18, G5/G9/G10←Report accuracy, G6←Correctness 3, G7←Out-of-scope, G8←Test adequacy, G11←D3) and every finding warranting action appears in `gaps.md`, G11 included — it was raised after that file was written and has since been filed there, so the dangling reference this row previously recorded is closed. The one inconsistency was inside `gaps.md` itself (A4's preamble arithmetic), plus G2's internal contradiction between its own evidence ("no hand-written corpus will separate them") and its *Done when* (which demanded exactly such a separation). | Preamble rewritten; G2's contradiction removed by re-basing the entry. |

**Residual doubt:** the largest one is no longer carried as doubt. **D3's round-trip claim from the other
direction** — enumerating the *rendered report* top-down and checking each figure back into the store,
which is the literal shape of the plan's *Done when* — was not performed and cannot be performed from
this clone, so D3 is now PARTIAL and the unrun check is carried as **G11** rather than as a hunch. The
remaining doubt: `G6`'s reachability bound — this review confirmed no *current*
...
**Verdict on the audit:** SOUND AFTER CORRECTION — every mutation result and every count in the original
audit held up under independent re-derivation, but its single most-argued gap (G2) rested on two claims
that the run's own report and one further mutation refute, and it was filed at the wrong severity, the
wrong kind and with an unsatisfiable *Done when*. One deliverable verdict did not survive: **D3 was
CONFIRMED on evidence for the forward direction only**, while its *Done when* is stated in the reverse
direction, so it is now PARTIAL with the unrun check carried as G11.

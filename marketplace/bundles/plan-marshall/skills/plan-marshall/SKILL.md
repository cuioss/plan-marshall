...
**Step 0 — executor / config staleness preflight (deterministic, runs first).** Before the worktree re-anchor and routing calls below, run the deterministic `generate_executor preflight` verb once. It compares the executor's embedded `MARSHALL_VERSION` and `marshal.json`'s `system.provisioned_version` against the installed `dist-manifest.json` (emitted by the target generator) under two **asymmetric ownership** rules: the executor is safe derived state (ADR-002) and is regenerated in place when stale, while `marshal.json` holds user decisions and is **never** auto-mutated. The verb runs from the main checkout using main's present executor (which stays present throughout phase-5+), so it precedes the worktree re-anchor:

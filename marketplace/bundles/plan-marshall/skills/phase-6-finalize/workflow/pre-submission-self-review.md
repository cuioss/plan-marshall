...
⛔ **This path performed NO analysis, and its verdict says so rather than reporting an absence of defects.** No surfacer ran, so no file was searched and no check executed; the outcome is `done` only because a consumer without a domain surfacer must not be blocked from pushing, NOT because the diff was reviewed and found clean. This is exactly why the not-run verdict is a distinct string from the nothing-to-check one: that verdict reports a surfacer that RAN over a real file set and produced no candidate, which is a weak statement about the diff, whereas this one is a statement about the executor and makes no claim about the diff at all. Recording both under one string would let an un-run analysis read as a clean review — the un-run-versus-un-observed collapse this step's verdict vocabulary exists to prevent. This clean run lets the promoted default step ship safely to consumers without a domain surfacer, with the un-run dimension named in the record rather than only in a log line.
...
- Empty `findings` AND `{N} == 0`, from a round that **did** run a surfacer (the **nothing-to-check** verdict) → `"self-review clean: surfacer ran, zero candidates surfaced"`. The surfacer ran over a non-empty file set and produced no candidate, so no check had anything to run against. This is a real, if weak, statement about the diff — unlike the not-run verdict above, which is a statement about the executor.

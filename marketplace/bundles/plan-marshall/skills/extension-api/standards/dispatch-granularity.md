...
| `pre-submission-self-review` | `tools-self-review:self_review` (deterministic candidate-surface script). | **`--phase phase-6-finalize`** (no `--role`; tracks `phase-6-finalize.default`) — LLM applies five structural-review checks against the surfaced candidates + loaded contract sources. |

...
`.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/530-detector-and-auditor-integrity.md`. It was ingested into this ledger on
2026-08-22 as `PLAN-CIS-051` and it will be EMITTED INTO THE PLAN-MARSHALL LIFECYCLE, not into the
cloud lane.
...
For each defect named in D1–D7, run the reproduction the deliverable names and record the observed
...
*Done when:* the report carries the reproduction table with one row per D1–D7 item, each row naming
...
landed plan's exported directory was deleted when the orchestrator collected it, and the PR is
what survives.
...
the retired export tree, which **is gone** — it is collected, so never depend on it;
...
The primary target is that run's report, whose exported copy **is gone** — it was collected
...
### D7 — A delta round can be blind to its own delta, and return clean over it

⛔ **SCOPE-BLOAT RATIONALE (required — this fold takes the plan to SEVEN implementation deliverables,
past the ~6 threshold).** D7 is admitted unsplit because it is the same defect class as D1–D6 — a
detector that cannot distinguish an unread input from a clean one — and shares this plan's D0 gate and
its re-derivation discipline, so splitting it would duplicate the gate for one deliverable. ⭐ It lifts
out cleanly if the operator prefers: it shares no file with D1–D6. `plan-doctor`/outline may split it
on that basis without re-litigating the subject.

⚠ **FOLDED IN 2026-08-24 from inbox `truthful-signals-049`, which RE-HOMED it here and withdrew it from
its own ledger.** It arrived pointed at `PLAN-CIS-043` arm A — but **`CIS-043` is SHIPPED (PR #1189)**,
so the arm that established the family has landed and cannot absorb a new consequence. It lands here
instead because this plan's charter is exactly *detectors that cannot tell an unread input from a clean
one*, which is what this is.

**The established parent (CIS-043 arm A, shipped):** `_detect_count_prose` opens **only**
`{skill_dir}/SKILL.md`, while its sibling `_collect_skill_contract_sources` in the same file returns
*"SKILL.md plus every `standards/*.md`"* — so *"full surface"* means the full **FILE** surface, not the
full **DETECTOR** surface. A full-surface pass can be blind to a whole document class.

⭐⭐ **THE NEW CONSEQUENCE, which arm A does NOT carry and which is why this is a deliverable rather
than a note:** *when a delta's whole content is uncovered prose, the candidate set is byte-identical to
the previous round's, and a clean verdict certifies nothing.* Arm A establishes that a **full** pass can
be blind to a document class. This adds that a **DELTA round can be blind to its own delta** — if
everything that changed lives in a content class the surfacer does not emit, the round re-surfaces the
*previous* round's candidate set verbatim and returns clean over it.

⛔ **So a clean delta round is not merely "a filter over a narrower file set" — it can be a round that
never saw the change it was convened to review.** That is this epic's signature archetype (a guard
reporting success over a population it could not read) landing on the self-review surface.

**The observation** (source: `PLAN-TRUTH-095`, merged `b95d78437` / PR #1339; the sending orchestrator
corroborated the surrounding mechanics but did not re-reproduce the run): `pre-submission-self-review`
fired **8 times** (7 prior `failed`) over **115 candidates / 20 findings**. One over-claim appeared at
**four sites** and was corrected in the order *the tooling could see them* — docstrings first, then
assertion messages, then a comment — **because the surfacer emits only `context: docstring` prose**.
**Two of the four copies were structurally invisible to every round that preceded their correction.**

*Deliver:* make a delta round state what it could not see. Either widen the surfacer beyond
`context: docstring`, or — if that is out of scope here — emit a **delta-coverage** fact per round
naming the content classes present in the delta versus the classes the surfacer can emit, so a clean
verdict carries the population it was computed over.

*Done when:* a round whose delta is entirely uncovered prose is **distinguishable from a genuinely clean
round** in its own output. ⛔ **A matched negative control is required** — a delta in a covered class
must still report clean-with-full-coverage, or the new fact proves nothing. ⛔ **Population-derived, per
this epic's standing rule**: the check must publish the class counts, not merely a boolean.

### Folded from the 2026-08-26 inbox drain (PLAN-CIS-060 / PR #1355)

**(N1) `channel_completeness` divides all-phase dispatch lines by finalize-only completions, and
grades the result `nominal`.** Folded from inbox `-002`. This extends D1 (`check-dispatch-audit`
publishes measurements, not defaults) — same script, same self-trust grade.
⭐ OBSERVED, first-party from the run: the D3 block emitted `dispatch_line_count: 22`,
`completion_count: 22`, `ratio: 1.0`, `confidence: nominal`. **Both figures are correct in isolation
and are NOT measured over the same population**: `dispatch_line_count = len(dispatch_lines)` counts
every `[DISPATCH]` line across **phases 2–6** (22 here, of which only **12** carry the finalize
dispatcher caller), while `completion_count` counts `[STEP] … Completed step:` lines, which
`mark-step-done` emits **only for finalize steps** (22 here).
⛔ **The `1.0` is an arithmetic coincidence of two unrelated populations.** The finalize-scoped ratio
is **12/22 = 0.545** — barely above the `_SPARSE_RATIO = 0.5` floor below which `confidence` would
have degraded to `low`.
⛔ **`cmd_run` computes the correctly-scoped figure and then does not use it**: it passes
`finalize_dispatch_line_count` to D2 (`evaluate_dispatch_coverage`) and the UNSCOPED
`len(dispatch_lines)` to D3 (`evaluate_channel_completeness`) alongside D2's finalize-scoped
`coverage['dispatched']`. Two checks in one function disagree about their own scope, and the mismatch
is invisible because both published counts look plausible.
⭐⭐ **The consequence on the very run that found it**: the audit graded the channel `nominal` over a
four-envelope, ~893 K-token hole in that same channel (the CIS-052 N2 defect). **This is a vacuous
green in the instrument that measures the instruments** — the epic's own subject, one level up.
*Done when:* `finalize_dispatch_line_count` is passed to `evaluate_channel_completeness` so numerator
and denominator name the same phase, as D2 already does; the ratio's two inputs are **labelled with
the population each was taken over** in the emitted fact block, so a future scope divergence is visible
in the fragment rather than only in the source; and a regression test in which the all-phase and
finalize-scoped counts **differ** asserts the graded `confidence` follows the finalize-scoped figure —
⛔ a single-phase fixture cannot distinguish the two and would pass over this defect.
...
- **Adding a producer for `_executive-summary`, or deleting the row.** Both are contract changes with
  defensible arguments on either side; D2(b) records the proposal. ⚠ **CORRECTED 2026-08-24** — the
  stated reason was that a run picking one "would be self-approving a change to a documented
  structure **with no operator in the loop**". There IS an operator in the loop here. The exclusion
  stands, but for the right reason: it is a genuine fork, so record the proposal AND escalate it with
  `AskUserQuestion` instead of leaving it merely deferred.
- **The report-defect gaps of other buckets, and any file under the landed-run archive outside this
  plan's own scope.** ⚠ **CORRECTED 2026-08-24** — this read "any exported-lane file outside this
  plan's own directory", justified by "the lane contract forbids status or bookkeeping writes under
  the export tree". Neither the tree nor that contract applies: `.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/`
  is deleted from git and the surviving records are the read-only archive at `cloud-runs/`. The
  exclusion is now **absolute rather than conditional** — nothing under `cloud-runs/` is written by
  this or any plan. The two report restatements this plan carries (D5(f)) therefore always take their
  recorded-note form: file them as an epic inbox `finding` and reproduce them in the PR body. ⛔ Their
  "degrade if the target has been collected" branch is no longer a fallback to be tested for — it is
  the only branch.
...
- `marketplace/bundles/pm-plugin-development/skills/ext-self-review-plan-marshall/scripts/self_review.py`
  — D6(c), **and D7** (attribution added 2026-08-24 by the `cleanup` pass that followed D7's fold: the
  file was already declared, so the disjointness algebra was never wrong here — only the per-deliverable
  attribution was incomplete, which would have understated who contends for it).
...
## Claim Labels
- verdict: unverifiable | checked_at: 91a07aaa4 | by: code-intelligence-substrate/analyze | rescoped: n/a | evidence: Section read in full at HEAD 91a07aaa4 and found SUBSTANTIVELY INTACT: it carries a labelled claim table with an OBSERVED/HYPOTHESIS discipline and a named git-reachable confirm/refute artifact per row. The block was a PARSER-READABILITY artifact - the claims are expressed as a markdown table or a prose paragraph rather than top-level bullets, which the pre-1355 parser silently read as zero claims - and NOT a refutation of any premise. UNVERIFIABLE, not corroborated, is the honest verdict: this pass did NOT re-execute the confirm/refute artifacts, which require running scripts and driving mutations across roughly forty claims, and stamping corroborated without doing so would be exactly the vacuous-authority archetype this epic tracks. Re-grounding by execution REMAINS OWED and belongs to a cleanup pass. unverifiable admits by contract because an unreached population is not a refutation, so this unblocks the spec while leaving the debt on the record rather than hiding it. Contrast PLAN-CIS-055, which was checked on the same pass and came back contradicted.
...
6. **Suites.** Run the affected test trees individually (`test/plan-marshall/plan-retrospective/`,
   ...
   manage-solution-outline module) and then the full verify — ⚠ **CORRECTED 2026-08-24**, this said
   "the full build gate the lane contract requires"; there is no lane gate here, so run it
   unconditionally as
   `python3 .plan/execute-script.py plan-marshall:build-pyproject:pyproject_build run --command-args "verify"`
   (Bash timeout ≥ 600000 ms), reading the result TOON `status`/`errors[]` rather than the exit code,
   which is 0 even on failure. Record
...
for example `.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/170-finalize-dispatch-evidence-is-missing/gaps.md#G13`.
...
`{source-plan}/gaps.md#{id}` under `.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/` — corroboration, not
required reading (§ Notes).

| Deliverable | Gaps discharged |
|---|---|
| D0 (gating) | none — it derives the state D1–D7 act on |

...
def _plan_lane(
    ...
    *,
    ...
    (pd / 'references.json').write_text(json.dumps({'scope_estimate': scope}), encoding='utf-8')
    ...
    if total_tokens is not None:
        (pd / 'work' / 'metrics.toon').write_text(f'[5-execute]\ntotal_tokens: {total_tokens}\n', encoding='utf-8')
...
def test_lane_lever_rows_sorted_by_plan_id(tmp_path):
    ...
    inputs = [audit.collect_inputs(tmp_path / '.plan' / 'local' / 'archived-plans' / n) for n in ('zeta', 'alpha')]
...
def test_emit_lane_lever_block_renders_header_and_severity(tmp_path):
    ...
    genuine_row = next(ln.strip() for ln in block.splitlines() if ln.strip().startswith('surg-over,'))

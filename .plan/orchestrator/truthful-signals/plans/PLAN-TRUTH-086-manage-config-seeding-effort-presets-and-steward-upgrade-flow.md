...
>
> ⚠ **Known already-closed ids cited by this plan (1):** `100/G7`.
...
> It names `report-01.md` under `150`, `290`. Those reports were ingested
...
The run should touch these and report anything else it had to change.

⚠ **Lane-residue correction (2026-08-24):** this sentence previously read *"per the lane contract"*. That contract governs a run executing from `doc/plans/` and does **not** bind a `/plan-marshall` run. ⭐ **The REQUIREMENT is kept and is correct** — a run must report surface it had to touch beyond the declared set; that is `affected_files` discipline and this epic tracks its under-recording. **Only the vehicle was wrong.** Recorded rather than silently deleted, because the same class was reported to `review-apparatus` as `truthful-signals-033.md` and this epic carried an instance of it.
...
## Folded from the PLAN-TRUTH-096 drain (2026-08-24) — finding `5721cb`

Inbox `-011`. The `sync-plugin-cache` finalize step invokes `sync.py` as bare `python3`. On this
machine it hard-failed with

```
staleness_guard: cannot import source_fingerprint helper
```

and **directed the operator to re-run the generator**. ⛔ That remedy is wrong: `target/claude` had
been freshly emitted by `deploy-target` moments earlier, so the artifact the operator was told to
rebuild was **already current** — re-running it changes nothing and the step fails again.

The real cause is an import path, not a stale tree: a stdlib-only helper is reached **through the full
`marketplace.targets` package**, which drags in `yaml`, absent under bare `python3`.

⭐ **Two defects, and the second is the one worth the deliverable.** The import is a one-line fix. The
durable defect is that **a guard reported the failure it was written to detect rather than the failure
that occurred** — an unrelated `ImportError` was absorbed into the "stale target tree" verdict and
carried its remedy. ⇒ A guard whose probe could not run must say so, not fall through to its own
subject. This is the same shape as `PLAN-TRUTH-104`'s channel-absent case, in a different component;
serialize with it rather than re-deriving the rule.

⚠ The finding was **`pending` at plan end** and the plan directory has since been archived — it
survives only through this fold and the epic's Open Defects.

...
## Claim Labels
- verdict: contradicted | checked_at: 7a028157eab86d5c5e9c8ee181671c6093ac5963 | by: code-intelligence-substrate/cleanup | rescoped: yes | evidence: RE-GROUNDED 2026-09-15 at HEAD 7a028157e (previous stamp 53ab7dd2e). SCOPE STATED: this spec has the LIGHTEST exposure of the three staged - only 2 of its 12 declared surface entries were touched by the 334 files changed since the previous stamp (audit.py and test_generate_executor.py), so the bulk of its claims sit on documents nothing has moved and retain their previous verdicts unre-executed. ⛔ That LOW exposure is itself misleading and the reason must be recorded rather than left for a later reader to rediscover: this spec's surface is dominated by GLOB entries (marketplace/bundles/plan-marshall/skills/*/SKILL.md, .../standards/*.md, .../references/*.md, doc/user/, doc/concepts/, doc/adr/), so a two-of-twelve reading counts GLOBS, not files. Dozens of individual documents under those globs moved in this window. A per-entry exposure count is the wrong instrument for a glob-declared surface, and treating 2/12 as low risk would be exactly the count-without-its-population error this plan exists to correct. NEW WORK ARRIVING FROM THE LANDING, not from this re-read: PLAN-CIS-049 (#1489) carried six out-of-footprint self-review findings forward as lessons 2026-09-14-21-001/-002/-003, and FIVE of them are documentation sites restating a superseded module_edges/content_search status vocabulary - this spec's subject exactly, now with their locations named. ⭐⭐ They are the SELF-SEEDING archetype (PLAN-TRUTH-089: 27% of finalize findings self-seeded across 5 chains), so the terminating move is to REPLACE THE RESTATEMENT WITH A POINTER AT ITS SOURCE, not to correct five copies into five new copies. Fold them in when this spec is next touched. D6's absorbed refutation is unaffected by this window: cloud-runs/ still holds 46 directories, 37 with a gaps.md.

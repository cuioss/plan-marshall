...
The other direction is the **9 rows this table marks `Conditional` or `Optional`** (`mutates_source`, `head_dependent`, `verdict_inputs`, `advances_main_via_rebase`, `records_facts`, `post_run_review`, `requires_prompt_fields`, `reads`, `destroys`). None is on the record, and none is an unread declaration: each has a live consumer that reads it from the step doc's own frontmatter directly, via `extension_discovery._read_frontmatter_fields`. The record deliberately carries the seeding/preset consumers' fields, not every declared key — see [Hook API](#hook-api).

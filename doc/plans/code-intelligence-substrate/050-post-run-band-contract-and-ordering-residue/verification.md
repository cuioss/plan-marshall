...
Re-derived at `a90adeb` (the audit was written at `61a43e5`). Every mutation below was taken by me
against a byte snapshot under `$TMPDIR/adv-050-post-run-band-contract-and-ordering-residue-mutsweep/`,
restored from that snapshot, and confirmed with `git status --porcelain` clean for the mutated path —
never `git checkout`/`restore`/`stash`. Each mutation was verified present **both before and after** the
test run, so no reading was taken against a file a concurrent agent had silently restored.

| # | Attack | What was found | Correction applied |
|---|---|---|---|
| A1 | False positives | **No false gap.** Walked every `path:line` in all nine gaps and every non-CONFIRMED verdict. G1 verified by *execution* (see A3), G2/G3/G4/G5/G6/G9 verified at source. Two stale citations: G1's raise site given as `_footprint_resolver.py:98-111` (the function is `:60-111`, the operative relative-path raise `:107-111`); G4's touched-file count given as `audit.py:1808-1809` (the code is `:1810`; `:1808` is its comment). | Both citations corrected. G4's `Where` expanded to all six real call sites (`:1266`, `:1329`, `:1571`, `:1810`, `:3061`, `:4178`); G5's expanded to include `:98`. |
| A2 | False negatives | **One material miss, and one overstatement.** (a) **D1's coverage claim is now false and its canary cannot catch it.** The module asserts the consumer-side vocabulary is empty; `ext-point-finalize-step.md` now defines `reads` **and** `destroys`, `finalize-step-order-bands.md:76-99` states their ordering obligation, and two steps declare `destroys` today. `_ABSENT_CONSUMER_MARKERS` watches only the four read-side spellings, so the guard written to force a re-measurement stayed green when exactly the event it names occurred (`308528d`, #1211, one day after this plan landed). Confirmed empirically by re-running the module's own discovery with `destroys` added to the probe set. (b) G4 overstated its blast radius: `:1810`/`:3061`/`:4178` read `modified_files_count or affected_files_count` and so **fall back** to the declared set rather than reading zero, and `_plan_shipped` carries an independent `plan_pr_number` criterion, so the shipping partition is not meaningfully at risk. | **New gap G10** added (medium, `measurement/metrics`) with the empirical evidence; D1's verdict re-stated as CONFIRMED **as of landing** with a new caveat (c); Correctness review item 7 added; D1's Test-adequacy row rewritten — the canary is not "passing trivially, which is its intent". G4's Evidence, Why-it-matters, Done-when and Risk rewritten per call site; verification.md item 3 given the same correction. |
| A3 | Vacuous evidence | **Both audit-claimed mutation sweeps re-run and both reproduce**; three further sweeps run that the audit did not. (1) D1 GATE: `record-metrics` `order: 998 → 5` → `1 failed, 5 passed`, offending edge named. (2) D3 fold: `cmd_generate`'s reconcile → `pass` → `1 failed`, `KeyError: 'total_tokens'`. (3) **New** — D3 *partiality* half: making `_reconcile_accumulator_into_phase` stamp an `end_time` → red on `any_phase_missing_end_time`, so **both** halves of D3's *Done when* are genuinely guarded (the audit had proven only the non-zero half). (4) **New** — D1 mutual exclusion: `record-metrics` declaring both markers → cardinality test red (`15` vs `14`), proving a claim the audit had only asserted. (5) **New** — D4 negative control: resolver's terminal `return FOOTPRINT_UNRESOLVED` → `return set()` → **4 tests red** across two files, including `assert 'pass' == 'skip'`, proving the graded-zero prohibition is enforced, not decorative. G7's claim was also verified directly: deleting Step 2.5 entirely leaves **1420 tests passing**. | Test-adequacy rows for D1, D3 and D4 rewritten to record the new proofs; the D1 detail section gained the mutual-exclusion mutation; G1 gained an execution-level proof (documented invocation raises; same plan with `--diff-file` omitted returns `status: success, footprint_source: resolved`). |
| A4 | Counts and quotes | **One count does not reproduce.** The report-accuracy row claimed a fresh grep for `three-tier\|two-tier\|3-tier\|2-tier` returned **19** hits; re-derived today it returns **45** (42 mentioning no footprint, 3 in plan 050's own `plan.md`/`report-01.md`). The load-bearing conclusion — no stale 2-tier resolver claim survives in a live surface — re-derives as **true**. Every other number re-derived and held: 14 edges / 14 marker-carrying / 25 non-gate steps / gate order 70; 6 D1 tests; 14 `test_footprint_resolver.py` tests; 235 resolver lines; 26 files in `0e7f644`; 14 `footprint.txt` hits across exactly 7 files, all inside `plan-retrospective`. Every quoted line checked verbatim against its attributed file (G6's quote, the `ext-point-finalize-step.md` "not a dead-end" passage, `_plan_shipped`'s docstring, the canary docstring). | The `19 hits` figure replaced with the re-derived 45/42/3 breakdown plus an explicit note that the count moves with `doc/plans/` and is not a measurement of the tree. |
| A5 | Actionability | **No "review/consider/investigate" entry.** Every gap already carried a concrete `path`, a concrete change and an observable *Done when*. Two *Done when*s were observable but under-specified: G4's would have been satisfied by the existing `affected_files` fallback (it asked only for "a non-zero `modified` count"), and G5's ("returns a non-empty value") did not say what the current failure actually is. | G4's *Done when* now requires a fixture whose `realized_footprint` is **disjoint** from its `affected_files`, so the fallback cannot satisfy it. G5's now names `status: success` vs `field_not_found` as the observable. G10 written to the same bar. |
| A6 | Severity and topic | **One mis-severity.** G6 was rated `low`, but it is a false claim in shipped documentation — `medium` by the stated calibration, and the same class as G3, which is rated `medium`. It is also the load-bearing premise of D2's worked case, not a cosmetic inconsistency. All other severities hold: G1 `high` (confirmed by execution — the documented command hard-errors), G2/G3/G4/G5/G7 `medium`, G8/G9 `low`. All `Topic` values check out against the owning surface; G10 placed under `measurement/metrics` so it groups with G8, the other D1 coverage/publication gap in the same file. | G6 raised `low` → `medium` with the reason recorded inline. |
| A7 | Coverage | **No hole.** All five deliverables carry a verdict and a detail section; out-of-scope compliance is checked exclusion by exclusion against `git show 0e7f644 --name-status`; report accuracy is itemised; both declared residue items are re-checked and both re-derive as accurate. The one coverage weakness was not a missing deliverable but a missing *finding* — the `reads`/`destroys` vocabulary — now added. | Correctness review item 7 and G10 added; gaps.md intro re-counted nine → ten and its third cluster rewritten to name the canary failure. |
| A8 | Internal consistency | **Consistent.** The overall verdict CONFIRMED WITH GAPS follows from the rows (3 CONFIRMED, 2 PARTIAL). Every actionable finding in the Correctness review maps to a gap (item 1→G1, 2→G2, 3→G4+G5, 4→G3, 5→G6, 6→G9, new 7→G10) and every gap traces back (G7 from the D3 verdict + Test adequacy, G8 from the D1 caveat + Report accuracy). No orphan in either direction. | Item 7 → G10 added to preserve the mapping; gap count reconciled in the intro. |

**Residual doubt:** The largest remaining unknown is the one both this audit and the run report already
name honestly — D3's and D4's literal *Done when*s are end-to-end assertions about a real finalize run,
and neither is reachable from a clone whose `.plan/` tree does not exist. A further round with a real
archived plan directory could still find that the capture→resolve→recall path, though correct tier by
tier, never fires in production for a reason none of the unit fixtures model; G1 is exactly that shape of
defect found at the one seam that *was* reachable, which raises rather than lowers the prior that a
second such seam exists between `branch-cleanup`'s capture call and the archived-plan consumers. Second,
I checked the finalize *ordering* vocabulary but did not audit whether the two steps declaring `destroys`
are themselves correctly ordered against every step that reads what they destroy — G10's fix would
surface that, and I would expect it to find at least one real violation.

**Verdict on the audit:** SOUND AFTER CORRECTION — every gap it raised is real and its two mutation
proofs both reproduce, but it missed that D1's coverage guard had already failed silently, and it
overstated G4's blast radius and G6's severity.

---
name: plan-marshall-manage-config
...
| `interaction-mode` | `get`/`set --field interaction_mode --value {basic\|advanced\|expert}` (whitelist-guarded read/write of the top-level `interaction_mode` scalar preference, a sibling of `plan`: absent key falls back to the `advanced` default; an out-of-schema value fails closed. See `standards/interaction-mode.md` for the mode-to-behaviour mapping.) |
...
|-------|------|---------|---------|
| `auto_route_recipe` | bool | `true` | Whether a high-confidence Tier 1 recipe match (top confidence `>= auto_route_recipe_threshold`) auto-routes to the matched recipe without prompting. `false` proposes the ranked matches via `ask_question` first. |

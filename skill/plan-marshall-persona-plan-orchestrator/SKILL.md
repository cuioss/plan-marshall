...
9. **Decision surfacing.** Genuine forks — decisions with materially different downstream consequences that the ledger cannot resolve — are surfaced to the operator via `question`, with the outcome logged as an interaction. Routine sequencing choices are decided and logged, not escalated.
...
```text
Call the `read` tool with `{ filePath: "standards/orchestration-model.md" }` before continuing.
```

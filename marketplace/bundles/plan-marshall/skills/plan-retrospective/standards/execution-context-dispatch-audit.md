...
- **`ran_inline` is an UPPER BOUND on inline execution, never proof of it.** The bucket means *a recorded zero token attribution* — a zero somebody MEASURED. A DISPATCHED step whose `<usage>` tag was captured as zero produces one too, so reading the bucket as "these steps ran inline" over-claims by exactly the size of that population. What it does rule out is an absent, unreadable, or explicitly-`unmeasured` record, all of which land in `no_evidence`. Never quote `ran_inline` as evidence a step ran inline; quote it as the ceiling on how many could have. ⛔ And do NOT read the converse either: since the writer records an omitted flag as `unmeasured`, an inline step whose caller measured nothing lands in `no_evidence`, so a small `ran_inline` is not evidence that few steps ran inline.

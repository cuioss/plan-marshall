...
### Two populations in one note

An aggregated note carries two numbers over two different populations, and each is named by the population it measures:

| Number | Population | Where it comes from |
|--------|------------|---------------------|
| `N reference(s) suppressed` | Source **references** | The sum of each suppressed candidate's [`component_refs.occurrences`](module-discovery.md#component_refs-optional-extension-specific), which defaults to `1` |
| `(+N more)` after the sample | Distinct **candidates** | The suppressed candidates beyond `NOTE_SAMPLE_LIMIT`, which is the population a sample is drawn from |

The split exists because `component_refs` elements are deduplicated on the `(target_bundle, dep_type, resolved)` triple, so one candidate can stand for many source references. Counting candidates and labelling the result "reference(s)" reports a number of one population under the name of another; the two agree only when every element stands for exactly one reference. `DerivationResolverBase._aggregate_notes` owns both numbers so every resolver reports them alike.

A resolver MUST NOT close the gap by emitting one suppression entry per reference — that re-inflates `component_refs` to the raw reference count the dedup clause exists to avoid, and makes the per-resolver report unreadable. It passes the multiplicity instead, as a `(description, occurrences)` pair. A resolver whose materializer does not track multiplicity passes bare description strings and each counts as one, which is the pre-existing rendering.
...
| Python distribution-name join | `pyproject` | B | `build-pyproject` (`BuildExtension(BuildExtensionBase, DerivationResolverBase)`) | Derives edges by joining each module's published distribution name (carried on `metadata.name`, filled by discovery from PEP 621 `[project] name`, Poetry `[tool.poetry] name`, or `setup.cfg` `[metadata] name` — the resolver reads the field, never the descriptor) against every module's `dependencies` strings (`name:scope`), compared in PEP 503 normalised form so the `-`/`_`/`.`/case variants of one distribution resolve to the same module. Scoped to modules the Python build system discovered — see [§ A name join must be build-system scoped](#a-name-join-must-be-build-system-scoped). |
...
**Why the markdown, python, documentation, and lsp joins are separate resolvers, not one.** All four are Axis-A, where the registration site is the bundle (see [§ 2 Discovery](#2-discovery)), so the split across `pm-plugin-development`, `pm-dev-python`, `pm-documents`, and `plan-marshall` is what makes per-edge provenance — was this edge a marketplace markdown reference, a Python import, a cross-document reference, or a parser-resolved symbol reference? — expressible at all. Collapsing them into one resolver would forfeit exactly that distinction.

That same one-per-bundle cardinality is what keeps `lsp` off `pm-dev-python`, whose Python-language ownership would otherwise have made it the obvious host: a second resolver there would have been stamped `python`, and its parser-resolved edges would have become indistinguishable from the AST-import join's. Where both derive the same pair they have corroborated rather than disagreed, and the union preserves that only because the two ids stay distinct. `plan-marshall`'s own registration site was unused, so `lsp` is hosted there.

**The resolver is inert in a project with no harvest, not absent from it.** `discover_derivation_resolvers()` applies no domain filter (see [§ 2 Discovery](#2-discovery)), and the host bundle is now core — so `lsp` registers in **every** consumer project, including every project whose discovered modules carry no `lsp_harvest` record at all. That is the intended null-on-absent shape rather than a leak: wherever a project has discovered modules and none of them carries a harvest record, `derive_edges()` reports `harvest-did-not-run` instead of a confident zero, so the resolver's presence contributes a *stated* absence where its absence would have contributed a silent one. A project with an empty module set is the one case that draws no note — there are no modules to report a harvest about, so the note would be noise rather than honesty. This is the capability-provision arm of [ADR-010](../../../../../../doc/adr/010-Domain_content_is_active_only_when_its_domain_is_active.adoc) — the capability is provided unconditionally and reports honestly on its own applicability, rather than being gated into invisibility — and it leaves [ADR-014](../../../../../../doc/adr/014-An_aggregation_over_N_independent_producers_carries_producer_identity_and_no_producer_suppresses_an_element_silently.adoc)'s producer-identity requirement intact, since every edge it does derive is still stamped `lsp` and no other id.

**The permanent consequence: core's one resolver slot is now spent on `lsp`.** Axis-A registration is one resolver per bundle, and the cardinality is structural rather than conventional (see [§ 2 Discovery](#2-discovery)). `plan-marshall` therefore cannot host a second derivation that has to stay distinguishable in `producers[]`; a future resolver of that kind needs its own bundle — the same constraint that keeps `lsp` off `pm-dev-python`, now binding core as well.

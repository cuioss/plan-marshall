...
An earlier adversarial pass on this document was terminated mid-edit by a session limit; its partial
corrections are the D1-verdict paragraph committed in `a90adeb`. This pass therefore re-derived the
document from scratch rather than continuing from it, and treated the partial edit as unverified
input like any other claim — one defect below is a consequence the interrupted pass left behind.

| # | Attack | What was found | Correction applied |
|---|---|---|---|
| 1 | A1 | **No false positive.** All eight gaps re-derived at their cited `path:line` and all eight reproduce: G1 (probe → identical `content_search` entries, comparison `True`), G2 (probe → stale row on call 2), G3 (`cache_key = tuple(sorted(module_names))` read at `_architecture_core.py:1176`), G4 (guard-deletion mutation → test file still green), G5/G6 (probe → `files_scanned: 2`, two identical `unreadable` entries), G7 (no leaf artifact anywhere in `test/plan-marshall/manage-architecture/`), G8 (GitHub API → eight commits). | None needed on existence. |
| 2 | A1 | **Five stale citations.** `135-remove-lsp-query-facade/plan.md:52-56` (introduced by the interrupted pass) does not contain the quoted Goal text — it is at `:56-61`, and the quote's second sentence fell outside the cited range entirely. `plan.md:20-61` for "states the goal" spans the title too. G7's `plan.md:154-157` starts on a blank line. `unreadable[]` cited at `:1233,:1237` — the OS-error append is at `:1236`; `:1237` is the `continue`. `test_search_content.py:540-667` over-runs a 666-line file. | All five corrected in place. |
| 3 | A2 | **Two false negatives.** (a) The `path_attribution` entry (`_cmd_client_handlers.py:236-242`) carries no `derived_count`, while `client-api.md:1383-1390`, the handler docstring `:172-176` and `code-intelligence.adoc:262` all document a three-state shape keyed on that field — so "ran and claimed nothing" and "ran and claimed fifty" are the same payload. (b) `client-api.md:572`, cited by the audit as evidence the surviving doc surface is "consistent with the code", itself says `capabilities` reports `derivable`/`not_derivable` for **content search**, which it never does. | (a) filed as new gap **G9** and as Correctness-review defect 4 and D2 Defect 3; (b) folded into G1's action and *Done when*, and added as the fourth D5 doc-vs-behaviour mismatch. D2's and D5's verdict text updated. |
| 4 | A2 | **Two suspected fail-open shapes checked and absent**, recorded so a later round need not re-check: `resolver_count` is `count_dispatched(...)` (`_cmd_client_query.py:412`, `:942-957`), excluding only `STATUS_NOT_DISPATCHED`, so `producer_count` can never exceed `len(producers)`; and `resolve_path_attribution`'s `except ImportError: return None, []` (`_architecture_core.py:1173-1174`) degrades toward `not_derivable`, the conservative direction. | Added to the Correctness review's closing paragraph. |
| 5 | A3 | **All four mutation sweeps re-run, all reproduce.** D2 constant-`'derivable'` → 2 failed / 8 passed, naming exactly `test_empty_envelope_reports_no_capabilities_not_false_ones` and `test_module_edges_not_derivable_when_no_resolver_ran`. D4 M1 (drop `re.IGNORECASE`) → 2 failed / 18 passed. D4 M2 (`file_count` → `len(results)`) → 1 failed / 19 passed. D3 guard-block deletion → 3 passed **green**, with `grep -c "Underivable guard"` = 0 at the moment of the run, confirming the vacuity claim. Every mutation applied and reverted as a targeted single-line/single-block edit from a scratchpad snapshot; `git checkout`/`restore`/`stash` never used; `git diff` clean for my lines after each revert. | Test-adequacy and per-deliverable "Checks run" updated with the re-taken figures. |
| 6 | A3 | **The audit's memo probe had no positive control** — an equal result on two calls is also what a *correctly uncached* run returns if the injected attributor never took effect. Re-ran with a third call after `_PATH_CLAIM_CACHE.clear()`: the row flips to `derivable, producer_count 1, producers ['probe-attributor']`, pinning the memo as the cause. | Control added to D2's "Checks run". |
| 7 | A3 | **The D3 vacuity claim was under-stated**, resting on one test file. A whole-tree sweep of `test/` for `UNDERIVABLE` returns hits only inside `test_feasibility_underivable_guard.py` itself, so no test anywhere reads the shipped guard. | D3 "Found" strengthened. |
| 8 | A4 | **A half-applied edit left the document self-contradicting.** The interrupted pass corrected D1 to "26 top-level subcommands … plus 10 `enrich`" but left "~30 registered subcommands" in the *Declared residue* section. Both figures re-derived from the `subparsers.add_parser` / `enrich_subparsers.add_parser` calls: 26 and 10 are correct. | Residue section aligned to 26 + 10. |
| 9 | A4 | **"enumerates six verbs" is wrong.** `report-01.md:48-56` is a five-row table naming eight *distinct* existing verbs (`module`, `impact`, `find`, `resolve` as targets; `path`, `impact`, `find`, `which-module`, `graph`, `neighbors` on the residue row). | Corrected in D1 and in the residue section. |
| 10 | A4 | **Every other number re-derived and correct:** 18 changed files (`git show --stat 8d5055f` and GitHub `changed_files: 18`); 8 commits with the exact eight SHAs, all carrying the trailer (GitHub `get_commits`); `merged: true`, `merged_at 2026-08-13T11:02:03Z`, `merged_by cuioss-oliver`; D3's 7-line footprint (`git show --numstat`); 10 / 3 / 20 baseline test counts; 5 D4 tests; `git branch --contains` puts both `8d5055f` and `064b387` on `main`; all eight branch SHAs unresolvable locally. Every quoted doc string checked verbatim against its file — one quote elided `(--ignore-case, file_count)` without marking it. | Elision restored in the 135 quote; a line added recording the GitHub-side figures. |
| 11 | A5 | **One unexecutable *Done when*.** G7's "with the plan's constraint marked discharged" named no artifact and no location, so a later run could not tell when it was done. | Rewritten to name both concrete arms (a committed leaf-obtained payload, or a named paragraph in `client-api.md` § `capabilities`). G1–G6, G8 already carried a path, a change and an observable condition. |
| 12 | A6 | **All nine severities re-checked against the calibration; none changed.** The contested one is G5: "a measurement misreports" reads as *high*, but duplicate attribution can only inflate `files_scanned`, which is zero exactly when the distinct count is zero, so the documented `files_scanned > 0` conjunction never flips — the misreport is in the number quoted, not in the decision. Topics re-checked against owning surfaces; G8 stays `plan-lane-contract` because its subject is the run record, not the query surface. | G5's severity reasoning written into the entry so it is visibly deliberate. |
| 13 | A7 | **Coverage complete.** All five deliverables carry a verdict and a detail section; out-of-scope compliance, collateral, report accuracy, declared residue and method are all present. No deliverable is silently unmentioned. | None. |
| 14 | A8 | **Two findings had no gap entry.** `report-01.md:69`'s "per-call (uncached)" and `:104-105`'s "All five deliverables verified as implemented-as-specified with tests" were both marked as defects in § Report accuracy but appeared nowhere in `gaps.md`, so a later fix run would have left them standing. | G8 widened from the commit count alone to all three report claims, and both § Report accuracy bullets now name G8. Every gaps.md entry traces to a verification.md finding and vice versa; the CONFIRMED WITH GAPS verdict follows from the D2/D3 PARTIAL rows. |
| 15 | A8 | **One stale header.** The document was written at `61a43e5` but re-derived at `a90adeb`. `git diff --name-only 61a43e5 a90adeb` touches only `doc/plans/**`, so no shipped file this audit cites moved. | Header records both tree states and the fact that nothing cited changed between them. |

**Residual doubt:** the likeliest next find is more of the G9 shape — entry-key sets in `client-api.md`
that name a field the handler does not always emit. Two such mismatches surfaced in the one section
examined closely (`§ capabilities`), which is weak evidence that the payload documentation across the
other twenty-odd verbs has not been checked field-by-field against its handler. Three things stay
genuinely unchecked here and are not defects of the audit: the `./pw verify` figure (excluded by the
brief), the D5 cold-read verdict (the sub-agent's artifact is not in the tree and cannot be
reconstructed), and real dispatched-leaf behaviour (G7 — the environment cannot be synthesised in
this session, which is exactly why it is filed rather than resolved).

**Verdict on the audit:** SOUND AFTER CORRECTION — every gap it filed is real and every mutation it
claimed reproduces, but it missed two doc-versus-payload defects, left five stale citations and one
self-contradicting count behind an interrupted edit, and filed two of its own report-accuracy
findings nowhere.

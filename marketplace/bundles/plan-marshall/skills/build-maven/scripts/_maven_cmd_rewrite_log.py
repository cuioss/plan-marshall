...
#: The three fail-closed verdicts. ``observed`` is the only state carrying
#: findings; ``not_observed`` and ``domain_inactive`` are distinct non-clean
#: skips, never a vacuous "clean".
...
def consume_rewrite_log(log_file: str, *, resolve_verb=resolve_domain_verb, dispatch=dispatch_parser) -> dict:
    ...
    Branches on the three fail-closed states:
    ...
    - It reached ``rewrite:run`` but no active domain provides the
      ...
      a false clean).
    - It reached ``rewrite:run`` and the verb resolves → dispatch the domain
      parser and surface its structured findings (``observed``).

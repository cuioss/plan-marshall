...
Before the per-step stamping below, the composer walks every task's `verification.commands` and routes each build command by its resolved `execution_tier` (see [standards/decision-rules.md](standards/decision-rules.md) § "execution_tier Routing" for the full predicate). For `orchestrator`-tier commands the verb maps to a bare phase-5 step ID: the four canonical verbs route through the `_VERB_TO_PHASE_5_STEP` fast path (`quality-gate` / `verify` / `module-tests` / `coverage`), and every other verb whose `verify:{verb}` generalization **resolves** routes to that bare step — the command is removed from the task, so no leaf runs it inline. Each non-canonical routed verb is named in its own `decision.log` line (canonical prefix `(plan-marshall:manage-execution-manifest:compose) execution_tier routing`), in addition to the summary line carrying `mutated_tasks` and the final step list.

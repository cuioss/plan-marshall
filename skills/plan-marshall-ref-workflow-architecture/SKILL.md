---
name: plan-marshall-ref-workflow-architecture
...
| [dispatch-logging.md](standards/dispatch-logging.md) | Standardized dispatch log line | Seam `[DISPATCH]` emission contract — five literal fields (target, level, role, workflow, plan_id), emitted by `effort resolve-target` itself (per firing) when passed the dispatch context (`--workflow`/`--plan-id`/`--caller`), plus the paired decision-log record and the corroboration limit, positive example + anti-pattern |
...
| [execute-task-skills.md](standards/execute-task-skills.md) | Execute-task skills | Profile dispatch, shared workflow, extensibility |

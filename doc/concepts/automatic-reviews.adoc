...
All three external roundtrips declare `requires: [ci-complete]` in their workflow-doc frontmatter; the phase-6-finalize dispatcher resolves the precondition before each body runs. The default order is `ci-verify` → `automated-review` → `sonar-roundtrip`, with `ci-verify` placed immediately after `create-pr` so the CI verdict is already triaged before the reviewer-bot and Sonar consumers run. Two precondition modes are in use: `ci-verify` runs in **`consume-failures`** mode (failing checks become structured findings, no short-circuit); `automated-review` and `sonar-roundtrip` keep the default **`strict`** mode (a non-success CI marks the step `failed` and the dispatcher continues). Triage budgets are independent of CI queue depth.
...
Workflow: link:../../marketplace/bundles/plan-marshall/skills/plan-marshall/workflow/verification-feedback.md[`plan-marshall/workflow/verification-feedback.md`]. **One workflow, twelve producer modes** — five base (`build-runner`, `sonar`, `pr-comment`, `plugin-doctor`, `pr-state`) plus the seven `ci-verify-{build,policy,timeout,cancelled,action-required,stale,missing}` subtypes.

...
- [ ] For each comment:
      - Real issue + sensible fix → apply, commit, push; reply via:
        `... pr prepare-body --plan-id refactor-06-developer-workflow --for edit --slot reply-<n>` (write text), then
        `... pr reply --pr-number $PR --plan-id refactor-06-developer-workflow --slot reply-<n>`

...
`affected_files`, `outline_affected_files`, and `affected_files_count` are a **frozen declaration**. They are captured once, at outline time (phase-3), from the solution outline's `**Affected files:**` blocks (`manage-solution-outline` `_extract_affected_files`) and mirrored into `references.json::affected_files`; they are **not re-derived when scope moves during execution**. They express a *lower-bound estimate of the surface a plan expects to touch* — a **declaration of intent**, and **never a record of what was actually touched**. A plan that legitimately widens its footprint mid-execution leaves them stale, and this holds regardless of whether the widening was sanctioned: approval of a scope change is not the same event as recording it, so a "declared surface" figure can under-count a fully-approved change. Treating that figure as if it were the record is the false signal this distinction exists to prevent.

...
The *plan-scoped* audit trail is strictly local. Storage lives under `.plan/local/{plans,archived-plans}/{plan_id}/`, which is gitignored. Completed plans move to `archived-plans/` via `/plan-marshall action=cleanup`; archived plans are read-only from the retrospective's perspective. Nothing under `.plan/local/` is ever surfaced to remotes by Plan Marshall itself — if a retrospective is worth sharing, the developer copies `quality-verification-report.md` out by hand. This is deliberate: the trail records local LLM behaviour, model usage, decisions, and per-developer state, and that content should not propagate to a shared repository.

The epic ledger is the deliberate exception, and it runs the other way. `.plan/orchestrator/{slug}/` — `epic.md`, `status.json`, the workstream charters, the staged plan specs, the landing-analysis records, and `history.md` — IS versioned and pushed, because an epic's decomposition and its landing record are project state that must survive a clone and travel between checkouts. Only its `logs/` subtree stays local, on the per-developer-state reasoning above.
...
* xref:orchestration.adoc[Concepts › Orchestration] — the orchestrator store's log-everything posture: the same record-as-side-effect discipline applied to epic-level decisions and reconciliations under `.plan/orchestrator/{slug}/logs/`, the one git-ignored subtree of an otherwise git-tracked epic store.

# Cloud bridge — code-intelligence-substrate
...
| This epic's cloud plans (the tree IS the state) | `doc/plans/code-intelligence-substrate/` | yes |
| This epic's cloud plans | `doc/plans/code-intelligence-substrate/{cloud-plan}/` | yes |
| The run contract a cloud session follows | `.claude/skills/cloud-plan-lane/SKILL.md` | yes |
| This epic's orchestrator ledger | `.plan/local/orchestrator/code-intelligence-substrate/` | **no** |
...
- **Never** write epic state into `doc/plans/`, and never expect a cloud session to have written
  anything into `.plan/local/orchestrator/code-intelligence-substrate/`. Git is the only shared medium.

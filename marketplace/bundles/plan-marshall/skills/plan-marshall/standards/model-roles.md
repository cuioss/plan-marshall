...
| `plugin-doctor` | Plugin / agent / command / skill diagnostics with `scope` as a runtime input (`agents`, `commands`, `skills`, `scripts`, `metadata`, `skill-content`, `skill-knowledge`, `test-conventions`, `marketplace`, `plan-marshall`). Absorbs the legacy `tool_coverage_analysis`, `component_analysis`, `inventory_analysis` keys. | Developer tool outside any plan AND `project:finalize-step-plugin-doctor` in the plan-marshall meta-project |

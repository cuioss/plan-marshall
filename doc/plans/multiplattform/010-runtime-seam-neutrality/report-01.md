...
| S4 | `OpenCodeRuntime.metrics_capture` reports `status: success` for an explicit `--total-tokens` while persisting nothing. The Claude implementation writes the token cursor and calls `manage-metrics end-phase` before succeeding; the OpenCode one reaches no boundary at all, so the count is acknowledged and lost | **(b)** Pre-existing; this branch rewrote the docstring that asserted the false behaviour and has now corrected it. Fires only when a caller passes an explicit count on an OpenCode project, and OpenCode is not a tested runtime. **Not fixed here** because the persistence boundary is target-neutral in substance but lives in `claude_runtime`, so wiring it up means relocating a helper across the target boundary — a plan, and one that would otherwise add exactly the coupling this epic removes. Corrected in three places: the ABC now states persistence as a **requirement** (persist before `success`, or decline with `no-op`) and names this as a known violation rather than asserting the behaviour; the OpenCode docstring says the value is not persisted; and `contract.md` warns that the OpenCode success block does not mean the count was stored, pointing at the absent `cursor_updated` as the distinguishing field. Inventory row added | Raised by CodeRabbit on PR #1291, confirmed by reading both implementations. Its re-review then caught that the **ABC** still claimed storage — an n−1-of-n miss in my own first fix, which corrected the concrete runtime and the survivor record but left the target-neutral contract asserting the behaviour |
...
|---|---|---|
| `coderabbitai` | **reviewed** on the second push, after a rate-limit window closed the first attempt. **11 actionable comments**, merge risk 🟡 Moderate. Four were real defects confirmed by reading the implementations and are fixed: the install-hook target mismatch, the `push-title-token` no-op/success contract, the unclosed-TOON-fence drop, and the inventory row misassigned to plan 010. Two more were real and are recorded as residue with the reason they are plans rather than review fixes (a single registration record; a content-level pin for the terminal-title inventory). The rest overlap residue this report already carried. On re-review of the fixed head it confirmed all six fixes present and correctly covered, dropped **Merge Risk 🟡 Moderate → 🟡 Low**, and found **one further defect**: the ABC's `metrics_capture` still stated that an explicit count "stores that value directly" — true of Claude, false of OpenCode. That is an n−1-of-n miss in this run's own S4 fix, which corrected the concrete runtime and the survivor record but left the target-neutral contract asserting the behaviour. Fixed. | yes |
| `sourcery-ai` | **declined on size** — "your pull request is larger than the review limit of 150000 diff characters". Measured: `git diff origin/main...HEAD \| wc -c` = **160 040**, over the limit by ~7%. Its `Sourcery review` check re-reported `skipped` on every subsequent head, confirming the refusal is structural rather than a one-off. Separately, on PR #1292 it declined for a *different* reason — a weekly 500 000-character account limit — so even a small PR would not have been reviewed by it at this point in the session. | no |
| `cuioss-review-bot` | **did not run on this PR** — no review, no comment, no check run, across repeated polls. It is **not** broken: the same bot ran on PR [#1292](https://github.com/cuioss/plan-marshall/pull/1292) minutes later in this same session, posting a "PR Reviewer Guide" with "No major issues detected". Why it skipped #1291 is unexplained and is **not** guessed at here. | n/a |

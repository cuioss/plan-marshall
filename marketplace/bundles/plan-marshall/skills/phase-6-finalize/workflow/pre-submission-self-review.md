...
**Stop-path confirmation checks — convergence signal and deterministic evidence.** Before recording `done`, confirm the close rests on BOTH legs the stop needs, not on the empty findings list alone:
...
- **Deterministic evidence** — the selected clean verdict must match the round's deterministic surface output: `counts.total == 0` selects nothing-to-check; `counts.total > 0` with empty findings selects examined-clean; `delta_coverage.files_with_candidates == 0` over non-zero `files_in_scope` selects zero-observation (with the deviation log). A verdict inconsistent with the evidence — examined-clean claimed over `total == 0`, or any clean verdict over an absent `counts` block — does not close; re-derive the verdict from the surface TOON rather than recording `done`. The zero-generator fallback is carved out (no surface ran; the not-run verdict carries no evidence claim).

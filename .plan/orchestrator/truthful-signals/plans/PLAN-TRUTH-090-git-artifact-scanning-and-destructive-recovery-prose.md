...
>
> ⚠ **Known already-closed ids cited by this plan (1):** `310/G1`.
...
> It names `report-01.md` under `310`. Those reports were ingested
...
**DA — `switch-and-pull --plan-id` resolves to the WORKTREE, contradicting its documented
main-checkout-root contract.** *(folded 2026-08-23 from PR #1332 / L6)*
The verb documents `--plan-id` as resolving the **main checkout root** via `marketplace_paths`. Invoked
from a plan running in a worktree it resolved to the **worktree** and refused:
`pre_sha=ad857be8, 'main is already used by worktree'`. The `--project-dir` escape hatch was required.
⭐ **The context is what makes it sharp:** the call site was itself repairing a stale-base defect — local
`main` at `92d61b52` while `origin/main` was at `e8324d24`, which inflated `compute-footprint`'s three-dot
`main...HEAD` diff from 20 files to **181**. So a verb whose purpose is to keep the main checkout current
**could not be aimed at the main checkout by its documented flag, at the exact moment the main checkout
was stale.**
⚠ The failure is not silent — `main is already used by worktree` is diagnosable — but the caller must
already know to reach for `--project-dir`, and the documented flag is the one that fails in the situation
the verb most needs to handle.
**Remedy, option 1 preferred:** make `--plan-id` resolve the **main checkout root** as documented, reading
the plan's status through the main-anchored handle rather than the cwd-keyed `base_path()` — the same
correction `delete-plan`'s lesson carry-back applies for the lessons store. *"A verb that pulls `main`
wants the checkout that has `main` checked out."* If worktree resolution is instead intended, correct the
documented contract and name `--project-dir` as the main-checkout form, so the escape hatch is documented
rather than folklore.
⚠ **Impact is every phase-5+ caller**, since cwd is pinned to the worktree from phase-5 entry until
finalize cleanup.
⭐ **Same resolver-face family as PLAN-TRUTH-100 D6(c)/(d)** — cwd-keyed resolution answering for a
main-anchored question. Read that deliverable's clauses before fixing this one; the two should not invent
...
## Claim Labels
- verdict: unverifiable | checked_at: 5f972ac15 | by: truthful-signals/cleanup | rescoped: n/a | evidence: Table-form claim section, section-scope address. Sampled 1 of 6 rows at this sha, NOT exhaustive. The 140/G1 artifact reference has DRIFTED: the quoted single line _split_ignored(get_gitignored_files(root) if respect_gitignore else set()) no longer exists in git-workflow.py; scan_artifacts at :700 now carries an if/else block at :739-751 plus a new gitignore_resolved degradation path that the claim does not mention. The substance (respect_gitignore is the only exclusion at the scan root) still appears to hold, but the claim must be re-read against the restructured symbol before it is used. Remaining rows not reached.

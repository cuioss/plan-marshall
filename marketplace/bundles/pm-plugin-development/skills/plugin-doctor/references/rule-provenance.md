...
| `MANAGE_STATUS_PROSE_CONFLATION` | content | `_analyze_toon_prose_status_conflation.py` | Plan `new-standards-docs-must-cross-reference-siblings-a` — inline-code prose of the form `status: {specific_code}` (where `{code}` is neither `error` nor `success`) collapses the two-tier TOON error envelope into a single field, misdescribing the contract defined in `pm-plugin-development:plugin-script-architecture/standards/output-contract.md` § "Operation Failure". The canonical shape is `status: error` + `error: {code}`; a reader who copies `status: plan_not_found` from prose will write code that branches on the wrong key. Detection scope: inline-code spans in plan-marshall bundle markdown only; fenced code blocks and plain prose are exempt. |

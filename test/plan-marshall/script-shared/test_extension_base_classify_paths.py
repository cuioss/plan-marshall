...
#: The inputs the default must answer identically — ordinary paths, no paths,
#: and paths a caller should never be able to make the no-op raise on.
_DEFAULT_CLASSIFY_INPUTS = [
    ['scripts/foo.py', 'README.md'],
    [],
    ['', '../etc/passwd', '/abs/path', 'é'],
]

_DEFAULT_CLASSIFY_IDS = ['ordinary-paths', 'no-paths', 'adversarial-paths']


@pytest.mark.parametrize('paths', _DEFAULT_CLASSIFY_INPUTS, ids=_DEFAULT_CLASSIFY_IDS)
def test_default_classify_paths_claims_nothing(paths: list[str]):
    assert _MinimalExtension().classify_paths(paths) == _EMPTY_FOUR_ROLE_CLAIMS
...
def _claims(**overrides) -> dict[str, list[str]]:
    """The four-role claim dict with the named roles filled and the rest empty."""
    return {**_EMPTY_FOUR_ROLE_CLAIMS, **overrides}


#: ``(paths handed to the override, the full four-role claim it produces)``. The
#: WHOLE dict is stated per row rather than the one role under test, so a row
#: also pins what the override did NOT claim — which is how the unclaimed row
#: proves an unmatched path lands in no role at all (the aggregator's ``unknown``
#: bucket owns it, not the extension).
_SUBCLASS_CLAIM_CASES = [
    (
        ['scripts/foo.py', 'scripts/bar/baz.py'],
        _claims(production=['scripts/foo.py', 'scripts/bar/baz.py']),
    ),
    (
        ['test/foo_test.py', 'tests/bar_test.py'],
        _claims(test=['test/foo_test.py', 'tests/bar_test.py']),
    ),
    (['pyproject.toml', 'uv.lock'], _claims(config=['pyproject.toml', 'uv.lock'])),
    (['README.md', 'docs/foo.adoc'], _claims(documentation=['README.md', 'docs/foo.adoc'])),
    (['scripts/foo.py', 'mystery.xyz'], _claims(production=['scripts/foo.py'])),
    (
        ['scripts/foo.py', 'test/foo_test.py', 'README.md', 'pyproject.toml'],
        _claims(
...
_SUBCLASS_CLAIM_IDS = [
    'production-sources-under-scripts',
    'test-sources-under-test-and-tests',
    'config-descriptors',
    'documentation-suffixes',
    'an-unmatched-path-is-claimed-by-no-role',
    'a-mixed-input-list',
]


@pytest.mark.parametrize('paths,expected', _SUBCLASS_CLAIM_CASES, ids=_SUBCLASS_CLAIM_IDS)
def test_subclass_override_claims(paths: list[str], expected: dict[str, list[str]]):
    assert _ClassifyingExtension().classify_paths(paths) == expected
...
#: ``(path, role, build_class)`` for the base default mapping. ``documentation``
#: is deliberately a ``none`` row rather than a class of its own: it is not a
#: build_map role and has no build owner, so the default treats it exactly as it
#: treats the two unmatched-role rows beneath it.
_DEFAULT_BUILD_CLASS_CASES = [
    ('scripts/foo.py', 'production', BUILD_CLASS_PROD_COMPILE),
    ('test/foo_test.py', 'test', BUILD_CLASS_TEST_RUN),
    ('pyproject.toml', 'config', BUILD_CLASS_BUILD_CONFIG_FULL),
    ('README.md', 'documentation', BUILD_CLASS_NONE),
    ('whatever', 'unknown-role', BUILD_CLASS_NONE),
    ('whatever', '', BUILD_CLASS_NONE),
]

_DEFAULT_BUILD_CLASS_IDS = [
    'production',
    'test',
    'config',
    'documentation-has-no-build-owner',
...
class _NoDomainKeyExtension(_RouteExtension):
    """Declares routes but resolves to no domain key, so they can be filed nowhere."""
...
class _BadRoleExtension(_MinimalExtension):
    """Declares a route whose role is outside the closed BUILD_MAP_ROLES set."""
...
_NO_ROUTE_EXTENSION_SETS = [
    [],
    [_MinimalExtension()],
    [_NoDomainKeyExtension()],
    [_BadRoleExtension()],
]

_NO_ROUTE_EXTENSION_IDS = [
    'no-extensions-at-all',
    'an-extension-declaring-no-routes',
    'an-extension-with-no-resolvable-domain-key',
    'a-route-whose-role-is-not-in-the-vocabulary',
]

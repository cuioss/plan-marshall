...
`.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/560-documentation-surface-truthfulness.md`. It was ingested into this ledger on
2026-08-22 as `PLAN-CIS-054` and it will be EMITTED INTO THE PLAN-MARSHALL LIFECYCLE, not into the
cloud lane.
...
>
> ⭐ **The pairing is causal, not filing convenience.** D1–D6 correct documents that state figures;
> D7–D11 SETTLE the figures those documents state. Split across two plans, every figure the
> measurement moved would leave a stale sentence standing in a document the other plan owns, until
> that plan ran — the exact drift class this spec exists to close, reproduced by its own scheduling.
> Merged, a figure and the sentence quoting it move in one act. D4 (the measurement and retrospective
> documentation) is the specific bridge.
>
> ⚠ **One refutation absorbed in the same act.** D6's gating derivation asserted *"That directory IS
> absent entirely"* of the epic's landed plan records. That is FALSE — the archive holds 46
> directories, 37 with a `gaps.md`, re-derived at HEAD `53ab7dd2e`. The sentence is deleted and the
> corrected derivation is now mandatory rather than precautionary; see the ⛔⛔ block at D6.
...
**ELEVEN deliverables.** D1–D6 are the documentation sweep, grouped by **owning bundle and shared
mechanism** rather than by severity: a deliverable that fixes one wrong mechanism at all its sites is
worth more than one per gap. Every gap in scope is named in § Gap coverage, by source plan and gap id.
**D7–D11** (merged 2026-09-12 from the retired `PLAN-CIS-056`) settle the exploration-split figures
that D4's documents quote — D7 is their gating population read.

⛔ **The two halves are ORDERED: settle the figures (D7–D11), then write them down (D4).** A document
corrected to a figure this plan is about to move is corrected twice, and the second correction is the
one that reveals the first was guesswork.
...
3. **The refusal does not exit non-zero** (`040/G3`, `040/G4`, `040/G5`, `040/G6`, `040/G7`).
   ...
   *Done when:* a whole-tree search for `non-zero exit` in the `tools-script-executor` skill returns
   ...
   ⚠ **Surface contention noted 2026-09-22 at cleanup, recorded rather than scheduled around.**
   `test/plan-marshall/tools-script-executor/test_generate_executor.py` has grown a NEW test,
   `test_dry_run_surface_stats_is_a_copy_of_the_shared_default` (`:3025`), that `PLAN-CIS-050` D1(e)
   must rewrite as part of its own fix. This item's edit is a comment correction elsewhere in the same
   file, not the same lines — the two plans touch the same file, not the same content — and under
   `parallelization_scope = 1` nothing runs concurrently regardless, so no sequencing constraint is
   owed beyond what already governs. Recorded so whichever of the two lands first is not surprised by
   the file having moved under it.
...
Forty-odd defects live in this epic's landed plan records — run reports, plan files, proposal
documents. They are records of past executions, not documentation of current state, and they are
corrected under a different rule from everything above.

⚠ **RE-EXPRESSED 2026-08-24 — READ THIS BEFORE THE GATING DERIVATION BELOW IT.** These records no
longer live under `.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/*/`; the collect has happened and that tree
is deleted from git. They are all present, and readable, at
`.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/{source-plan}/`.

⛔⛔ **The original gating derivation is now a VACUOUS-PASS TRAP, and it is disarmed.** As written it
said: list `.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/`; treat every absent directory as
**discharged-by-collection**; and *"if the epic directory is absent entirely, D6 is closed as
not-applicable on that evidence alone."* — so the rule as written would close D6 over a population of
forty-odd defects that were never examined, and report it as a clean discharge. That is this epic's
signature archetype reproduced inside the deliverable meant to correct records. **Do not run it.**

⛔⛔ **REFUTATION ABSORBED 2026-09-12 — the sentence that used to stand here, *"That directory IS
absent entirely"*, was FALSE and is deleted.** The 2026-09-05 re-grounding (row 13) read three
`cloud-runs/*/gaps.md` files directly and found them present; re-derived first-party at HEAD
`53ab7dd2e`, the archive holds **46 directories, 37 of which carry a `gaps.md`**. So the trap is worse
than the original wording implied and better than it feared: the not-applicable arm was never
reachable in the first place, because the evidence it claimed was absent is on disk in bulk. ⇒ **The
corrected gating derivation below is not a precaution against a hypothetical — it is the only
derivation with a population, and D6 must run it.** ⚠ Any deliverable that resolves a directory as
`discharged-by-collection` now owes the listing that shows it genuinely missing from those 46;
"absent" is a measured claim here, never an assumed one.

⛔ **Gating derivation, corrected.** List
`.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/`. For every source plan in
§ Gap coverage → D6, resolve its directory THERE. `discharged-by-collection` is now reachable only
for a source plan genuinely missing from the archive — record the listing as evidence, and state the
archive path you listed, so the population behind the verdict is named. **Do not reconstruct a
missing record from git history.**

⛔ **The corrections are not a repository deliverable.** `cloud-runs/` is git-ignored, so no
correction to it can appear in a diff, and the archive must not be edited in place — it is the
evidence the corrections are about. Per the provenance header, file D6's corrections as an epic inbox
`finding` message (`orchestrator inbox write --slug code-intelligence-substrate --kind finding`),
one entry per corrected claim quoting the false text and its correction, and reproduce them in the PR
body. The correction RULE below is unchanged and still governs what may be corrected.
...
### D7 — Population gate for the exploration measurement (gating; D8–D10 depend on it)

⚠ **MERGED IN 2026-09-12 from `PLAN-CIS-056` (retired) D0.** Before any measurement work, derive and
publish the corpus reachable from this checkout: how many archived plans carry a `metrics.toon`, how
many of those carry the per-phase exploration sub-source fields, and at which schema version.
**Publish the count with the command that produced it.** If the population is smaller than the schema
change that introduced those fields allows, say so and report what the measurement can and cannot
answer at that n — do not proceed to D9/D10 on a population that cannot support them. ⛔ A figure
without its denominator is the defect this epic exists to remove and is not permitted in this plan's
own output.

⛔⛔ **THIS IS THE SAME GATE `PLAN-CIS-050` D8 RUNS, over the same git-ignored
`.plan/local/archived-plans/` tree, and the two MUST NOT PRODUCE DIFFERENT NUMBERS.** Whichever plan
runs second **cites the first's published population and re-derives only what its own fields require**,
reporting any difference as a finding rather than quietly carrying its own count. Two independently
derived populations over one corpus is precisely the disagreement-between-instruments this epic keeps
finding; producing it here, in two plans that both exist to remove it, would be the archetype
reproduced by its own fix. ⭐ The last recorded walk found `exploration_index_answerable_bytes` in
**35** `work/metrics.toon` files (up from a cited 13) — a lead, not a fact. Re-derive.

### D8 — The per-phase exploration sub-source aggregator

⚠ **MERGED IN 2026-09-12 from `PLAN-CIS-056` D1.** Build the check that reads `index_answerable`,
`doc_residency` and `unattributed` **per phase** and reports the split with its population. This is
the instrument `PLAN-CIS-036`'s report claimed already existed. It is git-derivable and depends on no
corpus. Three requirements carried from that plan's audit (G3, G4, G7): the aggregator **MUST NOT**
pool phases into one figure; it **MUST** apply the schema-partiality read, so a phase whose fields are
absent reports `unmeasured` rather than a zero; and it **MUST** apply the re-entry guard, so a
re-entered phase is not double-counted.

### D9 — Settle or refute the exploration split

⚠ **MERGED IN 2026-09-12 from `PLAN-CIS-056` D3.** Run D8's aggregator over D7's population and publish
`index_answerable` / `doc_residency` / `unattributed` shares, plus exploration's share of tool-result
bytes, **each with its population**. Compare against the standing n=1 observations (index-answerable
15.9 %, doc-residency 65.2 %, exploration ≈ 77 %) and state, per figure, whether it is confirmed,
moved, or refuted. ⚠ The one prior partial result already contradicts the epic's assumption:
`2-refine` measured as the worst phase (3.3 % index-answerable / 90.7 % doc-residency), not
`6-finalize`. **Expect the ranking to move.**

⭐ **This is why the merge with a documentation plan is the right pairing rather than a convenient
one.** D9 and D10 SETTLE the figures; D4 above CORRECTS the documents that currently state them. Under
the retired split, the settling plan and the correcting plan were separate, so every figure this plan
moves would have left a stale statement standing in a document owned by another plan until that plan
ran. Merged, a figure and the sentence quoting it change in one act — which is this plan's whole
subject applied to its own output.

### D10 — Settle or refute the re-read multiple

⚠ **MERGED IN 2026-09-12 from `PLAN-CIS-056` D4.** `PLAN-CIS-040` shipped the currency correction and
**deliberately deleted** §6's numeric figures rather than restating them, leaving the
resident-context-times-turns model confirmed but its multiple unmeasured. Publish `resident_context`
and `turns` per phase over D7's population and derive the re-read multiple, stating whether the
standing 44.6× and the `cost(byte) ≈ 1.25 + 0.1 × turns_remaining` form hold. ⛔ The **model** is
already confirmed and independently re-derived twice — **do not re-litigate it.** Only the multiple is
open.

### D11 — Re-scope the residency instrument, and say what it can answer

⚠ **MERGED IN 2026-09-12 from `PLAN-CIS-056` D2.** `PLAN-CIS-039`'s D1 rests on
`exploration_doc_residency_bytes`, which is **a one-integer-per-phase proxy, not a per-document
consumption measure**. Establish from source what question that field can actually answer, and either
name the instrument that answers the per-document question or record that none exists and what it
would take. ⛔ **Do NOT re-run `PLAN-CIS-039` as written** — re-running it would measure the wrong
thing with a clean conscience.

⚠ **Ordered last on purpose, though it gates nothing.** It is the deliverable most likely to conclude
*"the instrument cannot answer this"*, and that conclusion is more useful once D9 and D10 have shown
what the existing instrument DOES answer. ⭐ `PLAN-CIS-039` is `parked` with PR #1149 — check its live
state at outline before scoping, since a parked plan's surface can move under this one.
...
- **Deciding where two configuration keys belong** (`220/G4`'s second half) and **where the
  ...
  Both are placement judgements about a contract's ownership. ⚠ **CORRECTED 2026-08-24** — the reason
  given was "per the lane's no-operator constraint", and that constraint does NOT bind here: a
  `/plan-marshall` run has an operator channel the cloud lane lacked. Recording a proposal stays the
  DEFAULT, because these two are out of scope either way. But where a placement judgement has
  materially different downstream consequences, escalate it with `AskUserQuestion` rather than
  deferring it — the original "does not make the call" was a limitation of the lane, not a considered
  property of this deliverable, and it should not silently outlive the lane it came from.
...
`.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/*/` — "D6 only, and only if the directories are present".
That tree is deleted from git, so **D6 writes no repository file** and this plan's repository surface
is the list above and nothing else. D6's corrections are filed as an epic inbox `finding` message and
reproduced in the PR body; see the re-expressed D6 gating derivation above, which also disarms the
vacuous "closed as not-applicable" branch the deletion would otherwise have triggered.


**Added by the 2026-08-31 inbox drain (PLAN-CIS-051 / PR #1370).**

One path this plan did not previously declare. `phase-3-outline`, `phase-4-plan`, `retro_sections.py`,
`compile-report.py` and `manage-solution-outline` were already declared, so the folds below add
surface at exactly one place. The same-act rule applies: this landed in the SAME edit that appended
§ Folded-in signals below.

- `marketplace/bundles/plan-marshall/skills/plan-retrospective/references/report-structure.md` —
  F-003: item 1 specifies a MANDATORY 3–5 sentence Executive Summary that nothing in the system has
  ever produced. Whichever way the producerless row is settled, this document's mandatory-content
  requirement has to be resolved with it.

**Added by the 2026-09-12 re-cut (absorbing `PLAN-CIS-056`, D7–D11).**

- `.plan/local/archived-plans/**` — **D7, D9, D10, READ ONLY.** The archived-plan corpus the
  population gate walks and the two settle-or-refute sweeps measure. ⛔ Git-ignored and outside the
  crawled inventory: a `search --content` zero over it is a coverage boundary, never a clean negative.
  Walk it directly and publish the walk. ⛔ **This plan writes nothing here.** ⛔ **`PLAN-CIS-050` D8
  reads the SAME tree for the same purpose** — see D7 for the must-not-diverge rule.
- `marketplace/bundles/plan-marshall/skills/plan-retrospective/scripts/analyze-logs.py` — **D8**, the
  per-phase exploration sub-source aggregator. ⛔ Contended with `PLAN-CIS-050` and `PLAN-CIS-052`;
  under `parallelization_scope = 1` this is a sequencing constraint, and the survivor re-grounds.
- `marketplace/bundles/plan-marshall/skills/manage-metrics/standards/data-format.md` — **D8**, the
  sub-source field definitions and their schema-partiality contract. ⛔ Contended with `PLAN-CIS-050`.
- `marketplace/bundles/plan-marshall/skills/manage-metrics/scripts/manage-metrics.py` — **D8**, the
  reader side of those fields. ⛔ Contended with `PLAN-CIS-050`.
- `.claude/skills/audit-archived-plan-retrospectives/scripts/audit.py` and its `checks/` documents —
  **D8, D9**, the retrospective auditor check that must not pool phases, must report `unmeasured`
  rather than zero on absent fields, and must apply the re-entry guard. ⭐ Already declared above for
  D4; tied here to D8/D9.
- `test/plan-marshall/plan-retrospective/` and `test/plan-marshall/manage-metrics/` — **D8**, the
  aggregator's tests, population-derived with a matched control.

## Claim Labels
- verdict: corroborated | checked_at: 7d82d5d906c62312c708ac8993dc5f8f4d46bfa6 | by: code-intelligence-substrate/cleanup | rescoped: n/a | evidence: RE-GROUNDED 2026-09-22 at HEAD 7d82d5d9 (previous stamp 7a028157e). SCOPE STATED, AND THE GLOB CAVEAT FROM THE PREVIOUS PASS STILL GOVERNS: only 5 concrete files in this spec's declared surface moved, but the surface is GLOB-dominated (skills/*/SKILL.md, */standards/*.md, */references/*.md, doc/user/, doc/concepts/, doc/adr/), so a small moved-file count counts globs not files and must not be read as low exposure. Re-checked the merged-from-CIS-056 rows and the two folded rows whose artifacts moved; every other row sits on a document not re-executed and retains its previous verdict. ALL FINDINGS CORROBORATING, no re-scope needed: the exploration_doc_residency_bytes one-integer-per-phase PROXY holds (data-format.md:172/193, no per-document breakdown field exists); the D8-adjacent aggregator still does not exist (audit.py 9583 lines and analyze-logs.py 2243 lines both hold zero occurrences of index_answerable/doc_residency/exploration_unattributed); F-003's Executive Summary mandate holds (report-structure.md:13); the resident-context x turns billing model holds (data-format.md:239/56/153, cache_creation weight 1.25 unchanged). Recorded a surface-contention note in D6/the relevant deliverable: test_generate_executor.py has grown a new test PLAN-CIS-050 D1(e) must rewrite, different lines than this spec's own comment-correction edit, no sequencing constraint owed under parallelization_scope=1. The five carry-forward lessons flagged at the previous stamp (2026-09-14-21-001/-002/-003 from PLAN-CIS-049/#1489) were NOT folded in this window and are still owed. Corroboration dispatched to execution-context-level-5 under the analyze.md Step 2 corroboration form per the Dispatch Decision Rule; this record is the orchestrator's own application of the returned verdict.
...
| This epic's landed plan directories are still present in the clone | HYPOTHESIS | A listing of `.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/`. D6's gating derivation performs it and closes on the result. |

| **[merged from `PLAN-CIS-056`]** No check in the retrospective auditor reads the three exploration sub-source fields | CORROBORATED at `28b578f1e` over a stated population (full-text scan) | The aggregator D8 builds is the instrument `PLAN-CIS-036`'s report claimed already existed |
| **[merged from `PLAN-CIS-056`]** `exploration_doc_residency_bytes` is a one-integer-per-phase PROXY, not a per-document consumption measure | CORROBORATED at `28b578f1e` | `data-format.md` field definition. D11 — and the reason re-running `PLAN-CIS-039` as written would measure the wrong thing |
| **[merged from `PLAN-CIS-056`]** The standing exploration figures (index-answerable 15.9 %, doc-residency 65.2 %, ≈77 % exploration) rest on n=1 | CORROBORATED, and STRENGTHENED — the cited count in the retired spec was itself wrong | D9 settles or refutes each figure with its population. ⚠ `2-refine`, not `6-finalize`, measured worst (3.3 % / 90.7 %): expect the ranking to move |
| **[merged from `PLAN-CIS-056`]** The resident-context × turns MODEL is confirmed; only the 44.6× multiple is open | CORROBORATED — independently re-derived twice | D10 measures the multiple only. ⛔ The model is not re-litigated |
| **[absorbed 2026-09-12]** ~~The epic's landed plan-record directory is absent entirely~~ | **REFUTED at HEAD `53ab7dd2e`** | `cloud-runs/` holds **46 directories, 37 with a `gaps.md`**. The re-grounding read three of them directly (row 13). D6's not-applicable arm was never reachable; its corrected derivation is now mandatory |
| **[recorded, not resolved]** D7's population gate and `PLAN-CIS-050` D8 read the SAME git-ignored corpus for the same purpose | OPEN — a cross-plan consistency obligation, not a defect | Whichever runs second cites the first's published population and reports any difference as a finding. Two independent counts over one corpus is the disagreement-between-instruments archetype this epic exists to remove |
...
⚠ **CORRECTED 2026-08-24** — this said "run the gate where the lane requires it". There is no lane
gate; run the full verify through the executor
(`python3 .plan/execute-script.py plan-marshall:build-pyproject:pyproject_build run --command-args "verify"`,
Bash timeout ≥ 600000 ms, reading the result TOON `status`/`errors[]` and not the exit code) whenever
this plan touches Python. ⛔ **The substantive warning is unchanged and is the point of this
paragraph: do not treat a green gate as evidence for any claim below.** The proof of this plan is
per-kind.
...
⚠ **CORRECTED 2026-08-24** — this named "the lane's pre-PR verification sub-agent
(`cloud-plan-lane` § Step 6)", which does not exist here. Dispatch a
`plan-marshall:execution-context-{level}` leaf with a reading task, not a requirements-conformance
task. Give it the corrected text and **nothing else** — no gap entry, no plan section, no commit
message — and have it report *which reading it took*. ⭐ The cold-read property is what makes this
verification worth anything, so the leaf must be a fresh dispatch carrying none of this plan's
context:
...
`.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/{source-plan}/gaps.md#{id}`.
...
Three signals folded here. Each is a prose surface asserting something the shipped code does not do —
this plan's exact subject.

**F-002 — `phase-3-outline` normatively promises a phase-4-plan Q-Gate guard that phase-4-plan does
not implement.** `phase-3-outline/SKILL.md:480` states, in the bucket→profiles table, that bucket
`unknown` *"BLOCKS the deliverable […] Phase-4-plan emits a Q-Gate finding requiring the user to
correct the affected-files list […]"*. **No such guard exists**, established by three independent
reads at HEAD `24c8ec5ee`: (1) `phase-4-plan` carries no `unknown` predicate and no Q-Gate action
keyed on the classification bucket — the single "bucket" occurrence is the unrelated missing-profile
guard at line 423, whose predicate names only `test_only` / `mixed_code` / `mixed_with_docs`;
(2) `_check_declared_bucket` warns only on a value OUTSIDE the vocabulary, and `unknown` is INSIDE
`CLASSIFICATION_BUCKETS`, so it produces no signal; (3) a deliverable whose resolved bucket is
`unknown` therefore proceeds to task creation unblocked while the outline contract says it is blocked.
⭐ **The divergence is PRE-EXISTING and lives wholly on `main`** — `git diff main...HEAD` over both
files is empty for PR #1370. The originating plan took the only in-surface action available: it
DELETED the `_manifest_core.py` docstring clause that asserted the non-existent enforcement, per its
standing decision `a80dc5` (resolve an over-claiming clause by deletion, not by rewriting). ⛔ That
stops the code asserting an absent enforcement; it does not close the gap.
*Two options, recorded not decided* — **A:** implement the guard, adding an `unknown` predicate that
emits the promised Q-Gate finding routed back through the existing auto-loop, making the normative
sentence true; **B:** retract the promise, rewriting line 480 to state what actually happens.

**F-003 / F-001(3) — the Executive Summary section can never be written on the documented workflow
path** (`plan-retrospective`, error; filed twice, folded once). `compile-report run` returned
`sections_written[17]` and placed **Executive Summary** in `sections_omitted` — the BENIGN half,
meaning "the trigger fragment was absent or carried nothing renderable, so there was nothing to
lose" — on a report carrying 8 error-severity findings and no headline.
⛔⛔ **Three facts compose into structural impossibility:** (1) `build_document` renders the section
ONLY from a `_executive-summary` fragment already present in the bundle, used verbatim — it
synthesizes nothing; (2) `retro_sections` states the underscore prefix means the compiler injects the
record itself and it "must never be registerable through `collect-fragments add`", and
`valid_aspect_keys` filters underscore-prefixed keys, so **no producer may register it**; (3) no
injection site exists — every production occurrence is a read, a comparison or documentation.
**The only writer in the repository is a test fixture.**
⛔ **Why four self-review rounds and 42/42 mutation kills all missed it:** the drop-versus-omit
predicate is well tested *because the fixtures inject the key directly*, so the predicate is
exercised on the populated path while production always takes the quiet `omitted` branch. **The
mutation population is the code the fixtures reach.**
⚠ **Verified at HEAD `7845a4b9a` by this drain, with one correction to the source message:** the
mechanism is confirmed exactly as described, but the published ratio "15 of 18 corpus occurrences are
in tests" does NOT reproduce — the corpus-wide population is **24, of which 16 are under `test/`**.
Note what that is: a count published without its population, inside a finding about counts published
without their populations. Use the mechanism, re-derive the ratio.
*Two options, recorded not decided* — **A:** add a producer (an LLM synthesis step writing the
fragment before compile), restoring the specified headline but adding an LLM-authored surface to a
compiler documented as a pure assembler; **B:** delete the row and the `report-structure.md` entry,
keeping the assembler pure. **The originating plan recommended B**, on the grounds that the documented
assembler boundary is the stronger invariant and A forks it; the synthesis, if wanted, belongs in the
fragment-producing LLM pass that already exists. ⛔ Either option must ALSO resolve the
mandatory-content requirement in `report-structure.md` item 1.
*Rule:* a section whose only production writer is a test fixture is a section that never renders.
When a key is structurally unregisterable by producers, the CONSUMER must compute it — and a test
that supplies the key by hand is testing the renderer, not the pipeline.

**F-001(obs) — `manage-solution-outline` parses only one `Command:`/`Criteria:` pair per
deliverable.** A cross-module deliverable can therefore only structurally declare HALF its
verification; on this run the second command had to be written into the criteria prose to reach
phase-4 at all. The documented deliverable contract and the parser disagree about how much
verification a deliverable may declare, which is this plan's subject on the outline surface.

...
8. Plan Efficiency — renders the `plan_efficiency` aspect fragment as totals (including `total_build_seconds`, a NUM or the literal `unavailable` when `summed_rows == 0` — a floor when `suspect_count > 0` AND a number was published at all; see `plan-efficiency.md`) plus ratios plus a per-phase breakdown.
...
Within that section the override is keyed on **findings-presence, not on a status**: the gate is consulted ahead of the status guard and asks only whether `findings` is a non-empty list. A skipped fragment is therefore not a special shape but one instance of the rule, and a fragment reporting `status: error` with findings renders for the same reason — an aspect that failed mid-run but still produced findings has something to say, and refusing it would lose precisely the findings the failure produced. Reading the override as *the skipped-with-findings case* under-states it by one status and expects a drop where the report correctly renders.

The counterpart holds too: a fragment that IS present and WOULD have rendered a usable body, yet was refused by the gate, is a **drop**, not an omission, and must be reported as such — under `sections_dropped`, with the run's status raised to `warning`. A dropped section is content the aspect produced and the report lost, so it can never ride the same clean-run signal as an omission.

Both halves are decided by one question, asked of the fragment the renderer would have been handed: **is there content this section's renderer would have shown, or content it structurally cannot show?** For a dict, "would have shown" means reader-facing content — the `summary` prose or the `findings` bullets — never the JSON dump, which is mechanical and is emitted for every fragment whatsoever. For a non-dict the value itself is the content, so a bare non-empty string, int or list renders a body and is a drop. A fragment whose own status says it produced nothing, and a clean-run fragment whose only non-envelope content is provenance and zero-valued counters, render no usable body and are therefore omissions — which is why a plan with no script failures and a plan with no `execution.toon` both report a clean run.

The second half of that question — *content the renderer structurally cannot show* — is live for exactly one section, and the asymmetry is a property of the renderers rather than a special case. Every conditional section is rendered by a renderer whose JSON dump is **total**: each key the fragment carries is shown whenever the section emits, so nothing it holds is unshowable and the question collapses to its reader-facing half alone. The Executive Summary is rendered from its `summary` field alone, with no dump and no fallback, so a narrative written under any other key is content no render path can ever reach a reader with — a loss, and therefore a **drop**, even though the reader-facing half calls it empty. One question, resolved against two renderers; the reverse move — carrying the unshowable-content half back to a conditional row — is what made every clean run report a dropped section.

## Footprint Derivation Aggregate

Section 3 is the one section the compiler DERIVES rather than renders from a producer's fragment. Every aspect that consumes the shared footprint derivation degrades honestly on its own when that derivation cannot be resolved — `check-artifact-consistency` and `check-routing-decisions` report `inconclusive`, `check-outline-vs-shipped` reports `comparison: inconclusive` and withholds its counts entirely, `check-manifest-consistency` reports `inconclusive` under a check `status` and under `footprint_resolution.status`, `analyze-logs` reports `ARTIFACT_COVERAGE_UNMEASURABLE`, and manifest compose-time reports `pre_push_quality_gate_inactive`. Each of those reports is correct and none of them is altered, rewritten, or replaced by this section.

Membership is decided by whether a producer publishes a degradation verdict, not by whether it names the resolver: a producer that can only ever read as `resolved` would suppress the record on every run, leaving the aggregate structurally unable to fire. `check-manifest-consistency` (`manifest-decisions`) is now a roster member — it resolves through the shared chain and publishes `inconclusive` (under a check `status`, and under `footprint_resolution.status`) when no tier answers. The declaration in `scripts/retro_sections.py` is where that judgement lives, and it carries the measurement behind it. What the aggregate adds is the plan-level statement none of them can make alone: that N independent consumers went unmeasurable on the *same* missing derivation.
...
- `sections_omitted` — sections that would have rendered no usable body. Benign: nothing was lost.
- `sections_dropped` — sections that WOULD have rendered a usable body, yet did not reach the report. Loud: content the aspect produced never reached the report.
...
The compiler still does NOT make judgement calls. The written/omitted/dropped partition is a mechanical probe over the fragment bundle — "is there content this section's renderer would have shown, or cannot show?" — not an assessment of whether the content mattered. All interpretation remains the LLM's responsibility and happens in the pass that produces the fragments.

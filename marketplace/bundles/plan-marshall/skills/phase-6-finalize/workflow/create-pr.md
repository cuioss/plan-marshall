...
- `status: error` → the read did **not** establish that no PR exists. Branch on `error_cause`, the machine-readable discriminator the `pr view` error envelope carries; a bare `status: error` is NOT a no-PR signal, because that one envelope collapses three materially different causes and its human `error` message is hard-coded to the no-PR wording:
  ...
  - **`error_cause: auth_failed` / `provider_call_failed` / `malformed_response`** → the question *does a PR exist?* is **UNANSWERED**, which is not the same as answered "no". STOP and return an error TOON per the middle clause, preserving the stdout error envelope as emitted — **every field it carries**, copied whole rather than against a fixed field list. `error` is hard-coded to the no-PR wording on these arms, so the actual provider failure is stated in `context` **when that field is present** — it is, on the two arms that have provider output to report, and it is absent on the pre-provider `auth_failed` arm, which has none. Copying the envelope whole is what makes the instruction true on all three arms; enumerating fields would name a `context` that the auth arm does not carry. See [`tools-integration-ci/standards/api-contract.md`](../../tools-integration-ci/standards/api-contract.md) § "`pr view` and the `error_cause` discriminator" for the per-arm field sets. Do NOT create: a transient failure on a branch that already has an open PR is exactly how this step opens a **duplicate** PR, and that failure and a genuine absence previously shared one envelope.

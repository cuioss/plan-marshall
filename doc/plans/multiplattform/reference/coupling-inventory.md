...
|---|---|---|
| `extension-api/scripts/configurable_contract.py` — the project-local step-doc resolver | Builds `project_root / '.claude' / 'skills'` segment-wise instead of routing through `get_project_skill_roots()` — the same shape as the implementor scan one file over, which is closed; this half is not | — |

...
## Step 5b: Effort Ladder & Pin Setup (Optional)

Seed-then-tune for machine-local model pins and effort ladders, parallel to the Step 5 effort seeding above. Seed the machine-local effort ladder defaults (`.plan/local/effort-ladder.json`) for the configured targets:

```bash
python3 .plan/execute-script.py plan-marshall:marshall-steward:effort_pins ensure-defaults
```

Then validate the ladder schema:

```bash
python3 .plan/execute-script.py plan-marshall:marshall-steward:effort_pins validate
```

- **`status: success`** → the ladder is readable and schema-valid. For open-model-set harnesses, materialize and hand off to the harness provisioning seam (never through `manage-config effort`):
  ```bash
...
- **`status: error`** → malformed ladder. Fails closed — report detail and fix the file, never hand-edit the levels to compensate.
...
Ask the user to accept defaults (all generic verify steps + the default-on finalize steps, default iterations) or configure individually. The default finalize steps are the `default_on: true` built-in finalize-step implementors discovered via `extension_discovery.find_implementors` (the SOLE finalize-step discovery path — membership, order, and default-seed inclusion are declared in each step doc's frontmatter, NOT a hand-maintained constant). The authoritative default-on roster is whatever that query returns, ordered by each step's declared `order`; consult the discovery contract in [`ext-point-finalize-step.md`](../../extension-api/standards/ext-point-finalize-step.md) for the live set rather than relying on a roster snapshot here. `pre-push-quality-gate` is a built-in default whose activation is derived from `build.map` — it activates whenever the live footprint touches a glob registered in the build_map (those globs are the explicit `(pattern, role)` routes each *applicable* extension declares via `classify_globs()`, seeded at Step 8b, not author-shipped static literals). CI completion is a dispatcher-resolved precondition (`requires: [ci-complete]` declared on `ci-verify`, `automated-review`, and `sonar-roundtrip` frontmatters), not a sibling step. If configuring, discover available steps and apply.
...
- **Drive the multi-select from `list-finalize-steps` verbatim.** `list-finalize-steps` already enumerates the full discovered set — built-in (`default:*`), project (`project:`), and bundle-optional skill steps — sourced from the `extension_discovery.find_implementors` query. Present exactly those steps. The wizard MUST NOT invent thematic "Add-ons" questions, MUST NOT group steps into invented categories, and MUST NOT couple semantically-unrelated steps under one toggle (e.g. bundling `sonar-roundtrip` with `plan-retrospective`, which forces a nonsense "retrospective but not sonar" undo). When the discovered step count exceeds the `AskUserQuestion` 4-option cap, **paginate** the multi-select across successive `AskUserQuestion` calls over the same `list-finalize-steps` output — never collapse the set into improvised thematic groups to fit the cap.
- **Pre-check built-in defaults; offer bundle-optional and project-local steps opt-in.** Present every step `list-finalize-steps` enumerates, but pre-select only the `default_on: true` built-in (`default:*`) steps. Bundle-optional (`{bundle}:{skill}`) and project-local (`project:*`) steps are surfaced UNCHECKED so the operator opts into them deliberately — they are absent from the default seed by design and MUST NOT be pre-selected. Among the pre-checked built-ins, a step is REMOVED from the selection ONLY when the operator explicitly declines it; a built-in (e.g. `default:finalize-step-print-phase-breakdown`) MUST NOT be silently omitted because it fits no invented group — every built-in default is pre-checked and survives unless the operator declines it.
...
If `set-steps` returns `missing_order` or `order_collision`, resolve it at the **config layer** — the steward sequences the explicit `phase-6.steps` array directly. The `phase-6.steps` array is runtime-authoritative; a step's source `order:` frontmatter (frontmatter on built-in standards docs / `SKILL.md` for `project:` and bundle-optional steps) only governs the seed/presentation sort `marshall-steward` applies when first writing the array. To resolve a collision or a missing order, set the desired sequence in the `phase-6.steps` array via `set-steps` and re-run.
...
field says so. Display each `lane_report[]` row branching on `declared`: a row
whose `declared` is `-` (no stored setting) as `{step}: no setting stored,
running at {effective}`; a row with a real `declared` value as `{step}: asked
for {declared}, running at {effective}`. The branch matters — `declared: -` is
not a claim that a request was ignored (see `manage-execution-manifest/SKILL.md`'s

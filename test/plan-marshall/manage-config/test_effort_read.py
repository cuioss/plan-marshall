...
# =============================================================================
# Retired legacy keys (cross.* + retired phase-6-finalize.*) error with remediation
# =============================================================================
...
def test_legacy_cross_triage_errors_with_remediation():
    ...
    with PlanContext() as ctx:
        ...
        result = cmd_effort(_ns(role='cross.triage'))
...
def test_legacy_cross_research_errors_with_remediation():
    with PlanContext() as ctx:
        _write_marshal_with_models(ctx.fixture_dir, {'default': 'medium'})
        result = cmd_effort(_ns(role='cross.research'))
        assert result['status'] == 'error'
        assert 'retired' in result['error']
...
def test_legacy_cross_q_gate_validation_errors_with_remediation():
    with PlanContext() as ctx:
        _write_marshal_with_models(ctx.fixture_dir, {'default': 'medium'})
        result = cmd_effort(_ns(role='cross.q-gate-validation'))
        assert result['status'] == 'error'
        assert 'retired' in result['error']
...
def test_legacy_cross_plugin_doctor_errors_with_remediation():
    with PlanContext() as ctx:
        _write_marshal_with_models(ctx.fixture_dir, {'default': 'medium'})
        result = cmd_effort(_ns(role='cross.plugin-doctor'))
        assert result['status'] == 'error'
...
def test_legacy_cross_enrich_module_errors_with_remediation():
    with PlanContext() as ctx:
        _write_marshal_with_models(ctx.fixture_dir, {'default': 'medium'})
        result = cmd_effort(_ns(role='cross.manage-architecture-enrich-module'))
        assert result['status'] == 'error'
        assert 'retired' in result['error']


def test_legacy_phase_6_create_pr_errors():
    """The pre-rename `phase-6.create-pr` legacy key errors with remediation."""
    with PlanContext() as ctx:
        _write_marshal_with_models(ctx.fixture_dir, {'default': 'medium'})
        result = cmd_effort(_ns(role='phase-6.create-pr'))
        assert result['status'] == 'error'
        assert 'retired' in result['error']
        assert 'phase-6-finalize.default' in result['error']


def test_legacy_phase_6_pre_submission_self_review_errors():
    with PlanContext() as ctx:
        _write_marshal_with_models(ctx.fixture_dir, {'default': 'medium'})
        result = cmd_effort(_ns(role='phase-6.pre-submission-self-review'))
        assert result['status'] == 'error'
        assert 'retired' in result['error']
...
def test_legacy_phase_6_lessons_capture_errors():
    with PlanContext() as ctx:
        _write_marshal_with_models(ctx.fixture_dir, {'default': 'medium'})
        result = cmd_effort(_ns(role='phase-6.lessons-capture'))
        assert result['status'] == 'error'
...
def test_legacy_phase_6_retrospective_errors():
    with PlanContext() as ctx:
        _write_marshal_with_models(ctx.fixture_dir, {'default': 'medium'})
        result = cmd_effort(_ns(role='phase-6.retrospective'))
        assert result['status'] == 'error'
        assert 'post-run-review' in result['error']
...
def test_legacy_phase_6_pr_doctor_errors():
    with PlanContext() as ctx:
        _write_marshal_with_models(ctx.fixture_dir, {'default': 'medium'})
        result = cmd_effort(_ns(role='phase-6.pr-doctor'))
        assert result['status'] == 'error'

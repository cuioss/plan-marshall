...
Ask the user to accept defaults (`branch=feature`, `compatibility=breaking`, `commits=per_plan`, `rebase_on_execute_start=true`, `rebase_strategy=merge`) or configure each field interactively. If configuring, apply each choice via manage-config:

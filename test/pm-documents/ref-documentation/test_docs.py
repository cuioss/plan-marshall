...
def test_verify_links_file_not_found():
    ...
    assert 'error' in result.stdout.lower() or 'error' in result.stderr.lower(), 'Error when file does not exist'
...
def test_verify_links_both_file_and_directory():
    ...
    if not empty_file.exists():
        return  # Skip if fixture doesn't exist
    result = run_script(
        SCRIPT_PATH, 'verify-links', '--file', str(empty_file), '--directory', str(LINK_VERIFY_FIXTURES)
    )
    combined = result.stdout + result.stderr
    assert 'cannot specify both' in combined.lower(), f'Error when both --file and --directory specified: {combined}'
...
def test_classify_links_stdin_stdout():
    ...
    input_data = json.dumps(
        {'issues': [{'file': 'test.adoc', 'line': 1, 'link': '<<anchor>>', 'type': 'broken_anchor'}]}
    )
...
def test_classify_links_with_files():
    ...
    try:
        run_script(SCRIPT_PATH, 'classify-links', '--input', str(input_file), '--output', str(output_file), '--pretty')

...
>
> **2. ⛔ THIS PLAN CITES ARCHIVED RUN REPORTS THAT NO LONGER EXIST IN VERSION CONTROL.**
> It names `report-01.md` under `060`. Those reports were ingested
> into `cloud-runs/` (git-ignored) and are no longer tracked. **A run report is a dated record of
> one execution, not documentation of current state** — correcting one edits a file nothing reads.
> ⇒ **Every deliverable of this plan whose target is a run report is MOOT and must be dropped at
> outline.** Where the same false claim is ALSO restated on a live surface, only that live
> restatement is actionable, and it must be RE-DERIVED — never inherited from the gap's `Where`
> line. See `cloud-runs/_epic/ingestion-analysis.md` § RULING 1.
...
- `marketplace/bundles/plan-marshall/skills/manage-change-ledger/scripts/_ledger_core.py` *(added 2026-08-23, DA)*
...
**DA — the build-time oracle publishes a confident figure over a population it did not cover; and D-074-b's recorded cause is CORRECTED.** *(folded 2026-08-23 from PR #1330 / L8 — SPLIT VERDICT, adjudicated first-party)*

**The symptom is CORROBORATED, at scale, on this machine.** The source run published
`build_count: 1, total_build_seconds: 0.0` for a plan whose own script log recorded **19**
`pyproject_build` invocations consuming 1,376,200 ms (40.8 % of all script time). Sampling this
repository's ledger 2026-08-23 (`.plan/work/change-ledger.jsonl`, 468 rows / 433 `kind=build`):

| Figure | Count | Population |
|---|---|---|
| build rows carrying a real `plan_id` | **9** | of 433 |
| build rows with `plan_id` null or `NO_PLAN` | **424** | of 433 |
| build rows with `duration_seconds` null or 0 | **402** | of 433 |
| ...of those, rows whose `exit_code` is **0** (successful builds) | **398** | of 402 |
| timestamp span | 2026-06-11 -> 2026-08-22 | 433/433 stamped |

⇒ A per-plan build query finds ~nothing because **424 of 433 rows are plan-unattributed**, and the
duration sum is ~zero because **402 of 433 rows carry no usable duration — 398 of them SUCCESSFUL
builds**, which the suspect-zero rule then excludes. Either mechanism alone produces the published
`0.0`.

⛔⛔ **The source lesson's stated ROOT CAUSE is CONTRADICTED here, and the epic's own D-074-b is
corrected in the same breath. Do not re-derive either.**
- L8 asserts the ledger "lives in the tree that gets deleted" — that `branch-cleanup` destroys a
  worktree-scoped ledger before the retrospective reads it. **Not what happens on this machine.** The
  ledger holds 433 rows spanning ten weeks; it is not being destroyed. `resolve_ledger_path()` returns
  `get_tracked_config_dir()/work/change-ledger.jsonl`, which resolves via
  `_find_plan_root_from_cwd()` — the nearest ancestor containing `.plan/local`. A worktree at
  `.plan/local/worktrees/{plan_id}` has no `.plan/local` of its own (it is gitignored), so the walk-up
  passes through and anchors on the MAIN checkout. Worktree builds write main-side here. The residency
  claim could still hold on a layout that places worktrees OUTSIDE the main repo's `.plan/local`; it does
  not explain OUR data.
- **D-074-b currently records "the write is not reached on the routed/daemon path". That is too strong.**
  The write IS reached — 433 rows, 398 of them `pyproject_build`. What fails is what the row CARRIES:
  it lands **unattributed** (`plan_id` null/`NO_PLAN`) and **undurated** (`duration_seconds` null/0),
  including on successful builds. ⇒ The defect is in the row's CONTENT, not in whether it is written.
  D-074-b's earlier refutation of reader-resolution stands and is unaffected.

**Remedy (re-scoped by the above):**
1. **Fix what the row carries** — attribute `plan_id` on the routed/daemon write path, and record
   `duration_seconds` for successful routed builds. This, not residency, is what makes the population
   queryable.
2. **Make the oracle disclose its coverage instead of publishing a bare count.**
   `summarize_build_ledger` should cross-check its row count against the build-notation invocation count
   already extractable from `script-execution.log` and report a coverage state when the two disagree.
   The precedent is in this codebase: `dispatch_boundary_rows_recorded` vs `subagent_samples`, with
   `partial` / `exact` / over-coverage `FAILURE` verdicts.
3. Until (1) or (2) lands, treat any `build_time` block from a worktree-using plan as **unmeasured**,
   not as a measured near-zero.
⚠ **Distinct from PLAN-TRUTH-087 DA** (routed build reports `tests_run: 0`): that is a FIELD on a build
result the client did receive; this is about rows the reader never receives in usable form.
⛔ **Cross-spec:** `plan-retrospective/scripts/check-routing-decisions.py` is ALSO claimed by
**PLAN-TRUTH-089 DB** (the `unmeasured` reader update). NEW `-088 <-> -089` collision — serialize.
...
⛔ **The one `doc/plans/truthful-signals/**` entry below no longer exists in the working tree** — PR
**#1328** (`00ec21c28`) deleted that directory when the cloud-run corpus was ingested. It is **not
dead**: the deletion is an ordinary commit, so retrieve it in any clone with
`git show 00ec21c28^:doc/plans/truthful-signals/{NNN}-{slug}/report-01.md` (verified 2026-08-22).
Prefer that form over the `.plan/local/.../cloud-runs/` working copy, which is git-ignored. It is a
**read-only input**, never a file this plan edits.
...
Two findings drained from PLAN-TRUTH-074's inbox and corroborated by the orchestrator at HEAD
`31211a99b`. Both belong to this plan's subject — a reader that mistakes an absence for a
measurement — and neither is yet a deliverable. **Outline decides whether each becomes one; if it
does, re-count against the split guard.**

### F1 — the change-ledger observed none of PLAN-TRUTH-074's 144 builds (from inbox `-003`)

`analyze-logs` reported `build_time.build_count: 0` / `total_build_seconds: 0.0` for that plan while
the same fragment's cost rollup ranked `plan-marshall:build-pyproject:pyproject_build` at **144 calls,
16,054,170 ms, 62.4% of all script wall time**, with 96 build-result log files on disk. The builds
ran; the ledger observed none of them.

⭐ **The orchestrator narrowed this from a two-way guess to one testable claim — do not re-derive the
refuted half.** The inbox message offered two candidate causes: (a) the write is skipped on the
routed/daemon path, or (b) the ledger is written where the retrospective's live-mode reader cannot
resolve it post-worktree-removal.

- **(b) is REFUTED.** The ledger is at `.plan/work/change-ledger.jsonl` — **main-anchored, present,
  and readable after worktree removal**; a `query --kind build` returns 433 rows today.
- **(a) is sharply supported.** Exactly **4** of those 433 rows fall in the plan's active window
  (2026-08-09/10), and **all four are `run --help` probes** carrying `plan_id: NO_PLAN`,
  `status: unknown`, `duration_seconds: null`. Zero real build runs wrote a row. `--help`
  short-circuits before daemon routing; the plan's real builds resolved `mechanism=daemon`
  (`[BUILD-SERVER] resolved build (requested=auto, resolved=routed…)`).

⇒ **The ledger write is not reached on the routed/daemon build path.** Verify that claim directly
before fixing anything, and publish the population either way.

A second, independent item the same evidence raises: `analyze-logs` already holds both figures and
does not cross-check them. When `build_count == 0` **and** the cost rollup ranks a build notation with
a non-zero call count, it should emit a finding naming both. A document that contains its own
contradiction should say so rather than leaving it to a reader to notice.

⛔ **The honest labelling is already correct and must survive the fix.** `plan-efficiency.md`
instructs its consumer that `build_count: 0` means UNAVAILABLE, not zero, and the retrospective
followed it — reporting `total_build_seconds: unavailable`. No false figure was published. Do not
"fix" this by letting an unmeasured build time render as `0`.

### F2 — no cost preview exists, so predicted-vs-actual is permanently `not_attempted` (from inbox `-006`)

`status.metadata.execution_profile_cost_preview` was never recorded for PLAN-TRUTH-074, so
`check-routing-decisions` returned `comparison: not_attempted` with
`comparison_reason: no cost preview recorded`. The aspect implements the comparison; it has never had
an input. Record the preview at manifest-compose time so the comparison has a left-hand side.

**Why it earns a slot here rather than being a cost complaint.** That plan spent **61.7%** of its
tokens in finalize against **16.8%** in the phase that did the work, and came in at **3.6× its
`single_module + feature` error anchor** — while nothing in the run could observe the breach as it
happened. The anchor table, the phase totals and the accumulators all exist mid-run. This is the
measurement gap, not the spend: the spend was 20 finalize dispatches all terminating `step_complete`,
zero error, zero retryable.

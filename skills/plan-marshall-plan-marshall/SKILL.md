---
name: plan-marshall-plan-marshall
...
- All user interactions use `ask_question` tool with proper YAML structure
...
(See [`tools-script-executor` SKILL.md](../tools-script-executor/SKILL.md) Canonical invocations → `generate_executor — preflight` for the verb's argparse surface and its seven-field TOON contract.) This Auto-Detect Step 0 is the **single documented home for the preflight branch contract**; the `task=` / `issue=` init entry runs the exact same preflight and defers here for the branch handling — see [`workflow/planning.md` § Action: init → Step 0](workflow/planning.md). Branch on the returned fields:

- **`executor_action == regenerated`** — the executor was regenerated inline (safe derived state, ADR-002) because its embedded version was older than the manifest's `executor_changed_at_version`. Log a `[STATUS]` decision; because a regenerated executor may emit a changed agent set and Claude Code's agent registry is session-pinned at startup, surface the session-restart guardrail (see [`marshall-steward` SKILL.md](../marshall-steward/SKILL.md) § "Session Restart Required After Executor / Agent Changes").
- **`executor_action == fresh`** — the executor is current (its embedded version is not older than the manifest's `executor_changed_at_version`); proceed.
...
/plan-marshall-plan-marshall task="Add user authentication"

# Create new plan from GitHub issue (action=init implied by issue=)
/plan-marshall-plan-marshall issue="https://github.com/org/repo/issues/42"

# Create new plan targeting a non-default merge base (seeds references.base_branch)
/plan-marshall-plan-marshall task="Add feature X" base_branch="develop"

# Outline specific plan
/plan-marshall-plan-marshall action=outline plan="user-auth"

# Execute specific plan
/plan-marshall-plan-marshall action=execute plan="jwt-auth"

# Finalize (commit, PR)
/plan-marshall-plan-marshall action=finalize plan="jwt-auth"

# Auto-detect: continues from current phase
/plan-marshall-plan-marshall plan="jwt-auth"

# Cleanup completed plans
/plan-marshall-plan-marshall action=cleanup

# List lessons and convert to plan
/plan-marshall-plan-marshall action=lessons

# Convert specific lesson to plan (action=lessons implied by lesson=)
/plan-marshall-plan-marshall lesson="2026-05-18-11-001"

# Aggressive cross-lesson aggregation + superseded-stub prune in one batch
/plan-marshall-plan-marshall action=lessons-aggregate

# Create plan from predefined recipe — lists available recipes for selection
/plan-marshall-plan-marshall action=recipe

# Create plan from specific recipe (action=recipe implied by recipe=)
/plan-marshall-plan-marshall recipe="refactor-to-standards"

# Explicit action= always wins over implicit inference
/plan-marshall-plan-marshall action=init task="Add user authentication"
```
...
The actionable-vs-knowledge classification is a **fixed, hardcoded** rule in `plan-marshall/scripts/_invariants.py` (`_ACTIONABLE_FINDING_TYPES`) — not per-phase configuration, no `marshal.json` key, no wizard seed. ACTIONABLE types block when pending at a guarded boundary; KNOWLEDGE types never block. The two sets are enumerated once, authoritatively, in [`references/phase-handshake.md`](references/phase-handshake.md) § `pending_findings_blocking_count` resolution — not restated here, so this doc cannot drift from `_invariants.py` when a type is added.

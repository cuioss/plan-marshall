...
- **This project's own mypy gates are cold by construction — deleting `.mypy_cache` before running them changes nothing.** `build.py`'s `_run_mypy` passes `--no-incremental` on every invocation, so `compile`, `test-compile`, `quality-gate` and `verify` never consult an incremental cache and always re-check the same file set CI does. There is no warm-cache local pass to guard against on those commands, and advice to clear the cache first would imply a risk the mechanism has already removed.
- **The cache-clearing discipline still applies to mypy invocations made OUTSIDE `build.py`.** An IDE integration, a hand-typed `mypy some/path`, or a new gate that shells out to mypy itself all honour the incremental cache. A run of that kind finishing in a couple of seconds over hundreds of files is a red flag that the cache, not the code, produced the green — pass `--no-incremental` yourself, or run in a clean checkout, before treating its result as evidence. What the in-house gate does with the resulting freshness signal, and the coverage-boundary and structural-limit verdicts it prints alongside it, are specified in [`phase-6-finalize/standards/pre-push-quality-gate.md`](../../phase-6-finalize/standards/pre-push-quality-gate.md) § "Coverage parity with CI, freshness, and honest coverage" and are not restated here.

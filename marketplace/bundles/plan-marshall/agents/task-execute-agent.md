...
python3 .plan/execute-script.py plan-marshall:manage-logging:manage-log \
  work --plan-id {plan_id} --level INFO --message "[SKILL] (plan-marshall:task-execute-agent) Loaded plan-marshall:ref-development-standards"
```
...
```bash
python3 .plan/execute-script.py plan-marshall:manage-logging:manage-log \
  work --plan-id {plan_id} --level INFO --message "[STATUS] (plan-marshall:task-execute-agent) Starting task {task_number}"
```
...
```bash
python3 .plan/execute-script.py plan-marshall:manage-logging:manage-log \
  work --plan-id {plan_id} --level INFO --message "[SKILL] (plan-marshall:task-execute-agent) Loading domain skills from task.skills: [{task.skills}]"
```
...
```bash
python3 .plan/execute-script.py plan-marshall:manage-logging:manage-log \
  work --plan-id {plan_id} --level INFO --message "[SKILL] (plan-marshall:task-execute-agent) Using task_executor: {task_executor} for profile: {task.profile}"
```
...
```
Skill: {task_executor}  # e.g., plan-marshall:task-implementation
```
...
```bash
python3 .plan/execute-script.py plan-marshall:manage-logging:manage-log \
  work --plan-id {plan_id} --level INFO --message "[STATUS] (plan-marshall:task-execute-agent) Complete - task {task_number}"
```
...
component: "plan-marshall:task-execute-agent"

...
**`quality-gate` vs `scan` for marketplace skill `.md` edits**: when a `documentation_only` (or `mixed_with_docs`) deliverable touches marketplace skill `.md` bodies (`marketplace/bundles/**/skills/**/*.md`), its per-deliverable verification MUST use the rule-complete, worktree-scoped `pm-plugin-development:plugin-doctor:doctor-marketplace quality-gate --marketplace-root {worktree}/marketplace` form — NOT `scan --paths`. `scan` is a navigation/inventory mode whose rule-set OMITS `analyze_lesson_id_in_skill_prose` (and other quality-gate-only rules), so a clean local `scan` pass is NOT evidence that CI's `verify / verify` plugin-doctor stage (which runs the full `quality-gate` rule set) will pass — a local-gate-≠-CI-gate divergence on the operation-mode/rule-set axis. The `--marketplace-root {worktree}/marketplace` form is the canonical pre-push proxy for CI's plugin-doctor stage: it exactly reproduces CI's rule set while sidestepping fresh-worktree newer-than-CI ruff noise that blocks using the whole-tree `quality-gate` as a clean local proxy. Reserve `scan --paths` for non-marketplace documentation paths where the marketplace quality-gate rule set does not apply.
...
**Verification:**
- Command: `python3 .plan/execute-script.py pm-plugin-development:plugin-doctor:doctor-marketplace quality-gate --marketplace-root {worktree}/marketplace`
- Criteria: the forward-looking rule content appears in the component's canonical docs; the ported skill prose carries NO narrative lesson-ID citation; `quality-gate` returns `status: pass` with no `analyze_lesson_id_in_skill_prose` findings.

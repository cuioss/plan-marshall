...
image::../resources/diagrams/extension-topology.svg[Extension topology — Plan Marshall core (orchestrator + phase skills + execution-context + manage-* + the extension-discovery scanner) on the left; the ten named extension points (build, triage, recipe, outline, provider, execution-context-workflow, dynamic-level-executor, self-review-surfacing, verify-steps, finalize-steps) on the right with declaration mechanism and impl count annotations. All ten production bundles implement get_skill_domains; the other hooks are optional., align=center]

The required hook is `get_skill_domains()` — every bundle declares its domain identity and organises its skills by profile (`core`, `implementation`, `module_testing`, `integration_testing`, `quality`, `documentation`). Everything else is optional: the seam exposes ten extension points beyond that minimum, and a bundle fills in only the slots it actually needs. A frontend bundle might implement three; a documentation bundle might implement five. The unfilled hooks return the default no-op and the core uses its generic behaviour.
...
* link:../../marketplace/bundles/plan-marshall/skills/extension-api/SKILL.md[`extension-api/SKILL.md`] — overview index of the ten extension points: build, triage, recipe, outline, provider, execution-context-workflow, dynamic-level-executor, self-review-surfacing, verify-steps, finalize-steps.

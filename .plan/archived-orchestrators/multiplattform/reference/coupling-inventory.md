...
| `tools-permission-doctor/scripts/permission_doctor.py` analysis rules + the three permission standards documents (`permission-architecture.md`, `permission-validation-standards.md`, `permission-anti-patterns.md`) | **NARROWED at the PLAN-14 landing.** The Claude permission model as analysis subject matter (`Skill()`/`Bash()`/`Write()` grammar, anti-pattern regexes, `.claude/commands/`) — rule-pack-class knowledge, a different shape than live render/resolve residue. ✅ The class is now **DECLARED**: the module docstring and all three standards documents carry Claude-rule-pack provenance naming the rule set and stating it binds only on a Claude target, citing `plugin-doctor/references/rule-provenance.md` § "Engine / Claude rule-pack split" as precedent. Relocation was **considered and rejected** — the knowledge remains by design, declared rather than moved. ⛔ **What REMAINS open is enforcement, not declaration**: the declaration is structural, not a dispatch path, so the direct-script `detect-*` route still runs the Claude rules on a non-Claude target and reports a FALSE ZERO — an unchecked negative presented as a clean one. The documented `platform_runtime permission analyze` path is already honest; fail-closed enforcement on the direct route is what is left | — (unclaimed) |

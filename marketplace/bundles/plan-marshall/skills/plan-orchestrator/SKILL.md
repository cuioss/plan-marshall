...
Returns one staged spec file's body through the sanctioned script-mediated read path — the compliant alternative to a direct `Read` of the ledger tree. Resolves `PLAN-NN` to its spec via exact-or-prefix stem match (with the separating hyphen, so `PLAN-1` never claims `PLAN-10-*.md`), main-anchored so the result is identical from a worktree and from the main checkout. Carries `spec`, `size_bytes`, `line_count`, and the verbatim `body`. An absent spec returns `spec_not_found` carrying `available_specs` (never an empty body); an unsafe slug (`invalid_slug`), an unsafe plan value (`invalid_plan`), an unreadable file (`unreadable`), and a slug with no store tree (`not_found`) are refused without writing — the verb is read-only.

...
- **Entry-point placement.** The push fires after slug resolution and before the verb's first read, so the title is already correct while the verb does its work. When a verb's epic `status.json` may not exist at entry (`lessons`, whose fixed `lessons-routing` slug is resolved but whose epic tree may still need scaffolding), the push moves to the first point at which both the slug and the epic's `status.json` exist — the same reason `init` fires a follow-up repaint after `manage-status create`: an entry push cannot resolve epic state before `status.json` exists.

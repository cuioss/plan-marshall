...
- The aspect-level `footprint_resolution` block carries `status: inconclusive`, `tier` (the resolving tier or `unresolved`), and `chain` (the tiers consulted); the reason itself is named at the check level, in each affected check's own message.
...
- Every check whose evidence is THIS script's own footprint (`docs_only_diff`, `early_terminate_diff`, `tests_only_diff`, `branch_cleanup_changes`) that ran against a reduced input set carries the reduction in its message. The membership is the script's `_FOOTPRINT_FED_CHECKS` set — the `_DIFF_FED_RULES` dispatch registry minus `declared_vs_realized_set` (Rule M6), whose evidence is the forwarded upstream fragment, not this script's footprint; see [Rule M6](#rule-m6-declared-vs-realized-set-comparison-is-received). `manifest_version_recognized` is exempt for the same reason as M6: it reads the manifest body alone, so no amount of filtering affects it.

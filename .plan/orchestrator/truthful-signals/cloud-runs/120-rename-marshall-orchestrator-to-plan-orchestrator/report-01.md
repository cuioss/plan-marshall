...
- Derivation population: `Grep` over the whole working tree (ripgrep, which honours `.gitignore`). ⚠ **The original bullet said this excluded `.plan/` as a whole. It did not.** `.gitignore` ignores `.plan/*` and then RE-INCLUDES `!.plan/marshal.json` and the `project-architecture/` tree, so ripgrep's negation handling put those tracked paths **inside** the swept population — 13 files at the base below. They contributed no hits, which is why their inclusion was invisible in the result and the exclusion claim survived unchallenged.
- Hyphen token `marshall-orchestrator`, **re-derived at the rename commit's merge parent `68a21cac`** (the tree extracted to a scratch checkout and swept with the same tool): **264 matching lines across 73 files**, of which **0 lines across 0 files** were under `.plan/`. ⚠ **The figure this report originally carried — 265 matching lines across 74 files — does not reproduce at that base, and is not recoverable.** It was derived from a working tree that no longer exists, and the run recorded no commit for it. The +1 line / +1 file delta is stated as a delta and nothing more: attributing it to a cause would be manufacturing the provenance this correction exists to remove. **The re-derived figure is the one attributable to `68a21cac`; the original is attributable to nothing.** Underscore form `marshall_orchestrator`: **0** (no Python identifier uses it). Other-cased variants found in the initial sweep: the single uppercase identifier `_MARSHALL_ORCHESTRATOR_SKILL` (3 uses, one test file). ⚠ **The initial variant sweep was incomplete** — it did not cover the **space-separated title-case display form** `Marshall Orchestrator`, which survived in two `SKILL.md` H1 headings and was caught by the verification sub-agent (findings #1/#2 below, fixed). A later exhaustive `Marshall.Orchestrator` sweep confirmed exactly those two and no others.
- **Subset relation confirmed** (claim-label check): `persona-marshall-orchestrator` is a strict substring of `marshall-orchestrator`, so a single exact-token replace transforms BOTH skills and the 3-part notation; the file count is the union, not a sum (73 at the re-derived base — the original bullet said 74, carrying the unreproducible working-tree figure corrected above).
...
**D6 — the git-ignored `.plan/` ledger explicitly NOT rewritten.** Non-goal asserted for the git-ignored part — `.plan/local/**` (the orchestrator ledger, plan specs, landing records, epic trees) and the generated `.plan/execute-script.py` are absent from a fresh clone, so their historical orchestrator references are untouched and unreachable. ⚠ **`.plan/` is not git-ignored as a whole, and the original bullet's flat "`.plan/` is git-ignored and absent" was wrong about the rest of it.** Two paths are tracked and present in every clone: `.plan/marshal.json` and the `.plan/project-architecture/` tree (13 files at the merge parent `68a21cac`). Those were **inside** the D0 sweep's population and were searched; they returned zero hits, which is why nothing under `.plan/` needed rewriting — an empty result, not an exclusion. In-tree records under `doc/plans/**` (other specs + the 090 report) likewise left as records.

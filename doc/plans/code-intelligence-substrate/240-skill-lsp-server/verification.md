...
|---|---|---|---|
| A1 | False positives | Walked all 27 gaps to their cited `path:line`. **Every gap is a true positive** — G1 reproduced end to end (exit 1, TOON on the frame stream, third request unanswered), G5/G6 confirmed by re-running the mutations, G7/G8/G9 confirmed by grep returning exactly the claimed hits (`cmd_query` once at `test_corpus_lsp_optin.py:138`; `corpus_path` and `did_*` zero times in `test/`), G18–G20 confirmed as the only three dangling links (line 46 resolves), G24 confirmed against the emitted zero-width range, G25 confirmed (`lspServers` in no manifest), G27 confirmed (156 + 2 + 2 = 160 vs the stated 157). One stale citation. | **G21's `Where:` moved from `report-01.md:178` to `:175`** — the quoted sentence *"`230-validate-precision.md` is still an unexecuted plan file"* is at 175; 178 opens the table beneath it. **G13** given its missing line number (`:286`). |
| A2 | False negatives | Read the three production modules in full and re-derived each *Done when* independently. **One defect the audit missed, found only by driving the protocol rather than reading it:** `on_references` discards the `verified` flag, so the **162 of 5 020 sites (3.2 %)** the index marks unconfirmed reach an editor as ordinary `Location`s — while `SKILL.md:121` and the user page's line 182 both promise such sites are "never presented as exact". Demonstrated live: an unverified edge emits `manage-architecture/SKILL.md` line 515, a line about `--pattern` search. The audit's D2 evidence ran the `query` CLI verb only and drove `serve` no further than `initialize`, so the projection layer was asserted from code — exactly the gap the brief warned about. Separately, **G10's lean is wrong**: the current 61-row unresolved set is 25 non-notations (41 %) and 36 well-formed notations with an absent target (59 %), so the ~97 %-false-positive premise has inverted, not merely drifted. Checked and found sound: the fail-open surfaces (`read_corpus_config` fails closed on all 7 pinned branches), the guard that could not fire (none — M1 proves the negative-length guard is live), the disabled path driven with real requests (`{}` / `null` / `[]` / `null`, exit 0, no index built). | **New gap G28** (high, `lsp/resolvers`) with a two-option concrete fix and an observable *Done when*. **New Correctness-review finding 2b.** **G10 rewritten** with the measured 25/36 classification and its lean reversed to "presume the decision needs reversing". D2 row and section record that the protocol path is now exercised. |
| A3 | Vacuous evidence | **Re-ran the audit's entire mutation sweep.** M1 (delete `parsed < 0`) → **RED**, one test, and its sibling `test_negative_content_length_does_not_consume_the_stream` stayed green, independently confirming G6's masking claim. M2 (delete the `_candidate_files` cache read) → **GREEN, all 78 pass**, confirming G5. M3 (`_version_key` → lexical) → **RED** (`0.1.9` selected over `0.1.62`). M4 (drop `lspServers`) → **RED**. All four match the audit exactly. Files snapshotted to `/tmp/adv-240-skill-lsp-server-mutsweep/`, restored by `cp`, `md5sum -c` clean, `git status --porcelain` free of all four. No `git checkout`/`restore`/`stash`. G5's evidence overstated *why* its second test is vacuous. | G5's evidence corrected: `walked` is derived from what the cache *contains*, not whether it is *read*, so the test passes with the read deleted; the "walked is empty" case is the different mutation that also drops the write. |
| A4 | Counts and quotes | Re-derived every figure at the moment of checking. Confirmed exactly: **308 components / 5 081 dependencies / 61 unresolved / 5 020 resolved**; **4 858 verified, 296 (6.1 %), 1 331 (27 %)**; 78 tests; 8 generator tests; 156/2/2; 12 `.py` files (6 + 6); 17 skill dirs (4 + 12 + 1); `3d96e40` 08:54:13Z and `5edca5a` 10:08:21Z (**74 min**); landing commit "20 127 passed" vs the report's "20 119"; `ruff>=0.16.2` in both files; zero `tools-marketplace-inventory/` files in the diff; G10's per-target counts (`lint:js:fix` ×9, `lint:style:fix` ×6, `project:core:compile` ×3, `HH:mm:ss`, `trivy:…`). All quoted lines verbatim. Two numbers did not survive. | **D1's re-measured table replaced.** The audit's 2.50 s / 2.61 s / 29.4 ms were contention from concurrent agents, not "~30 % higher on this machine"; a quiet re-take gives **1.80 s build, 20.1 ms first call, 0.51 µs / 0.94 µs** — reproducing the report's own figures and the docs' "~20 ms at 443 inbound edges" exactly. The 443-edge component is named (`plan-marshall:manage-logging:manage-logging`, re-derived as the corpus maximum). **"14 files naming this skill" → 12**, enumerated; the 14 counted the audit's own two documents. **"4 858 resolved sites" → "4 858 `verified: true` of 5 020 returned"**, since 5 020 is also a resolved count. |
| A5 | Actionability | 26 of 27 entries carry a concrete path, a concrete change and an observable *Done when*. One did not: **G13** said *"restate once G10's re-evaluation is done"* with *Done when:* "matches the current validator behaviour" — neither independently executable nor checkable, and blocked on another gap. | G13 rewritten to name the exact sentence to replace, the measured split to replace it with, and a *Done when* that a later run can evaluate without reading G10. G10's *Action* now carries the classification so the successor need not re-derive it. |
| A6 | Severity and topic | Re-rated all 27 against the calibration. Severities hold: G1 high (server dies, stream corrupted); G2 medium is right rather than high, because the evidence shows the resolution is *unguaranteed*, not that it is wrong at scale; the doc-defects sit correctly at medium (shipped docs, false claim) vs low (report-only, cosmetic). One topic was wrong. | **G17 re-topiced `bundle-docs` → `documentation-surface`**: `marketplace/targets/claude/plugin_json_gen.py` is the multi-target generator, not a bundle, so `bundle-docs` would route it to the wrong fix plan. G15/G16/G24 keep `bundle-docs` correctly (docstrings inside bundle scripts). New G28 rated **high** under "a documented contract is unimplemented", with the reasoning stated in the entry so it can be re-rated. |
| A7 | Coverage | All six deliverables are covered (seven rows, D0 split into (a)/(b)), plus every "Out of scope" clause, report accuracy, the declared residue and the collateral diff. No deliverable is silently unmentioned. Verified independently that the out-of-scope claim "the index is consumed, not edited" holds: `git show 5edca5a --name-status` lists zero files under `tools-marketplace-inventory/` (an earlier `--name-only` count of 2 was commit-message text, not files). D5's cold read re-done — the user page opens *"You do not need this page."* / *"you have lost nothing"*, which is the correct reading. | None needed. |
| A8 | Internal consistency | The overall verdict follows from the rows (6 CONFIRMED + 1 PARTIAL + code defects → "CONFIRMED WITH GAPS"). Two linkage defects: **G24 was orphaned** — `_lsp_location` appears nowhere in `verification.md`, so a gap existed with no finding behind it; and the collateral paragraph's marker **"→ G14" pointed at the wrong gap** (G14 is the `SKILL.md` D3 wording; the finding described is G17). | **Correctness-review finding 5 added** for `_lsp_location`, so G24 traces. **"→ G14" corrected to "→ G17"**, with the `PASSTHROUGH_FIELDS` line range added. **"→ G5" marker added** to the M2 mutation row, which previously described G5's defect without naming it. |

**Residual doubt:** The two things this round still could not close are the ones no offline pass can: the four D0(a) web searches and the operator conversation quoted under D0(b) remain UNVERIFIABLE, and the "first registered wins" extension-collision behaviour is still a claim sourced from the run's own research rather than an observation against a real client — it is the load-bearing premise for shipping no `lspServers` declaration at all, and nothing in the tree tests it. A further round would most likely find more in the same place this round found G28: the **protocol projection**, which is thinner than the index beneath it and correspondingly under-tested. `on_definition` and `on_hover` were each driven for exactly one notation; `notation_at_position`'s URI-normalisation assumption (`self.documents` is keyed by the client's raw URI string, while `_lsp_location` emits a `resolve()`d one) is untested and would misbehave for a client that sends a non-canonical URI. A round that fuzzes URIs and positions against a running server, rather than calling the index directly, is the highest-yield next step.

**Verdict on the audit:** SOUND AFTER CORRECTION — every gap it filed is real and its mutation sweep reproduces exactly, but it verified D2 through the `query` verb while the plan shipped a language server, and that blind spot hid a high-severity contract defect (G28) and left its D1 figures distorted by measurement contention.

...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `parallelization_scope` | int (`>= 1`) | `1` (seeded; a genuinely-unset legacy block keeps the ask's hard-coded `1`) | Project-level default that pre-fills the per-epic `parallelization_scope` ask in `marshall-orchestrator` init. The stored per-epic answer and the ask's own positive-integer validation are unaffected — this knob only supplies the initial suggestion for a NEW epic's first ask. `bool` is rejected even though it is an `int` subclass. |

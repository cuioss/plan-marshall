...
`display_detail` is ≤80 chars, ASCII, no trailing period. `plan` and `landing_report` carry `-` for the observation granularity. `emitted[]`/`shortfall[]` mirror the `orchestrate.md` `next` verb output shape (see there) for both granularities, with one blocking reason per unemittable candidate.

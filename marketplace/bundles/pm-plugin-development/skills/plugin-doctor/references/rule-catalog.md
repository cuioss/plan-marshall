...
**mutates-source-step-post-merge-order** (class: structural, severity: error, fixable: false): Flags a finalize step that declares `mutates_source: true` in its frontmatter yet is ordered at or after the merge gate (`default:branch-cleanup`). Such a step's source edits land after the plan's feature branch has been squash-merged, so they cannot be pushed onto that branch, cannot ride the PR, and cannot be CI-covered or reviewed with the rest of the change. Findings carry `details.step_name`, `details.step_order`, and `details.merge_gate_order`, anchored at the step doc's `order:` line.
...
- **Structural discriminator**: the co-presence of a truthy `mutates_source:` frontmatter key and a declared integer `order:` at or above the merge-gate order. The merge-gate order is resolved **dynamically** from the discovered `default:branch-cleanup` record rather than a hardcoded literal, so moving the merge gate moves the threshold with it.
- **Legitimate-occurrence note**: a step doc that declares no `mutates_source:` key makes no source-mutation claim and is out of scope — it is silently skipped and never produces a false positive. A source-mutating step ordered anywhere in the settle band (below the merge gate) is the correct shape and is likewise not flagged. When the merge-gate step is not discoverable (a synthetic `tmp_path` marketplace with no phase-6-finalize tree), the rule skips cleanly rather than guessing a threshold.
...
- **Fix**: move the step into the pre-merge settle band (an `order:` below the merge gate), or drop the step's source mutation and its `mutates_source: true` declaration. The ordering contract itself is owned by the central standard — see [source-edit-pushability.md](../../../../plan-marshall/skills/phase-6-finalize/standards/source-edit-pushability.md) and its § "Mutation-settling stage" counterpart in [phase-6-finalize/SKILL.md](../../../../plan-marshall/skills/phase-6-finalize/SKILL.md); it is deliberately not restated here.
- **Exemptions**: None beyond the out-of-scope cases above — every step that claims `mutates_source: true` is expected to run pre-merge.
...
**Recommended fix**: Move the prompt to the inline orchestrator. The leaf computes the prompt's options and recommended default, then returns a prompt-required envelope (a block on its return TOON, as `phase-6-finalize/workflow/automated-review.md` does with `status: escalate_ask`); the main-context orchestrator fires the `AskUserQuestion` and performs any resulting persistence.

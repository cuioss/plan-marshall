...
* **S1 — positional argv-template check.** The command is non-empty; `command[0]` (the interpreter) is accepted — against an explicit interpreter pin when the caller supplies one, and otherwise against the canonical `python3` / `python` basenames, since no registration field carries an interpreter and the check is therefore daemon-wide argv shape rather than per-project registered state; `command[1]` is exactly `{exec_path}/.plan/execute-script.py` inside the submitted tree; `command[2]` (the executor notation) is on the project's allowlist; the remaining args are plain, NUL-free strings.

...
- **No shell file operations** — Use Glob/Grep/Read/Edit tools, not `ls`, `find`, `cat`, bare `grep`. Sole carve-out: `git grep <pattern> -- <pathspec>` for a broad content sweep when `Grep` is revoked at runtime — see `plan-marshall:persona-plan-marshall-agent`.

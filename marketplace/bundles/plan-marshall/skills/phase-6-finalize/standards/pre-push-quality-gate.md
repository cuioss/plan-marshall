...
Branch on the **probe's** TOON, not on the build wrapper's:
...
- **`status: error`** → the whole-tree `quality-gate` cannot run at all in this project. The resolver reports this as `error: architecture_error` with `message: Command not found` and an `available[]` list — the commands the `default` module *does* expose — that omits `quality-gate`. Take the **honest-degradation branch** below and do NOT run the invocation.

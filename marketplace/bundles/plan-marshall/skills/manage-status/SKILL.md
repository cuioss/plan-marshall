...
`scope_provenance` explains **why** the band came out as it did: `distinct_path_count` and `fan_out_marker` are the two measurements the band table reads, and `band_rule` names the row that fired (one of `unscoreable_body`, `fan_out_marker`, `path_count_at_or_above_multi_module_floor`, `path_count_middle_band`, `path_count_at_or_below_surgical_max`, `pathless_non_empty_body` — see the `scope-estimate-heuristic` row of the Scripts table). It is observability, not a gate; `--lane-override` / S6 remains the way to disagree with a verdict.
...
| `classification-validate` | `--plan-id` | Deterministic classification-validation gate (flag-not-block). Cross-checks `change_type` / `scope_estimate` against cheap request signals; flags `feature_as_bug_fix` (bug_fix stamp over a non-ambiguous feature narrative), `non_empty_affected_files_with_null_scope`, and `scale_mismatch_light_routing` (a persisted `surgical` band over a body naming >= 8 distinct paths), recording a `warning` `anti-pattern` Q-Gate finding against `2-refine` per mismatch. NEVER blocks routing; runs automatically as a pre-route pass inside `planning-lane route`. |

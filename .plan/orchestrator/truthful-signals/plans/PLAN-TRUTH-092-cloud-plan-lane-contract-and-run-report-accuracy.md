...
> It names `report-01.md` under `040`, `060`, `110`, `160`, `220`, `260`, `370`, `430`, `440`, `450`. Those reports were ingested
...
> ---
>
> **⛔ SCOPE DECISION TAKEN AT INGESTION — this plan is RE-SCOPED to its contract half.**
>
> The plan states its own split: *"D2–D4 and D7 are the contract half — prose in a governing skill;
> D1, D5 and D6 are the record half — corrections inside landed run reports."* RULING 1 settles the
> record half.
>
> | Deliverable | Disposition |
> |---|---|
> | **D0** — GATE, re-derive the defect population | **KEEP, re-worded.** Its premise *"Nothing in this plan requires a file outside the clone"* is now **FALSE**: the `{source-plan}/gaps.md` it reads were ingested and are no longer tracked. Read them from `cloud-runs/{NNN}-{slug}/gaps.md` in this ledger, and say so. Sub-step (b), the 15-defect class map, now feeds **only** D4. |
> | **D1** — the record half's high gap | **DROP — moot.** A correction inside an archived run report. |
> | **D2, D3, D4, D7** — the contract half | **KEEP unchanged.** These edit `.claude/skills/cloud-plan-lane/SKILL.md` and `.claude/skills/author-cloud-plan/SKILL.md` — live, tracked, governing surfaces. |
> | **D5, D6** — the record half | **DROP — moot.** Corrections inside archived run reports. |
>
> ⚠ **This costs the plan its only `high` gap**, because that gap is a report correction. That is the
> correct outcome, not a loss of signal: the *class* those 15 report-accuracy defects evidence is
> preserved structurally by **D4**, which installs a Step-9 receipt check so future run reports cannot
> repeat them. **A retro-correction to a dated record buys nothing; a check that stops the next one
> buys everything.** The individual corrections are closed-as-recorded in the gap entries under
> `cloud-runs/`, which remain readable.
>
> ⇒ **Re-scoped shape: D0 (re-worded) + D2 + D3 + D4 + D7 — five deliverables, contract-only.**
...
⛔ **READ THIS BEFORE RESOLVING ANY `doc/plans/truthful-signals/**` PATH BELOW.** That directory was
deleted from the working tree by PR **#1328** (`00ec21c28`) when the cloud-run corpus was ingested. The
paths are **not dead** — the deletion is an ordinary commit, so every file is reachable in any clone:

```bash
git show 00ec21c28^:doc/plans/truthful-signals/{NNN}-{slug}/report-01.md
```

Verified 2026-08-22 against `060-invented-plan-scoping-flags-…/report-01.md`. A working-tree copy also
exists at `.plan/local/orchestrator/truthful-signals/cloud-runs/{NNN}-{slug}/`, but `.plan/` is
git-ignored, so **prefer the `git show` form** — it is the one that works from a fresh clone and from a
cloud session. ⚠ These reports are **read-only inputs**: this plan corrects the CONTRACT that produced
them, never a landed report (a run report is a dated record — see the epic's binding ruling 1).
...
## Claim Labels
- verdict: unverifiable | checked_at: 5f972ac15 | by: truthful-signals/cleanup | rescoped: n/a | evidence: Table-form claim section, section-scope address. Sampled the highest-risk row - the asserted ABSENCE 220/G5 - and CORROBORATED it at this sha: a content search of .claude/skills/cloud-plan-lane/SKILL.md for CHECK_ERA or era_stamp_fill returns 0 hits. The remaining rows, including a second asserted absence (040/G5) and the three report-01.md line references, were not reached. An absence is the higher-risk half of the verify-first contract, so the other one must be re-derived before D4(b) writes anything.
...
## ⛔ RE-GROUND AGAINST PR #1336 BEFORE STARTING (2026-08-24)

**PLAN-TRUTH-075 landed on this plan's declared surface.** PR #1336 / `77c9dc70a` modified
`.claude/skills/cloud-plan-lane/SKILL.md`, which this spec names in its Expected Surface. The defect is
**different** — `-075` fixed the *build gate* report phrase; this plan owns the *merge gate* and
run-report accuracy — so this is a **collision, not a duplicate**, and no scope moves.

Two consequences, and the second is the one that will bite:

1. **The file has moved.** Any line number or quoted phrase in this spec predates `77c9dc70a` and must
   be re-verified. `-075`'s own experience is the warning: its D1 line numbers (`:278`, `:376-377`) had
   moved between staging and execution while the content requirement remained, and its claim-labels had
   already flagged line numbers as the least durable part of the spec.

2. ⭐⭐ **A NEW GUARD WILL FAIL THIS PLAN IF IT EDITS EITHER SURFACE ALONE.**
   `test/pm-plugin-development/cloud-plan-lane/test_build_gate_lockstep.py` (10 tests, 740 lines) parses
   **both** the Step 5 trigger table and the run-report template's `## Build gate` block out of the
   document and **fails when they disagree**. If this plan edits one without keeping the other in
   lockstep, the suite goes red.

   ⛔ **That is the guard working as intended — do NOT weaken, skip, or "fix" it to get green.** The
   correct response is to keep the two surfaces in lockstep, exactly as the guard demands. ⚠ Note also
   that the guard binds the verdict sentence's **paragraph**, not the whole block: `-075` tried
   block-wide uniqueness first and it was **wrong**, because the block legitimately quotes a second
   phrase in its stale-base paragraph. A future author reading the guard as over-strict should read
   that history before loosening it.

⚠ **Queue-latency caveat, from `-075`'s own corrected diagnosis.** This spec was staged before #1336
landed. `-075` demonstrated that a spec accurate on the day it was written can have its target changed
by unrelated work while it waits in the queue, and that the remedy is **re-verification at execution
start**, not better sourcing at staging time. Treat every claim here as dated, not wrong.

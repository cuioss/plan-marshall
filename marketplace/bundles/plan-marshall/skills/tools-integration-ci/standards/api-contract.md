...
| `pr wait-for-comments` | `--pr-number` | `--timeout` (default 300), `--interval` (default 30) | `pr_number`, `timed_out`, `duration_sec`, `polls`, `baseline_count`, `final_count`, `new_count`, `rate_limited_bots[N]{bot_kind,rate_limit_class,eta,cause,cap,layer,body}`, `movement_matched_bots[N]{bot_kind}`, `detector_answerable`, `unanswerable_reason` |
| `pr update-branch` | _exactly one of_ `--pr-number` _or_ `--head` | — | `pr_number` |

**`pr list` field semantics, and the provider asymmetry.** `total` is the number of rows the listing returned, and it is never read alone: `limit` is the bound it was read at, and `truncated` reports whether the row count REACHED that bound. `truncated: true` means the population could not be enumerated — the listing is a page, and the count is a floor rather than a total — so a consumer deriving a population MUST re-read at a higher `--limit` until `truncated: false` rather than quoting the number it has. Reaching the bound and being exactly that large are indistinguishable from outside, which is why both report `truncated: true`.
...
- **`pr list` CLI differences**: GitHub `gh pr list --head {branch} --state open|closed|all --limit {n} --json number,url,...` — the bound is always passed explicitly, so the listing is never silently capped at `gh`'s own default page size; GitLab `glab mr list --source-branch {branch} --state opened|closed|all --output json`, which passes no page bound and so reports a page-bounded count with no truncation signal.
...
- **`pr wait-for-comments` field mapping**: the poll counters (`baseline_count` / `final_count` / `new_count`) are provider-agnostic — derived from the `pr comments --unresolved-only` unresolved count. The completion predicate is TWO-ARMED: the count-growth arm above fires for a bot that appends a new comment per review (`participation_requires_update: false`), while a movement arm fires for a bot that re-reviews by EDITING one persistent comment in place (`participation_requires_update: true`), keying on the LATER of that comment's `updated_at` / `created_at` moving strictly past the wait-start. `movement_matched_bots[]` names one `{bot_kind}` record per bot the movement arm matched (empty when none), so a caller can tell WHICH bot's edit ended the wait rather than inferring it — and a `timed_out: false` return with `new_count == 0` is the normal shape for an in-place re-review, not an anomaly. `detector_answerable` / `unanswerable_reason` report whether the await COULD have succeeded at all: `false` means the observable could never move (the bot registry declares no bot kinds, or every registered bot declares an empty `participation_evidence`), which is a structurally different operator signal from a genuine timeout where answerable bots simply stayed silent. The signal is derived from the REGISTRY alone and never from the observed comment set, so a wait that merely starts with no comments on the PR reports `detector_answerable: true`. `rate_limited_bots[]` is additive and bot-agnostic: it carries one `{bot_kind, rate_limit_class, eta, cause, cap, layer, body}` record per REGISTERED reviewer bot whose newest comment on the PR is a rate-limit / service notice posted in place of a review, and is empty (the default) when no registered bot is rate-limited. `cause` and `cap` are the orthogonal SIZE-vs-QUOTA axis, carried alongside the awaitability axis rather than derived from it — a size refusal and a quota refusal can share one `rate_limit_class`, so the class cannot answer which remedy applies. Both keys are present on EVERY record whatever the cause, so a consumer reads one shape. No bot is named in the detection path — the bot set and each bot's author login are derived from the per-bot registry docs under `automatic-review/standards/{bot_kind}.md`, and the notice itself is recognised by a structural two-part recogniser (a limit-exceeded statement AND a notice shape) paired with that bot's registry `ignore_patterns`.
  ...
  | `cause` | Why the bot declined: `size` (the diff is over a per-PR ceiling — the remedy is a smaller diff, and waiting never helps) or `quota` (a rate/budget limit — the remedy is backoff). Derived from the bot's registry `refusal_size_patterns`, a subset overlay on `refusal_patterns`. This is a SEPARATE axis from `rate_limit_class`, not a restatement of it: one bot can refuse for both causes at one class, so a consumer routing a remedy MUST read `cause`. A refusal matching no declared size marker is `quota`, the default. |
  | `cap` | The ceiling the size notice itself stated, extracted via that bot's registry `refusal_size_cap_patterns`, or `""` when it stated none. Read the figure together with `measured_diff_size` where available: the pair is what makes an accepted coverage gap auditable rather than asserted. An empty `cap` means *unknown* and is never defaulted — a figure nobody observed would make the gap look audited when it was not. The two figures are an ORDER-OF-MAGNITUDE comparison, since a provider's unit (Sourcery counts diff characters) need not match the measured one. |
  | `layer` | The recognition arm that read the notice, from the shared `_github_pr.REFUSAL_LAYERS` vocabulary: `registry_refusal_patterns` (the bot's own declared phrasing matched — reported even when the structural arm also fired) or `structural_fallback` (recognised by notice shape alone). The same field the `github_re_review` `refusals[]` record carries. |
  | `body` | The notice itself, whitespace-collapsed and truncated to one line — the same excerpt the `refusals[]` record carries. Untrusted bot text: a consumer interpolating it into a shell argument owns its quoting. |
...
**Merge-shaped rows are corroborated.** `pr merge`, `pr auto-merge`, `pr safe-merge`, and `pr merge-queue` never derive their success claim (`merged` / `disposition` / `enqueued`) from the CLI exit code — each establishes it from a read of provider state, and each fails closed (for `enqueued`, an unobservable membership is `indeterminate`, never `true`). The normative statement lives once in [`pr-operations.md`](pr-operations.md) § "The corroborate-not-report contract"; the rows below record only the resulting fields.

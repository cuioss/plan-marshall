...
**Rule 1 — insert the marker pair.** Applies **only when both hold**: `epic.md` carries a `## Ordered Queue` section, AND that section contains no `<!-- BEGIN GENERATED: ordered-queue -->` marker. When the pair is already present, Rule 1 does not apply at all. Its **two steps** run in the order below, and they are the whole of Rule 1. Zone creation is not a third step — it is the instruction above, which fires only when the zone is **absent** *and* there is content to move into it (a table carrying `Notes`). So it precedes step 1 on a ledger with `Notes` and no zone, and does not fire at all on one whose zone already exists or whose `Notes` column is empty:

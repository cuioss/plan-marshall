...
- [`plan-marshall:persona-security-expert/standards/secure-design-principles.md`](../../persona-security-expert/standards/secure-design-principles.md) § "Agents Rule of Two" — the general design principle this threat model instantiates. The reader holds all three high-risk corners (processes untrusted input, can reach sensitive state via `Read`, can communicate outward via `WebFetch`/`WebSearch`); the deterministic `validate_struct` boundary plus the read-only reader tool surface are how this surface downgrades the state-change/exfiltration corner rather than holding all three unmediated.

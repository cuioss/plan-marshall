---
name: pm-dev-java-junit-weld-testing
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/weld-testing-autowired.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "templates/cdi-test-class.java.tmpl" }` before continuing.
```

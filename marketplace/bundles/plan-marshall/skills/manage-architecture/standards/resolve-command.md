...
The canonical trigger is an OpenRewrite-bearing profile: a `quality-gate` that resolves `verify -Ppre-commit` rewrites tracked sources in place.

**Gate-context behavioural contract**: a worktree gate context (per-task verification, per-deliverable focused build, end-of-phase sweep, pre-push gate) runs the resolved command as resolved — including a `mutating: true` one — with **no operator confirmation and no preference for a different candidate**. Authoring a profile id into `build.maven.profiles.mutating` is a deliberate statement that this profile IS the gate; auto-fix is the gate's normal behaviour, not a hazard to be confirmed around. The signal rides through `derive-verification`'s command rows unchanged (the deriver copies the resolved dict), so a derived gate command carries the same field.

What the field is FOR is the settle-stage commit contract, not a prompt: the pre-push gate step declares `mutates_source: true`, so after it runs, the phase-6-finalize dispatcher's item-5f instrumentation checks `git status --porcelain` and commits whatever the gate rewrote onto the feature branch, which the order-11 push barrier then ships. See [`phase-6-finalize/SKILL.md`](../../phase-6-finalize/SKILL.md) § Step 3 item 5f. Removing the confirmation therefore removes a prompt, not a safeguard — the rewritten sources are committed and pushed rather than stranded.

**There is no competing candidate to prefer, and that is a structural property rather than a convention.** `_build_commands` (build-maven) builds its command map as one entry per canonical: it seeds `quality-gate` with the plain `verify` form and then **overwrites** that entry with `verify -P{profile}` when a profile maps to the canonical — the two never coexist, and only the first matching profile wins. `resolve` performs a single lookup on that one entry. A "prefer the non-mutating candidate" rule would therefore have had nothing to select among; stating it invited a reader to build the silent substitution it described, which is why it is stated as absent here rather than left unsaid. (The `verify` canonical keeps its own non-mutating entry, but it is a different canonical and is not a candidate for `quality-gate`.)

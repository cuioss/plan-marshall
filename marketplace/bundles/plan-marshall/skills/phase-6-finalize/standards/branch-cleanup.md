...
The `worktree-remove` verb resolves **main-anchored**: it derives both the move-back probe and the `git -C` target from `marketplace_paths.main_checkout_root()` (git's common dir), which names the main checkout from any linked worktree, so neither the tree it acts on nor the guard it applies is decided by where the caller happens to stand. The one thing it reads from the caller is `Path.cwd()`, and only in order to refuse — the containment check above (see `workflow-integration-git` Canonical invocations → `worktree-remove`):

...
- [ ] **Testing**:
      ...
      - By-reference triage path — OpenCode subagent dispatched as `verification-feedback` (under `--phase phase-6-finalize --role verification-feedback`, with `producer` runtime input) loads `manage-findings`, queries the per-plan findings store, processes findings with smart grouping, returns TOON.
      - Per-iteration parallel dispatch — `enrich-module` (dispatched under `--phase phase-6-finalize`) fans N dispatches in parallel under OpenCode's `task` tool model.

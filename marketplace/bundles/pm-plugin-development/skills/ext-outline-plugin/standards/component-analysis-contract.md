...
Each finding MUST be logged to decision.log. Hash IDs are automatically generated by the logging system.

```bash
python3 .plan/execute-script.py plan-marshall:manage-logging:manage-log \
  decision {plan_id} INFO "({agent_name}) {file_path}: {certainty} ({confidence}%)
  detail: {reasoning}
  evidence: {evidence}"
```

**Note**: The logging system automatically adds a 6-digit hash ID to every entry. Agents do not need to compute hashes.

### Logging Examples

Log output format: `[{timestamp}] [{level}] [{hash}] {message}`

```
[2025-12-11T11:14:48Z] [INFO] [a3f2c1] (skill-analysis-agent) path/skill.md: CERTAIN_INCLUDE (95%)
  detail: Has ## Output section with ```json block
  evidence: Lines 45-60

[2025-12-11T11:15:20Z] [INFO] [b7e4d9] (skill-analysis-agent) path/skill2.md: UNCERTAIN (45%)
...
FOR each file_path from filter script:
  ...
  # Log finding to decision.log (hash auto-generated)
  LOG_TO_DECISION_LOG(plan_id, file_path, certainty, confidence, reasoning, evidence)

...
**`terminalSequence` is the SOLE delivery channel, and it requires no tty ownership.** The hook subprocess never touches the terminal itself — it hands the escape sequence to Claude Code, which writes it on the subprocess's behalf. That is why the envelope works from any process the hook fires in, including ones with no controlling terminal. There is **no second channel**: a direct `/dev/tty` write needs a controlling terminal that no context here holds, so it silently cannot land inside a dispatched agent, a CI runner, or a backgrounded process, and the bundle no longer performs one anywhere.

The consequence is load-bearing for hook authors: **the channel is event-driven, not callable on demand.** A blocking window that no hook event spans (a long build, a CI wait, a lock hold) has NO channel — you cannot repaint during it. Write the state you want painted and let the next render event deliver it; see `plan-marshall:platform-runtime`'s `session push-title-token`, which binds and persists state rather than repainting. This is exactly why the `build-busy` clear rides the `PostToolUse:Bash` render trigger instead of a live push.

### Installed render-trigger hook entries

The terminal-title bundle installs eight render-trigger entries (plus `statusLine`): `SessionStart` (ONE matcher-less entry — it fires for every source, and the renderer turns `source == "clear"` into a session teardown rather than a render), `UserPromptSubmit`, `Notification`, `Stop`, `PreToolUse:AskUserQuestion`, `PreToolUse:Bash`, `PostToolUse:Bash`, and `PostToolUse:AskUserQuestion`. `PostToolUse` is **matcher-scoped**, not matcher-less: the title refreshes after the tool calls that change it, keeping the render cadence narrow.
...
That silence is why the bundle **removed** its `/dev/tty` write rather than keeping it as a fallback. A path that provably never lands is worse than none: it makes the write look implemented, so the missing repaint reads as a mystery instead of an absent channel.

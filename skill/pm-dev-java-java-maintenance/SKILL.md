...
```text
Call the `read` tool with `{ filePath: "standards/maintenance-prioritization.md" }` before continuing.
```
...
```text
Call the `read` tool with `{ filePath: "standards/refactoring-triggers.md" }` before continuing.
```
...
```text
Call the `read` tool with `{ filePath: "standards/compliance-checklist.md" }` before continuing.
```

---
name: plan-marshall-marshall-steward
...
```text
/plan-marshall-marshall-steward                        # Interactive menu or first-run wizard
/plan-marshall-marshall-steward --wizard               # Force first-run wizard
/plan-marshall-marshall-steward upgrade                # Post-change reconciliation — asks before each stage
/plan-marshall-marshall-steward upgrade integrate=true # Post-change reconciliation — runs all four stages end-to-end
```
...
The Main Menu has 7 options, which exceeds the `ask_question` 4-option cap. It is presented as a paginated menu following the "More actions..." pattern documented in `plan-marshall/workflow/planning.md` (§ Action: list): each page presents at most 4 options, and every non-final page reserves its 4th slot for a "More..." continuation that triggers the next page's `ask_question`. Page 2 carries 4 options and needs no further continuation.
...
| "More..." | Present Main Menu Page 2 `ask_question` |
...
| `menu-maintenance.md` | Regenerate executor, cleanup. The cleanup operation routes to the `Action: cleanup` workflow — see [`../plan-marshall/workflow/planning.md`](../plan-marshall/workflow/planning.md) § "Action: cleanup", the sole authority for what that pass does. | Menu option 1 |
...
The `docs/` prefix is explicitly retired and must not be re-admitted — a
...
`ask_question` gate that consumes a deterministic diff, mirroring the existing
...
gate any re-seed behind an `ask_question` so deliberate hand-edits are never
clobbered:
...
itself is unscoped over the whole working tree. The full procedure — diff detection, the land/leave `ask_question`

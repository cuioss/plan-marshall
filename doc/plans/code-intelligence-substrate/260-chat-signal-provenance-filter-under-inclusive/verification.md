...
|---|---|---|---|
| A1 | False positives | No gap is fabricated. All eight were re-derived at their cited `path:line`: G1's predicate escape and its 30-turn end-to-end reading reproduce exactly (`raw 30 kept 30 operator 30 no_signal False`); G2's contract sentence at `chat-history-analysis.md:52` is verbatim; G3's balanced-only nesting tests are at `test_chat_provenance.py:63`/`:73` inside `TestSyntheticClasses`; G4's `3224ea1` is confirmed *after* `809022d` via the PR commit list, with 11+30+34 = **+75** lines across the three named test modules and the PR body's "20379 passed, 14 skipped" against the report's 20371; G5's merged head is `07c8c00e735…`; G6's raw-text render at `:251-253`; G7's `is_operator_authored('[Request interrupted by user]')` → `True`; G8's zero `AskUserQuestion` tool calls. Two stale citations found | Fixed `:286-288` → `:289-290` (skipped-branch counters) and `:55-64` → `:54-64` (allow-list rationale) in the D3/D1 rows |
| A2 | False negatives | **The material finding of this review.** G1 is real but was characterised as one shape when it is two, and the demonstrated one is the *less* reachable. (i) A body quoting its own **close** tag escapes by a different branch — the quoted close pairs with the outer open, `_drop_above(0)` clears the stack, and the tail becomes residue: `is_operator_authored('<sr>a</sr>b</sr>')` → `True`. G1's proposed greedy re-pair fixes the open-quote variant and **not** this one. (ii) The quoted-open variant is neutralised by nesting: inside a `<task-notification>`'s `<result>` body it classifies **synthetic**, so on the only envelope class observed reaching the reducer inline (36 `<task-notification>` turns; `<system-reminder>` appears as an inline `user` text block in **0 of 6,190** turns) the *close*-quote variant is the reachable one. Re-read D1/D3/D4 independently against the shipped code — the counters, the gate channel, `reduced + dropped == raw`, the untouched threshold, the fail-open surfaces (`errors='replace'`, the `FileNotFoundError`-only except, the `is_file` guard, self-closing and blank `<command-args>`, CRLF skill-load) — no further false CONFIRMED | Rewrote D2's defect detail, Correctness-review Defect 1 and G1 to carry both variants, the asymmetric reachability, and an Action/Done-when that covers both (with the `OPERATOR_BEARING_TAGS` constraint that rules out the naive whole-span fallback). Widened G3 to two shapes and required the nested case, since a flat quoted-open test would pass pre-fix and be vacuous |
| A3 | Vacuous evidence | Mutation sweep **re-run end to end**, snapshots to `$TMPDIR/adv-260-…-mutsweep/`, restored by hand, `git status --porcelain` clean after: 10 of 10 KILLED, reproducing the audit's table. One attribution defect: M8 (gate-decision `role == 'user'` → `True`) **survives** `test_extract_chat_signal_verdict.py` alone (17 passed) and is killed only by `test_extract_chat_signal_io.py::TestRoleGuards::test_a_tool_result_on_an_assistant_turn_is_not_a_gate_decision` — a module the Test-adequacy table does not map to D3. Non-vacuity of the discriminating regression independently confirmed by extracting the pre-fix reducer from `8a11858^` (`no_signal = len(turns) == 0`) and re-measuring the shipped fixtures: 4/5 and 51/51 kept, both `no_signal: false`; post-fix 0 and 1, both `true`. G6's "11,563 bytes" is not reproducible from any shipped fixture or corpus turn — it came from an unnamed constructed input, and the corpus-measured value is **0 bytes** of envelope across all 82 kept operator turns | Recorded M8's real killer in the mutant table; replaced G6's figure with a reproducible fixture measurement (199 bytes rendered vs 27 of residue, ~86 % envelope) plus the corpus-measured zero, and marked its false-Tier-2-refusal rationale prospective |
| A4 | Counts and quotes | Every number re-derived at review time and **held**: 160 tests; per-module 34/14/22/17/20/53; 303 `assert`s (55+21+27+52+28+120); modules 359/207/108; `test_extract_chat_signal.py` 575 lines; 318 over-budget test modules; `test-module-line-budget` `severity='warning'` at `_analyze_test_conventions.py:61`; `parse_turn` with one definition and 15 references all in one test module; `DEFAULT_READ_BUDGET_BYTES` byte-identical pre/post; zero `by construction` in the three chat modules; commits `2b5d1aa`, `18b1b5c`, `bb85899` and `coupling-inventory.md:84`. Quotes verbatim against source. Only the corpus figures moved (42/8,382/42 → 81/16,163/82) — that directory grows with every session on this machine, so the figure is a snapshot, not a fact about the code | Re-stated the corpus counts as a snapshot with both readings and the invariant qualitative result |
| A5 | Actionability | Seven of eight entries are executable by a run that has read neither plan nor audit. Two weaknesses: G1's Done-when pinned only the shape that would not have been fixed on the real block anyway, and G8's action is contingent on an `AskUserQuestion` exchange that exists in **no** reachable transcript, which the entry did not say | G1's Done-when now names three concrete inputs including the real nested shape; G8 now states the contingency and the correct disposition (record as blocked, do not substitute another hand-built fixture) |
| A6 | Severity and topic | Calibration holds for all eight. G1 `high` is right — a guard that cannot fire plus a measurement that misreports. G7 was probed for under-severity and stays `low`: an interrupt *is* an operator action, so `no_signal: false` is the correct verdict and only the counter attribution is wrong; the corpus carries 0 such turns. G2 `medium` (false claim in shipped documentation), G3 `medium` (missing test on a load-bearing path), G4/G5 `low` (stale claims confined to the run report), G6 `low` (behaviour unchanged from pre-fix). Topics match the owning surfaces | Added the severity rationale to G7 so the `low` is defensible rather than asserted |
| A7 | Coverage | All four deliverables carry a verdict; out-of-scope compliance, report accuracy and the full declared-residue list are each covered. Nothing in `plan.md` is silently unmentioned — the four exclusions, the Expected-surface entries (including the two `HYPOTHESIS` ones) and the validation trap all appear | None needed |
| A8 | Internal consistency | The `CONFIRMED WITH GAPS` verdict follows from one PARTIAL and three CONFIRMED rows. Every actionable finding maps to a gap (Defect 1→G1, Defect 2→G2, test gap→G3, Observation 3→G6, Observation 4→G7, report items→G4/G5). One weak trace: G8 rested only on a "Could not check" bullet, with no finding in the body | Added Observation 5 to the Correctness review recording the unwitnessed gate-decision channel, so G8 traces to a stated finding |

**Residual doubt:** the highest-value next probe is the pairing walk itself rather than the audit.
This review found the second escape variant by enumerating shapes by hand, which is the same
sampling method the plan's own thesis calls unreliable — a systematic search (a fuzzer over
balanced/unbalanced tag sequences asserting *"no turn consisting solely of tag tokens and whitespace
classifies operator"*) would very likely find a third. Second: every reachability claim here rests on
one machine's `~/.claude/projects` corpus, which represents one harness surface; the contract itself
records at `chat-history-analysis.md:62` that other surfaces persist these blocks differently, so
"latent" is a statement about this surface, not about the harness. Third, minor: G1's severity is
argued on the escape's consequence, and the *upper* bound on that consequence — text reaching the
Tier-1 prompt labelled as operator — was reasoned through, not exercised against a live retrospective
run.

**Verdict on the audit:** SOUND AFTER CORRECTION — every gap it filed is real and every count it
stated re-derives, but its headline defect was scoped to one of two escape variants and demonstrated
on the variant that does not fire on the block shape the reducer actually receives, so the fix it
prescribed would have closed half the hole.

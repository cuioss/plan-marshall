...
class TestGatedZero:
    ...
    def test_a_gate_that_declined_every_row_is_gated_not_disciplinary(self):
        assert audit._classify_zero(_gated_block(excluded=2), 0, corpus_size=4) == audit._ZERO_GATED
    ...
    def test_a_gate_that_declined_nothing_is_still_disciplinary(self):
        ...
        assert audit._classify_zero(_gated_block(excluded=0), 0, corpus_size=4) == audit._ZERO_DISCIPLINARY
    ...
    def test_a_block_that_fired_is_not_gated(self):
        ...
        assert audit._classify_zero(_gated_block(excluded=2, genuine=3), 3, corpus_size=4) == audit._ZERO_NONE
    ...
    def test_starved_outranks_gated(self):
        ...
        """
        assert audit._classify_zero(_gated_block(excluded=2, examined=0), 0, corpus_size=0) == audit._ZERO_STARVED
...
class TestZeroClassification:
    def test_unmeasured_status_is_a_structural_zero(self):
        assert audit._classify_zero(_UNMEASURED_BLOCK, 0, corpus_size=5) == audit._ZERO_STRUCTURAL

    def test_empty_corpus_is_a_starved_zero(self):
        assert audit._classify_zero(_MEASURED_ZERO_BLOCK, 0, corpus_size=0) == audit._ZERO_STARVED

    def test_measured_zero_over_a_real_corpus_is_disciplinary(self):
        assert audit._classify_zero(_MEASURED_ZERO_BLOCK, 0, corpus_size=5) == audit._ZERO_DISCIPLINARY
    ...
    def test_structural_outranks_starved(self):
        ...
        """
        assert audit._classify_zero(_UNMEASURED_BLOCK, 0, corpus_size=0) == audit._ZERO_STRUCTURAL
    ...
    def test_structural_outranks_no_count(self):
        ...
        """
        assert audit._classify_zero(_UNMEASURED_BLOCK, None, corpus_size=5) == audit._ZERO_STRUCTURAL
...
class TestMissingCountVersusMeasuredZero:
    ...
    def _row(self, per_check_genuine: dict) -> dict:
        rows = audit.suspect_zero_census([_MEASURED_ZERO_BLOCK], per_check_genuine, {}, corpus_size=5)
    ...
    def test_a_check_absent_from_the_counts_is_no_count_with_a_blank_signal(self):
        ...
        assert row['genuine_signal_count'] == '', (
            'the blank is absence of a reading; a 0 here would be indistinguishable from a measured zero'
        )
    ...
    def test_the_two_states_tally_on_different_axes(self):
        ...
        measured = audit.emit_suspect_zero_census_block(
            audit.suspect_zero_census([_MEASURED_ZERO_BLOCK], {self._CHECK: 0}, {}, corpus_size=5),
            corpus_size=5,
        )
...
class TestCensusRows:
    ...
    def test_quiet_streak_is_carried_per_check(self):
        rows = audit.suspect_zero_census([_MEASURED_ZERO_BLOCK], {'dispatch-topology': 0}, {'dispatch-topology': 7}, 5)
...
class TestCensusEndToEnd:
    ...
    def test_a_starved_detector_is_structural_on_a_real_sweep(self, tmp_path: Path):
        ...
        row = next(ln for ln in output.splitlines() if ln.strip().startswith('merge-window-accounting,'))
        assert audit._ZERO_STRUCTURAL in row
        assert ',true,' in row

    def test_a_measuring_check_over_the_same_sweep_is_classified_differently(self, tmp_path: Path):
        """The discriminator — without it this would pass on an all-structural census."""
        inputs = minimal_corpus(tmp_path)
        output = audit.run_checks(inputs, list(audit.CHECK_NAMES), tmp_path)

        row = next(ln for ln in output.splitlines() if ln.strip().startswith('dispatch-topology,'))
        assert audit._ZERO_STRUCTURAL not in row

    def test_no_registered_check_reports_disciplinary_over_an_empty_population(self, tmp_path: Path):
        ...
        output = audit.run_checks(inputs, list(audit.CHECK_NAMES), tmp_path)
        blocks = {name: output.split(f'check: {name}\n', 1)[1].split('\ncheck: ', 1)[0] for name in audit.CHECK_NAMES}
        census = output.split('check: suspect-zero-census', 1)[1]

        offenders = []
        for check in audit.CHECK_NAMES:
            row = next(ln for ln in census.splitlines() if ln.strip().startswith(f'{check},'))
    ...
    def test_a_readable_lock_log_with_no_merge_events_is_a_measured_zero(self, tmp_path: Path):
        ...
        census = output.split('check: suspect-zero-census', 1)[1]
        row = next(ln for ln in census.splitlines() if ln.strip().startswith('merge-window-accounting,'))
        assert f',{audit._ZERO_DISCIPLINARY},' in row

    def test_no_registered_check_is_classified_no_count_on_a_real_sweep(self, tmp_path: Path):
        ...
        census = output.split('check: suspect-zero-census', 1)[1]
        offenders = [ln.strip() for ln in census.splitlines() if f',{audit._ZERO_NO_COUNT},' in ln]
        assert offenders == []

    def test_a_check_narrowing_its_own_population_to_zero_is_starved(self, tmp_path: Path):
        ...
        census = output.split('check: suspect-zero-census', 1)[1]
        row = next(ln for ln in census.splitlines() if ln.strip().startswith('exploration-share,'))

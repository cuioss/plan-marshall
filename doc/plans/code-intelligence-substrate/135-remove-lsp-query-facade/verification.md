...
| D1 — `lsp` no longer parses | none — asserted by direct CLI invocation here | **Unguarded, and D1's acceptance rests on it.** The invalid-choice behaviour is cited as D1 evidence in the verdict row (`:24`) and re-derived in the D1 detail (`:59-81`) — twice, through `architecture.py` and through `.plan/execute-script.py` — but both are one-off invocations made by this audit, not standing assertions. Nothing in the suite would go red if the `lsp` subparser were re-added. Two honest dispositions, and a later run must pick one rather than leave the row as it stands: **(a)** add a focused parser/CLI test asserting `lsp` is not among `architecture.py`'s subcommand choices, which pins the invariant D1 was accepted on; or **(b)** drop the invalid-choice behaviour from D1's acceptance criteria and rest D1 on the removal evidence that *is* guarded (no `cmd_lsp_*` symbol, no import, no dispatch entry, `test_lsp_facade.py` absent, 573 wrapped-verb tests green). The earlier judgement that a "verb X must not exist" test is a poor guard shape is a reason to prefer (b) over (a); it is not a reason to cite the behaviour as evidence and guard it with nothing |

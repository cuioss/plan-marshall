...
| `documentation_only` | every claimed path resolves under the `documentation` role (e.g., `pm-documents` `*.md`, `pm-plugin-development` `marketplace/bundles/*/skills/*/SKILL.md`) | `implementation` only — NEVER `module_testing`, NEVER a paired pytest task | when any claimed path is a marketplace skill `.md` body (`marketplace/bundles/**/skills/**/*.md`): `pm-plugin-development:plugin-doctor:doctor-marketplace quality-gate --paths {skill-dir} --marketplace-root {worktree_path}/marketplace` (rule-complete, scoped to the touched skill; NOT the rule-less `list-components`); otherwise (non-marketplace docs, outside the marketplace tree where the rule set does not apply) markdown validation + manual end-to-end where relevant |
...
**Canonical plugin-doctor notation**: `pm-plugin-development:plugin-doctor:doctor-marketplace quality-gate --paths {skill-dir} --marketplace-root marketplace` (NOT the stale `:plugin-doctor:plugin-doctor`, and NOT the rule-less `scan`/`list-components` enumerate verbs). All `documentation_only` and `mixed_with_docs` deliverable Verification fields MUST cite this rule-running, scopeable gate.

**`quality-gate --paths` vs the rule-less enumerate verb for marketplace skill `.md` edits**: when a `documentation_only` (or `mixed_with_docs`) deliverable touches marketplace skill `.md` bodies (`marketplace/bundles/**/skills/**/*.md`), its per-deliverable verification MUST use the rule-complete, scoped `pm-plugin-development:plugin-doctor:doctor-marketplace quality-gate --paths {skill-dir} --marketplace-root {worktree_path}/marketplace` form — NOT the rule-less `list-components` enumerate verb. `list-components` is a navigation/inventory mode that runs ZERO rules, so its output OMITS `analyze_lesson_id_in_skill_prose` (and every other quality-gate rule); a clean local `list-components` pass is NOT evidence that CI's `verify / verify` plugin-doctor stage (which runs the full `quality-gate` rule set) will pass — a local-gate-≠-CI-gate divergence on the operation-mode/rule-set axis. The scoped `quality-gate --paths {skill-dir} --marketplace-root {worktree_path}/marketplace` form is the canonical pre-push proxy for CI's plugin-doctor stage: it runs CI's exact rule set, filtered to the touched skill, while sidestepping fresh-worktree newer-than-CI ruff noise that blocks using the whole-tree `quality-gate` as a clean local proxy. Non-marketplace documentation paths (outside the marketplace tree, where the quality-gate rule set does not apply) use markdown validation plus a manual end-to-end pass — there is no rule-running doctor gate for them.
...
- phase-4-plan's contract-violation guard reads the resolved bucket from the `<!-- bucket: ... -->` comment to decide whether to emit a `*-Test` task or a Q-Gate finding (see `marketplace/bundles/plan-marshall/skills/phase-4-plan/SKILL.md`).

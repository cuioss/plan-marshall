# PLAN-PR-026: Nobody-reviewed and reviewed-clean become two signals, on both surfaces that claim to separate them
...
> shared target surface; D1–D6 and D3a are carried there as D1–D7, with **D1 merged with `PLAN-PR-047`
> D0** because the two name the same artifact from the producing and consuming side. ⛔ **This file is
> NOT dead and is NOT deleted**: it remains the AUTHORITATIVE TEXT of every deliverable body, and
> `PLAN-PR-061` points here rather than retyping it.

epic: review-apparatus
workstream: WS-03

> Staged plan spec, ingested 2026-08-23 from the cloud-lane corpus (``PLAN-PR-026` (this directory)`,
...
Separate "nobody reviewed" from "reviewed clean" on both surfaces that claim to distinguish them, and give the deficit signal a caller.
...
Every load-bearing claim reproduces at HEAD, several at exact line. The three grade strings measure
73 / 50 / 109 characters and none is ASCII — re-derived, not accepted.

⛔ **Correct before handing this spec to a run:**

1. **D1's central Done-when is unfalsifiable as written.** It requires tests that exercise "both step
   paths — the producer's write and the retrospective's read". **Neither step path is code**: both are
   prose instructions an LLM executes. Replace with a seam that exists — a `manage-files write` ->
   `read` round trip driving `review_retrospective.main(argv)`, which no test invokes today.
2. **D0 leg 4 gates vacuously.** The four legs confirm the verbs exist; none confirms the ORDERING
   property the claim table itself labels HYPOTHESIS. (The premise does hold at HEAD — the
   retrospective already writes its artifact at `order: 990` — but the gate must say so.)
3. **D3's "(it derives neither today)" is half-contradicted:** `assess_deficit` already takes
   `required_bots` (`:615-616`). Only `bot_lists_provenance` is missing.
4. **D1 and D4 both rewrite the same three table rows** (`SKILL.md:190-192`). Assign them to one.
5. **Collateral omission:** `test_pre_merge_barrier.py` reads `verdict['bot_states']` (`:372`, `:636`)
   and is absent from § Expected Surface; D3's `quorum_basis` addition changes what it observes.
6. **Fold in, from the ingestion audit:** `040 G10` — D4's worked example is character-for-substance
   the string that gap forbids ("no reviewers configured" claims the operator answered none, false in
   the default case); restore the `bot_lists_provenance` clause to its Done-when. And `040 G15` —
   change the `min_deficit` default OR defend it in the contract with a pinning test; a run-report
   proposal is neither.

⚠ **Scope: 7 deliverables — over the guard. Split before emitting**: (a) D0+D1+D2, the handoff seam at
both ends; (b) D3+D4+D5+D6.

## Dependencies and Sequencing

- **Depends on: PLAN-PR-025** landing first, so the `deficit`-synopsis handshake resolves in the
  documented direction and `review_completeness.py` is not contended.
- **MUST NOT run concurrently with PLAN-PR-027** — both write inside `automatic-review/SKILL.md`
  § Canonical invocations -> `review_completeness — deficit` (this plan rewrites the verdict list at
  `:1022-1025`; that plan adds a flag-FORM note to the same block), and both edit the barrier block of
  `branch-cleanup.md` within twenty lines of each other.
- **MUST NOT run concurrently with PLAN-PR-030** — `finalize-step-review-retrospective/SKILL.md` is
  shared and has **no split authority at all**; PLAN-PR-030 D3 also narrows
  `test_counting_rule_parity.py`, which pins the delegation this plan touches.
- ⚠ **This plan is a FOURTH editor of the § Consumers table the shared-document table does not name**:
  D2's three-valued `participation` change invalidates the `finalize-step-review-retrospective` row.
  Either add the row to the table or record the exemption here.
- Overlaps with: PLAN-PR-025 and PLAN-PR-031 (`review_completeness.py`, `test_structural_refusal.py`),
  PLAN-PR-024 and PLAN-PR-028 (`branch-cleanup.md`).
...
## ⛔⛔ THE POPULATION IS NOW n=4 — four consecutive PRs, one bot deep or less

Folded 2026-08-27 from inbox `-005` (PR #1356, first-party) and `truthful-signals-038` (PR #1359,
forwarded). **Derived from the four landings, not asserted:**

| PR | Required (`pr-agent`) | `coderabbit` | `sourcery` | Barrier said |
|---|---|---|---|---|
| #1340 | empty clean guide | credited on **stale** evidence | **reviewed nothing** across 6 HEADs | `participation_complete: true` |
| #1349 | contentless | — 1 empty / 2 refused, **no bot produced a finding** — | | round complete |
| #1356 | **empty on every HEAD** | **all 22 comments**, at its 1/hour ceiling | **structural refusal**, read none of 4493 lines | 6/6 shipped |
| #1359 | **0 actionable over 4 rounds** | carried the gate | — | merged |

⭐ **The invariant across all four: the REQUIRED bot contributed nothing, and an OPTIONAL bot carried
whatever review happened.** That is no longer an anecdote — it is the steady state, and it sizes both
this plan and PLAN-PR-025 D7.

⛔ **Read it precisely.** On #1349 the measurable-review population was **zero**, so that PR supports no
quality claim in either direction. #1356 and #1359 DO carry measurable reviews, and in both the
optional bot produced everything. ⇒ The supportable statement is about **coverage and roster
composition**, not about model quality: *a required set of one, whose member is reliably empty, is a
gate that cannot fail.*

⚠ Sourcery's structural size refusal recurs at #1356 (4493 lines) — the diff-size dimension this epic
already measured at 179,695 vs a 150,000 cap. **The source/test split remedy applies here too.**

## ⭐⭐ FOLDED IN 2026-08-27 — the participation signal IS the defect (8 lessons)

From inbox `lessons-handling-26-08-26-01-002.md`. Its suggested fold targets (`PLAN-PR-005/006/007/013`)
have all **shipped**, so the cluster lands here. Every lesson is already in the global corpus; this
fold turns them into scope and allocates nothing new. ⚠ **Eight lessons is past the scope-bloat guard**
— split at outline; the two sub-tables below are the natural cut.

⛔⛔ **READ FIRST — the directive that rules out the obvious plan.** `2026-08-26-05-004` is the
corpus's strongest reviewer-comparison datapoint and it argues AGAINST a charter round: *"Do not spend
another round on bot charter exhortation. The failure here is empty/refused/refused-structural, not
weak findings — a charter change addresses none of the three."* ⚠ And it is equally explicit about
what it does NOT support: the population of **measurable** external reviews on that PR is **zero**, so
**no quality claim about the bots is supportable from it in either direction**. What IS supportable:
**the merge barrier let a PR through with zero measurable external participation.** ⭐ This supersedes
the framing in this epic's own 2026-08-26 Watch — keep the observation, drop any reading of it as a
verdict on bot quality.

**A. The three participation-detection defects — the actionable core.** Each is a specific named
mechanism by which a NON-review is recorded as a review:

| Lesson | Mechanism |
|---|---|
| `2026-08-26-16-001` | A Sourcery rate-limit refusal **credited as participation** — a third budget phrasing matches no `refusal_pattern` and no structural fallback |
| `2026-08-26-17-001` | pr-agent reports `absent` because its response **outruns `review_bot_buffer_seconds`** and it publishes **no check-run to wait on** |
| `2026-08-25-07-001` | `review_retrospective._grade_comparison` returns clean on a NON-EMPTY intersection, so one `participated_but_empty` reviewer grades a 1-of-3-with-two-refusals run **identically to 3-of-3-reviewed-clean** |

**B. The four contract-shape lessons** — why the detection keeps failing the same way:

| Lesson | Rule |
|---|---|
| `2026-08-08-21-001` | **A boolean derived from a fallible read has THREE outcomes; a two-branch `if` folds the third into the `else`**, asserting a definite state on evidence nobody gathered. It **neutered a deliberate fail-loud guard**. ⭐ Second-order: *a change that establishes a fail-closed discipline is not thereby compliant with it* — its own new call sites are where to look |
| `2026-08-08-21-005` | **A correctness fix has a direction, and the direction has an error budget.** Filtering runs by `pull_requests[].number` is unsafe — GitHub populates it unreliably (routinely empty for forks), so a strict filter **blocks merges** on the common fork case. ⭐ *A field an upstream API populates "usually" is not a key* |
| `2026-08-08-21-004` | **When a change splits one state into N, the deliverable is N remedies, not N names.** `not_triggered`'s remedy shipped as prose with no invocation. ⭐ Cheap check: **grep the new members' remedy prose for an actual invocation** |
| `2026-08-25-09-015` | **A consumer gated on ONE of two sets a producer reports DISJOINTLY is unreachable, and its test pins the gap** — PLAN-PR-034's own shipped defect, caught at round 7 of 7 |

### D3 — Stop rendering an empty required population as a positive result

**Gaps: 040/G2, 040/G3, 040/G12, 050/G13, 040/G15.**

⭐⭐ **LIVE INSTANCE FOLDED IN 2026-08-26, from PR #1349's drain (inbox `-009`): "a required-bot set of
ONE, whose member's default output is contentless, cannot tell a reviewed PR from an unreviewed one."**
This is this deliverable's thesis, observed rather than argued. On #1349 `automatic-review` recorded
**0 comments, 1 empty / 2 refused** — the required set is `pr-agent` alone, its default output is
content-free, and two of three bots never read the diff. ⛔ **The step still reported a completed
review round.** ⚠ Note the compounding: an empty required set and a required set of ONE whose member
returns nothing are *different* inputs that this surface renders identically, so D3's empty-population
fix does not by itself cover the singleton-contentless case. **Cover both**, and pin the singleton case
with its own test. ⭐ Sizes D7 of PLAN-PR-025 (promoting CodeRabbit to required) from the other
direction: a quorum of one cannot be a quorum.

⛔ **Precondition: D0 reported PASS on all four legs.** If D0 halted, this deliverable does not start.

⛔⛔ **PRE-EMIT AMENDMENT 2026-08-25 (amendment 3 of `../cloud-wave-audit.md` § 9) — `040 G15` IS
FOLDED IN HERE AS A REAL DELIVERABLE. A RUN-REPORT PROPOSAL DOES NOT DISCHARGE IT.** `040 G15`'s
Done-when is *"the default is either changed or defended in `bot-participation-contract.md` § 'The
comparative deficit signal', and the test pins whichever value is chosen"* — a proposal recorded in a
run report is neither a change nor a defence, and pins nothing. § D6 previously carried this as one of
its three record-only proposals with an explicit **"Do not change the default in this run"**; for
`min_deficit` that instruction is **superseded by this amendment** (the other two D6 proposals are
untouched and stay record-only).

**Do here, choosing exactly one and recording the reasoning:**
- **Change** `assess_deficit`'s `min_deficit: int = 1` default to a genuinely material gap — 2, or a
  proportional rule — updating the docstring AND `automatic-review/SKILL.md`'s restatement; **or**
- **Defend** `1` explicitly as the right floor, in `bot-participation-contract.md` § "The comparative
  deficit signal", which today repeats "materially" at two sites and names no threshold at all.

⭐ **Either way the test pins the chosen value** — `test_min_deficit_threshold_is_honoured` currently
pins the behaviour at `1`, so leaving the default while adding no defence would leave a test pinning
an undefended number. The threshold is already a parameter, so only the default and its justification
are at stake. ⚠ The stake is real: at `1` a one-finding gap between two reviewers on the same diff is
ordinary variance, so the signal fires routinely — which is how an observability signal becomes noise
and stops being read, the exact failure this plan's own ⛔ on cases (b) and (c) warns about, arriving
through the threshold instead of through the baseline.
...
#### D3a — The same collapse at the QUORUM layer, not just the per-bot layer

⭐ **Folded from `truthful-signals-050.md` on 2026-09-06**, relayed from `PLAN-TRUTH-128`'s finalize
(`freshness-gate-says-fresh-unexamined-tree`, PR #1425). Their first-party observation; folded here
rather than staged because the relaying epic explicitly asked for the duplication check against this
spec and `PLAN-PR-048` first, and it is a **third instance of the shape D3 already owns** — one layer up.
...
⛔ D3 establishes that a single bot's silence and its clean review are one signal. This says **the
aggregate over required bots inherits the same collapse**: a quorum computed from participation cannot
distinguish *three reviewers found nothing* from *three reviewers said nothing*.

⭐⭐ **Their run is a positive control for the distinction it lacks** — CodeRabbit reviewed **three
times** and filed **9 actionable items, 8 fixed, none suppressed, two of them genuine fail-open
contract defects.** The yield was real and high, **and the quorum's verdict would have been
byte-identical had it been zero.** ⇒ The gate's output carries **no information about the thing that
mattered.**

*Done when:* D3's remedy is applied at the quorum layer too — the aggregate verdict names its yield
population, not only its participation population — and a test pins that a zero-yield quorum and a
high-yield quorum do not render the same verdict.

⛔⛔ **A FIFTH instance, and it closes a step as `done` over its own outstanding work — folded
2026-09-15 from `truthful-signals-058` (relayed from `plan-truth-148`, PR #1488).** The
`automatic-review` step recorded `outcome=done` with `0 comment(s) found` while its OWN
`display_detail` named three different states: *"1 reviewed, 1 empty, 1 refused-structural (triage
pending)"*. The retrospective independently recorded *"1 of 3 reviewers measurable, 8 actionable
comments"*.

⇒ **`done` + `0 comments found` is not "the reviewers were happy"** — it is one measured reviewer and
two that produced no signal. ⛔ And `(triage pending)` inside a `done` detail is a **contradiction**:
pending triage is outstanding work the record is reporting as complete. *Done when (added):* a review
step's close publishes the **per-bot participation split** rather than a bare comment count, and a
pending triage cannot coexist with a `done` outcome. ⚠ The remedy is already carried by the emitted
`PLAN-PR-061` D1/D3 — this fold sharpens the brief it points at, and adds no file surface.

⭐ **A FOURTH instance, folded 2026-09-13 from `PLAN-PR-033`'s inbox `-002`, first-party on #1473.**
`cuioss-review-bot` — the REQUIRED bot, whose participation gates the merge barrier — **published
nothing but contentless status summaries across an entire run**, while CodeRabbit (optional) filed the
5 actionable findings. The merge gate was satisfied by the reviewer that found nothing and would have
been satisfied identically had it read no diff at all. ⛔ This is the same collapse as D3a's, on a
DIFFERENT run and a different repository state — the invariant's population grows, and the remedy does
not change. ⚠ Keep it in the plan-marshall population (a first-party PR), unlike the using-project
observation the epic's Watches explicitly fence off.

⚠ **The clean path's own log line is a third unscoped zero — folded 2026-09-11 from
`truthful-signals-054.md` item 7** (relayed, API-Sheriff `-003`). Re-read first-party at `356973d80`:
`branch-cleanup.md`'s clean-barrier work-log line reads *"Pre-merge review barrier: clean — zero pending
pr-comment findings, required-bot participation complete"*. The GATE half is already fixed (#1199 —
`phase_handshake findings-check` counts every actionable type), so this is not a gate defect; it is a
reader-facing zero stated over ONE finding type with no population, rendered by the very clean path
D3 rewrites. *Done when (added):* the clean line names the finding types and the population its zero
was counted over, the same way D3 makes the quorum name its provenance. Adds no file surface
(`branch-cleanup.md` already declared).
...
- **The empty-roster fallback (040/G10)** at `automatic-review/SKILL.md` ~798 falls back to
  ...
  the *composition* site, leaving `compose_review_state_summary`'s `''` return and its stated
  rationale unchanged — inventing a bucket for reviewers that were never configured would be a claim
  about nothing.

  ⛔⛔ **PRE-EMIT AMENDMENT 2026-08-25 (amendment 2 of `../cloud-wave-audit.md` § 9) — THE WORKED
  EXAMPLE HERE WAS THE STRING `040 G10` FORBIDS, AND THE `bot_lists_provenance` CLAUSE IS RESTORED.**
  This bullet previously offered `"{N} comment(s) found - no reviewers configured"` as the example
  rendering. `040 G10`'s Task rules that out in as many words:

  > ⛔ Do **not** assert configuration from the empty roster: a string such as `"no reviewers
  > configured"` is a claim that the operator answered *none*, and it is false in the default case,
  > where the question was never put.

  An empty roster and an answered-empty roster are **two different facts**, and the epic's own reason
  for the EMPTY default is that *"a never-asked key stays distinguishable from an answered-empty
  value"* (`automatic-review/SKILL.md:24`). Rendering both as "no reviewers configured" collapses
  exactly the distinction this plan exists to preserve — the same defect one level up, in the fix.

  **Do instead:** consume `bot_lists_provenance` (`never_asked` / `migrated` / `answered`) and render
  the two cases **distinctly** — the split `050 G7` owns — mirroring the house pattern in
  `create-pr.md`'s `skip-bot-review` label rules (~`:258-259`), which refuse the label on an
  empty-plus-`never_asked` posture and apply it only on an answered one. ⚠ A `never_asked` posture
  must render as *the question was never put*, never as *the answer was none*.
...
- **The `min_deficit` default (040/G15).** `assess_deficit` takes `min_deficit: int = 1`, documented
  ...
  ⛔⛔ **SUPERSEDED 2026-08-25 — this item is no longer record-only. `040 G15` is now folded into D3
  as a real deliverable (amendment 3 of `../cloud-wave-audit.md` § 9), because G15's Done-when
  requires the default to be CHANGED or DEFENDED with the test pinning the chosen value, and a
  run-report proposal is neither.** The former instruction here — *"Do not change the default in this
  run"* — no longer applies to `min_deficit`; act on it at **D3**. ⚠ The other two proposals in this
  section (the refusal pre-filter `040 G13`, and the third) are UNCHANGED and remain record-only.
...
`marketplace/bundles/plan-marshall/skills/workflow-integration-github/scripts/_github_pr.py`, `marketplace/bundles/plan-marshall/skills/automatic-review/scripts/bot_registry.py`, and
`.claude/skills/cloud-plan-lane/SKILL.md`.

## ⭐⭐⭐ FOLDED IN 2026-08-31 — a MATCHED PAIR for the wait-vs-override question, same reviewer, same week

Transferred from `truthful-signals-041` under the three-way rule. ⚠ **RELAYED, not re-derived here.**

This spec's thesis is that *nobody reviewed* and *reviewed clean* are one signal. Two plans landing
the same week took **opposite** decisions on that signal, and **both outcomes were observed** —
which converts the question from an anecdote into a population:

| Plan | Decision | Outcome |
|---|---|---|
| **PLAN-TRUTH-109** (#1369) | **WAITED** — closed #1367 unmerged and reopened to recover a CodeRabbit review | The recovered review produced **5 findings that 5 internal self-review passes had missed**, including a `RuntimeError` escaping a guard whose own function documented a `(None, reason)` contract — **in the code that plan had just added**. ⛔ Merging on the contentless quorum would have shipped it. |
| **PLAN-TRUTH-090** (#1371) | **OVERRODE** — merged under an explicit operator HEAD-bound override | **No CodeRabbit review object covered the merge commit.** `automatic-review` reported 9 findings → 2 loop-backs → 0 pending; `review-retrospective` reported 3 reviewers, 1 producing findings. |

⛔⛔ **The override case is this spec's defect in its purest observed form: *findings filed and fixed*
is NOT *the merge candidate was reviewed*.** A clean pending-count over a HEAD nothing reviewed reads
identically to a reviewed-clean HEAD, which is exactly the collapse D3 exists to break.

⭐ **Use this pair as the spec's evidence base rather than re-deriving one.** It is stronger than
either half: same reviewer, same week, opposite decisions, both outcomes known. ⛔ It does **not**
settle which decision is right — that is a policy question — but it does establish that the two
states are operationally distinguishable and that the gate currently cannot tell them apart.

**Also folded:** a **per-bot currency exemption silently disables the currency test for the whole
ANDed gate** (`git-artifact-scanning-and-destructive-recovery-003`) — a single exemption widening to
the conjunction. That is the vacuous-gate shape this epic already tracks, now on the currency gate.
...
- OBSERVED (added 2026-08-31, RELAYED from `truthful-signals-041`, NOT re-derived): the
  ...
  - verdict: corroborated | checked_at: 14d8f3ccd | by: review-apparatus/cleanup | rescoped: n/a | evidence: Only declared path touched in this window is branch-cleanup.md, and the sole change there is the inserted ADR duplicate-number gate ahead of Merge routing - unrelated. review_completeness.py, automatic-review/SKILL.md, bot-participation-contract.md, review_retrospective.py and both test trees did not move. Premise intact.
...
| `review_rate_window_await` defaults to `false`, so an awaitable refusal is discarded rather than awaited under the default posture | OBSERVED | `automatic-review/SKILL.md` § "Rate-limit refusal recovery" — the `review_rate_window_await == false` sentence and the config declaration in the frontmatter block |
  - verdict: corroborated | checked_at: 14d8f3ccd | by: review-apparatus/cleanup | rescoped: n/a | evidence: Same window, same read: branch-cleanup.md is the only declared path that moved (ADR duplicate-number gate), unrelated to the nobody-reviewed-vs-reviewed-clean collapse this claim's table enumerates. Every named symbol sits in files untouched since the prior stamp. Line references remain leads; re-derive at outline.
...
- `../cloud-runs/040-canned-no-op-indistinguishable-from-a-review/gaps.md` — entries
  ...
  analysis.
- `../cloud-runs/050-coverage-shortfall-disclosed-against-the-roster-not-the-required-set/gaps.md`
...
**A plugin-cache sync IS owed.** This plan edits `marketplace/bundles/`, so the change does not take
effect until the cache is synced (CLAUDE.md § Plugin Cache Sync). ⛔ **This reverses the spec's earlier
text**, which said no sync was owed — that was true only in the `doc/plans/` standalone lane, where
`/sync-plugin-cache` reads a git-ignored build tree the lane may not touch. This plan runs in the
plan-marshall lifecycle, where on-main executor regeneration happens at finalize via

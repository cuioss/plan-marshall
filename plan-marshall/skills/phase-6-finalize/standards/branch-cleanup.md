...
The middle clause exists because the exit code alone does not carry the verdict on this path: `ci_base.output_error` prints `status: error` and returns exit 0, and both provider `main()` functions return 0 without branching on the result's `status`. A failed `ci` call therefore passes an exit-code-only reading of the first clause. What such a return lacks is the **success payload** the step wanted to read; what it carries — on stdout, where `output_error` wrote it — is the diagnostic envelope, whose fields vary by verb (`operation`, `error_cause`, and `context` on a `ci` provider call; `message` and `plan_id` on a plan-resolution failure). That envelope is why the middle clause routes to the stdout envelope as emitted rather than to the stderr the third clause names: on the exit-0 path stderr is typically empty, so carrying it alone would discard the only cause the step could report.

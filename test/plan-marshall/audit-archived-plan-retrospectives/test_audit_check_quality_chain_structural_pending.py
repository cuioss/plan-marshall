...
def _plan_with_findings(repo_root: Path, plan_id: str, findings_by_file: dict[str, list[dict[str, Any]]]) -> Any:
...
class TestPromotedRecordsAreNotStructural:
    ...
    def test_the_split_stays_exhaustive_with_promoted_records_present(self, tmp_path: Path):
        inputs = _plan_with_findings(
            tmp_path,
            'plan-exhaustive',
            {
        ...
        assert result['corpus_actionable_pending'] + result['corpus_structural_pending'] == result['corpus_pending']
        assert result['corpus_actionable_pending'] >= 0
...
class TestPendingSplitIsReported:
    ...
    def test_matrix_still_counts_every_finding(self, tmp_path: Path):
        ...
        could be cleared".
        """
        inputs = _plan_with_findings(tmp_path, 'plan-census', {'tip.jsonl': [_seeded(type='tip')]})
        result = audit.cross_quality_chain([inputs])

        matrix_total = sum(result['corpus_matrix'][m][r] for m in result['mechanisms'] for r in result['resolutions'])

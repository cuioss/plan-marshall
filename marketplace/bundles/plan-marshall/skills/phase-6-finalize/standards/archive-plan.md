...
The gate reads two deterministic signals — the `foreign` column on `manage-solution-outline list-deliverables` intersected with the declares-a-change predicate (`declares_change` in `_plan_parsing.py`), which is the population it iterates, and the `ci pr landing-state` verb (the per-repository done-ness discriminator) — not artifact prose. Every foreign repository is classified on the plan's own branch, `feature/{plan_id}` (echoed as `plan_branch` on the result), never on whatever branch that checkout happens to have checked out: after branch cleanup the foreign tree typically sits on `main`, which would read `pushed_no_pr` for a plan branch that already merged. A foreign path declared `read`, including a marker-less `Files to survey` bullet, names a file the deliverable only consults, and no commit can carry it; a foreign path with no intent marker still enters the population.
...
- **`status: clear`** — no foreign deliverable is `pushed_no_pr`, and every foreign repository's landing state was read (this includes the common zero-foreign-deliverable case). The result also carries the `excluded_read_only[]` rows — one per `(deliverable, path)` pair, so a path two deliverables both declare `read` appears twice — and `excluded_read_only_count`, the number of those rows: the foreign paths the predicate kept out of the population as read-only. A path any deliverable declares as a change is in the population and appears in no row. A non-zero count means the clear was reached over a narrowed population — those paths were excluded, not evaluated and found landed — so report them alongside the verdict rather than as part of it. Proceed to "Mark Step Complete" below.

...
| `canonicals` | list[str] | Never — sibling ext-point | A finalize-step doc MUST NOT declare this. It is a **verify**-step field whose contract is owned by [ext-point-build-verify-step.md](ext-point-build-verify-step.md), and it appears in this table for one reason only: `find_implementors()` parses a single union key set across ext-points, so the record a finalize-step query returns carries `canonicals` too — always at its empty default `[]`. Declared here so no key the record carries is undeclared; see [Keys the record carries that no finalize step declares](#keys-the-record-carries-that-no-finalize-step-declares). |
| `verification_profile` | str | Never — sibling ext-point | A finalize-step doc MUST NOT declare this. It is the ext-point-verify producer declaration whose contract is owned by [ext-point-verify.md](ext-point-verify.md), riding the same union parse target as `canonicals`. It differs from every other key in one respect: it is surfaced **only when declared with a non-empty value**, so its ABSENCE from a record is the contract signal that the producer does not participate in the verify stage — never a defaulted empty value. |
...
#### Keys the record carries that no finalize step declares

The table above is the **declaration** surface and `find_implementors()`'s record is the **read** surface, and the two are deliberately not the same set. Reconciling them needs both directions stated, because a key on one side and not the other is a defect in whichever direction it runs:

| Record key | Where it is declared | On the record? |
|------------|---------------------|:--------------:|
| `name`, `order`, `default_on`, `presets`, `description` | This table, as finalize-step frontmatter | Always |
| `canonicals` | [ext-point-build-verify-step.md](ext-point-build-verify-step.md) — a sibling ext-point | Always, at the empty default `[]` for a finalize step |
| `verification_profile` | [ext-point-verify.md](ext-point-verify.md) — a sibling ext-point | Only when declared non-empty; absence is the non-participation signal |
| `source`, `path` | Neither — **derived by the scanner**, not read from any frontmatter | Always |

`source` and `path` are the reason this section exists rather than two more table rows: a frontmatter table cannot honestly carry a field no doc declares. `source` is the scanner's own classification of where it found the doc (`built-in` / `bundle-optional` / `project`) and `path` is the resolved doc path. Both are read by consumers, so both are declared — here, where their provenance is stateable — rather than left as undeclared reads.

The other direction is the **9 rows this table marks `Conditional` or `Optional`** (`mutates_source`, `head_dependent`, `verdict_inputs`, `advances_main_via_rebase`, `records_facts`, `post_run_review`, `requires_prompt_fields`, `reads`, `destroys`). None is on the record, and none is an unread declaration, but the reader is not uniformly a script, and not uniformly `extension_discovery._read_frontmatter_fields`: eight of the nine have a real Python consumer — some a script's own direct frontmatter parse (`mutates_source`'s plugin-doctor analyzer never goes through the shared helper), some `extension_discovery._read_frontmatter_fields` itself (`head_dependent`, `verdict_inputs`), and some only a guard test's parse (`records_facts`, `post_run_review`, `requires_prompt_fields`, `reads`, `destroys`). `advances_main_via_rebase` is the exception — its declared consumer is the phase-6-finalize dispatcher's own re-resolution procedure (`SKILL.md` § "Post-rebase step-doc re-resolution contract"), which the orchestrating agent follows directly rather than a script or test reading the field. `test_every_declared_field_has_a_reader` derives both populations mechanically and is what this claim is checked against — not a hand count kept in step with this sentence. The record deliberately carries the seeding/preset consumers' fields, not every declared key — see [Hook API](#hook-api).
...
def find_implementors(ext_point: str) -> list[dict]:
    ...
    plus verification_profile, which is present ONLY when the doc
    declares it with a non-empty value — its absence is the
    ext-point-verify non-participation signal, never a defaulted empty.
    ...
    obligation fields — every field the table above marks Conditional or
    Optional — are NOT on this record; a consumer that needs one reads
    it from the step doc's frontmatter directly (see
    extension_discovery._read_frontmatter_fields). Two record keys are
    not frontmatter at all: source and path are derived by the scanner.
    See the "Keys the record carries that no finalize step declares"
    table above for the full provenance of every key.

...
**The resolver is inert in a project with no harvest, not absent from it.** `discover_derivation_resolvers()` applies no domain filter (see [§ 2 Discovery](#2-discovery)), and the host bundle is now core — so `lsp` registers in **every** consumer project, including every project that carries no `lsp_harvest` record at all. That is the intended null-on-absent shape rather than a leak: `derive_edges()` reports `harvest-did-not-run` instead of a confident zero, so the resolver's presence contributes a *stated* absence where its absence would have contributed a silent one. This is the capability-provision arm of [ADR-010](../../../../../../doc/adr/010-Domain_content_is_active_only_when_its_domain_is_active.adoc) — the capability is provided unconditionally and reports honestly on its own applicability, rather than being gated into invisibility — and it leaves [ADR-014](../../../../../../doc/adr/014-An_aggregation_over_N_independent_producers_carries_producer_identity_and_no_producer_suppresses_an_element_silently.adoc)'s producer-identity requirement intact, since every edge it does derive is still stamped `lsp` and no other id.

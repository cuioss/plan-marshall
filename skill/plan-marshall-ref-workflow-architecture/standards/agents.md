...
The `execution-context` agent declaration is reconciled with this runtime reality: it does NOT list `AskUserQuestion` (the prompt-required envelope below is the sanctioned operator-input path). Its declared `Grep` / `Glob` are legitimate search intent, but the harness MAY deny them to a subagent; when it does, the leaf discovers through the structured architecture inventory (`architecture find` / `search --content` / `which-module` / `files`, structured queries first) and scans an already-known file with `Read` — Bash `grep`/`find` and git's `grep` subcommand are blocked unconditionally by the file-operation hard rule and its hook, so they are NOT a fallback. A broad content sweep is served by `architecture search --content --pattern P`, which the leaf reaches for FIRST; only a sweep over a tree the inventory does not walk (`.claude/**`, `.github/**`, gitignored paths) is returned to the orchestrator as a coverage gap — and in neither case may the leaf silently degrade to spot-checks. See [`execution-context.md` § "Runtime tool availability for dispatched leaves"](../../../agents/execution-context.md) for the agent-side contract.

**`architecture find` is a path-name match, not a content search — do not read its zero-result as an empty population.** `find --pattern` matches against inventory PATHS (`fnmatch`), never file bodies; a pattern shaped like content you expect to find inside a file (a renamed enum value, a retired token) can legitimately return `count: 0` while the tree still carries dozens of live occurrences whose paths simply don't contain the pattern. Reach for the sibling verb instead: **`architecture search --content --pattern P` is the sanctioned tree-wide content sweep**, answering "which files CONTAIN this" over the same module-attributed inventory. Add `--literal` for a fixed token. Its `files_scanned` field is what makes a zero trustworthy — `count: 0` with `files_scanned: N` is a real negative, `files_scanned: 0` means nothing was searched — and `unreadable[]` reports any file it could not open rather than dropping it silently. See [`../../manage-architecture/standards/client-api.md`](../../manage-architecture/standards/client-api.md) § search for the response contract and the inventory-scope boundary.

When a deliverable's own acceptance criteria require the sweep to be *pinned as a regression test* (proving zero orphaned references to a retired token stays true), a dedicated pytest module that walks the text files directly and asserts zero hits per token is still the shape to author — see `test/plan-marshall/test_lane_refactor_cleanup_sweep.py`'s `_grep`/`_assert_zero` pair (one assertion per retired token, failure message naming the offending `file:line`). One authoring pitfall to avoid when copying it: gate the walk's skip-list (`__pycache__`, `.git`, `node_modules`, etc.) against each file's path RELATIVE to the walked root, never against the file's absolute `path.parts` — a plan executes from inside a worktree whose absolute path already contains a directory named `.plan` (`.../.plan/local/worktrees/<plan>/...`), so an absolute-path skip-list check matches that segment on every file and silently skips the entire tree, passing every "zero hits" assertion by scanning nothing.
...
This is the same escalation-envelope pattern the marketplace already uses for the other leaf-cannot-do-it cases, generalized from these shipped precedents:

| Dispatched leaf | Prompt-required envelope it returns | Orchestrator that owns the prompt |
|-----------------|-------------------------------------|-----------------------------------|
| `automatic-review/SKILL.md` § "`escalate_ask` return (timeout escalations)" (re-review timeout) | `status: escalate_ask` with `prompt_options[]` | `phase-6-finalize/SKILL.md` Step 3 (item 7a) |
...
| `execute-task/SKILL.md` Handle-Verification (scope-deviation / `smart_and_ask` gates) · `phase-5-execute/SKILL.md` Step 11 triage | `status: escalate_ask` with `prompt_options[]` | `plan-marshall/workflow/execution.md` § Post-return `escalate_ask` batched deviation dispatch |

All four entries are **persist-and-continue blocks** (`escalate_ask`, `outline_prompt`, `refine_prompt`): they ride back on an otherwise-complete return, and the orchestrator fires the prompt and applies the resolution post-return (persisting metadata, adjusting references, or baking answers into an at-most-once re-dispatch). None requires the leaf to accept a resolution *input* back into the same leaf turn. Every option string an orchestrator assembles from one of these envelopes obeys the option-text-names-the-branch authoring rule: each option's label MUST name what the branch it selects actually does. [`operator-prompt-authoring.md`](operator-prompt-authoring.md) is the governing authoring standard for every option string in a prompt-required envelope — it defines the two-sided contract, the verify-per-option obligation, and the re-check-both-sides-on-either-change rule.

**Corollary — main-context workflows are exempt.** A workflow that runs in the main context (loaded via `Skill:`, not dispatched via `Task:`) — e.g. `phase-1-init` (which runs inline in the orchestrator, firing all six of its operator prompts natively), `plan-marshall/workflow/planning.md`'s list / cleanup / lessons menus, `triage.md`, `verification-feedback.md`, and the inline `phase-6-finalize` dispatcher steps — CAN and does fire `AskUserQuestion` directly, because it is not inside a dispatched envelope. The contract binds only the dispatched leaf. **Every dispatched-leaf prompt site now routes through this pattern:** `phase-2-refine` Step 11's below-threshold confidence-loop clarification returns a `refine_prompt` envelope owned by `planning.md` § 2-Refine Phase, and the `execute-task` / `phase-5-execute` scope-deviation and `smart_and_ask` gates return an `escalate_ask` envelope owned by `execution.md` § Post-return `escalate_ask` batched deviation dispatch — no dispatched-leaf `AskUserQuestion` call site remains.

### Leaf must record its terminal outcome BEFORE composing its return

A dispatched leaf **must land its terminal `mark-step-done` before it composes its return TOON** — never as a trailing action taken after the return is assembled. This corollary of the leaf/dispatch-topology invariant binds **every dispatched leaf**, not only the external (`project:` / `bundle:skill`) finalize steps whose own contract document happens to spell it out. Once a leaf returns, its turn is over: it has no further opportunity to write, so a record deferred until "after" the return is a record that never happens. The dispatcher then observes a step that reports success while leaving no terminal outcome on `status.metadata.phase_steps`, which deadlocks the `phase_steps_complete` handshake at the phase transition.

The contract is two-sided, and both halves are structural:

> **Leaf side** — before composing the return TOON, the leaf MUST call `manage-status mark-step-done` with its terminal `--outcome` (`done` / `skipped` / `loop_back` / `failed`). Record first, then compose and emit the return. A leaf that treats the record as a post-return step has violated the contract even when its work succeeded.
>
> **Dispatcher side** — a `status: success` return whose step fails `assert-step-recorded --require-terminal` with `step_record_missing` is a **leaf-attributable contract violation**, not a reconcilable condition the dispatcher should paper over. The dispatcher records the violation against the offending step (`mark-step-done --outcome failed`), logs it attributed by step id, and HALTS; it MUST NOT synthesize the missing record on the leaf's behalf, because doing so would launder a broken leaf into an apparently-clean run.

The dispatcher-side guard that detects the violation — `manage-status assert-step-recorded --require-terminal`, run after every dispatched-step return by `phase-6-finalize/SKILL.md` Step 3 item 5d ("Post-dispatch completion guard") — is a **backstop detector, not the fix**. It converts a silent handshake deadlock into an attributed per-step failure, but it cannot make the omitted write happen. The fix is authoring the call in the right place: before the return is composed. Detector and authoring rule are paired, never redundant.

The single sanctioned non-terminal return is the **`escalate_ask` carve-out**: a leaf returning `status: escalate_ask` legitimately records no terminal outcome, because the continuation — firing the operator prompt — is owned by the orchestrator, not the leaf (see § "Leaf cannot fire AskUserQuestion — return a prompt-required envelope" above for why the leaf cannot own that prompt). The dispatcher's completion guard skips the terminality assertion for exactly that return shape; every other return shape is subject to it.

The per-step authoring mechanics — the `--step` composed-manifest-catalog-key contract and the `--display-detail` constraints — are the external-step contract's own concern and live in [`../../phase-6-finalize/standards/external-step-contract.md`](../../phase-6-finalize/standards/external-step-contract.md) § "Required termination". This section owns the invariant; that document owns the mechanics.

### Leaf cannot reap a backgrounded build — run `per_task` inline, hand `orchestrator`-tier back

A dispatched `execution-context` leaf **cannot reap a backgrounded build.** Only the component that owns a build's `run_in_background: true` dispatch receives its completion notification; a leaf that backgrounds a build then returns loses the running job — the orchestrator that re-reads the leaf's return never receives the background-completion signal, and the build's exit status is silently dropped. This is the fourth corollary of the leaf/dispatch-topology invariant above (a leaf cannot spawn a subagent, cannot reach the operator, must record its terminal outcome before composing its return, and cannot reap a backgrounded build), and it is the **single source of truth** for build-tier ownership on the leaf boundary:

> A dispatched leaf runs only `per_task`-tier build/verify steps and branches on `execution_tier`:
...
**The tier is resolved live, at the moment the leaf would run the build.** `execution_tier` is derived from two run-config quantities `manage-architecture` reads — whether the command key has been **measured at all**, and the **adaptive learned build duration** behind `bash_timeout_seconds` — and every intervening build moves both. An unmeasured step tiers `orchestrator` until its first observed run, and a step whose learned duration sits near the Bash ceiling (the Claude-target `harness bash-timeout-ceiling` value, resolved per active target) crosses that ceiling in ordinary operation, so no compose-time value can stand in for the tier at execute time. The leaf accordingly resolves the tier itself, immediately before running each build/verify step, and branches on that live verdict.

`manage-execution-manifest` `compose` also stamps each `phase_5.verification_steps` entry's compose-time `execution_tier` into `phase_5.step_execution_tier` (a per-step record list), but that stamp is **advisory** — it tells the orchestrator how many orchestrator-tier steps to expect and gives a retrospective the compose-time picture. When the stamp and the live resolve disagree, the **live resolve wins**; `plan-marshall/workflow/execution.md` § "Orchestrator-tier phase-5 verification" is the handler for the yield the leaf returns when the live tier is `orchestrator`. See [`../../manage-execution-manifest/SKILL.md`](../../manage-execution-manifest/SKILL.md) § "Per-step execution_tier stamping" and [`../../manage-execution-manifest/standards/decision-rules.md`](../../manage-execution-manifest/standards/decision-rules.md) § "execution_tier Stamping" for the composer contract.

This mirrors the escalation-envelope pattern of the other leaf-cannot-do-it cases: the leaf does the work it *can* do (run the `per_task` build inline) and hands back the work it *cannot* (the `orchestrator`-tier long build) rather than attempting a backgrounded run it can never reap. The tier-agnostic `kind=build` freshness stamp is written at the executor dispatch boundary regardless of which tier ran the build (see [`../../manage-change-ledger/SKILL.md`](../../manage-change-ledger/SKILL.md)), so the orchestrator-detached build is ledger-stamped identically to an inline `per_task` build — the per-deliverable commit that consumes the stamp fires unconditionally at the chain-tail regardless of the tier that produced it.

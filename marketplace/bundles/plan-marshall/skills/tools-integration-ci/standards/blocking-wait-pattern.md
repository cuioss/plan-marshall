...
The `checks wait` handler (`cmd_ci_wait`, GitHub `_github_ci.py` / GitLab `gitlab_ops.py`) waits for a whole CI run to reach a terminal conclusion. That is a **different shape** from the section-3 snapshot-diff waits: those block until a single scalar snapshot *changes* (unresolved-comment count grows, status flips), and a fixed-interval `poll_until` re-fetch is the right tool. A CI run instead has a **known-busy window** — it will be running for roughly as long as it ran last time — so polling `pr checks` every 30 s from second zero burns API quota during a window where the answer cannot yet be "done". `cmd_ci_wait` therefore uses a two-stage **seed-then-watch** wait instead of the fixed-interval loop:

...
`.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/550-test-suite-anti-vacuity.md`. It was ingested into this ledger on
2026-08-22 as `PLAN-CIS-053` and it will be EMITTED INTO THE PLAN-MARSHALL LIFECYCLE, not into the
cloud lane.
...
- `test_dispatch_roster_closure.py:197,420-443,797-816` — the roster-correctness check derives its
  ...
  **re-derive both by calling the module's own helpers; do not trust these figures**) and then
  ...
  projects outside this repository. ⚠ **CORRECTED 2026-08-24** — the stated reason was that "the lane
  forbids self-approving" such a change, and that lane rule does not bind here. **The split still
  holds, on its own merits**: a contract change reaching consumer projects outside this repository is
  a genuine fork, and the orchestrator's escalation rule sends those to the operator rather than
  letting a run self-approve them. So: **record the proposal** (both options, the migration hazard,
  and the excluded discovered-but-unregistered doc) in the PR body and the verification record, and
  **escalate the contract half with `AskUserQuestion`** rather than merely deferring it — a channel
  the cloud lane did not have. Land only the non-contract half here: a coverage assertion that
  reports, without failing, how many registered steps contribute a comparison. *(§ G11)*
...
- **Any change to the finalize-step extension-point frontmatter contract.** *Reason:* it governs
  ...
  their builds until they migrate. ⚠ **CORRECTED 2026-08-24** — the second reason given was that "the
  lane forbids a run self-approving a change to a contract that governs it", which does not bind
  here. The exclusion stands on the FIRST reason alone, which is the substantive one; D5 records the
  proposal and escalates it with `AskUserQuestion` per the corrected § G11 item.
...
`.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/{280,220,180,040}-…/report-01.md` — "four coverage claims
corrected in place or by appended note". That tree is deleted from git. The four reports survive
ONLY in the git-ignored archive at
`.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/{280,220,180,040}-…/report-01.md`,
so correcting them is not a repository deliverable and **this plan's repository surface is the list
above and nothing else**. Per the provenance header, file the four corrections as an epic inbox
`finding` message (`orchestrator inbox write --slug code-intelligence-substrate --kind finding`),
quoting each false claim and its correction, and reproduce them in the PR body. ⛔ Do NOT edit
anything under `cloud-runs/` — that archive is the evidence being corrected.
...
Three paths this plan did not previously declare, named by the folded signal that adds them. The
same-act rule applies: these landed in the SAME edit that appended § Folded-in signals below.

- `test/conftest.py` (the `_load_module` wrapper) and
  `test/pm-plugin-development/tools-marketplace-inventory/test_resolve_dependencies.py` — F-001(obs): the loader-collision guard
  bounds statically-unresolvable call sites at **90** while the wrapper is used in **97 files**, and
  that test still hides a real `_dep_detection` collision the guard cannot see.
- `marketplace/bundles/pm-plugin-development/skills/tools-marketplace-inventory/scripts/_dep_detection.py`
  — the module whose collision is hidden; read to establish the collision is real, changed only if the
  fix belongs on the production side rather than in the guard's bound.

## Claim Labels
- verdict: corroborated | checked_at: 28b578f1ed435973e53c510f0c8225446cc024aa | by: code-intelligence-substrate/cleanup | rescoped: n/a | evidence: RE-GROUNDING BY EXECUTION IS NOW DONE for the reachable half - the debt the previous section verdict left open is discharged as far as a read-only pass reaches. All 10 table rows re-derived at HEAD: 4 corroborated; 6 unverifiable (rows 3,4,6,7,8 need applied mutations or live pytest runs and row 8 names a machine-local transcript corpus outside any checkout); 0 refuted. The LOAD-BEARING row holds: conftest.py:117-134 still uses check=True/timeout=120 and catches only CalledProcessError/TimeoutExpired/OSError, never branching on TOON status, and conftest.py:95-97 returns early when the executor exists so the fail-open guard is unreachable; generate_executor.py:2413-2419 still returns 0 unconditionally with no status branch; test_freshness_notation_crosscheck.py:269-309 is still unstubbed. CHECKED NEGATIVE on the three intervening landings: bf1b7ed67, 8d8c17bd1 and 80da16e30 close NONE of these premises - bf1b7ed67 touched conftest.py and the crosscheck test only for cosmetic noqa retirement, not the _ensure_executor_present or collision logic. Row 9 is corroborated by observed drift: UNRESOLVED_CALL_SITE_BOUND moved 86 to 87 in 8d8c17bd1 and back to 86 in bf1b7ed67, so quoted counts do move and must be re-derived at outline.
...
| The "38 tests fail across shared directories" residue is real | HYPOTHESIS | `.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/220-resolver-configuration/report-01.md` § Residue, and the two named test modules — re-run the multi-directory invocations and record the outcome; adversarial review could not reproduce it |
...
⛔ Note on the gap citations: entries are cited as `{source-plan}/gaps.md § Gn`. ⚠ **CORRECTED
2026-08-24** — this paragraph used to say those files were "git-tracked and on `main` today" and
might be "gone by the time this runs". Both halves are now settled and neither is true as written:
the collect HAS happened, so they are **not** on `main`, and they are **not** gone either. They are
readable at `.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/{source-plan}/gaps.md`,
a git-ignored local archive. Resolve every citation there. Every item above still restates its
defect, its file and its observable fix inline, so the citation remains corroboration rather than
required reading — but a citation that fails to resolve is now a real anomaly worth reporting, not
the expected outcome it once was.
...
2. **Whole-suite regression.** ⚠ **CORRECTED 2026-08-24** — this said "run the build gate as the lane
   requires"; there is no lane gate here. Run the full verify unconditionally as
   ...
   which is 0 even on failure. Additionally run each
...
4. **Independent cold read of the interpretation-bearing text.** ⚠ **CORRECTED 2026-08-24** — this
   named "the lane's pre-PR verification sub-agent", which does not exist here. Dispatch a
   `plan-marshall:execution-context-{level}` leaf with a second, separate instruction, and keep it a
   FRESH dispatch carrying none of this plan's context — the cold-read property is the whole value of
   this check. Have it read — cold, without this plan —
...
73 gaps: 2 high, 41 medium, 30 low. Paths are relative to `.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/`;
...
One signal folded here, and it is this plan's subject in its purest form: a guard whose stated bound
is smaller than the population it guards, so the excess is invisible by construction.

**F-001(obs) — the `_load_module` conftest blind spot is systemic, and the bound hides it.** The
loader-collision guard bounds statically-unresolvable call sites at **90**; the wrapper is used in
**97 files**. `test/pm-plugin-development/tools-marketplace-inventory/test_resolve_dependencies.py` still hides a real
`_dep_detection` collision the guard cannot see.
⭐ **PLAN-CIS-051 deliberately did NOT raise the bound** — it reduced the blind spot by three
registrations instead, on the reasoning that raising a bound to match an unmeasured population is how
a guard stops biting. The structural gap remains, and it is this plan's to close.
⛔ **This is the population-derived-detector rule applied to a guard that fails it**: a bound written
as a literal cannot track a population that grows, so the guard's coverage silently decays with every
new call site. Per this epic's standing rule, the remedy is to DERIVE the bound from the population
(count the wrapper's call sites) and publish that population beside the verdict — never to bump the
literal.
*Also owed from the same source, and already routed elsewhere:* the mutation-methodology
admissibility gap (a plan that mutates deliberately manufactures a threshold-clearing recurrence out
of its own induced RED observations) is folded into `PLAN-CIS-050` alongside F-013; it is a
preference-emitter admissibility question, not a suite-vacuity one.

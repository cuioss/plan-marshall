...
**Checked in that order**, and the ordering claim is exact: the four message-state checks (11–14) run AFTER the base envelope sweep (4–10), so the base rejection codes are unchanged by their addition and a message carrying none of the state fields — the virgin `live` case — passes every one of them. See the validator error-code table in [`standards/inbox-envelope.md`](standards/inbox-envelope.md) for the rejection condition of codes 4–14 — that table is exhaustive over ENVELOPE-VALIDATION verdicts and so does not carry rows 1–3, which this verb raises itself before `validate_envelope` is reached; the "Raised by" column above is their authority. The verb also carries an `invalid_envelope` fallback for a rejection reporting no code; it is unreachable while `validate_envelope` returns a code on every rejection branch, and is listed here as the defensive default rather than as a fifteenth outcome.

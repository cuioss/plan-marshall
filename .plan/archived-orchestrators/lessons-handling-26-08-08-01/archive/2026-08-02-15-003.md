...
persisted artifact. Concretely, PLAN-CIS-027 replaced the merge-output assertion with one
against the merged graph itself (`70b93270e`, `test_merge_stage_produces_edges` explicitly
...
Applies to every multi-stage pipeline — resolver to merge to query, scan to index to lookup,
discovery to registration to dispatch. The recognition signature is a green

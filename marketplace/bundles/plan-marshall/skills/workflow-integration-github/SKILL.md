...
1. **FIND — file findings** (untrusted body quarantined under `raw_input`):
   ...
   **A refusal the STRUCTURAL arm read ALONE is reported as wording DRIFT.** Alongside the fields above the producer emits **`refusal_pattern_drift[]`** — one `{bot_kind, layer}` record per bot whose notice the structural arm recognised **while the bot's own declared `refusal_patterns` did NOT**, where `layer` (from the shared `_github_pr.REFUSAL_LAYERS` vocabulary) names the arm that read it, and is therefore always `structural_fallback`. It is derived from the `_github_pr.refusal_layers` provenance seam, which reports WHICH arms matched where `_is_refusal_notice` reports only THAT one did, and is computed at the filing pre-filter only — never in the participation loop — and only for comments arriving in one of the bot's declared `participation_evidence` publish shapes. ⛔ **The predicate is DIRECTIONAL — deliberately not "exactly one arm fired".** The mirror case, the registry arm matching while the structural arm does not, is the DESIGNED state for a whole class of refusals and is never recorded: a diff-size ceiling (`your pull request is larger than the review limit of …`) is a COMPARISON rather than an "exceeded / reached / hit" statement, so it is invisible to the structural arm BY CONSTRUCTION and its registry-only match is the registry arm doing precisely the job it is load-bearing for. Recording it would report the architecture working as designed as though it were decay. A `structural_fallback` record is the actionable one: the notice was caught by SHAPE alone while the bot's own declared `refusal_patterns` missed it, so that registry record has gone stale against the bot's current wording, and the next rewording may clear the structural shape too — at which point the refusal is filed as review feedback and the bot is credited with a review it declined. Emitting it is what makes that decay observable BEFORE it fails, since the boolean returns `True` whether one arm fired or both. The remedy is to add the observed phrasing to that bot's `refusal_patterns` in `automatic-review/standards/{bot_kind}.md`. Records are deduped on `(bot_kind, layer)` — drift is a property of the bot's declared wording, not of each comment carrying it. ⚠ **Diagnostic ONLY**: it changes no verdict, denies no participation credit, gates nothing, and no `review_completeness` member routes on it; a bot named here was still correctly recorded in `refused_bots[]`.
   ...
   **An unresolved identity is disclosed, never read as resolved.** The identity is read at most once per fetch, and only when some fetched comment opens with the heading or carries the section signature. `workflow_identity` reports how it stood: `resolved`; `unresolved` — the viewer read failed, so the self-response stage fell back to the heading-only test and the emitter-bypass stage could not run; or `not_needed` — no comment carried either shape, so no stage asked for it.
...
4. **RESPOND — apply dispositions back to the PR** (keyed by hash_id):
   ...
   **The pass is idempotent per (finding, disposition).** The findings store is plan-scoped and persists across rounds, and terminality (`_RESPONDABLE_RESOLUTIONS`) is the *selection* criterion — a terminal finding re-qualifies on every pass. So each transmitted reply stamps its finding with a `responded` marker (in the same unit of work as the send), and a later pass skips any finding already carrying it (`skipped[]`, reason `already responded`). Consequently `count_responded` reports only the dispositions transmitted **this round**, never a standing re-count of every terminal finding. A disposition that genuinely CHANGED between rounds is transmitted again: `manage-findings resolve` clears the marker whenever it changes a finding's resolution or reply body, so the guard is a per-`(finding, disposition)` key, not a blanket suppression.
...
**The same predicate decides both rows; only the evidence FIELD differs** — a review's reviewed-commit field, and a comment's body. The two rows are therefore not "verified" versus "unverifiable": they differ in the STRENGTH of the artifact (a review object outranks a comment, which is why the review signal resolves first), not in whether a reviewed commit can be named.

**Both** signals additionally reject a **refusal notice** — a bot declining to review (rate-limit, diff-size, quota). The rejection runs the SAME refusal-recognition stack the producer applies, arm for arm, and is identical on both paths: the awaited bot's registry `refusal_patterns` (its OBSERVED refusal strings — a DEDICATED field, never `ignore_patterns`, which lists the routine sections a bot emits on a *successful* review and would therefore over-match), the author-independent structural `_is_rate_limit_notice` for an unknown or renamed bot, and the enumerative `_is_unrecognised_refusal` for a body no earlier arm could read. `_github_pr.REFUSAL_LAYERS` is the single place those arms are named, and the list is open — an arm added there is nameable here with no count to correct. A refusal carries no review, so counting it as a completion signal would assert review coverage that never happened. This applies to the review path as much as the comment path: a bot may submit its refusal as a **review object** rather than an issue comment, and the review path resolves first — so without the check the strongest signal (`head_sha_verified: true`) would be the one most likely to be false.
...
⛔ **`head_sha_verified` on the comment path is DERIVED from the body, never hard-coded `false`.** The retired wording — that this signal never names the commit it reviewed — was a premise nobody had checked against the registry, and it is false for a bot whose ONLY declared publish shape is an issue comment. `cuioss-review-bot` declares `participation_evidence: issue_comment` with an empty `completion_check_name` (it submits no review object at all) and names its reviewed commit inside that comment as a `…/commit/{sha}` permalink — the exact URL-embedded form the predicate above already recognises. While the field was hard-coded, every correct re-review that bot performed was published as `matched: true` / `head_sha_verified: false`, which both consumers route as an incremental-review **decline**; that member is blocking and its documented remedy is to demote the bot to `optional_bots` or take a merge-authorization waiver, so a *required* bot in this shape could never verify.

A comment that does NOT reference the awaited HEAD still reports `false`, and that remains the genuine decline — the predicate compares for equality, so naming some other commit does not verify either. The match itself is unchanged and still proves only that the bot **responded**: the SHA reference is a match condition on the review path but a property read off the matched record on the comment path, so a decline stays observable as a match rather than collapsing into a bare timeout. The comment path uses the LATER of `updated_at` / `created_at` so a bot that edits ONE persistent comment in place still registers as fresh activity. No bot is named in code — both matchers are generic across the registry.
...
is this size, so waiting is a NON-OPTION for it whatever its class says — the remedies are splitting

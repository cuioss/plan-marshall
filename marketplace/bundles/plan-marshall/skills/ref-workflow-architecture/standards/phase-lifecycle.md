...
- User review gates (config flags: `plan_without_asking`, `execute_without_asking`, `finalize_without_asking`, `loop_back_without_asking`) must be respected. The first three gate **forward** phase transitions (3→4, 4→5, 5→6); `phase-6-finalize.loop_back_without_asking` is the symmetric **reverse-direction** gate that controls whether a phase-6 step's `outcome: loop_back` re-dispatches the execute pipeline inline (capped by `phase-6-finalize.max_iterations`, default 3) or halts and prompts the user. All four default to `false` for the conservative interactive shape; full unattended execution requires opting into the relevant pair (`finalize_without_asking` + `loop_back_without_asking` for the phase-5 ↔ phase-6 cycle). See [`manage-config/SKILL.md`](../../manage-config/SKILL.md) § "Symmetric auto-continuation knobs" for the four-corner truth table and [`phase-6-finalize/SKILL.md`](../../phase-6-finalize/SKILL.md) Step 3 § "Loop-back continuation hook" for the dispatch shape.

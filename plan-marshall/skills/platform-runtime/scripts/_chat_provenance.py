...
Answers one question for the ``chat extract-signal`` runtime operation: **is
this ``user`` turn operator-authored, or did the harness inject it?** The
contract it implements — why the predicate is positive rather than an
enumeration, which envelopes carry operator content, and the published marker
inventory with its residual gap — is specified in the ``chat extract-signal``
operation contract — ``standards/contract.md`` § "The operator-provenance
predicate" — and is not restated here.
...
# until it is added. The ``chat extract-signal`` operation contract (§ "The
# operator-provenance predicate") publishes the residual gap rather than
# implying this list is closed.

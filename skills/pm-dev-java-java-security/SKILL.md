...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/java-input-validation.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/java-security-patterns.md" }` before continuing.
```

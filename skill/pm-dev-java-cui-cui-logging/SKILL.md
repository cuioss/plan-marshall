...
```text
Call the `read` tool with `{ filePath: "standards/logging-standards.md" }` before continuing.
```
...
```text
Call the `read` tool with `{ filePath: "standards/logmessages-documentation.md" }` before continuing.
```
...
```text
Call the `read` tool with `{ filePath: "standards/logging-maintenance-reference.md" }` before continuing.
```

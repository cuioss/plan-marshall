...
into ``{output_dir}/{bundle}/``. Excludes every ephemeral / generated
directory named in ``EXCLUDED_DIR_NAMES`` (the constant is the list; naming
its members here would be a second copy to keep in step) and the bundle's
``.claude-plugin/plugin.json`` (regenerated by ``plugin_json_gen.py``).

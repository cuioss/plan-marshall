...
shape_violation:
  ...
  violations: N
  foreign_caller_line_total: N   # sum of by_role[].foreign_caller_lines — OMITTED when not_evaluated,
                                 # because no role was walked and a 0 there would read as "none found"
  corroboration: {seam_corroborated|uncorroborated|not_evaluated}
                                 # how much the verdict above is worth: `uncorroborated` means at least
                                 # one role's [DISPATCH] lines came from a caller that resolved nothing
                                 # for it, so a cancelled emission can be hiding inside a clean delta
...
dispatch_coverage:
  ...
  ran_inline_claim_strength: {ceiling|no_claim}
                                 # how much the count above is worth: `ceiling` over a non-empty
                                 # bucket (a dispatched step with a captured-zero usage lands here
                                 # too), `no_claim` over an empty one
  no_evidence: N                 # no token row, OR a row whose total_tokens was unreadable
                                 # (including the writer's `unmeasured` token)
  no_evidence_steps[N]: […]
...
- **Read `ran_inline` together with the emitted `ran_inline_claim_strength`.** The payload states the strength of that count, so it no longer has to be recovered from this document: `ceiling` means quote it as the ceiling on how many steps *could* have run inline, never as evidence that they did. ⛔ And do NOT read the converse either: an inline step whose caller measured nothing lands in `no_evidence`, so a small `ran_inline` is not evidence that few steps ran inline. The two-state derivation and what each value rules out are owned by the `evaluate_dispatch_coverage` docstring in `scripts/check-dispatch-audit.py`.
...
- **Read `shape_violation.violations` together with the emitted `corroboration`.** The payload states how much the verdict is worth, so it no longer has to be recovered from this document: `uncorroborated` means at least one role's `[DISPATCH]` lines came from a caller that resolved nothing for it, so a cancelled seam emission can be hiding inside a `violations: 0`; `foreign_caller_line_total` and the per-role `by_role[].foreign_caller_lines` say how much. Even `seam_corroborated` is a **consistency** verdict, never a completeness one — both surfaces are written by the same seam, so their agreement proves the seam ran and nothing more. The **Corroboration limit** and **Caller-blindness** blockquotes in § Inputs own that reasoning; do not re-derive it here.

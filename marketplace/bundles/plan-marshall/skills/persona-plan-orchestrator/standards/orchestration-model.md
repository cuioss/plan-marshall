...
|------|------|--------------|-------|
| **Epic** | A long-running goal too large for one plan — a roadmap, a campaign, a multi-plan feature | One `.plan/orchestrator/{slug}/` tree (the per-concern ledger files — see [§ Directory Layout](#directory-layout)) | The orchestrator |
...
Each epic lives in one git-tracked, repo-local tree under the orchestrator store. The ledger is split into files that each hold ONE concern, so two sessions that write different concerns — on one machine or on two — write different files, and git merges them without a conflict:

```text
.plan/orchestrator/{slug}/
├── status.json          # MACHINE AUTHORITY, header: kind, title, phase, workstreams,
│                        #   metadata, created — no updated stamp, no queue, no anchor
├── resume_anchor.md     # MACHINE AUTHORITY: the resume anchor text alone
├── queue/               # MACHINE AUTHORITY: the plan queue, one row per file
│   └── {PLAN-ID}.json   #   id, slug, workstream, status, plan_marshall_plan_id, pr,
│                        #   landing, seq — staging a plan creates ONE new file
├── queue-view.md        # GENERATED, git-tracked: START HERE + the Ordered Queue, rendered
│                        #   from the files above; never hand-edited
├── epic.md              # Hand-written narrative only: vision, queue annotations,
│                        #   decisions, open defects, watches
├── history.md           # Frozen record of the closed epic (written at close)
├── settled.md           # Relocated settled narrative, written MID-LIFE by the compact
│                        #   stage; epic.md keeps a pointer at each origin resolving here
...
└── logs/                # decision.log, work.log (written via manage-logging) —
                         #   the one sub-path that stays git-ignored
```

The store root resolves through `get_store_dir('orchestrator', slug)` (`plan-marshall:tools-file-ops`) — composed onto `get_tracked_config_dir()`, the git-tracked, cwd-relative config tier, so the tree is versioned with the repository and travels with a clone rather than with the machine. The orchestrator store is a sibling of `.plan/local/plans/`, never inside it: plan discovery globs only `.plan/local/plans/`, so orchestrator epics are structurally invisible to the plan lifecycle. Epics are discovered by scanning the store roots on query — enumerating BOTH `.plan/orchestrator/` and `.plan/archived-orchestrators/` — never boot-indexed, so an archived epic stays discoverable by slug.

A closed epic MAY be relocated by the optional `archive` verb to a sibling tree under `.plan/`:

```text
.plan/archived-orchestrators/{slug}/   # relocated home of a closed epic (identical tree layout)
```

`archived-orchestrators/{slug}/` resolves through `get_archived_orchestrator_dir(slug)` (composed onto the same `get_tracked_config_dir()` tier, so both homes of one epic sit on the same tracked storage tier). The read verbs resolve an archived epic transparently via the `allow_archived` read-fallback on `get_store_dir` — a slug is looked up at the active `orchestrator/{slug}/` path first, then at the archived path when the active tree is absent.

Document templates for `epic.md`, `workstreams/WS-NN-{slug}.md`, `plans/PLAN-NN-{slug}.md`, and `landings/PLAN-NN.md` live in the `plan-orchestrator` skill's `templates/` directory and mirror this layout contract one-to-one. `queue-view.md` has no template: the renderer is its only source of shape.

### Per-file decision

Each ledger file is cut along the write pattern [§ Ledger Write Pattern](#ledger-write-pattern) measures: what is written together at the same frequency stays together, and what is written at a different frequency, or by concurrent sessions, gets its own file. The frequency classes are that section's closed set.

| File | Concern | Written by | Frequency class | One file or aggregated | Why |
|------|---------|------------|-----------------|------------------------|-----|
| `status.json` | The epic header: `kind`, `title`, `phase`, `workstreams`, `metadata`, `created` | `manage-status create`; `update-field` (`phase`, `workstreams`); `metadata --set` | `once`, `per-epic-lifetime`, `per-decompose` | Aggregated — six fields in one file | Every header field is written rarely, so keeping them together buys no contention. The header carries no `updated` stamp: a stamp every write restamps is the collision line that made every pair of concurrent writes conflict, whatever they touched. |
| `resume_anchor.md` | The resume anchor — the exact next action | `update-field --field resume_anchor` | `per-verb` | One file | The one field written on nearly every verb. Isolating it keeps an anchor move from colliding with a header or queue write. Its contention with another session's anchor write is genuine — see [Accepted residual contention](#accepted-residual-contention). |
| `queue/{PLAN-ID}.json` | One plan-queue row | `orchestrator queue --add-row` (creates the file); `queue --transition`; `queue --set-row` | `per-queue-change` | One file per row | Staging a plan creates one new file and a reconcile rewrites only the row it locates, so two sessions acting on different plans touch disjoint files. |
| `queue-view.md` | START HERE and the Ordered Queue, rendered together | The renderer — see [§ Persist / Stop-Resume Contract](#persist--stop-resume-contract) | `per-queue-change` for the Ordered Queue; `per-verb` for START HERE, which renders the anchor | One file, aggregating both blocks | Both blocks render from the same assembled view and are regenerated together. A separate file takes the generated churn out of `epic.md`, so a hand edit to the narrative never collides with a regeneration. It is tracked so the queue stays readable on a plain clone without the executor — on GitHub, or in a cloud session. The file is derived: never hand-edited, and never part of the machine authority. |
| `epic.md` | Hand-written narrative: Vision, Queue annotations, Decisions, Open Defects, Watches, and any epic-specific section | The orchestrator, through the [direct-file-write carve-out](#carve-outs) | `once` (Vision) to `per-queue-change` (annotations, decisions, defects, watches) | Aggregated — every narrative section in one file | The narrative sections move with queue changes rather than on every verb, and each is hand-curated. A concurrent edit to the same section is a genuine disagreement, and splitting per section would not remove a conflict between two appends to one section. |

Workstream charters, plan specs, landing records, `history.md`, `settled.md`, `references.json`, `inbox/`, and `logs/` keep their own files, as above.

### Accepted residual contention

Two conflicts remain by design, and they differ in kind:

- **`resume_anchor.md` — genuine.** Two sessions that both rewrite the anchor disagree about the next action. The conflict carries information, and a person resolves it. The same holds for two sessions editing one narrative section of `epic.md`.
- **`queue-view.md` — mechanical.** Two sessions that each regenerate the view after a queue change conflict on it, but the conflict carries no information, because the file is derived. ⛔ **Never merge `queue-view.md` by hand.** Merge the source files, run `orchestrator regenerate-view --slug {slug}` on the merged tree, and `git add` the result. The render is deterministic and carries no timestamp, so two machines rendering the same ledger state write byte-identical files and do not conflict at all.

### Staging identity

- **One machine — prevented.** `queue --add-row` publishes `queue/{PLAN-ID}.json` by hard-linking a completed temp file into place (`os.link`), which never replaces an existing file, so two local sessions staging the same plan id cannot both succeed: the second is refused with `duplicate_plan_id` and writes nothing. The duplicate-slug check runs inside a critical section scoped to the queue, before the create.
- **Queue order.** Each row carries `seq`, allocated at append time as the local maximum plus one. The view renders rows in `(seq, id)` order, so two machines that allocate the same `seq` still write disjoint files and still render one deterministic order.
- **Across machines — detected, not prevented.** No lock spans machines. A duplicate plan id staged on two machines surfaces as a git add/add conflict on the same filename, which is the correct signal for a genuine duplicate. A duplicate slug surfaces through the `shared_slugs` detector `resume-summary` already runs.

### Readers outside the layout module

Every read and write of the ledger files goes through ONE layout module, `manage-status/scripts/_orchestrator_ledger.py`: it resolves the paths — grammar-checking a plan id before it becomes a filename (ADR-016) — assembles the ledger into one view for every reader, and detects the monolithic layout. `manage-status`, `orchestrator.py`, `_orchestrator_inbox.py`, and `pm-plugin-development:tools-epic-surface-partition` all read through it.

**The one deliberate exclusion** is the platform-runtime terminal-title seam, `_read_orchestrator_title_state` (reached through `claude_runtime.py`), which `session push-title-token --store orchestrator --slug S` calls. It opens the epic `status.json` directly and stays outside the layout module, for four reasons:

- **It reads header keys only** — `kind` and `title_token`. The split keeps both in the header `status.json` with the same meaning, so the read is correct on the per-concern layout and on the monolithic one alike.
- **It reads no queue row and no anchor text.** The `legacy_layout` refusal exists to stop a legacy queue or anchor from being read as empty, which cannot happen to this read, so the refusal does not apply to it.
- **Routing it through the module would put a cross-skill import on the render path.** The render-hook path would import a private helper from another skill (`manage-status`), and that path runs on every render event — one of the largest recurring script costs.
- **It writes nothing to the ledger.**

**Rule for the future.** A new reader of queue rows or anchor text outside `manage-status` MUST go through the layout module. A new header-only reader MAY follow the title seam's exclusion, and this section lists every such exclusion by name.
...
- **The ledger's JSON files and `resume_anchor.md` are the machine authority.** The header `status.json`, the `queue/{PLAN-ID}.json` rows, and `resume_anchor.md` together hold the epic's machine state — workstream states, the plan queue with each plan's lifecycle state, and the resume anchor (`kind=orchestrator`, managed via `manage-status --store orchestrator` and `orchestrator queue`; schema documented in [`manage-status/standards/status-lifecycle.md`](../../manage-status/standards/status-lifecycle.md)). `status.json` alone is only the header. `queue-view.md` is a derived view of these files, and any disagreement between the two is resolved by regenerating the view. Any statement in `epic.md` that conflicts with them is stale prose — the ledger files win, and the reconciliation direction is always ledger files → `queue-view.md` and `epic.md`, never the reverse.
- **START HERE and the Ordered Queue are GENERATED, and persisted only in `queue-view.md`.** One renderer produces both from the assembled ledger and the staged specs, and one writer persists them. This is the one place the writer set is named: `orchestrator regenerate-view`, the `compact` stage, and `orchestrator migrate-layout` each write the file through that writer, and nothing else writes it. Nothing is pasted into `epic.md`, and `resume-summary` writes nothing — it returns the rendered blocks together with a `view_current` flag that says whether the committed `queue-view.md` matches a fresh render. Hand-editing `queue-view.md` is prohibited: a hand-written block silently forks the authority and defeats the resume contract. Per-row narrative the renderer cannot derive lives in `epic.md`'s `## Queue annotations` zone, keyed by plan id. Regenerate the view after every state change that touches the queue or the anchor.
- **Regeneration cannot hide a source conflict.** `regenerate-view` renders from the assembled ledger and never reads `queue-view.md` itself, so conflict markers left in the view by a merge are simply overwritten. When the header or any row file is unreadable — for example because it holds git conflict markers from a genuine duplicate id — the verb refuses with that file's unreadable state, names the file, and writes nothing, so a genuine source conflict stays visible until it is resolved.
- **The monolithic layout is refused, never read.** A ledger whose `status.json` still carries the queue or the anchor is reported as `legacy_layout`, naming `orchestrator migrate-layout` as the remedy — the only path off the monolithic layout. There is no read-fallback: the ledger is git-tracked with no compatibility shim (ADR-024), and a legacy ledger is a distinct state, never read as an empty one (ADR-019).
- **The resume anchor is kept current.** `resume_anchor.md` names the exact next action a resuming session takes (e.g. "await PR #912 CI, then analyze landing"). Every session updates it before stopping and whenever the next action changes. A stale anchor is a defect, not a cosmetic issue — it is the single field a fresh session trusts first.
- **Stop is always safe.** Because every decision, interaction, plan-status change, and reconciliation is persisted (to the ledger files, `epic.md`, and `logs/` via `manage-logging --store orchestrator`), no orchestration state lives only in model context. A session that ends mid-thought loses nothing that the resume contract needs.
- **Close freezes, never deletes.** Closing an epic writes the final state into `history.md` and marks the header `phase` `closed`; the tree remains on disk as the audit record.
- **Compact regenerates and relocates, never deletes.** The `compact` stage (the ledger-compaction stage `cleanup` Phase B calls) reconciles a **live** epic's ledger record: it regenerates `queue-view.md` from the ledger files and the filesystem, and relocates settled narrative from `epic.md` to `settled.md` with a pointer left at the origin. It is the third content-level operation, and the only mid-life one — `close` freezes and `archive` relocates the *whole tree* post-close, while `compact` touches *content* inside a live epic. It deletes nothing: the generated view is overwritten with its own re-derivation, and a narrative item is moved with a pointer, never dropped. The full contract is [§ Ledger-Compaction Stage](#ledger-compaction-stage).
...
Which verb writes which part of the ledger, and how often. This is the collision surface: two sessions on two machines that both write the same file produce a git merge conflict, whether or not their writes meant anything to each other. The tables below name, for each top-level `status.json` field and each `epic.md` template section of the **monolithic layout** — one `status.json` holding every field, one `epic.md` holding the generated blocks, the layout `orchestrator migrate-layout` converts from — the verbs that write it and the frequency class it is written at. The per-concern layout in [§ Directory Layout](#directory-layout) is cut along exactly these lines, and its per-file decision table carries the same frequency classes forward.

**Frequency classes** — a closed set:

| Class | Written |
|-------|---------|
| `once` | At creation, and never again |
| `per-epic-lifetime` | At a lifecycle transition — a handful of times across the whole epic |
| `per-decompose` | On each `decompose` run |
...
| Field | Written by | Frequency class |
|-------|------------|-----------------|
| `kind` | `manage-status create` (`init`) | `once` |
| `title` | `manage-status create` (`init`) | `once` |
| `created` | `manage-status create` (`init`) | `once` |
| `phase` | `manage-status create` (`init`, value `init`); `update-field --field phase` at `decompose` and `lessons` (`orchestrating`) and at `close` (`closed`) | `per-epic-lifetime` |
| `workstreams` | `update-field --field workstreams` (whole array) at `decompose` | `per-decompose` |
| `metadata` | `metadata --set` at `init` (`parallelization_scope`, including its correction on `init` re-entry) | `per-epic-lifetime` |
| `plans` | the bulk `update-field --field plans` seed at `decompose`; `orchestrator queue --add-row` at `decompose`, `analyze` (stage disposition) and `lessons`; `queue --transition` at `analyze` and `next`; `queue --set-row` at `analyze` | `per-queue-change` |
| `resume_anchor` | `update-field --field resume_anchor` at `init`, `decompose`, `next`, `analyze`, `resume`, `close`, `cleanup`, and `lessons` | `per-verb` |
| `updated` | stamped by EVERY write — `create`, `update-field`, `metadata --set`, and all three `queue` write forms | `per-verb` — **collision line shared by every write** |

⛔ **`updated` is the collision line shared by every write.** Every write re-serializes the whole document and restamps `updated`, so two sessions that write `status.json` on two machines conflict on that one line even when they touched disjoint fields — a staged row on one machine and a moved anchor on the other still collide. The local `rmw_json` critical section serializes writers on ONE machine; nothing spans machines, so the stamp turns every pair of concurrent writes into a merge conflict. It carries no information a reader acts on, which is what makes the collision false rather than genuine.

### `epic.md` sections

| Section | Written by | Frequency class |
|---------|------------|-----------------|
| Title heading and `slug:` line | `init` (instantiated from `templates/epic.md`) | `once` |
| `## Vision` | `init` | `once` |
| `## START HERE` — the `resume-summary` generated block | pasted from `orchestrator resume-summary` by `decompose`, `next`, `analyze`, `resume`, `close`, and `lessons`; rewritten in place by `compact` at `cleanup` | `per-verb` — the block renders the resume anchor, so it moves whenever the anchor does |
| `### Annotations` (under START HERE) | hand-written by the reconciling verb | `per-queue-change` |
| `## Ordered Queue` — the `ordered-queue` generated block | the same `resume-summary` paste set as START HERE; rewritten in place by `compact` at `cleanup` | `per-queue-change` |
| `### Queue annotations` | hand-written at `decompose` and `analyze`; `cleanup` evacuates hand-written content into it | `per-queue-change` |
| `## Decisions` | hand-curated at `decompose` and `analyze` beside their `logs/decision.log` records; `cleanup` relocates settled entries to `settled.md` | `per-queue-change` |
| `## Open Defects` | `analyze` (invalid-message, failed-archival, incomplete-landing, and absorb-arm entries); `cleanup` relocates resolved ones | `per-queue-change` |
| `## Watches` | `analyze` (absorb-arm and unverified-lead entries); `cleanup` relocates resolved ones | `per-queue-change` |
| Epic-specific sections outside the template (e.g. `## Lesson Sweeps`) | the verb that owns the section — `lessons` appends one dated sweep record per run | `per-verb` |

**What the pattern shows.** In the monolithic layout, three distinct write surfaces share two files. The header fields (`kind`, `title`, `created`, `phase`, `workstreams`, `metadata`) are written rarely. `resume_anchor` and the START HERE block move together on nearly every verb, and that contention is genuine — two sessions rewriting the next action disagree about it. `plans[]` and the Ordered Queue move on every queue change, and the narrative sections move in the same sessions. Because all of it serializes into one `status.json` and one `epic.md`, a rare header write, a per-row queue change, and an anchor move all collide with one another — through the `updated` stamp in `status.json`, and through the wholesale re-paste of the generated blocks in `epic.md`. The per-concern layout gives each surface its own file and drops the stamp.
...
- **Canonical invocation.** The settlement is the single platform-runtime seam, invoked with the orchestrator store and the epic slug:
  ...
  The call has a single purpose: it **establishes the session→epic binding** (`bind_orchestrator`) as a best-effort side effect and settles the epic's title state, so subsequent hook-driven renders resolve the epic and deliver the orchestrator title on the `terminalSequence` channel — the sole channel that lands. The seam itself repaints nothing; delivery is deferred to the next render event. That binding side effect is what gives the orchestrator a path into the delivering channel, and it lights up every epic-scoped verb with **zero per-verb doc edits** — the delivery path rides the existing per-verb call. It does not reach `preflight`, whose plan-scoped form establishes no session→epic binding.

- **Entry-point placement.** The push fires after slug resolution and before the verb's first read, so the title is already correct while the verb does its work. The discriminator is not "the epic might be absent" — nearly every epic-scoped verb could hit that — it is whether the verb may have to SCAFFOLD its epic before it can read it: a verb whose slug is a fixed constant it re-enters, and whose epic tree may or may not already exist (`lessons`, entering the fixed `lessons-routing` epic), moves the push to the first point at which both the slug and the epic's `status.json` exist. A verb that CREATES the epic as its own purpose (`init`) keeps the entry push and instead fires a follow-up repaint after `manage-status create` — a different remedy for a different shape of the same underlying constraint: an entry push cannot resolve epic state before `status.json` exists.
...
The orchestrator MAY use Write/Edit directly — but ONLY within its own `.plan/orchestrator/{slug}/` tree. This is a deliberate, bounded exception to the `.plan/` access via `manage-*` scripts only rule: the orchestrator's ledger documents (`epic.md`, workstream charters, plan specs, landing records, `history.md`, `settled.md`, `references.json`) are free-form authored artifacts with no owning manage-* script.
...
- **Ledger state** — the ledger JSON files (the header `status.json` and the `queue/{PLAN-ID}.json` rows) and `resume_anchor.md` are created, read, and mutated via `manage-status --store orchestrator` and the `orchestrator.py queue` verb, never by direct file writes.
- **The generated view** — `queue-view.md` is written only by the renderer (see [§ Persist / Stop-Resume Contract](#persist--stop-resume-contract)), never by a direct file write.
...
The plan queue — the `queue/{PLAN-ID}.json` row files — is written by exactly two sanctioned forms, and which one applies is decided by the SHAPE of the change rather than by which verb happens to be running:

| Occasion | Form | Scope of the write |
|---|---|---|
| **Single append** at stage — and at `decompose`, which seeds a new queue by calling it once per row | `orchestrator.py queue --add-row` | Creates one new row file, atomically (see [§ Staging identity](#staging-identity)) |
| **Single mutate** at reconcile | `orchestrator.py queue --transition` (status) and `queue --set-row` (the three result fields) | Rewrites one located row file |

Three rules bind this boundary:

- **The three `queue` flags are mutually exclusive per call.** Supplying more than one returns `wrong_parameters`. The accepted flag set per form — including that `--status` is REQUIRED with `--transition` and OPTIONAL with `--add-row` — is published at [`plan-orchestrator/SKILL.md` § Canonical invocations → queue](../../plan-orchestrator/SKILL.md#queue) and is not restated here.
- **There is no whole-queue write form.** A whole-array rewrite cannot exist over per-row files, so `manage-status update-field` does not accept the queue as a field on the orchestrator store. Seeding a queue from nothing is a sequence of single appends.
- **Direct edits of the ledger files remain prohibited outright**, per the script-mediation bullets above. This subsection narrows WHICH script-mediated form to use; it grants no direct-write path.
...
**The executing plan MUST NOT create or edit any file under `.plan/orchestrator/{epic}/`** — not `status.json`, not `resume_anchor.md`, not `queue/`, not `queue-view.md`, not `epic.md`, not `workstreams/`, not `plans/`, not `landings/`. The orchestrator owns every ledger write and reconciles the epic from the landed PR through the `analyze` verb. A plan has exactly two channels back to the epic: its PR, and its `inbox/` OUTBOX.
...
- **One-way per location, and the ledger is never read by a plan.** The epic QUEUE is one-way — the plan writes, the orchestrator drains — the drain being the `analyze` verb's inbox-scan input mode ([`plan-orchestrator/workflow/analyze.md`](../../plan-orchestrator/workflow/analyze.md) § The four input modes), which enumerates through `inbox list` and consumes through `inbox archive` ([`plan-orchestrator/SKILL.md`](../../plan-orchestrator/SKILL.md) § Canonical invocations). The addressee MAILBOX carries the other direction: a message aimed at a running plan is delivered into `inbox/to/{plan_id}/`, and the addressed plan takes it through `inbox read --plan-id` ([`plan-orchestrator/SKILL.md`](../../plan-orchestrator/SKILL.md) § Canonical invocations), which resolves that one mailbox address and reaches no other path in the epic tree. What stays absolute across both is the load-bearing half — **a plan never reads the ledger to make a decision**: the ledger files, `queue-view.md`, `epic.md`, `workstreams/`, `plans/`, and `landings/` stay orchestrator-owned, and a mailbox read is a read of messages addressed to that plan, never of the epic's own state.
...
- **The three tests.** All three must pass; any failure means the sub-step runs inline.
  ...
  - **Write-freedom** — the sub-step **performs** no ledger write. Drafting the content of a ledger write is not performing it, and the test turns on that distinction rather than on being connected to a write at all: a sub-step that only composes what a write will *say* passes write-freedom, because the leaf returns the draft as structured data and the orchestrator performs every resulting write itself, through the sanctioned calls it already uses. A sub-step that performs the write fails, and no drafting framing rescues it. The boundary, concretely — a drafting leaf MAY compose a document body, a proposal, or a spec draft; it MAY NOT call `Write`/`Edit` inside `.plan/orchestrator/{slug}/**`, `manage-status` or `manage-logging --store orchestrator`, `orchestrator queue`, or `corpus set-verdict`. Every ledger-file, `queue-view.md`, `epic.md`, `logs/`, workstream-charter, plan-spec, and landing-record write therefore still stays in the orchestrator: the draft crosses the dispatch edge as data in the leaf's return, and the write never crosses it at all. Containment for this widened class is **S1 and S2 below, at full strength** — S2's absolute prohibition on a leaf writing inside the epic tree is precisely what makes dispatching a draft safe, so widening this test reaffirms S2 rather than relaxing it, and S1's read-only instruction bars the leaf from writing, never from composing the draft it returns.
- **Canonical form.** One dispatch shape, used verbatim. The level resolves outside any plan context, and the resolve carries the same four facts the prompt body does, so the trail and the envelope agree:

  ```bash
  python3 .plan/execute-script.py plan-marshall:manage-config:manage-config \
    effort resolve-target --role orchestrator.{surface} --plan-id none \
    --caller plan-marshall:persona-plan-orchestrator \
    --workflow {the workflow/instructions doc the leaf loads}
  ```

  ⛔ **Pass an explicit `--role orchestrator.{surface}` at the `analyze` / `decompose` sites, never `--default`.** A `--default` resolve carries no payload role and renders the literal `default` in the trail, which is not distinguishable between the two surfaces. It is also the wrong resolver input wherever `orchestrator.effort` carries anything that changes the resolved level — **including a `max` that binds**. `--default` short-circuits to `plan.effort` *before* any orchestrator handling, so it enters neither the walk nor the clamp: the effort dimension below resolves each surface through `orchestrator.effort.{surface}` → `orchestrator.effort.default` → `plan.effort` and THEN clamps to `orchestrator.effort.max`, and only an explicit `--role` reaches either stage. ⛔ **Divergence is PER SURFACE, not per configuration.** The two spellings differ at a given surface exactly when `orchestrator.effort` resolves *that surface* to a level other than `plan.effort`. The same block can therefore diverge at one site and agree at another: with `orchestrator.effort: {"analyze": "level-6"}` the `analyze` site resolves `level-6` while `decompose` falls through to `plan.effort`.

Shapes that can resolve a surface away from `plan.effort`: a `{surface}` override; a `default`; the scalar shorthand (`orchestrator.effort: "level-6"`, which resolves with source `orchestrator.effort` — its own rung, not `default`, and so **not reachable by the `max` clamp**, which is read only from the object form); and a binding `max`. Each has a complement that agrees instead — any of the first three set equal to `plan.effort`, or a `max` at or above the resolved level. A value outside the level enum is neither: the resolve returns `status: error` and no level at all. With `orchestrator.effort` at its seeded empty `{}` both spellings resolve to the same level — the walk falls straight through to `plan.effort` either way — so the tier divergence is what an operator's configuration *makes* possible, not something observable at the shipped default. The trail divergence, by contrast, is unconditional. `--workflow` names the doc the leaf actually loads, `--plan-id none` states that an epic is not a plan, and `--caller` names this identity.

  **The resolve seam is what emits the dispatch record.** Each firing writes the `[DISPATCH]` line and its paired decision-log record to the dated global log, so the orchestrator lane leaves a dispatch trail on the same surface every other lane does — which is the whole reason the four facts above ride the resolve rather than the prompt body alone.
...
- **S2 — ledger writes stay in the orchestrator.** No leaf dispatched by an orchestrator verb writes inside `.plan/orchestrator/{slug}/**`, and none invokes `manage-status` / `manage-logging --store orchestrator`. This is the [direct-file-write carve-out](#carve-outs) extended across the dispatch edge, and it stays absolute for the class it governs: a sub-step leaf of an orchestrator verb has no inbox business — the orchestrator records its findings itself. It does NOT govern the plan-lifecycle leaves of an executing plan, whose `inbox/{sender}-{seq}` OUTBOX carve-out is stated in [§ Ledger Write-Boundary](#ledger-write-boundary) with its three qualifiers.
- **Effort dimension — every dispatch this rule sanctions runs at a config-resolved, bounded tier.** The sanctioned dispatchable sub-steps are these **seven**, and this list is a complete index of them: (1) `analyze.md` Step 2 landing ground-truth corroboration; (2) `analyze.md` Step 2 untrusted-text reader ingestion; (3) `analyze.md` Step 4 item 1 landing-report body draft; (4) `analyze.md` Step 5b per-message disposition proposals; (5) `analyze.md` Step 5b **Stage** new-spec body draft; (6) `decompose.md` Step 2 on-disk-corpus read, candidate mapping, and prior-art / collision search; and (7) the `decompose.md` Step 2 plan-spec body draft — the per-candidate draft the SAME Step 2 envelope as (6) returns (that verb doc names the return field), never a second dispatch, and never `decompose.md` Step 4, whose `Write` instantiating each `plans/PLAN-NN-{plan_slug}.md` is inline-only on write-freedom. The **drafting** sub-steps, admitted by the write-freedom test above, are (3), (4), (5) and (7); (1), (2) and (6) are reads. The drafting set is therefore NOT this list's positional tail, and (6) sitting inside that tail is why: at `decompose.md` Step 2 a read and a draft ride one envelope. Each verb doc dispatches its drafting sub-steps as internally-iterating envelopes rather than one envelope per item. All seven resolve their effort from the `orchestrator.effort` config block rather than always taking the `effort resolve-target --default` tier. Each surface resolves through `orchestrator.effort.{surface}` → `orchestrator.effort.default` → `plan.effort` → `inherit`, and the resolved level is then deterministically clamped by the `orchestrator.effort.max` uplift ceiling. This replaces the former free-prose discretion ("MAY run at a higher tier") with a config-resolved bound: the per-surface value carries the uplift, `max` bounds it, and no per-dispatch judgement picks the tier. **Unset resolves to exactly today's behaviour** — with `orchestrator.effort` unset every surface falls through to `plan.effort` (the tier `effort resolve-target --default` returns), and an unset `max` is a no-op (no clamp). The boundary is what makes a higher tier safe: **orchestrator = high-level plus analyze, plan = fine-grained.** A higher-effort dispatch gathers, verifies, and drafts; it returns a structured verdict or a draft, and never reproduces the plan lifecycle's fine-grained implementation work — reproducing it is mechanism duplication, the exact anti-pattern the tier boundary exists to prevent. The tier is the only thing that moves: the three tests, the canonical dispatch form, and S1 / S2 apply unchanged at every effort level. The role registry and the block schema live in [`effort-roles.md` § Orchestrator role group](../../plan-marshall/standards/effort-roles.md) and [`marshal-json-reference.md` § Orchestrator Configuration](../../extension-api/standards/marshal-json-reference.md).
...
**The section-level reading.** Every row above is addressed at claim scope. A row can also be addressed at **section** scope, because the claim section itself has a readable state and a spec whose section the parser could not read is not the same document as a spec that declares no claims. `corpus verdicts` reports that state over a four-member **parse-coverage** vocabulary — `absent`, `empty`, `unreadable`, `parsed` — tallied across the whole vocabulary so a state no spec is in publishes a stated zero. The discrimination rule is deliberately conservative: `absent` when the spec carries no `## Claim Labels` heading; `empty` when the section body carries no non-blank unfenced line; `unreadable` when it carries such content but yields no top-level claim bullet — the table-form and prose-only authorings; `parsed` when at least one claim bullet is found. A body whose only non-blank content sits inside a fence reads as `empty`, because the parser deliberately never treats a fenced `- item` as a claim.

An **unsettled `unreadable` section is both reported and blocking**: it contributes exactly one section-scoped row that reads precisely as the last table row above does — `indeterminate`, and therefore `admits: false` — so a section that could not be read can no longer pass the prep-ready test by contributing nothing at all. That is the whole correction: an empty claim list used to mean both *"this spec declares no claims"* and *"this spec's claims could not be read"*, and only the first of those admits. An `absent` or `empty` section contributes **no** row and still admits, unchanged.
...
Two of the three `true` bases are **fail-closed** — `surface_not_declarative` and `tree_diff_unavailable`: an uncomparable surface and an uncomputable difference are not evidence that nothing moved, and holding them apart from `declared_surface_unchanged` is what keeps *could not evaluate* separate from *evaluated and found unchanged*. The third, `declared_surface_touched`, is not a fallback — it is a **measured** positive, with the matched paths riding the row as direct evidence a declared entry changed. Governing authority: **ADR-019**, the same separation this section's sibling [§ The gate's reading contract](#the-gates-reading-contract) applies to the disjointness verdict. The tally `corpus verdicts` publishes spans the whole vocabulary, so a basis no row reached is a stated zero rather than a missing row.
...
- **Subject boundary.** The **spec corpus** — the staged `plans/PLAN-NN-{slug}.md` specs — is this verb's subject. The **ledger record** — the generated `queue-view.md`, settled-narrative relocation out of `epic.md` — is the compaction stage's subject and stays there (the resume anchor's shape and `inbox/` foldering are within that subject too but are owned by their own successor specs, not by the landed stage). `cleanup` CALLS the compaction stage ([§ Ledger-Compaction Stage](#ledger-compaction-stage), now landed) and never grows a second compaction implementation. Two implementations of ledger compaction is a worse outcome than no verb at all.
...
The `compact` stage is the ledger-compaction stage the `cleanup` verb's Phase B runs. It is the STAGE the § Cleanup Contract's verb-name settlement names — `cleanup` is the operator verb, `compact` is the ledger stage — and its subject is the **ledger record** (`queue-view.md` and the narrative in `epic.md`), never the spec corpus. It is a stage, not a router verb, so it has no workflow doc of its own: the deterministic surface (arguments, error codes, report shape) is documented once at [`plan-orchestrator/SKILL.md`](../../plan-orchestrator/SKILL.md) § Canonical invocations → `compact`, and the procedure (including the settled-narrative relocation judgement half) is [`plan-orchestrator/workflow/cleanup.md`](../../plan-orchestrator/workflow/cleanup.md) § Step 8 (Phase B). This subsection is the binding contract both bind to.

- **The discriminator is DERIVABLE vs NARRATIVE, not old vs new.** A statement is **derivable** when it is re-derivable from the ledger files and the filesystem — counts, the queue table, per-plan status mirrors, the Surface a spec declares, PR/landing stamps. A statement is **narrative** when the world offers no derivable source — a decision, a retraction, a refutation, a standing rule, a do-not-re-derive note. The high-value content is often old, settled, and reads like archaeology (a retraction exists precisely to prevent rework); a trim-by-age pass would delete exactly it. So: **regenerate the derivable; preserve the narrative regardless of age** — relocate it if bulky, never drop it.

- **The split is NOT applied mechanically per field.** The operator-confirmed `running` state is the standing counter-example: a queue **row** at status `running` mirrors its `queue/{PLAN-ID}.json` file and IS regenerated, but a narrative **note** about liveness ("the operator confirmed at session start that PLAN-03 is running") has **no machine field and no liveness signal** — nothing observes whether a `running` plan is alive. Regenerating that note would **fabricate a fact**. It is narrative because the world offers no derivable source, not because prose sits where a field belonged; the stage leaves it untouched and the boundary must be written so a cold reader never regenerates it.

- **The regeneration mechanism — a separate generated file.** Every derivable surface lives in `queue-view.md`, which the stage regenerates after its invariant pass through the same writer `orchestrator regenerate-view` uses, from the ledger files and the staged specs. `epic.md` carries no generated block, so the script half of the stage has no write path into it at all — that is what makes "a retraction survives a pass verbatim" a **structural** property rather than a matter of care. The render lists only LIVE rows in the Ordered Queue by construction, so no separate invariant has to guard a terminal row out of a hand-pasted table. The invariants the stage still verifies are the bidirectional queue-versus-spec reconciliation and the reachability of every relocation pointer. Moving the generated blocks out of an `epic.md` that predates the per-concern layout is `orchestrator migrate-layout`'s one-time job, not the stage's: it removes both GENERATED marker blocks with their guidance comments, leaves every hand-written byte untouched, and writes a fresh `queue-view.md` through the renderer rather than copying pasted text that may be stale.

- **The annotation-zone contract.** "GENERATED, never hand-edited" and the per-row annotations a renderer cannot derive (a blocked reason, a park caveat, a sequencing note) are reconciled by keeping them in different FILES. The annotations live in `epic.md`'s `## Queue annotations` zone, keyed by plan id; the derivable content lives in `queue-view.md`. A regeneration never reaches the annotations, and a hand annotation never lands inside the generated view, so neither destroys the other.

- **The `## Decisions` authority.** The append-only `logs/decision.log` (written via `manage-logging --store orchestrator`) is the authoritative decision record; the `epic.md` `## Decisions` section is a curated human-facing VIEW carrying rationale and alternatives the log summary need not. It is therefore **narrative**, not a derivable surface — `compact` never regenerates it. Declaring this authority is the whole fix: it closes the "duplicates the log with no stated authority" gap without turning a rationale-bearing section into a lossy regeneration.
...
- **Idempotence.** Content-addressed for the derivable half — `queue-view.md` is a deterministic render with no timestamp, so a second run produces byte-identical content and writes nothing. Pointer-presence-keyed for the narrative half — an item already carrying a relocation pointer is not relocated again. A second run immediately after the first is a no-op.

- **The report makes it safe to run unattended.** `compact` names every mutation AND every abstention AND every invariant verdict, each count carrying its population, and an abstention says WHY it abstained. A silent compaction is indistinguishable from a lossy one — this epic's whole theme — and a blind spot reported as a deliberate abstention is the same defect one level up. So the report, not the mutation, is the deliverable that makes the stage safe.

- **Refuses a closed epic.** `compact` mutates the ledger record, so it refuses a `phase: closed` epic (`refused_closed`) and resolves the store strictly (never the archived read-fallback). That tree is the frozen audit record `close` already sealed; compaction is a live-epic operation only.
...
- **Fixed-epic sweep.** The `lessons` verb ALWAYS targets the single standing epic `lessons-routing`. The slug is a fixed constant — neither an input nor derived — so the mode is repeatable over time by re-entering one durable epic rather than by opening a new one. The epic is scaffolded only when it is absent; a run against the existing tree neither re-creates nor reopens it. No `lessons-handling-{YY-MM-DD}-{NN}` epic is ever opened again: the retired dated epics — the live population is enumerable via `orchestrator corpus epics`, which is the derivation to re-run rather than a tally to maintain here — are closed history, kept as the audit record.
- **Local dedup/aggregate obligation.** The local pass enumerates the current repo's lessons and MUST cluster similar or duplicate lessons, aggregating each cluster into ONE bundled item — never one item per lesson. Every lesson receives an auditable per-lesson disposition in the epic ledger: clustered-into, already-covered, standalone, or stale.
- **Sweep findings route outward.** A sweep's clustered dispositions are ROUTED to each cluster's owning sibling epic over the inbox channel (`orchestrator inbox write`, `kind=finding` — a cluster's bundled statement is not one lesson body in the lessons corpus's shape, so it does not carry the `candidate-lesson` payload contract), never staged into `lessons-routing`'s own plan queue. `lessons-routing` is a distribution point that holds no plans of its own. The single narrow exception is a cluster that is a tooling defect in the routing/versioning MECHANISM itself — not lesson content — which MAY be staged as a `PLAN-LR-NN` spec in `lessons-routing`. That exception is taken only after confirming the defect is not already shipped and is not in fact `truthful-signals`' subject; failing either check routes the cluster outward like any other.
- **Sweep record.** Each run's disposition record is a dated subsection under `lessons-routing`'s own `## Lesson Sweeps` section in `epic.md`. `epic.md` is hand-written narrative only, so the section is narrative under [§ Ledger-Compaction Stage](#ledger-compaction-stage): `compact` never regenerates it. A sweep record is the auditable trail of what a run disposed of and where it routed each cluster, and the world offers no derivable source for it.
- **Cross-repo integrate-then-remove.** When the operator supplies a lessons directory from ANOTHER repo, the sequence is normative and strictly ordered: (1) read each remote lesson file directly — its text is externally-sourced content under the untrusted-ingestion boundary above; (2) classify applicability to the current repo; (3) INTEGRATE the applicable ones locally (fold into a cluster, or register into the current repo's `manage-lessons` store when the lesson is a standing rule); (4) only after the local integration is persisted, REMOVE the integrated lesson files from the remote repo.
...
- ADR-002 (`doc/adr/`) — the bounded main-anchored resolver exception set, which the orchestrator store no longer belongs to; ADR-024 records the amendment that moved it to the git-tracked tier

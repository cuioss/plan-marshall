...
│                        #   from the files above; written only by regenerate-view and compact
...
- **One machine — prevented.** `queue --add-row` creates `queue/{PLAN-ID}.json` with an exclusive create (`O_EXCL`), so two local sessions staging the same plan id cannot both succeed: the second is refused with `duplicate_plan_id` and writes nothing. The duplicate-slug check runs inside a critical section scoped to the queue, before the create.
...
- **The generated view** — `queue-view.md` is written only by `orchestrator regenerate-view` and the `compact` stage, never by a direct file write.

...
**Re-entry (accumulate, never replace)**: Calling `end-phase` more than once for the same phase — which is exactly what a finalize loop-back into an earlier phase produces — **accumulates** onto the existing row rather than replacing it, and increments the row's `close_count`. Three rules apply, because the fields do not share one arithmetic:

...
- **Canonical form.** One dispatch shape, used verbatim. The level resolves outside any plan context, and the resolve carries the same four facts the prompt body does, so the trail and the envelope agree:
  ...
  ⛔ **Pass an explicit `--role orchestrator.{surface}` at the `analyze` / `decompose` sites, never `--default`.** A `--default` resolve carries no payload role and renders the literal `default` in the trail, which is not distinguishable between the two surfaces. It is also the wrong resolver input wherever `orchestrator.effort` carries anything that changes the resolved level — **including a `max` that binds**. `--default` short-circuits to `plan.effort` *before* any orchestrator handling, so it enters neither the walk nor the clamp: the effort dimension below resolves each surface through `orchestrator.effort.{surface}` → `orchestrator.effort.default` → `plan.effort` and THEN clamps to `orchestrator.effort.max`, and only an explicit `--role` reaches either stage. **The rule, not a list of cases:** the two spellings diverge whenever `orchestrator.effort` carries anything that moves the resolved level off `plan.effort`. Worked examples, each run rather than reasoned — a `{surface}` override differing from `plan.effort`; a `default` differing from it; the **scalar shorthand** (`orchestrator.effort: "level-6"`, which the walk reads at the `default` rung); and a `max` below the level the walk produced. Each of their complements agrees instead — an override, `default` or shorthand equal to `plan.effort`, and a `max` at or above the resolved level. ⛔ **Treat that list as illustrative, never exhaustive**: the rule above is what governs, and a configuration absent from the list is decided by the rule, not by its absence. With `orchestrator.effort` at its seeded empty `{}` both spellings resolve to the same level — the walk falls straight through to `plan.effort` either way — so the tier divergence is what an operator's configuration *makes* possible, not something observable at the shipped default. The trail divergence, by contrast, is unconditional. `--workflow` names the doc the leaf actually loads, `--plan-id none` states that an epic is not a plan, and `--caller` names this identity.

...
| **issue comment** (fallback) | a comment whose author resolves to the awaited `bot_kind`, whose later of `updated_at` / `created_at` strictly post-dates the trigger time, and which is not a refusal notice | `matched_signal: issue_comment`, `matched_comment: {…}`, `head_sha_verified` derived from the comment body — `true` only when the body references `--head-sha` |

**"References" is not whole-field equality.** The review signal recognises the SHA wherever the reviewed-commit evidence carries it — as a bare token, or embedded in a commit URL — and compares it for equality either way, so a review of a *different* commit still fails to verify. Matching the whole field for string equality would recognise only the bare shape, drop a URL-shaped review onto signal 2, and publish `head_sha_verified: false` for a HEAD the bot HAD reviewed. The predicate is `_references_head_sha` in `github_re_review.py`, whose module docstring is the authoritative statement.
...
`head_sha_verified` on the comment path is read from the comment BODY through the same `_references_head_sha` the review path uses: a bot that republishes its summary in place names the commit it reviewed there, so a body referencing `--head-sha` verifies, while a body naming no commit — or a different one, or an abbreviation — publishes `false`, meaning the match proves only that the bot **responded**, not that it reviewed the new HEAD. The widening is in location only; every token is still compared for equality. The caller learns the strength of the evidence, not just the fact of a match. The comment path uses the LATER of `updated_at` / `created_at` so a bot that edits ONE persistent comment in place still registers as fresh activity. No bot is named in code — both matchers are generic across the registry.

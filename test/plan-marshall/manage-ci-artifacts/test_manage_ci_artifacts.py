...
def test_cmd_persist_with_populated_jobs_file_writes_job_rows(tmp_path, capsys):
...
def test_cmd_persist_without_jobs_file_labels_empty_source(capsys):
...
def test_read_latest_selects_newest_by_fetched_at_not_run_id():
...
def test_read_latest_returns_log_paths():
...
def test_read_latest_errors_cleanly_when_no_runs_persisted():
...
def test_cmd_read_latest_dispatches_to_latest_accessor(capsys):
...
def test_cmd_read_latest_error_envelope_when_no_runs(capsys):

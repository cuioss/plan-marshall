...
- `action`: `defer` when policy is `defer` (orchestrator skips the merge directly); `ask` when policy is `ask` (orchestrator fires `question` with `prompt_options[]`). All three rate-window variants always use `action: ask`.

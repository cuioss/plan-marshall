...
| `default:sonar-roundtrip` | `standards/sonar-roundtrip.md` | Sonar analysis roundtrip — **FIND-only**: files new-code issues via `sonar fetch_findings`, then marks done; dispatches no triage of its own. Triage runs via the same dispatcher-owned unified pass (Step 3 item 7c, `producer=finalize-feedback`) |

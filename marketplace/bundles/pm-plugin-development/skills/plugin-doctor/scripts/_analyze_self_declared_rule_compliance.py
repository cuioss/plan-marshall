...
def _claude_skill_source_targets(marketplace_root: Path) -> list[Path]:
    """Return every ``SKILL.md`` under the project-local skill trees.

    The project-local trees are resolved through the platform-runtime layout op
    (``_doctor_shared.resolve_project_skill_trees`` → ``marketplace_paths.get_project_skill_roots``),
    so the same scan covers both the Claude ``.claude/skills/**`` tree and the
    OpenCode layout — no segment-wise ``.claude/skills`` anchor is hardcoded.
    """
    sources: list[Path] = []

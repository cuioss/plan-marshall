...
## ⛔⛔⛔ 2026-09-26 — THE STAGED QUEUE IS PARKED: `plan-marshall-mcp` supersedes it (inbox `review-apparatus-044`)

**Operator decision** (relayed by `review-apparatus`, confirmed by the operator in this session). `plan-marshall-mcp`
(PM-MCP, `/Users/oliver/git/plan-marshall-mcp`) replaces BOTH the process prose AND the Python scripts of
plan-marshall. **Nothing Python-bound or process-prose-bound carries**; what carries is implementation-independent
content — rules, invariants, classifications, data, real-corpus fixtures. PM-MCP's own PM-TEST-1 Ported/Redesigned
split (which would keep store/integration scripts worth fixing) was **explicitly withdrawn** — do not re-derive it.

- **36 of 39 `staged` rows → `parked`**, each spec bannered `SUPERSEDED BY PM-MCP` ("Do NOT emit; un-park only by
  explicit operator decision"). The 5 already-`parked` rows (`-156`/`-158`/`-159`/`-163`/`-164`, all retired-and-merged)
  stay parked and carry the same banner. Spec bodies are intact — they are the evidence chain.
- **3 rows stay `staged`, NARROWED** (operator-confirmed delivery-breaking exceptions; each spec carries a
  `NARROWED BY PM-MCP` banner naming the only deliverables still in scope):
  `PLAN-211` D1/D4/D6/D7 (localized merge-tree prose parsed as conflict paths — aborts phase-5),
  `PLAN-TRUTH-172` D2 (`pr_title_missing` on every light-lane plan),
  `PLAN-TRUTH-167` D1/D2 (self-review on a consumer diff loops to `max_iterations`, blocks push).
  A `next` emit of any of the three MUST scope the command to those deliverables.
- **Extraction:** `plan-marshall-mcp/doc/known-defects/truthful-signals-carry-over.md` — 44 specs,
  340 deliverable rows, 270 carry content, 70 none; of the 270: **98 gap, 125 partial, 47 covered**. Eight ⛔
  contradictions where PM-MCP's current spec encodes a defect this corpus exists to remove — headline: the
  FindingRecord disposition enum `FIX|SUPPRESS|ACCEPT` cannot record a refuted finding; ConfigReconciler back-fills
  operator-removed steps (TokenSheriff#745); adaptive timeout learns from timed-out runs and defaults an underived
  budget to 300 s; PM-MCP's recipe lane skips the only `pr_title` producer. These are the first thing to carry.
- **No emitted-but-not-launched command existed** — nothing to void (the prior anchor named "emit 149 first" as a
  next step; no command was emitted, and 149 is now parked).
- Rows were extracted by six read-only sub-agents and **not re-verified line by line**; the orchestrator
  de-duplicated cross-spec restatements. The carry-over is **operator input**, not a PM-MCP edit.
- **Same day, operator amendment — two items re-opened / added despite PM-MCP:**
  (1) `PLAN-TRUTH-168` **un-parked → `staged`**: `/marshall-steward upgrade` blindly overwrites operator
  adaptations (re-adds a removed `default:verify:coverage`, TokenSheriff#745). The upgrade flow reaches the
  back-fill through `manage-config sync-defaults` (`marshall-steward/references/upgrade-flow.md:339`), now in
  its Expected Surface. (2) **Domain self-review surfacers — never staged before** (archived `PLAN-TRUTH-126`
  had scoped `ext-self-review-java` out as "a separate unowned item"; only `ext-self-review-plan-marshall`
  exists): staged `PLAN-TRUTH-181` (foundation — shared envelope, per-content-class multi-implementor
  dispatch; needed because Step 1 runs only the FIRST resolvable implementor) and four domain plans on top of
  it: `-182` Java, `-183` Python, `-184` JavaScript, `-185` Documents. Order: `167` → `181` → `182`–`185` (the
  four run in parallel). Their detector rule sets are domain knowledge that also belongs in the PM-MCP
  carry-over.
- The standing inbound routing rule below still governs NEW items, with one amendment: a finding whose remedy is a
  Python or process-prose change routes to the carry-over as a rule/fixture, not to a new staged plan, unless the
  operator grants a delivery-breaking exception.
...
⛔ **Live-plan exclusions at the disjointness gate — operator decision 2026-09-26.** `NO_PLAN` is a sentinel
(the ad-hoc PR id, never a real plan) and `antigravity` is dormant. **Ignore both** when reading `corpus
cross-check`: their `live_plan` `indeterminate` rows do NOT block a `next` admission. The script cannot express
this exclusion (no ignore list exists — a gap for the PM-MCP carry-over, not a plan here), so it is applied by
the orchestrator reading the payload: subtract exactly these two from `live_plan indeterminate` before judging
`candidate_comparison_determinate`. Any OTHER indeterminate live plan still fails closed. Re-check `antigravity`
if it is ever resumed (`manage-status list` shows it past `1-init`).
...
- **2026-09-25 drain dispositions (179 landing + 7)** — landing reconciled as shipped (PR #1619);
  `-001`/`-002`/`-003` staged as PLAN-TRUTH-180 (script-internal-error trio, D0 owns reproduction
  from archived logs); `-004`/`-005` discarded (python-module triage policy, out of epic scope,
  remote CI governs by the messages' own account); `-006` folded into 169 as timeout-noise
  recurrence; `lessons-routing-002` folded into 169 (ci_wait budget tuning = D3's subject).
  Layout migration (`migrate-layout`, PR #1609's per-concern split) applied mid-drain when `queue`
  refused the monolithic ledger — 47 rows + anchor migrated, generated blocks relocated to
  `queue-view.md` by the migration itself.
...
> ↪ Relocated to `settled.md` § "Inbox drain — 2026-09-06, 1 message, 11 items dispositioned (`review-apparatus-033`)" — settled: the drain/sweep is complete — every message dispositioned and archived, every resulting plan row reconciled

## Inbox drain — 2026-09-07, 12 messages, every one dispositioned (PLAN-TRUTH-128 full ship)

> ↪ Relocated to `settled.md` § "Inbox drain — 2026-09-07, 12 messages, every one dispositioned (PLAN-TRUTH-128 full ship)" — settled: the drain/sweep is complete — every message dispositioned and archived, every resulting plan row reconciled

## Inbox drain — 2026-09-07 (b), 24 messages, every one dispositioned (PLAN-TRUTH-099 full ship)

> ↪ Relocated to `settled.md` § "Inbox drain — 2026-09-07 (b), 24 messages, every one dispositioned (PLAN-TRUTH-099 full ship)" — settled: the drain/sweep is complete — every message dispositioned and archived, every resulting plan row reconciled

## Inbox drain — 2026-09-07 (c), 11 messages, every one dispositioned (PLAN-TRUTH-125 full ship)

> ↪ Relocated to `settled.md` § "Inbox drain — 2026-09-07 (c), 11 messages, every one dispositioned (PLAN-TRUTH-125 full ship)" — settled: the drain/sweep is complete — every message dispositioned and archived, every resulting plan row reconciled

## Inbox drain — 2026-09-08, 8 messages, every one dispositioned (no landing)

> ↪ Relocated to `settled.md` § "Inbox drain — 2026-09-08, 8 messages, every one dispositioned (no landing)" — settled: the drain/sweep is complete — every message dispositioned and archived, every resulting plan row reconciled

## Inbox drain — 2026-09-11, 40 messages, every one dispositioned (cross-repo lessons, no landing)

> ↪ Relocated to `settled.md` § "Inbox drain — 2026-09-11, 40 messages, every one dispositioned (cross-repo lessons, no landing)" — settled: the drain/sweep is complete — every message dispositioned and archived, every resulting plan row reconciled
...
> ↪ Relocated to `settled.md` § "Inbox drain — 2026-09-17, 14 messages, every one dispositioned (PLAN-TRUTH-166 full ship)" — settled: the drain/sweep is complete — every message dispositioned and archived, every resulting plan row reconciled

## Inbox drain — 2026-09-15 (c), 20 messages, every one dispositioned (no landing)

> ↪ Relocated to `settled.md` § "Inbox drain — 2026-09-15 (c), 20 messages, every one dispositioned (no landing)" — settled: the drain/sweep is complete — every message dispositioned and archived, every resulting plan row reconciled

## Inbox drain — 2026-09-15 (b), 1 message, dispositioned (no landing)

> ↪ Relocated to `settled.md` § "Inbox drain — 2026-09-15 (b), 1 message, dispositioned (no landing)" — settled: the drain/sweep is complete — every message dispositioned and archived, every resulting plan row reconciled

## Inbox drain — 2026-09-15, 99 messages, every one dispositioned (PLAN-TRUTH-148 + PLAN-TRUTH-157 double full ship)

> ↪ Relocated to `settled.md` § "Inbox drain — 2026-09-15, 99 messages, every one dispositioned (PLAN-TRUTH-148 + PLAN-TRUTH-157 double full ship)" — settled: the drain/sweep is complete — every message dispositioned and archived, every resulting plan row reconciled

## Inbox drain — 2026-09-14, 5 messages, every one dispositioned (no landing)

> ↪ Relocated to `settled.md` § "Inbox drain — 2026-09-14, 5 messages, every one dispositioned (no landing)" — settled: the drain/sweep is complete — every message dispositioned and archived, every resulting plan row reconciled

## Inbox drain — 2026-09-13 (b), 5 messages, every one dispositioned (PLAN-TRUTH-127 full ship)

> ↪ Relocated to `settled.md` § "Inbox drain — 2026-09-13 (b), 5 messages, every one dispositioned (PLAN-TRUTH-127 full ship)" — settled: the drain/sweep is complete — every message dispositioned and archived, every resulting plan row reconciled

## Inbox drain — 2026-09-13, 7 messages, every one dispositioned (PLAN-TRUTH-139 full ship)

> ↪ Relocated to `settled.md` § "Inbox drain — 2026-09-13, 7 messages, every one dispositioned (PLAN-TRUTH-139 full ship)" — settled: the drain/sweep is complete — every message dispositioned and archived, every resulting plan row reconciled

## Inbox drain — 2026-09-21, 20 messages, every one dispositioned (ledger relocation to `.plan/orchestrator/`)

> ↪ Relocated to `settled.md` § "Inbox drain — 2026-09-21, 20 messages, every one dispositioned (ledger relocation to `.plan/orchestrator/`)" — settled: the drain/sweep is complete — every message dispositioned and archived, every resulting plan row reconciled

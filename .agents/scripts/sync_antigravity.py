...
"""Deploy engine for the project-level ``sync-antigravity`` command.

Pipeline:

    marketplace/bundles/  →  target/antigravity/  →  ~/.gemini/config/plugins/plan-marshall/

The script consumes the multi-target generator output at
``target/antigravity/`` (or a supplied ``--source`` path) and copies each
emitted component into the Antigravity plugin directory.

Components deployed:

    skills/{bundle}-{skill}/  →  {dest}/skills/{bundle}-{skill}/
    agents/{name}.md          →  {dest}/agents/{name}.md
    commands/{bundle}-{skill}.md  →  {dest}/commands/{bundle}-{skill}.md
    plugin.json               →  {dest}/plugin.json
    install.sh                →  {dest}/install.sh
    README.adoc               →  {dest}/README.adoc

Deletion boundary
-----------------

The destination (default ``~/.gemini/config/plugins/plan-marshall/``) holds
the Antigravity plugin assets. The script removes only **managed entries** —
those whose names match the ``{bundle}-{skill}`` namespace of the bundles
being synced:
...
Outputs a TOON document on stdout:
    ...
    deployed[N]{kind,name}:
      skills/plan-marshall-sync-antigravity
      commands/plan-marshall-sync-antigravity.md
    removed[N]{kind,name}:
...
Flags:
    ...
    --source PATH       Override the source root (default: {cwd}/target/antigravity/).
    --target-dir PATH   Override the destination (default: ~/.gemini/config/plugins/plan-marshall/).
...
# Root assets deployed from source if present.
ROOT_ASSETS = (
    ('plugin.json', 'config'),
    ('install.sh', 'asset'),
    ('README.adoc', 'asset'),
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        '--source', type=Path, default=None, metavar='PATH', help='Source root (default: {cwd}/target/antigravity/).'
    )
    parser.add_argument(
        '--target-dir',
        type=Path,
        default=None,
        metavar='PATH',
        help='Destination directory (default: ~/.gemini/config/plugins/plan-marshall/).',
    )
    parser.add_argument(
...
def _resolve_source(args: argparse.Namespace) -> Path:
    ...
    if source is not None:
        return source
    return Path.cwd() / 'target' / 'antigravity'
...
def _resolve_dest(args: argparse.Namespace) -> Path:
    ...
    if target_dir is not None:
        return target_dir
    return Path.home() / '.gemini' / 'config' / 'plugins' / 'plan-marshall'
...
def _is_managed_skill(name: str, synced_bundles: set[str]) -> bool:
    ...
    synced bundle or equals ``{bundle}``. Prefix matching (not first-hyphen
    token matching) handles bundle names that themselves contain hyphens
    (e.g. ``plan-marshall`` → ``plan-marshall-sync-antigravity``).
    """
    for bundle in synced_bundles:
        if name == bundle or name.startswith(f'{bundle}-'):
            return True
    return False
...
def _enumerate_source_skills(source: Path, only_bundle: str | None) -> list[Path]:
    """Return sorted skill directories under ``source/skills/``."""
    ...
    if only_bundle is not None:
        skills = [s for s in skills if s.name == only_bundle or s.name.startswith(f'{only_bundle}-')]
    return skills


def _enumerate_source_agents(source: Path) -> list[Path]:
    """Return sorted agent files under ``source/agents/``."""
    agent_dir = source / 'agents'
...
def _enumerate_source_commands(source: Path, only_bundle: str | None) -> list[Path]:
    """Return sorted command files under ``source/commands/``."""
    command_dir = source / 'commands'
...
def _prune_managed(
    ...
    *,
    ...
    does not exist in the source. Agents and unmanaged user entries
    are never pruned.
...
def main(argv: list[str] | None = None) -> int:
    ...
    has_root_assets = any((source / asset_file).is_file() for asset_file, _ in ROOT_ASSETS)
    ...
    # Deploy root assets (plugin.json, install.sh, README.adoc) if present
    for asset_file, kind in ROOT_ASSETS:
        ...
        if asset_src.is_file():
            ...
            if not dry_run:
                ...
                if asset_file == 'install.sh':
                    target.chmod(0o755)
            deployed.append({'kind': kind, 'name': asset_file})

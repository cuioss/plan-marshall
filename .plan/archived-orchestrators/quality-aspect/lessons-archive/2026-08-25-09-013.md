...
### 2026-09-05-14-002 — Bot acknowledgment notices are filed as findings and block the gate until triaged, though comment-patterns.json exists to drop them

**Component**: `plan-marshall:automatic-review` · **Category**: improvement

## The gap

`comment-patterns.json` exists to drop provider acknowledgment noise before it becomes a finding. Two CodeRabbit status notices reached the findings store anyway on PR #1409 and had to be triaged before the gate would clear:

- *"GitHub couldn't provide a complete incremental comparison…"* (the full-review fallback notice)
- *"Already reviewed the last commit…"* (the reply to a re-review trigger)

Neither carries a finding. Both are the bot reporting on its own mechanics.

## Why it costs more than it looks

An untriaged finding contributes to `pending_findings_blocking_count`, which gates the merge barrier. So an acknowledgment notice does not merely add noise to a report — it **holds the merge open** until someone dispositions it, and the disposition is always the same: `accepted`, no detail, nothing transmitted.

The cost also compounds per re-review. Each loop-back iteration that re-triggers a bot produces another acknowledgment, so a plan with several review cycles pays this repeatedly.

## Remedy

Add both phrasings to CodeRabbit's `ignore_patterns` (or to the shared pre-filter, if these are provider-generic rather than bot-specific — the "already reviewed" reply is a trigger-response shape other bots may share).

Detect on the distinctive cause phrase rather than on a heading. That is the same discipline the registry already applies to the `Too many files!` skip notice, which is matched on its cause line specifically because `Review skipped` heads several unrelated skips and would over-match.

## Scoping note

Deliberately not fixed in the plan that observed it. `unreviewed-merge-gate-holes` had already widened twice and lost half its declared scope to an upstream landing, and adding a third unrelated fix would have made the PR harder to review than the noise is to triage.

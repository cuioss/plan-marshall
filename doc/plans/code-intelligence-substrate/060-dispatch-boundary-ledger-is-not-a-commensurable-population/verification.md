...
**Tree state:** `a90adeb` on `claude/code-intelligence-substrate-analysis-kah884` (the audit was first
written at `61a43e5`; the intervening commits touch only `doc/plans/` documents). Every finding was
...
| D4 | Equal figures annotated as agreement; a test pins smaller/equal/larger | `_reconciliation_relation_clause` renders the true relation | Implemented at `:661-683`. But the `equal` branch is unreachable for a same-population row (the tie resolves to `total_tokens`, which suppresses the annotation entirely), and the `equal` and `smaller` relations are reachable **only** on `inline`-population rows, where both compare across populations. (`larger` is also reachable same-population and is sound there) | PARTIAL |
...
- **Checks run:**
  ...
  - Independent registration sweep: the **phase workflow docs** hold **four** executable
    ...
    boundary row.
    Two further executable blocks exist in `manage-metrics/SKILL.md` (`:421`, `:768`); they are the
    command's own canonical-invocation documentation, not dispatch sites, and are excluded from the
    count for that reason. Every remaining match in the marketplace is prose.
    Positive control: the same pattern returns those hits, so a zero result elsewhere is a real
    absence, not a filtered search.
...
- **Checks run:**
  ...
  - **Second defect probe, the realistic shape.** A `5-execute` row carrying exactly the synthesized
    clean-exit row and nothing else — `dispatch_boundary_rows_recorded: 1`, `subagent_samples: 0`
    (the value `enrich` writes when its walk sampled nothing, `:3513`) — classifies `over` and
    renders **no bullet, no `FAILURE`, and no `(boundary sum, over-covering)` marker**: the
    `_unclosed_boundary_floor` fold at `:618-658` is gated on the same falsy sum, so both surfaces
    that could have carried the signal fall silent together. This shape needs no unusual input at
    all — one drained queue plus one window `enrich` sampled nothing in produces it.
...
- **Checks run:**
  ...
  - **The cross-population problem is not confined to the `equal` branch — but it is confined to
    two of the three.** Reachability, re-derived branch by branch: a `<` relation requires the
    winner to be smaller than `total_tokens`, which an eligible `total_tokens` would have beaten, so
    `<` — like `=` — is reachable only when `total_tokens` was ruled ineligible at `:593`, i.e. only
    on an `inline` row. `>` is different: it renders whenever a dispatched measure exceeds an
    **eligible** `total_tokens`, which is a same-population comparison and is sound. Probes against
    a pristine `HEAD` copy:
    - `total_tokens: 5000` stamped `inline` with `subagent_total_tokens: 3000` →
      ...
      dispatched population on a row the render simultaneously declares inline.
    - **Counter-probe** (the correction an earlier pass missed): `total_tokens: 439628` with **no**
      population stamp and `subagent_total_tokens: 577452` → `4-plan → subagent_total_tokens 577,452
      (> total_tokens 439,628)`. `_token_population` defaults an absent stamp to `dispatched`
      (`:433-453`), so this row reaches the clause with both figures in one population. A second
      counter-probe with an exact-coverage `dispatch_boundary_total: 150000` against
      `total_tokens: 100000` renders the same way. So the claim that only `inline` rows reach the
      clause is **false**; the defect is scoped to `=` and `<`.
    The `<` branch is the shipped test's own scenario (`:262-278`) and the `=` branch is the other
    (`:237-260`), so both regression arms sit on the unsound row shape; only the characterization
    arm (`:280-299`) exercises a same-population row.
  - Reachable on real data, not only in fixtures: `enrich` writes `subagent_total_tokens` /
    `subagent_samples` first (`:3509-3513`), then stamps `total_tokens_population = inline`
    (`:3578-3586`) on a row whose `total_tokens` is falsy at that moment **and** whose inline
    main-context sum is non-zero — the same branch that sets `total_tokens` to that inline sum. A
    dispatching phase that never closed with `--total-tokens` and did accrue main-context spend
    therefore carries dispatched measures *and* an `inline` stamp. This branch was verified by
    reading, not by running `enrich` over a transcript — see § Adversarial review, residual doubt.
...
- **Verdict:** PARTIAL — the wording defect the plan named is fixed and the three-way test exists.
  The agreement signal is unreachable in the same-population case, and the two relations that carry
  the deliverable's regression arms (`=` and `<`) are emitted only as cross-population comparisons.
  The `>` relation is sound. See G2, G3.
...
- **Checks run:**
  ...
  - **Fail-first independently reproduced.** Extracted `3f64b71^`'s `manage-metrics.py` to
    ...
    - D5(a), verbatim: `- **Dispatch-boundary total**: 900,000 (dispatched-subagent population; 8 of
      3 dispatch(es) recorded — complete; won the reconciliation maximum)` — the plan's OBSERVED-but-
...
2. **`manage-metrics.py:615` + `:1798` + `:1793` — the agreement identity is unreachable where it
   matters, and the `=` and `<` relations are emitted only cross-population.** `max()` over
   ...
   annotation. `=` and `<` are consequently reachable only when `total_tokens` was ruled ineligible
   as cross-population (`:593`) — and `beaten` is that same cross-population figure (`:1793`,
   `:1800-1801`), so both compare a dispatched measure against a main-context one, under a preamble
   that calls both "competing measures of the dispatched population". The `equal` case is the
   loudest instance ("measures agree"); the `<` case is the more common one. The `>` relation is
   **not** affected: it renders whenever a dispatched measure beats an *eligible* `total_tokens`,
   which is a same-population comparison (probe: an unstamped row `total_tokens: 439628` /
   `subagent_total_tokens: 577452` renders `(> total_tokens 439,628)`).
...
- Both D4 **regression** arms (`test_equal_boundary_and_total_annotated_as_agreement` `:237-260`,
  `test_smaller_dispatched_winner_annotated_as_below_total` `:262-278`) stamp
  `total_tokens_population = inline`, so the suite pins the cross-population comparison rather than
  catching it (G3). Only the characterization arm (`:280-299`) runs on a same-population row.
...
- **Read:** the epic README, `plan.md`, `report-01.md`; `manage-metrics.py` (the whole
  ...
  `ref-workflow-architecture/standards/call-graph.md`; the four `record-dispatch-boundary` call
  blocks in the workflow docs; the D5 test file in full; the neighbouring metrics test files for
  guard coverage.
...
- **Counts re-derived at audit time, and again in the adversarial pass:** 8 tests in the D5 file
  (299 lines); 456 tests in the metrics suite (re-run in the adversarial pass: `456 passed`);
  **4** executable `record-dispatch-boundary` invocation blocks **in the phase workflow docs**
  (plus 2 in `manage-metrics/SKILL.md` that document the command itself) resolving to
  **3** registering phases; 6 names in
...
- **Concurrency hazard, disclosed.** Sibling audit agents were mutating the same working tree during
  ...
  The adversarial pass hit the same hazard independently — `git status --porcelain` showed
  `manage-metrics.py` modified by another agent mid-pass (md5 `6b1ffcd4…` against `HEAD`'s
  `b9c88ef9…`). Consequence for method: the adversarial pass **never wrote to the repository file**.
  Its mutation sweep ran entirely against `git show HEAD:` copies, and every probe was re-taken
  against a pristine copy after the foreign edit was observed. The repository file was verified
  `b9c88ef9…` and `git status`-clean at the end.
...
|---|---|---|---|
| A1 | False positives | Every `path:line` in both documents re-opened against the tree; all nine gaps are real and G1/G2/G3 were re-reproduced by execution against a pristine `git show HEAD:` copy. **One false claim:** the audit asserted, in four places (D4 table row, D4 checks, D4 verdict, Correctness review #2) plus `gaps.md` G3, that an `inline` row is the only row reaching `_reconciliation_relation_clause`, hence that *every* emitted relation is cross-population. Counter-probe refutes it — an **unstamped** row (`total_tokens: 439628`, `subagent_total_tokens: 577452`) renders `(> total_tokens 439,628)` with both figures in the dispatched population, because `_token_population` defaults an absent stamp to `dispatched` (`:433-453`); a second counter-probe with an exact-coverage `dispatch_boundary_total` does the same. Two stale-citation defects: the header's tree state named `61a43e5` (HEAD is now `a90adeb`, code blob unchanged), and "the marketplace holds **four** executable `record-dispatch-boundary` invocation blocks" is unscoped — six exist, two of them the command's own documentation in `manage-metrics/SKILL.md:421` / `:768` | Cross-population defect re-scoped to `=` and `<` throughout both documents, with the reachability argument spelled out branch by branch and the counter-probes recorded. G3's severity note rewritten to say what the bounding does and does not change. Header tree state updated; the invocation-block count scoped to the phase workflow docs in § D1 and § Method |
| A2 | False negatives | Each *Done when* re-derived from `plan.md` and satisfied against the shipped code, not against this audit's reasoning. D1's `9` and `3` independently re-enumerated (`call-graph.md:66-72` five envelopes, `:343-382` four shared workflows, four call blocks resolving to three phases). `_boundary_coverage_state`'s `isinstance` guards checked against `read_metrics_raw`'s int coercion (`:1014-1021`) — they fire on real data, no fail-open. The `<` branch confirmed genuinely same-population-unreachable by the max argument (a winner below an eligible `total_tokens` would have lost to it). No CONFIRMED verdict was found unearned. **One strengthening:** G1 has a *minimal realistic* instance the audit did not name — a `5-execute` phase with one synthesized clean-exit row and `subagent_samples: 0` — where the coverage bullet **and** the `_unclosed_boundary_floor` over-covering marker (`:618-658`) fall silent together, both gated on the same falsy sum | The realistic shape added as a second defect probe in § D2 and as evidence in G1 |
| A3 | Vacuous evidence | The audit's mutation sweep was **re-run, not trusted**. Three mutations applied to `git show HEAD:` copies — drop the `over` branch of `_boundary_coverage_state`, drop the `equal` branch of `_reconciliation_relation_clause`, force `boundary_surface_present` false — each turned exactly its own D5 assertion RED and left the other two GREEN (M1→D5(a) only; M2→D5(c)-equal only; M3→D5(b) only). Pre-fix reconstruction from `3f64b71^` turned all three RED while the characterization arm stayed GREEN, reproducing the report's "7 failed / 1 passed" independently; the pre-fix render is `- **Dispatch-boundary total**: 900,000 (dispatched-subagent population; 8 of 3 dispatch(es) recorded — complete; won the reconciliation maximum)`. Nothing in the audit rested on re-reading. ⚠ The repository file was **never written**: a concurrent agent held `manage-metrics.py` mutated during this pass, so the sweep ran against copies and the single-test-file pytest arm was replaced by direct assertion probes over the same three D5 scenarios | No correction needed to the audit's non-vacuity claims. The method constraint and the concurrency incident recorded in § Method |
| A4 | Counts and quotes | Re-derived at check time: **8** test functions and **299** lines in the D5 file; **456 passed** in `test/plan-marshall/manage-metrics/`; **6** names in `DISPATCH_BOUNDARY_EXCLUDED_CLASSES`; 5 envelopes + 4 shared = **9** classes against **10** `execution-context` dispatch sites once the change-type fallback is counted. Anchors re-confirmed on the pristine blob after the foreign edit: `:515`, `:525`, `:565`, `:593`, `:600`, `:606`, `:618`, `:661`, `:1439`, `:1445`, `:1793`, `:1798`, `:1941`, `:2014`, `:2106`, `:2112`, `:2183`, `:2201`, `:2210`, `:3157`, `:3513`. Quotations checked verbatim against `data-format.md:333-336`, `SKILL.md:446`, `execution.md:219`/`:254`, `planning-outline.md:468`, `cloud-plan-lane/SKILL.md:1344-1345`; commits `3f64b71`, `85abeeb (#1293)`, `3a5e2ca (#1177)` all exist as described. **Two defects:** the D5(a) pre-fix quotation carried `**…**` emphasis inserted inside a code span, so it was not verbatim; and `test_manage_metrics.py:3871-3876` names the contract test's docstring rather than the test (`:3870-3890`) | Emphasis removed and the quote marked verbatim; the contract-test citation corrected and named in both documents |
| A5 | Actionability | Each of the nine entries carries a concrete path, a concrete change, and an observable *Done when*; none is a "review X" / "consider Y". **Two fell short:** G1's action ended with "Consider also persisting `dispatch_boundary_total` unconditionally", and G8's *Done when* admitted only two of the three dispositions its own action offered (delete / add a caller / correct the docstring) | G1's suggestion rewritten as a definite instruction with its line anchor and the `data-format.md` precedent; G8's *Done when* widened to cover all three dispositions and made checkable by a symbol search |
| A6 | Severity and topic | All nine re-checked against the calibration and the topic list. G3's **high** stands — the `<` branch alone is a shipped misreport and is the more common path — but its stated justification ("scope is wider than the `equal` branch … `>` and `<` as well") was partly false and is now bounded. G9 was **low** for an unimplemented clause of D3's literal *Done when* ("an explicit exclusion list **that the coverage figure references**"), which the calibration classes as "an incomplete deliverable" | G9 raised low → **medium** with the reasoning recorded in the entry. G2 (medium) and G6 (low), both re-severitied by the interrupted pass, independently re-checked and left as they stand. All nine topics verified against the owning surface; none changed |
| A7 | Coverage | `plan.md` declares D1–D5; each has a table row **and** a detail section. All four § Out of scope exclusions are checked against the landed diff (`3f64b71` touches only `manage-metrics.py`, `standards/data-format.md`, the new test, and the two plan documents — independently re-listed). § Report accuracy covers the report's substantive claims; § Declared residue covers all three items the report declared. `plan.md` § Verification's four requirements (D1 by derivation, D5 fail-first, the D3 negative control, `./pw verify`) are each addressed. No deliverable is silently unmentioned | None needed |
| A8 | Internal consistency | The interrupted pass left a **dangling forward-reference**: the header said "see § Adversarial review" and no such section existed. Every gap traces to a finding here and every actionable finding has a gap (G1←CR#1, G2/G3←CR#2 + D4, G4/G6/G9←D3, G5←D5, G7←Report accuracy, G8←CR#3). `gaps.md`'s opening said "its four behavioural deliverables are present, but three of them are incomplete" while the file also files G5 against D5 — an undercount of its own contents. Overall verdict `CONFIRMED WITH GAPS` over 2 CONFIRMED / 3 PARTIAL matches this epic's convention (34 of 36 sibling audits use it) | This section supplied. `gaps.md`'s opening rewritten to match what it actually files, and to carry the A1 bounding of the D4 defect |

**Residual doubt:** the likeliest further finding is a reachability constraint inside `enrich` that this
pass verified by reading rather than by running. G3's "reachable on real data" argument turns on a row
acquiring the `inline` stamp while already carrying `subagent_total_tokens`; the stamp site
(`:3578-3586`) requires `inline_main_context` truthy **and** `total_tokens` falsy at that moment, which
was confirmed by reading the branch but not by executing `enrich` over a real transcript. If that
combination proves unreachable in practice, G3 collapses to a fixture-only defect and its severity
falls to medium. Everything else in both documents was reproduced by execution.

**Verdict on the audit:** SOUND AFTER CORRECTION — every gap it files is real and reproducible, but
its D4 finding over-claimed by asserting that *all three* relations are emitted cross-population when
the sound `>` branch is reachable on an ordinary same-population row, and that over-claim had
propagated into four passages and one gap entry before being bounded here.

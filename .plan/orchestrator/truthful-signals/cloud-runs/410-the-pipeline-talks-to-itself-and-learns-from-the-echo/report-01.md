...
|---|---|---|
| `coderabbitai` | `reviewed` (fixes confirmed) | Full review at the fix head 380d514: walkthrough issue-comment + 3 actionable inline review-thread comments (bot_kind registry validation, population count, D0 provenance framing). All 3 fixed, replied on-thread, resolved, and **CodeRabbit explicitly confirmed each fix resolved** ("addresses the finding" / "confirmed" / "🐇 ✅"). Its *incremental* re-review of the docs-only final commit 06a52b5 then hit CodeRabbit's **own** OSS review-rate limit (resets ~43 min) — but that commit changed only the run report; the substantive code (audit.py, tests, contract doc) was fully reviewed and cleared at 380d514. |
...
**Coverage: 2 of 3 reviewed** (`coderabbitai` — code reviewed and all findings confirmed-resolved; `cuioss-review-bot` — no concerns); 1 `rate-limited` (`sourcery-ai`, weekly quota). CodeRabbit's stale walkthrough banner still reads "Merge Risk: 🟡 Moderate · up to `a5d7c`" — it is computed *up to the pre-fix head* and names exactly the two issues since fixed (the corpus-clean claim → D0 reframe, and the unknown-reviewer-identity path → registry validation), both confirmed resolved on their threads; the banner is a rendering lag, superseded by the per-thread confirmations. The § Step 8 shortfall disclosure fires (see Merge gate) — merge proceeds on 2-of-3 with the shortfall stated in words, per the disclose-not-block rule.
...
- **Condition 2 — every PR comment handled.** The 3 CodeRabbit inline findings were fixed, replied on-thread, resolved, and **CodeRabbit confirmed each resolved**. The 2 issue-comments (CodeRabbit walkthrough, pr-agent review-guide) are informational. The fix-head re-review was awaited and confirmed before arming, so no comment was left unhandled.
...
- **Condition 4 — review-coverage shortfall DISCLOSED (disclose, not block).** **Review coverage: 2 of 3** — `coderabbitai` reviewed the code (3 findings, all fixed AND confirmed-resolved by CodeRabbit); `cuioss-review-bot` reviewed (no concerns); `sourcery-ai` **rate-limited** (weekly 500 000-diff-character quota, window reopens on its own). Additionally, CodeRabbit's *incremental* re-review of the docs-only final commit was itself rate-limited (its own OSS limit) — immaterial, since that commit changed only the report and the code was fully reviewed at 380d514. The stale "Moderate" merge-risk banner (computed up to the pre-fix head `a5d7c`) names exactly the two now-fixed issues and is superseded by the per-thread confirmations. Per the disclose-not-block rule, the merge proceeds with the shortfall stated in words; the gate is not held for the rate-limited reviewers.

Auto-merge enabled (2026-08-14T19:02Z). GitHub reported the armed method as `MERGE` — the repository default — although `SQUASH` was requested; the repository's own merge configuration governs the final method, so the reported value is recorded as-is rather than asserted as SQUASH. The PR lands automatically once the required `verify` on the final head goes green. Landing confirmation is reported to the operator (the merge SHA does not exist until it lands) — not embedded in this pre-merge report.

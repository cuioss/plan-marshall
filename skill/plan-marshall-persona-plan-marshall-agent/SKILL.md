...
```text
Call the `read` tool with `{ filePath: "standards/agent-behavior-rules.md" }` before continuing.
```
...
```text
Call the `read` tool with `{ filePath: "standards/user-communication.md" }` before continuing.
```
...
```text
Call the `read` tool with `{ filePath: "standards/tool-usage-patterns.md" }` before continuing.
```
...
```text
Call the `read` tool with `{ filePath: "standards/argument-naming.md" }` before continuing.
```
...
```text
Call the `read` tool with `{ filePath: "standards/thoroughness.md" }` before continuing.
```
...
| coverage-gathering-contract.md | The reusable coverage-gathering contract: canonical `question` gather shape, the static cell→instruction expansion table, the gather→expand→consume implementor obligation, status.json persistence (identifier + expanded instruction), Current-Implementations table |

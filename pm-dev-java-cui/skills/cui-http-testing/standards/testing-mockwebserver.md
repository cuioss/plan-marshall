...
- **Atomic shared state.** Every mutable field on a dispatcher/fake/spy that is accessed from more than one thread MUST use a true atomic read-modify-write for any compound or in-place update: an `AtomicInteger`/`AtomicLong` for counters, an `AtomicReference` mutated via `compareAndSet`/`getAndUpdate`/`updateAndGet` to replace the reference outright, or explicit synchronization guarding the whole read-modify-write. Bare `volatile` is NOT sufficient for these: it provides visibility only, not atomicity, so a `volatile callCount++` can still lose increments under concurrent access; likewise an `AtomicReference` only makes *replacing* the reference atomic — it does not make in-place mutation of the referenced object thread-safe. Reserve `volatile` for simple publication/flag visibility (a one-shot "ready" flag written once and read elsewhere), never for counters or compound updates. A non-atomic `callCount++` can lose increments, so a concurrency assertion reads its expected value and PASSES while the regression it guards is live — the test is green precisely when it should fail. The Retry Logic Testing example above uses `AtomicInteger` for exactly this reason.
...
A new module's integration tests that import another module's classes need a DIRECT dependency with the appropriate scope (e.g. `test`) on exactly what they import. A full-reactor build masks a missing declaration — the reactor classpath supplies the class anyway — and the isolated CI integration-test job then fails to compile. Reproduce the CI condition locally with `./mvnw test-compile -pl {it-module}` (or `mvn test-compile -pl {it-module}`) WITHOUT `-am`: the isolated invocation compiles only the IT module against its declared dependencies, surfacing the drift before CI does. These integration tests are standard `src/test/java` sources, so `test-compile` is the correct phase to exercise the drift.

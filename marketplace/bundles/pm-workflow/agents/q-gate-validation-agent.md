...
- Use `manage-logging read-findings` for reading findings, NOT grep
...
```bash
python3 .plan/execute-script.py plan-marshall:manage-logging:manage-log \
  decision {plan_id} INFO "(pm-workflow:q-gate-validation-agent) {file_path}: {CONFIRMED|FILTERED} ({original_confidence}% → {validated_confidence}%)

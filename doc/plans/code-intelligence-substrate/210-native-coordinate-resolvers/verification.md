...
Reviewed at `a90adeb` (this document was written at `2a9aba4`). Everything below was re-derived by
the reviewer; nothing was accepted on the audit's word. Mutation snapshots were taken to
`/tmp/adv-210-native-coordinate-resolvers-mutsweep/` and written back by hand — no `git checkout`,
`restore` or `stash` — and `git status --porcelain` confirmed every touched file unmodified
afterwards.

| # | Attack | What was found | Correction applied |
|---|---|---|---|
| A1 | False positives | **No false-positive gap.** All nine original gaps reproduce. G1 driven end to end over a fresh synthetic Poetry monorepo → `modules ['default','pkg_app','pkg_core']`, all three `metadata={} deps=[]`, `edges: ([], [])` — matching the recorded block exactly. G2/G3 reproduced verbatim, including the exact mangled strings `'m-core @ file:///./core:runtime'` and `'m-core; python_version:runtime'`, with a live control (`m-core>=1.0` on the same four modules yields `[('default','core')]`) proving the apparatus can come back different. G4, G5, G6, G7, G8, G9 each confirmed at their cited site. | Three stale citations corrected: `run_config.py:825`→`:826`; `_name_edge_join.py:132-134`→`:131-133` and `:112-117`→`:112-116`; `_cmd_client_query.py:1000-1003`→`:1001-1002`. All other `path:line` citations across both documents — 40-odd — verified exact, including every `ext-point-derivation-resolver.md`, `dependency-intelligence.adoc`, `build-npm/SKILL.md`, `build-maven/SKILL.md` and `_gradle_cmd_discover.py` reference. |
| A2 | False negatives | **Two misses in the Correctness review, both reproduced.** (1) A **setuptools** project: `_find_descriptor_file` admits `setup.cfg`/`setup.py` and stamps `build_systems: ['python']`, putting the module inside the resolver's scope with `paths.descriptor` pointing at a file `_parse_pyproject_metadata` never opens → `metadata={} deps=[] edges: ([], [])`. (2) The bare `except Exception` at `_pyproject_cmd_discover.py:279-280`: a malformed descriptor is swallowed with no note and no log, silently killing the *dependent's* declared edge. Separately, D4's guard-against-vacuity was re-checked directly and holds: the impact assertions are exact non-empty lists, so "edges present, impact empty" fails them. | Added **C5** and **C6** to § Correctness review with their probe output, plus a ⚠ naming the coverage hole they came from. Filed as **G10** (high) and **G11** (medium). |
| A3 | Vacuous evidence | **The mutation sweep re-runs identically.** All ten mutations re-applied one at a time against a fresh baseline of `54 passed`: M1 6 failed, M2 3, M3 5, M4 2, M5 2, M6 3, **M7 SURVIVED (54 passed)**, M8 13, **M9 SURVIVED**, M10 24 — every figure matching this document. G5's *Done when* was then validated by construction rather than argued: adding `"unnamed": "^1.0.0"` to the fixture root turns the M7 mutant red (`2 failed`) while shipped code stays green (`34 passed`, shared e2e impact list included). Test counts re-collected per module: 13/16/18/7/5 = 59, `59 passed`. | M9's justification was a false universal ("every emitted string has exactly one `:`") — C2 in this same document emits three. Scoped to the fixtures with the contradiction named. G5's *Risk if fixed* replaced by the measured result. |
| A4 | Counts and quotes | Every headline number reproduces exactly: D0 rows 1/0/0, **8/11/0/11** and **31/83/0/83** (the last two from my own fresh `git clone --depth 1`), `.py` diff count **13** with single parent, 59 tests, 13/16/18/7/5, `7 impls` in the SVG, 8992 bytes. **Two quote/number defects.** (a) G1 presented `"a real, positive result … your modules genuinely declare no dependencies on each other"` as one quotation from `client-api.md:110`; the second half is from `doc/user/dependency-intelligence.adoc:120` — an ellipsis stitching two documents under one citation. (b) The full-crawl re-measurement in § Report accuracy no longer reproduces: I measure 13 modules, **32** edges over **13** nodes, markdown 32 / python 5 / documentation **5**, not 25/11 and documentation 0. | (a) Split into two correctly-attributed quotes and backed with the live resolver row `{'id':'pyproject','status':'ok','edge_count':0}`. (b) Rewritten to record both readings and to state that only the invariant (0 coordinate-bearing modules, 0 `pyproject` edges) is quotable forward. Also: G1's "still the majority of the Poetry installed base" was an unmeasured population claim stated as fact — softened to what is known, with the absence of a measurement stated. |
| A5 | Actionability | **G2's prescribed fix would not work.** Its Action justified `dep.split(' @ ')[0]` with "PEP 508 requires whitespace around the `@`". It does not: the grammar is `urlspec = AT wsp* URI_reference`, and `packaging.requirements.Requirement` parses `m-core@file:///./core`, `m-core@ file:///./core` and `m-core @ file:///./core` all to `name='m-core'`. I reproduced the whitespace-free spelling producing no edge — so the prescribed split leaves it broken while satisfying a *Done when* written only for the spaced form. **G7's Action offered a false escape hatch**: "if its Gradle subprocess dependency makes that impractical". `_extract_gradle_module` takes `gradle_data` as a **parameter** and runs no subprocess; I called it hermetically and got `{'artifact_id':'core','group_id':'com.example',…}`, and `{'group_id': None}` for a build declaring no group. | G2's Action rewritten to split on the bare `@`, with the legality argument from the `name` grammar, and its *Done when* widened to require **both** spellings. G7's Action rewritten around the demonstrated hermetic call (naming the real key, `gradle_data['group_id']`, not `group`), its escape hatch removed, its *Risk* corrected to none and effort M→S. G11's *Done when* was pointed at a test module that actually exists. |
| A6 | Severity and topic | Each of the nine original severities re-checked against the calibration and each `Topic` against its owning surface. No change: G1 high (the misreport, not the doc), G2/G3/G4/G5/G7/G8 medium, G6/G9 low; `lsp/resolvers` for the five discoverer/join gaps, `tests` for the three test gaps, `documentation-surface` for G8, `plan-lane-contract` for G9. New gaps assigned G10 high (identical misreport class to G1, and discovery names a descriptor it never reads) and G11 medium (fail-open reliability, narrower blast radius). | None to the existing nine. |
| A7 | Coverage | All six deliverables covered, plus out-of-scope, report accuracy, residue and method. Out-of-scope re-verified independently against `git show --name-only 87c71d3`: no `build-gradle/scripts/` file, no `build-maven/scripts/extension.py`, no new `ext-point-*.md`, `extension_discovery.py` and `run_config.py` absent. **One hole:** § Correctness review examined what the Python parser does with the table it reads but never what happens when the file it reads is the wrong file or unreadable. | Hole named in-place and closed by C5/C6. |
| A8 | Internal consistency | Overall verdict follows from the rows; every actionable finding traces to a gap and every gap back to a finding — re-checked entry by entry, now including G10/G11. **Two inconsistencies:** M9's justification contradicted C2 (above); and § Report accuracy called the `./pw verify` figures "correctly re-derived" while § Method lists them under "Not checked". | Both fixed: the verify figures are now described as an arithmetic consistency check with a pointer to the Method caveat, in both this document and G9. The pnpm "noted rather than raised" call was also converted from an assumption into evidence — a synthetic `pnpm-workspace.yaml` workspace drives through real discovery to 3 modules / 2 edges, so the shipped claim is true-but-untested rather than false. |

**Residual doubt:** A further round would most likely find more members of the C1/C5/C6 family, since
all three come from one function reading one table of one file and the family was not enumerated
systematically — PDM's `[tool.pdm]`, Hatch's `[tool.hatch]` metadata hooks, a `requirements.txt`-only
layout, and the `tomllib is None` arm on an interpreter without `tomli` are the untested candidates,
and each would present as the same `status: ok, edge_count: 0`. The second-likeliest finding is on the
npm side, where only `_extract_dependencies` was probed and `_build_module`'s own reach (nested
workspaces, a `packages/*/*` glob depth, a member whose `package.json` is unreadable) was not. Three
things are stated here and deliberately **not** measured by either round: the report's five commit
SHAs and all GitHub-side claims (branch deleted, API not queried); the `./pw verify` sub-step figures
(build not run); and the reverse-index injection experiment (requires mutating `_cmd_client_query.py`,
which concurrent agents hold).

**Verdict on the audit:** SOUND AFTER CORRECTION — every one of its nine gaps is real and every
headline measurement in it reproduces exactly, including the full ten-mutation sweep and two
from-scratch external clones; what it got wrong was two prescriptions that would not have worked
(G2's PEP 508 premise, G7's imagined subprocess), one quote stitched across two documents, and one
unswept corner of the Python discoverer that hid two further instances of the very defect the
document is about.

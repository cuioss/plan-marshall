...
python3 .plan/execute-script.py plan-marshall:manage-logging:manage-log \
  decision --plan-id {plan_id} --level INFO --message "(plan-marshall:workflow-outline-change-type) Context loaded: change_type={change_type}, compatibility={compatibility}"
```
...
```bash
python3 .plan/execute-script.py plan-marshall:manage-logging:manage-log \
  work --plan-id {plan_id} --level INFO --message "[SKILL] (plan-marshall:workflow-outline-change-type) Loaded domain skill: {resolved_skill}"
```
...
```bash
python3 .plan/execute-script.py plan-marshall:manage-logging:manage-log \
  decision --plan-id {plan_id} --level INFO --message "(plan-marshall:workflow-outline-change-type) Complete: {N} deliverables ({change_type})"
```
...
- `plan-marshall:manage-status:manage_status` - Read change_type metadata

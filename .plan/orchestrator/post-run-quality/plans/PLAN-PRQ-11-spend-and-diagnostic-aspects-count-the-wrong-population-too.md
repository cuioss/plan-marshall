...
- OBSERVED: `-007`, `-008` and `-023` are ONE signal, split by the filing plan itself —`-023` states it
  ...
  three.
  - verdict: corroborated | checked_at: 7d82d5d90 | by: post-run-quality/cleanup | rescoped: n/a | evidence: PRQ-06 archived inbox messages -007/-008/-023 share one sender_id/stream -- ONE signal split by the filing plan confirmed structurally via inbox validate (location=archived). Quoted body text not re-verifiable (archived bodies unreachable), carried forward from prior verdict
- OBSERVED: `architecture search --content --pattern P` accepts `--category`, `--literal`,
  ...
  - verdict: corroborated | checked_at: 7d82d5d90 | by: post-run-quality/cleanup | rescoped: n/a | evidence: architecture.py search --help declares exactly --content/--pattern/--category/--literal/--ignore-case, no path/dir/scope flag -- complete by construction from live argparse output
- ⚠ HYPOTHESIS: the per-row `script-failure-analysis` classification rule itself was NOT re-verified
  ...
  - verdict: contradicted | checked_at: 7d82d5d90 | by: post-run-quality/cleanup | rescoped: yes | evidence: script-failure-analysis.py already declares the two-axis vocabulary (docstring:47-53, classify_failure:374-388) -- staged premise is dead. Rescoped: narrowed D1's two real asks (no summed roll-up in cmd_run; unsplit script_internal_error for every non-2 exit) confirmed still open at HEAD
- ⚠ HYPOTHESIS: `script_cost_rollup` carries no wait/intent discriminator today — the filing message
  ...
  - verdict: corroborated | checked_at: 7d82d5d90 | by: post-run-quality/cleanup | rescoped: n/a | evidence: log-analysis.md:79-98 script_cost_rollup key set has no wait/intent/reason/deliberateness key; repo-wide literal sweep for wait_reason returns 0/3097; the recording layer itself (script-failure-analysis.py:95-105) also lacks an intent field
- Verify-first clause: D0's two populations are both first-party leads from one plan's retrospective, not
  ...
  - verdict: corroborated | checked_at: 7d82d5d90 | by: post-run-quality/cleanup | rescoped: n/a | evidence: both cited figure sets attributed to a single plan PLAN-PRQ-06 by the spec's own Provenance, no corpus-wide derivation exists reachable from this envelope; claim 2's re-derivation independently reinforces the exact D0-corpus-pass-catches-this-class-of-error concern

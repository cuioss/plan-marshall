...
| HEAD moved during the probe | `error` / `probe_mutated_head` | STOP. The classifier performs no writes, so a moved ref means it cannot be trusted to have classified the state it reported; it refuses to return a verdict derived from a mutated tree |

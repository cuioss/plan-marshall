...
⭐ **Recovery note (2026-09-22):** this payload is a verbatim re-file of `truthful-signals-001.md`, which
failed `epic_mismatch` — its envelope named `epic=next-level`, the epic's name before the 2026-09-21 pure
rename to `instrumentation-substrate`. Content is unchanged; only the envelope's `epic` field is corrected
by re-filing through `inbox write`. `next-level` in the body text above is the epic's own name AT THE TIME
the message was written, not a routing error, and is left as originally authored.

...
The surfacer echoes `surface_scope` (`delta` or `full`), `since_ref`, `files_in_scope`, and `scope_statement`, so the round variant that produced a verdict — and the scope any absence claim it produces must publish (see § "Absence claims state the scope they were drawn against") — is legible from the returned TOON without reconstructing it. A `--since-ref` that does not resolve is refused by the surfacer with `since_ref_unresolvable` — that is a helper non-zero exit and takes the halt path below; it is never silently widened into a full sweep.
...
Compute the variant target via the role resolver, passing the dispatch context so the seam emits the standardized `[DISPATCH]` work-log line and its paired decision-log resolution record itself — see [`../../ref-workflow-architecture/standards/dispatch-logging.md`](../../ref-workflow-architecture/standards/dispatch-logging.md) § Emission contract. `--role default` reproduces this dispatch's `role=default` label and resolves the identical target as a bare `--phase phase-6-finalize`:

```bash
python3 .plan/execute-script.py plan-marshall:manage-config:manage-config \
  effort resolve-target --phase phase-6-finalize --role default \

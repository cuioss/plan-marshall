...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/null-safety-core.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/null-safety-patterns.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "templates/package-info.java.tmpl" }` before continuing.
```

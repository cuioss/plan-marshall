...
def test_all_in_scope(plan_context):
    plan_dir = plan_context.plan_dir_for('vfs-all-in')
    _write_refs(plan_dir, ['src/a.py', 'src/b.py'])
...
def test_all_out_of_scope(plan_context):
...
def test_mixed_scope(plan_context):
...
def test_empty_error_paths(plan_context):
...
def test_blank_paths_filtered(plan_context):

...
| Error Code | Cause |
|------------|-------|
| `not_found` | The shared id-resolution seam resolved `absent` — no file exists at the resolved path (`get`, `update`, `remove`, `supersede`, `set-body`, `convert-to-plan`, and the explicit-ids mode of `cleanup-superseded`). See [Resolving a lesson id](#resolving-a-lesson-id-three-states-two-error-values) — `not_found` means absent, and only absent |
| `unresolvable` | The shared id-resolution seam resolved `unreadable` — the file EXISTS and no reader can resolve it (the read raised, or the metadata header did not parse). Reachable from `get`, `update`, `remove`, and the explicit-ids mode of `cleanup-superseded`. Carries `path` and `detail` naming the substrate. See [Resolving a lesson id](#resolving-a-lesson-id-three-states-two-error-values) |
...
The canonical argparse surface for `manage-lessons.py`. The plugin-doctor `missing-canonical-block` rule checks that this section is PRESENT,

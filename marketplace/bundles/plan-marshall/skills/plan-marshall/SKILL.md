...
1. **Plan + phase** — from the worktree cwd (`.claude/worktrees/<id>`) or the `$PLAN_ID` env variable, reading `current_phase` from the main checkout's `status.json`. Shown as `{icon} {plan_short}:{phase}`.
2. **Active slash command** — captured on `UserPromptSubmit` from the hook stdin payload's `prompt` field when it starts with `/`, stored per `session_id` at `~/.cache/plan-marshall/sessions/{session_id}/active-command`, and cleared on `Stop`/`SessionStart`. Shown as `{icon} {command}` when no plan/phase resolves.
3. **Fallback** — `{icon} claude` when neither a plan/phase nor an active command is known.

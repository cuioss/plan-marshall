...
**What this is NOT — the two axes this model is most often mis-remembered on.** Both bullets below state the mechanism positively and name the belief each forecloses. They are **additive**: no inventoried file states either mistaken model, so a reader arriving here should not go hunting for a wrong sentence to correct. That is a first-party finding rather than an assumption — a complete content sweep of the crawled inventory found no instance of either belief in any inventoried file, with nothing unreadable, nothing elided, and no result truncated, so the negative is trustworthy under the complete-coverage rule rather than merely empty. The scope word is load-bearing: the crawled inventory is strictly smaller than the repository — it excludes `.claude/**`, `.github/**`, and every `.gitignore`d path — so the sweep speaks for the inventoried files and not for those trees. See [`client-api.md`](../../manage-architecture/standards/client-api.md) § search for the coverage contract.
...
- **Pin resolution has exactly three arms, and `.in_use` is not one of them — but the pin is a DIFFERENT mechanism from the keep-set above, owned by a different skill.** The six-rule union above is the `marshall-steward` sweep's *keep* decision (`cache_retention.py`); the pin is `tools-script-executor`'s *marking-exclusion* decision, computed by `generate_executor._retention_pinned_versions`. **The pin is not the keep-set**, and their two arities are not two readings of one rule. That function returns exactly three arms — the selector-resolved newest-on-disk dir, `system.provisioned_version`, and the manifest-named version — which mirror three of the **four** *dir-naming* arms of the keep union (newest-on-disk, provisioned, manifest-named, and the executing dir). "Dir-naming" is the load-bearing label: these are the arms that name one specific directory, as opposed to the union's two *threshold* arms (newest-`N`, younger-than-`D`-days) which apply to all dirs and pin none. Do not call them the "non-count" arms — the `younger-than-D-days` arm is an age threshold rather than a count, so that label admits it and yields a third arity for the same phrase. The fourth dir-naming arm, the version dir the sweep is itself executing from, is deliberately **not** mirrored: self-deletion is the sweep's own hazard, not the marker-writer's. The `_retention_pinned_versions` docstring states the same partition and must agree with this section member-for-member. On neither mechanism does an `.in_use` marker participate in plugin-cache pin or retention resolution — an `.in_use` file is repair residue, not a contract.

The marker itself is **existence-only**: only its presence is ever consulted, never its content, because a foreign co-producer writes the same field in a different encoding. That invariant is declared at each of the **two** sanctioned existence-read sites — `marketplace_bundles._partition_version_dirs` (the selector) and the mirrored predicate in `generate_executor._CLAUDE_RESOLVER_TEMPLATE` that is emitted into the generated executor — and is not restated here. The template is a deliberate policy duplicate whose own docstring mandates mirroring, so the two sites are kept in step by hand, not by construction.
...
**`sync-defaults` materialises an explicit `lane` on every RESOLVABLE lane-less finalize step.** After the deep-merge, `manage-config sync-defaults` fills a `lane` value into every `plan.phase-6-finalize.steps` entry that carries none and whose lane it can resolve, by provenance: a **pre-existing** lane-less step is filled with its **frontmatter-class effective lane** — the value the composer would apply with no override (declared `lane.tier` ▸ class default: `core` / `derived-state` → `minimal`, `adversarial` / `prunable` → `standard`), a semantic no-op that surfaces the implicit default as an explicit value; a **freshly-merged default** step (one the config did not previously carry) is filled with `lane: off`, honoring the infra-steps-must-be-opt-in principle. A pre-existing step whose frontmatter lane resolves to no concrete lattice tier — an external `bundle:skill` step, or one whose source doc is missing or declares no `lane:` block — is left lane-less and unreported, because materialising a value the resolver cannot derive would not be a no-op. A step already carrying an explicit `lane` (`off` / `ask` / a resolved tier) is left untouched, so the pass is idempotent. Because a weakening `off` on a `core` / `derived-state` floor element is **immune** at compose time (ignored, the element stays at its class-default tier), materialising the finalize set to a fully-explicit shape can never silently drop a correctness-floor step — a hand-written `off` on such a step is inert.

**The four finalize ceremony gates ride the `lane` channel, not a run-at-all knob.** `qgate` / `self_review` / `simplify` / `security_audit` are each governed by their owning step's per-element `steps.<step>.lane` override — read from the merged plan-local-over-marshal source, so a plan-scoped declaration governs a ceremony gate exactly as a project-wide one does — and resolved by the manifest ceremony transform (`off→never`, `minimal→always`, every other value `→auto`). There is no flat `qgate` sibling and no `simplify` / `self_review` run-at-all param. The owning steps are `pre-push-quality-gate` (qgate), `default:pre-submission-self-review` (self_review), `default:finalize-step-simplify` (simplify), and `default:finalize-step-security-audit` (security_audit). The `default:pre-submission-self-review` step is config-less.
...
| — (pre-push-quality-gate activation) | derived | — | The `default:pre-push-quality-gate` finalize step's activation is **derived from `build.map`** — no dedicated config key. The manifest composer consumes the three-value `build-decision` verdict and drops the step on `not_necessary` ONLY (an absent build_map, or a resolvable footprint matching no glob); an `unknown` verdict — the footprint is unresolvable — KEEPS the step, and `build` keeps it. Its ceremony gate (`qgate`) rides `steps['pre-push-quality-gate'].lane`, not a flat run-at-all knob. |

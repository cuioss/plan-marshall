...
| The contradiction exists in **exactly one** registry document | OBSERVED — settled first-party | Re-run the sweep **at the pre-D0 baseline** (before D0's STRIP alignment deliberately adds a strip-rule to the second document): a case-insensitive search for `strip` over the second document returns zero matches there, and the third records that its bot emits no such block. ⭐ **The per-document fan-out is not owed** |

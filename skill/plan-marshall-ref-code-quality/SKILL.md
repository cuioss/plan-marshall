...
```text
Call the `read` tool with `{ filePath: "standards/code-organization.md" }` before continuing.
```
...
```text
Call the `read` tool with `{ filePath: "standards/error-handling.md" }` before continuing.
```
...
```text
Call the `read` tool with `{ filePath: "standards/documentation-principles.md" }` before continuing.
```

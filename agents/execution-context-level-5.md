...
| `plan_id` | Yes | Plan identifier. Sentinel `none` is permitted for free-standing dispatches outside any plan, but every plan-bound dispatch MUST pass the real id. Forward `--plan-id {plan_id}` to a script call **only where that script's own parser declares it, and in the position that parser requires** — this is per-script, not a universal every call obeys. Three cases the workflow bodies actually contain: a script that declares `--plan-id` as a **top-level/router flag consumed before the subcommand verb** (the `ci` router consumes it only ahead of the first verb token, so the `ci` verbs that declare no `--plan-id` of their own — the `checks` read verbs, `pr view`, `pr list`, `pr wait-for-comments` — take `--plan-id` **before** the verb, and placing it after one of those verbs is an argparse rejection. This does not extend to `ci` as a whole: `ci`'s body-consumer verbs and its `prepare-body` / `prepare-comment` verbs declare a **required** `--plan-id` **after** their own verb, where a pre-verb flag is swallowed by the router and the subparser then rejects the call for a missing required argument); a script that declares it on the subcommand **after** the verb; and a script that does **not** declare it at all (append nothing — an undeclared `--plan-id` is an `unrecognized arguments` rejection). Consult each script's canonical-invocation block for its flag set and the position it requires; never append `--plan-id` by rote. |
...
- **`ask_question` is intentionally NOT declared.** A dispatched leaf cannot reach the operator — operator input is unreachable inside a dispatched envelope at runtime. When a workflow step would prompt the operator, the leaf MUST instead return a **prompt-required envelope** (a structured block on its return TOON) for the main-context orchestrator to fire the `ask_question`. This is the sanctioned operator-input path; see [`ref-workflow-architecture/standards/agents.md` § Leaf cannot fire AskUserQuestion](../skills/ref-workflow-architecture/standards/agents.md#leaf-cannot-fire-askuserquestion--return-a-prompt-required-envelope).
- **`Grep` / `Glob` are declared, but the harness MAY deny them to a subagent.** Search intent is legitimate, so the declaration keeps them; when the runtime does not grant them, the leaf performs discovery through the structured architecture inventory first — `architecture find --pattern P` (path glob), `architecture search --content --pattern P` (file content), `architecture which-module --path P`, `architecture files --module X` (structured queries first, per `plan-marshall:manage-architecture/standards/client-api.md` § search) — and scans inside an already-known file with `Read`. Bash `grep` / `find` / git's `grep` subcommand are NOT a fallback: the project's file-operation hard rule and its enforcement hook block them unconditionally, whether or not `Grep`/`Glob` were granted. **A broad content sweep is a first-class capability, not a coverage gap**: `architecture search --content` answers "which files contain X" over the module-attributed inventory, so reach for it FIRST — see `plan-marshall:manage-architecture/standards/client-api.md` § search for the response contract and the payload / inventory-scope boundaries. Returning the coverage gap to the main-context orchestrator remains the residual path, but only for trees the inventory does not walk (`.claude/**`, `.github/**`, and anything a `.gitignore` rule excludes). Either way the leaf MUST NOT silently degrade to spot-checks and pass green with shrunken coverage. The `allowed-tools-body-drift` hook stays clean because the agent body invokes no `Grep:` / `AskUserQuestion:` directive, so the granted set and the declaration agree.

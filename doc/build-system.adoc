...
Each `Task:` dispatch carries a fixed overhead (skill-load preamble, Worktree Header echo, return-TOON marshalling). The granularity heuristics in link:../marketplace/bundles/plan-marshall/skills/dev-general-practices[`dev-general-practices`] govern when a step belongs in a dispatch envelope vs. inline in the orchestrator's context:

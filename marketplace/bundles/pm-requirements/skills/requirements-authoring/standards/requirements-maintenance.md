...
**CRITICAL RULE**: Default action is deprecation, not deletion. The process differs by project phase.

### Pre-1.0 Projects

**Rule**: Update requirements directly without deprecation process.

Pre-release projects are in active development — requirements can change freely without maintaining historical record.
...
**Rule**: Always ask the user whether to deprecate or remove functionality.
...
1. **Remove Requirement**: Delete requirement section completely, update document table of contents
2. **Update All References**: Remove from traceability matrices, update cross-references, remove from specifications
3. **Update Implementation References**: Remove code references, clean up test references
4. **Update Index/TOC**: Ensure no orphaned links remain

### Best Practices

**Do**:
- Mark as [DEPRECATED] in heading (post-1.0)
- Explain deprecation reason
- Reference replacement if exists
- Keep ID in sequence

**Don't**:
- Delete requirement completely without user approval (post-1.0)

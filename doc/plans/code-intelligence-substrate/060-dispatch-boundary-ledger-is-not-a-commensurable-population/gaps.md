...
- **Evidence:** The bullet — which carries every coverage verdict, including the `over` `FAILURE`
  ...
  declaration banner. All-zero boundary rows are reachable: `cmd_record_dispatch_boundary` defaults
  the `total_tokens` column to `0` (`:3157`) and `plan-marshall/workflow/planning-outline.md:468`
  instructs callers to "use `0` when the field is absent".
...
## G3 — Stop asserting "measures agree" across two populations

- **Kind:** bug
- **Severity:** medium
- **Topic:** measurement/metrics
- **Where:** `marketplace/bundles/plan-marshall/skills/manage-metrics/scripts/manage-metrics.py:1793`
  and `:1800-1801` (`beaten = raw_tokens`), consumed by `_reconciliation_relation_clause` at `:661-683`
- **Evidence:** `beaten` is the row's raw `total_tokens` regardless of whether that field was ruled
  *ineligible* as cross-population at `:593` (`if field == 'total_tokens' and population ==
  POPULATION_INLINE: continue`). Given G2, the `value == beaten` branch is reachable **only** on such
  a row. The shipped test proves it: `test_dispatch_boundary_ledger_population.py:237-260` sets
  `total_tokens_population = manage_metrics.POPULATION_INLINE` and asserts
  `'= total_tokens 5,000' in annotation` and `'measures agree' in annotation`. The module states at
  ...
  exists to prevent", and `standards/data-format.md:306-310` repeats it.
- **Why it matters:** the one case where the surface currently emits its "reconciliation identity"
  is a comparison between a main-context figure and a dispatched figure. A reader is told two
  independent producers agree when the module's own rule says the two are not comparable — the same
  class of cross-population mislabel this epic exists to eliminate, now dressed as its strongest
  trust signal.
- **Action:** suppress the relation clause (or render it as `(not comparable — total_tokens on this
  row measures the inline main-context population)`) whenever `total_tokens` was excluded from
  eligibility for population reasons. Update the D5(c) equal-case test to exercise a
  same-population row once G2 makes that path reachable.
- **Done when:** an inline-population row whose dispatched winner numerically equals `total_tokens`
  no longer renders "measures agree", and the equal-case regression test runs against a
  same-population row.
- **Effort:** S
- **Risk if fixed:** `test_equal_boundary_and_total_annotated_as_agreement` and
  ...
  rewriting; do not simply delete them, or D4 loses its regression coverage entirely.
...
- **Evidence:** the constant's preamble claims it is "Derived from the DISPATCHING code — the call
  ...
  pattern does return the analogous lock-step guards for `DISPATCH_TERMINATION_CAUSES`. The
  repository states this obligation explicitly at
  `marketplace/bundles/plan-marshall/skills/manage-metrics/standards/data-format.md:944`
  ("Restating surfaces (lock-step obligation)").
...
## G6 — Name the change-type-heuristic LLM fallback in the class enumeration and the exclusion list

...
Batch filing consults the drain before allocating: `manage-lessons drain-dedup --candidates-file {candidates} [--corpus-file {corpus}]` groups candidates with the corpus (strongest-wins) and only its `to_file` ids are subsequently allocated — recurrences ride counts, never duplicated bodies. The landing narrative republishes `manage-lessons created-counts --emitted-count {emitted} [--filed-id {id} ...] [--recurrences-file {recurrences}]` totals via the compile-report assembler: `created_count` names entries actually filed after dedup and housekeeping, never the emitted candidate total.

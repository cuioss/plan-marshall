...
`.plan/orchestrator/`. Everything it needs is in git; nothing in it expects an orchestrator
record to exist.
...
| Layout resolution is a runtime op with memoised helpers (`get_project_skill_roots()`, `get_bundle_cache_roots()`); the config, manifest, steward, inventory-discovery, and extension-discovery resolvers route through them — the implementor scan included. Several plugin-doctor analyzers and a handful of other resolvers keep inline anchors, registered open in the [coupling inventory](reference/coupling-inventory.md) §B | `script-shared/scripts/marketplace_paths.py` and its consumers |

...
are interpreter-dependent in a way nothing in the code, the tests, or the shipped documentation
acknowledges — the reference count, three of the four notes, and the wall-clock all move with which
Python pyright happens to resolve against, and there is no `pythonPath` / `workspace/configuration`
handling to pin it. G13 fixes the `.venv` symptom; it does not make the harvest reproducible. A

...
**Claude's settings-file resolution** is internal to that implementation, not part of this contract: `claude` resolves to `.claude/settings.local.json` in both install modes — the default terminal-title mode and `--enforcement` alike — because both payloads are machine-local operator wiring, and `.claude/settings.local.json` is the gitignored file, so absolute per-user paths never enter version control. That implementation additionally honours an absolute path ending in `.json` as a test/recovery override; it is Claude-internal, no other target need offer it, and any other value (a relative path, an unknown identifier) is rejected with `unknown_target` rather than silently creating a stray file.
...
- `reduced_bytes` — the UTF-8 byte length of the transcript **this reduction produced**. The runtime reports it and does NOT decide whether it fits a budget: read-budget policy is consumer-side, so the caller derives `over_budget` itself. The reduction is never truncated by the runtime.

  ⛔ **It is a PRODUCTION figure, not a DELIVERY measurement, and a budget decision must not be taken on it.** It describes what the runtime handed to its caller, and says nothing about what that caller in turn emits or what the eventual reader receives — the two diverge exactly when a consumer's serialization loses part of the payload, which is the case a budget decision most needs to see. A consumer that owns a read budget therefore measures the transcript IT emits and gates on that; `plan-retrospective`'s `extract-chat-signal.py` publishes `reduced_transcript_delivered_bytes` for precisely this and derives `over_budget` from it, forwarding `reduced_bytes` unchanged beside it so the gap between the two stays visible rather than being reconciled away.

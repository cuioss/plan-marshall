...
- **Content lookup across modules** — "Which FILES contain the string `foo`?" → `architecture search --content --pattern foo` (add `--literal` for a fixed string). Hits carry `module` / `category` / `path` / `match_count`; the response carries no line bodies, and `count: 0` always rides with the coverage fields so a negative is never bare. Read all four before believing a zero: a negative is trustworthy only with `files_scanned > 0` **and** `unreadable == []` **and** `truncated == false` **and** `elided == []`. `files_scanned > 0` on its own proves only that *some* files were opened — an unreadable file or an elided inventory bucket is a file that might hold the match and was never scanned, which is a coverage gap to report, not an absence to record. See [`manage-architecture/standards/client-api.md`](../../manage-architecture/standards/client-api.md) § search for the full contract.

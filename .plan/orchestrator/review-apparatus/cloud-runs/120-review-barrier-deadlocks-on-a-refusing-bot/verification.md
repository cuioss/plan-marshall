...
| Non-option pairing 2 (`refused_awaitable` + `size` → Branch 2 claim-and-await) — closed by "Recovery gains **Branch 0**" | **OVERSTATED — the fix is prose-only** | Branch 0 exists in `SKILL.md:376`, but `rate_limited_bots[]` is `{bot_kind, rate_limit_class, eta}` (`_github_pr.py:403-409`, and the authoritative contract `api-contract.md:147`) and `refusals[]` is `{source, bot_kind, layer, eta, body}` (`github_re_review.py:331-337`). Neither carries a `cause`. See G1 |
| Non-option pairing 1 (`refused_hard` + `size` → a wait offered first) — "**This is the defect D1 fixes**" | **OVERSTATED — fixed at the classifier and the barrier, NOT on the opt-in recovery path** | The classifier resolves `refused_structural` and the barrier renders split/accept/disable, both confirmed. But the leaf's Branch 0 cannot see a `cause`, so a Sourcery size refusal falls to Branch 1 (`SKILL.md:400`, guard `hard_quota` or `unknown` and cause not `size`) and returns `rate_window_not_awaitable`, whose `prompt_options[0]` is `"Wait another {review_rate_window_timeout_seconds}s"` (`SKILL.md:902`; item 7a renders it at `phase-6-finalize/SKILL.md:1345`). Sourcery declares both the size pattern and `hard_quota` (`sourcery.md:43`, `:49`), so no hypothetical registry entry is required. Gated only by `review_rate_window_await` defaulting to `false`. See G1 |
| "Both carry the same discriminators … the refusal's `cause` … plus … the stated `cap`" (`automatic-review/SKILL.md:361-364`) | **FALSE** | Neither producer record contains either field; `refused_causes[]` / `refused_size_caps[]` come from `fetch_findings`, produced in § "Producer: FIND" (`SKILL.md:589`) — **after** the recovery section (`:350`), as `:366` states |
...
| F6 fixed — `_WAIT_OFFER` widened past the literal "wait" | **ACCURATE** | `test_structural_refusal.py:437-451` |

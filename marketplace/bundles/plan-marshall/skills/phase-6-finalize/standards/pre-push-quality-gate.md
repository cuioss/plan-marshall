...
- `{worktree_path}` has been resolved at finalize entry (see SKILL.md Step 0). The immediately-following `default:push` (`order: 11`) step runs the `pre-commit-verify-freshness` gate. That gate's contract — its four status members, which two of them permit and on what basis each rests, the routes to a refusal, and the cross-check dimensions — is owned by `manage-tasks/SKILL.md` § "Pre-Commit Verify Freshness"; read it there. It is deliberately NOT restated here: a second copy of a contract is a second thing to keep true, and this bullet's own restatement had already drifted out of agreement with the gate. See `marketplace/bundles/plan-marshall/skills/manage-change-ledger/SKILL.md` for the ledger and `worktree_sha` primitive the gate reads.

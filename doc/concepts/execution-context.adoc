...
* xref:build-server.adoc[Concepts › Build Server] — when a project registers the build server, an orchestrator-tier build is submitted to the `marshalld` daemon and bounded-long-polled: the leaf/orchestrator polls a bounded wait instead of holding a long build inline, so a harness-reaped wait costs one re-poll.

...
The two levels are genuinely different, and conflating them is what makes a naive reading of this field unimplementable. Frontmatter is step-level — there is exactly one block per step doc — but a step doc has multiple terminal `mark-step-done` call sites with deliberately different honest fact sets (`branch-cleanup.md` has six terminal branches A through F — five `--outcome done` plus Branch F's `--outcome loop_back`, which is terminal for its dispatch and carries facts like the rest — alongside three further `loop_back` sites in the pre-merge barrier that record none; `sonar-roundtrip.md` has three `--outcome done` branches plus an `--outcome failed` call). The delta rule between the levels is set-valued, in both directions:

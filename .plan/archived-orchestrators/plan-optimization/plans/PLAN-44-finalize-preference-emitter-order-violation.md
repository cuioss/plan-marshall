...
**Disjointness:** finalize-step surface — disjoint from `marshall-orchestrator` (PLAN-40, shipped),
`_markers_search.py` (PLAN-23), `manage-execution-manifest.py` (PLAN-35), the CI/await seam
(PLAN-42), `manage-architecture` (PLAN-43), phase-1-init (PLAN-41). Emittable now.

## Notes

- The **symptom** (the uncommitted `enriched.json` on main) is owed a **follow-up PR per operator** —
  separate from this **root-cause** plan. Do not conflate: committing the valid hint cleans main;
  this plan stops the recurrence.

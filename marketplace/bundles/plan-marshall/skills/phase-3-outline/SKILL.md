...
Four deliverable classes carry a recurring engineering hazard that the outline MUST recognize at authoring time and route to its canonical home. These rules fire on **both** tracks — Simple Track Step 7 and Complex Track Step 10 — when a deliverable being authored matches the trigger. They are recognition triggers ONLY: the substance (mitigation menu, enumeration procedure, decision checklist, field-declaration semantics) lives once in its canonical home — a `dev-general-*` standard, or this skill's detail standard for the clean-break and survey-scope rules — and MUST NOT be restated here or in the deliverable.

...
The `execution-context` agent declaration is reconciled with this runtime reality: it does NOT list `AskUserQuestion` (the prompt-required envelope below is the sanctioned operator-input path). Its declared `Grep` / `Glob` are legitimate search intent, but the harness MAY revoke them at runtime — above both project surfaces, so neither the agent declaration nor the project settings can widen the grant back. The revocation is asymmetric: the `Grep` permission is withdrawn while the file-operation hard rule's prohibition on bare Bash `grep`/`find` stays fully enforced by its hook, leaving the rule prescribing a content-search remedy the leaf cannot reach. Navigation still works — the leaf discovers through the structured architecture inventory (`architecture find` / `which-module` / `files`, structured queries first) and scans an already-known file with `Read`. For a broad content sweep across prose that those cannot cover, `git grep` reaches the hard rule's own prescribed remedy through the one Bash program family the rule permits — see the carve-out in [`persona-plan-marshall-agent` § Bash: No file operations](../../persona-plan-marshall-agent/SKILL.md#bash-no-file-operations). Such a sweep is therefore run with `git grep` rather than silently degraded to spot-checks; returning the coverage gap to the orchestrator is the residual path only, for content outside git tracking. See [`execution-context.md` § "Runtime tool availability for dispatched leaves"](../../../agents/execution-context.md) for the agent-side contract.

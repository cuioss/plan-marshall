...
* *Axis-C — module-edge derivation.* "Which modules depend on which?" Declared by `DerivationResolverBase`, subclassed by whichever bundle owns the relevant domain knowledge.

The three ABCs inherit from none of each other. Axis-A and Axis-B are kept disjoint by xref:../adr/004-The_file-to-build_contract_is_owned_by_build-system_extensions_not_languagecontent_domains.adoc[ADR-004] — a build system is not a language domain — and that disjointness is precisely what forced Axis-C to be a third axis instead of a face on either. `build-maven` subclasses `BuildExtensionBase` and not `ExtensionBase`, so a resolver hook on `ExtensionBase` would have been unreachable from the bundle that owns the Maven coordinate join; putting it on `BuildExtensionBase` would instead have forced a future markdown resolver to masquerade as a build system. An implementor from either hierarchy opts into Axis-C by multiple inheritance, which is the only shape reachable from both sides.
...
Extension Architecture is the *plumbing* answer to "what is Plan Marshall." The xref:planning-workflow.adoc[Planning Workflow] is the *user-visible* answer. The eleven domain-facing extension points are the contact points between the two: every domain decision a plan makes — which build skill to dispatch (xref:build-management.adoc[Build Management]), which triage skill loads for a Sonar issue (xref:automatic-reviews.adoc[Automatic Reviews]), which skills enter the subagent context for a Java testing task (xref:skill-handling.adoc[Skill Handling]), which resolver derives the module graph (xref:code-intelligence.adoc[Code Intelligence]) — flows through one of them.
...
* link:../../marketplace/bundles/plan-marshall/skills/extension-api/SKILL.md[`extension-api/SKILL.md`] — overview index of all 16 extension-point contracts: the eleven domain-facing points (build, verify, triage, recipe, outline, provider, execution-context-workflow, dynamic-level-executor, self-review-surfacing, domain-verb, derivation-resolver) plus the core-side finalize-step, lane-element, domain-bundle, retrospective, and build-verify-step contracts.
...
* xref:code-intelligence.adoc[Concepts › Code Intelligence] — the code-intelligence substrate the derivation-resolver point sits on: the Tier 0/1/2 ladder, the resolver seam, N-resolver union, and the provenance contract.

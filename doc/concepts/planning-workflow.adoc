...
"What is the agent supposed to be doing right now?" is a question that goes unanswered through most of a long Claude Code session. Plan Marshall organises every non-trivial change as a *plan* that moves through six phases, each with a clear contract about what it produces and what it refuses to do. Each phase is a skill (`phase-1-init` … `phase-6-finalize`); `phase-1-init` runs inline in the orchestrator's own context, while phases 2–6 have their workflow docs dispatched through the xref:execution-context.adoc[execution-context dispatcher]. The `plan-marshall` meta-skill ties them together and is what `/plan-marshall` orchestrates.
...
== Recipe-match routing (Tier 1)

Before the planning-lane decision runs, `phase-1-init` scores the request narrative against the live recipe registry and classifies its aspect — a *Tier 1* routing tier sequenced ahead of the Tier 2 light/deep lane decision. For any source other than an explicit `recipe`, the init phase calls `manage-config recipe-match` (registry-wide deterministic scoring) and `manage-config aspect-classify` (analysis / planning / implementation classification), both heuristic-first with no LLM call inside the scripts. When the top recipe match clears the configurable auto-route floor, the matched recipe is persisted and the standard refine/outline discovery is shortcut into the recipe path; below the floor the ranked matches are surfaced to the user for selection. The auto-route gate (`auto_route_recipe`) and its confidence floor (`auto_route_recipe_threshold`) are configurable — see xref:../user/configuration.adoc#recipe-routing[Configuration › Recipe-match routing].

The enforcement-critical recipe-match process steps and the auto-route gate semantics live in xref:../../marketplace/bundles/plan-marshall/skills/phase-1-init/SKILL.md[`phase-1-init/SKILL.md`] Step 5c. The recipe shortcut itself is described in xref:recipes.adoc[Concepts › Recipes].
...
`phase-3-outline` opens its work by reading the affected modules' architecture hints — durable, project-specific guidance surfaced through `get-module-context` from `enriched.json`. Two kinds of signal feed that surface:

* **KNOWLEDGE-lesson hints** — actionable residue distilled from lessons learned, routed to `architecture enrich` so a recurring fix becomes a standing outline bias.
* **Learned user preferences** — recurring user gate-dispositions (`suppressed`/`accepted`/`taken_into_account`) generalized into best-practice / insight strings and routed to the SAME `architecture enrich` sink. The cross-plan auditor learns them corpus-wide; the per-plan `default:finalize-step-preference-emitter` finalize step learns them per plan. See xref:audit-trail.adoc#_from_dispositions_to_durable_preferences[Audit Trail → From dispositions to durable preferences].

Both kinds land in the same `enriched.json` `best_practices[]` / `insights[]` schema and surface identically in the phase-3-outline Architecture Hints section — so a future outline is biased not only by what past plans *learned to fix* but by what the project has *demonstrated it prefers*.

== Above the lifecycle

A goal too large for one plan — a roadmap, a campaign, a multi-plan feature — is orchestrated as an *epic* by the `marshall-orchestrator` skill, which decomposes it into workstreams and staged plan specs and emits ready-to-run `/plan-marshall` commands, each of which enters this six-phase lifecycle as an ordinary plan. See xref:orchestration.adoc[Orchestration].

== Related

* xref:orchestration.adoc[Concepts › Orchestration] — the epic tier that sits above this lifecycle and hands work down to it

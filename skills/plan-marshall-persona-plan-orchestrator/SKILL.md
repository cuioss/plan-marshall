...
3. **Direct-file-write carve-out.** Write/Edit are permitted ONLY within the epic's own `.plan/local/orchestrator/{slug}/` tree; logging and status transitions stay script-mediated (`manage-logging` / `manage-status` with the orchestrator store). Writes outside that tree follow the ordinary access rules.
...
6. **Resume discipline.** Keep `resume_anchor` current at all times — before stopping, and whenever the next action changes. The two generated `epic.md` blocks — START HERE and the Ordered Queue table — are regenerated from `status.json` (one `resume-summary` invocation emits both) after every queue-touching state change; they are never hand-written.

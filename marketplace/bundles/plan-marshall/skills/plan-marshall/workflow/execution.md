...
**The dispatched leaf's verification slice is `per_task`-only.** Before running any whole-tree verification step, the leaf resolves that step's `execution_tier` (`per_task` | `orchestrator`) live via `architecture resolve`. It runs only `per_task`-tier steps inline; every `orchestrator`-tier step is OUTSIDE the leaf's runnable slice and is owned HERE by the main-context orchestrator through the [`await-long-running`](await-long-running.md) detach-and-notify seam — the only component permitted to background a build (see [`../../ref-workflow-architecture/standards/agents.md`](../../ref-workflow-architecture/standards/agents.md) § "Leaf cannot reap a backgrounded build"). The manifest's `phase_5.step_execution_tier` stamp (composed by `manage-execution-manifest`) records the same field at compose time and is **advisory**: it tells this orchestrator how many orchestrator-tier steps to expect, but because the tier derives from the adaptive learned build duration it drifts between compose and execute, so the leaf's live resolve is what routes. See § "Orchestrator-tier phase-5 verification (await-long-running)" below for the handler.
...
**Trigger**: the just-returned execution-context dispatch is classified `termination-cause == baseline_drift` (`error: baseline_drift` with `conflict_count > 0` — the non-zero-overlap case; the zero-overlap case is self-absorbed inside phase-5-execute and never reaches here). The recovery is a `2-refine` re-cycle: log the drift, `set-phase 2-refine`, re-dispatch phase-2-refine via the standard `planning.md` § "2-Refine Phase" envelope, and re-enter phase-5-execute on return — bounded by a consecutive-drift-loop cap of **3** (escalate + `drift_loop_cap_exceeded` error TOON on cap; the counter resets on any non-`baseline_drift` return). The full step-by-step recovery procedure lives in [`../standards/execution-recovery.md`](../standards/execution-recovery.md) § "Baseline drift recovery (non-zero overlap)".
...
**Trigger**: the just-returned phase-5-execute dispatch yielded an orchestrator-tier verification signal — a `status: blocked` / `voluntary_checkpoint` terminal payload naming a `phase_5.verification_steps` step whose LIVE-resolved `execution_tier` is `orchestrator` (the leaf's Step 11b Final Quality Sweep or Step 11c Execute-Exit Verify Gate resolved a step outside its `per_task` runnable slice and yielded rather than running it). The named step is the whole-tree gate the leaf cannot reap. The leaf's live verdict is the trigger — a step the manifest stamped `per_task` legitimately yields here when its learned build duration has since crossed the Bash ceiling.

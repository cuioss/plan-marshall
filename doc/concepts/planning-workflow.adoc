...
|3. Outline
|`phase-3-outline`
|`solution_outline.md` produced via the Simple Track (localised changes) or Complex Track (cross-cutting discovery). Outline is strictly analytical — no source mutations and no `doctor` apply/fix calls.
...
|6. Finalize
|`phase-6-finalize`
|Git workflow, finalize-step pipeline (plugin-doctor, pre-submission self-review, post-run review, plus meta-project-only deploy and cache-sync steps), commit, push, and PR creation.
|===
...
* `verification-feedback.md` — triage envelope shared by phases 5 and 6 (build, lint, Sonar, PR review, plugin-doctor).
...
* **Outline never mutates source files or runs `doctor fix/apply`.** Outline is read-only outside the plan directory.

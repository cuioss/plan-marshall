...
**The ceiling binds both rows.** It is evaluated at the item-7b admission gate BEFORE `loop_back_without_asking` is consulted, and its count is persisted to `status.metadata.loop_back_iteration`. Previously the ceiling sat inside the `value == true` branch and counted in model context, which left the DEFAULT configuration unbounded in practice: every halt-and-prompt returned control and the next entry started the count at zero, so a plan could loop indefinitely one operator re-run at a time while `max_iterations` was nominally in force.

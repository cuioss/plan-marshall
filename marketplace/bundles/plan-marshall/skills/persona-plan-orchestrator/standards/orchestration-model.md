...
- **Canonical form.** One dispatch shape, used verbatim. The level resolves outside any plan context, and the resolve carries the same four facts the prompt body does, so the trail and the envelope agree:
  ...
  ⛔ **Pass an explicit `--role orchestrator.{surface}` at the `analyze` / `decompose` sites, never `--default`.** A `--default` resolve carries no payload role and renders the literal `default` in the trail, which is not distinguishable between the two surfaces — and the effort dimension below resolves those surfaces through `orchestrator.effort.{surface}` anyway, so `--default` would also take the wrong tier. `--workflow` names the doc the leaf actually loads, `--plan-id none` states that an epic is not a plan, and `--caller` names this identity.

...
## Per-file assessment record shape for downstream consumers

Every declared path carries one assessment record that downstream phases consume.
Each record names the file path, the certainty (`CERTAIN_INCLUDE`,
`CERTAIN_EXCLUDE`, or `UNCERTAIN`), a numeric confidence, and the rationale
linking the file to the request. One record per declared path is required; a
declared path without a record is an outline error, and consumers read the
certainty, confidence, and rationale fields directly without re-deriving them.

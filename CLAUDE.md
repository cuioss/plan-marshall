...
- **No shell file operations** — Use Read/Write/Edit/Glob/Grep, never `cat`, `head`, `find`, `ls`, or bare `grep` via Bash. (One carve-out: `git grep <pattern> -- <pathspec>` is the sanctioned broad-content-sweep primitive when the `Grep` tool is revoked at runtime — it reaches this rule's own prescribed remedy through the one permitted Bash program family; see `plan-marshall:persona-plan-marshall-agent` § "Bash: No file operations".)

...
* A **separate ordering key** — a timestamp, sequence number, or priority — that must stay consistent with the order a serialized structure is actually mutated, where that key is sampled OUTSIDE the serialization boundary. A `min(key)` / `sorted(..., key=…)` selection over the structure then disagrees with the structure's own positional order, and any consumer reading positional order sees a different "front" than the selector does.
...
* **(d) One source of truth for order — use the serialized structure's own order.** A list mutated entirely inside a serialized read-modify-write already records arrival order in its element order, so the front is `list[0]`, full stop. Do NOT introduce a parallel ordering key that has to be kept consistent with append order; that is a second source of truth, and the two silently diverge under concurrency (an actor can sample an earlier key value yet have its append serialize later). When a key genuinely MUST drive ordering — for cross-process comparison or persistence — assign it INSIDE the same serialized section that establishes the order, so the key is monotonic with serialization order. Sampling it before entering the section is the divergence bug; demote any such pre-sampled value to informational and keep no ordering decision reading it.

**Testing note — order divergence needs an adversarial fixture, not a stress test.** A concurrency stress test only *flakes* on an ordering divergence, and happy-path fixtures that seed a monotonic key matching append order hide it entirely. Pin the invariant with a deterministic fixture that **inverts** the key against the append order (append `[a, b]` while `a.key > b.key`) and assert the front is `a` (position), not `b` (min-key). Such a test fails against the divergent selector and passes against the fixed one, with no timing dependency.

...
|---|---|---|---|
| A1 | False positives | Every one of the fourteen original gap entries was re-derived at its cited `path:line` and **all fourteen hold** — `_check_step_loadable`'s external short-circuit (`_manifest_validation.py:464-469`), the backfill comprehension's three conditions (`:2998-3004`), the two-exception `try` (`git-workflow.py:828-846`), `automatic-review/SKILL.md:547` dispatching the rebase without consuming the payload while `finalize-step-sync-baseline.md:188` and `branch-cleanup.md:384` do, the non-string filter at `:2940`, the unenumerated error branches at `phase-6-finalize/SKILL.md:368-370`, `required-steps.md:114-120`'s asymmetric scope note, and the lane contract's unshipped proposal. No gap sends a later run at working code. Three citations were imprecise. | Composer pre-filter range `1975-2010` corrected to the five individual line numbers (`_apply_security_class_inactive` is at `:2023`, `_apply_scope_gated_finalize` at `:2043` — both outside the stated range), in this document and in G3. `tools-script-executor/SKILL.md (+10/−…)` corrected to `(+7/−3)`. The garbled compose-snapshot sentence ("nothing subtracts between 1948 and the snapshot" — 1948 *is* the snapshot) rewritten. |
| A2 | False negatives | **One high defect the audit cleared.** It read `git-workflow.py:916-931` and accepted *"the success verdict is taken from disk, not from `gen_rc`"*. It is taken from disk — but from **presence**, and `prepare_execute` guarantees presence in every real worktree (`prepare_execute.py:253-262`, `:318-327`, `:598`). Measured by adding a pre-existing executor to the fixture and re-running the landed-check scenario: `executor_regenerated: True`, `executor_detail: "…worktree executor regenerated"`, over bytes nothing wrote. The exit code is no backstop — `generate_executor.py generate` against an unresolvable `--marketplace-root` prints `status: error` at **RC=0** (measured), and `cmd_generate:1961-1965` asserts the opposite. So the F7 fix's guard cannot fire where it matters and the payload misreports. Also picked up the interrupted thread on the **indeterminate → regenerate nothing** bound: it **holds** — `drift` against a bundle-less tree returns no `drift_status`, the `:889` guard maps it to `unknown`, and an explicit `--marketplace-root` outranks the plugin cache (`marketplace_paths.py:775-784`), so the probe cannot answer from the cache instead of the worktree. | Filed **G15 (high, `dispatch/finalize`)** with the fix (require the generator's own TOON `status: success` in addition to the presence check) and a Done-when that seeds the slot first. D3's verdict text rewritten; the overall summary now leads with the high gap; the indeterminate bound recorded as checked-and-cleared rather than assumed. |
| A3 | Vacuous evidence | All three claimed mutations **re-run and reproduced** from fresh snapshots (`1 failed, 7 passed` / `1 failed, 18 passed` / `1 failed, 9 passed`, same named tests; `37 passed` on restore; `git status --porcelain` empty). No test in the three files is tautological. But the audit took D5's *pre-fix reds* from the report without re-deriving them, and one covering test is non-vacuous yet unrepresentative. | Each test file re-run against its own `d2e94b4^` module: collection `AttributeError` / `10 failed` / `7 failed, 1 passed` — all three of the report's reds reproduce, including the named pre-existing pass. Added mutation **M4** (a fixture mutation, not a source one) and a table caveat marking D3's disk-verdict test unrepresentative. |
| A4 | Counts and quotes | Re-derived at check time: nine `.py` paths in `d2e94b4` (five production, four test); 19 + 10 + 8 = **37** tests; `additions: 2115, deletions: 50, changed_files: 23` from the PR API; the PR body's *"32 tests across …"* quoted verbatim and wrong by five; `sourcery-ai[bot]`'s review body verbatim. Two internal miscounts: this document said "the **two** files I mutated" after describing three; `gaps.md` said "Fourteen entries" and "Four residue items … are still open" while filing only three residue entries. | "two files" → "all three files". `gaps.md` intro rewritten (sixteen entries; all four residue items now carried). |
| A5 | Actionability | Every entry carries a concrete path, a concrete change, and an observable Done-when. G3, G13 and G14 are two-option entries, but each names both options concretely and its Done-when covers either — not "review X". G12 cannot be scheduled inside the `doc/plans/` lane and says so in its own Effort field, which is the honest form. Nothing needed rewriting. | None. G15 and G16 written to the same standard (G15's Done-when names the fixture precondition that makes it non-vacuous). |
| A6 | Severity and topic | G8 was `low`: `reconcile --apply` silently deletes a non-string `phase_6.steps` entry on a path `SKILL.md:450` explicitly sanctions, from a verb whose stated premise is never to silently drop scheduled work — that is a real bug with silent data loss, not a cosmetic one. G7's topic was `documentation-surface` while G2, the same class of defect in the same bundle tree, was `bundle-docs`. G4 was weighed against the `high` calibration ("a documented contract is unimplemented" — the "non-fatal by contract" wording is in shipped standards) and **left at medium**: every failure mode the code can actually produce is still reported, and only exotic `OSError`s escape. | G8 `low` → `medium`. G7 topic → `bundle-docs`. G4 unchanged, with the reasoning recorded here. |
| A7 | Coverage | All six deliverables, out-of-scope compliance, report accuracy and the residue list were covered. Four **non-deliverable plan instructions** were not — most consequentially the plan's ⚠ split guard (*"If the outline finds D4 does not belong beside D1–D3, drop it to its own trivial plan"*), which the run explicitly evaluated and decided against. An unmentioned instruction is a hole in the same way an unmentioned deliverable is. | Added § "Plan instructions beyond the deliverable list" — split guard, D0-refutations-in-report, D2's observation point, D5's pre-fix reds — each with its evidence. The split-guard CARRY was independently checked and its strongest stated ground (D5(c) is a test of D4b) is true. |
| A8 | Internal consistency | The overall verdict follows from the rows. Two traceability breaks: **`gaps.md` G7** (Step 1.5's unenumerated error branches) appeared in no finding in this document, and this document's residue row for the interpreter-version-sensitive predicate cited a "Gap G14" that is actually the contract-proposal entry — the cross-references had drifted by one from G7 onward, so seven of them pointed at the wrong entry, and one gap existed with no audit finding behind it while one finding had no gap. | Seven cross-references renumbered to `gaps.md`'s own numbering (G9/G10/G11 for the report counts; G12/G13/G14 for the residue items). `gaps.md` G7 given its own finding (Correctness review item 6). The orphaned residue finding filed as **G16 (low, `tests`)**. |

**Residual doubt:** the highest-yield further round is the one this audit and the run both name and
neither can perform — executing `phase-6-finalize` Step 1.5 against a real `.plan/` tree with a
freshly-composed manifest (G12). Every claim about the *integration* (that `compose` writes
`phase_6.candidate_steps` where `reconcile` looks for it, that the dispatcher parses the TOON and
honours the mandated re-read) rests on unit tests that call `cmd_reconcile` directly, and G15 is
precisely an instance of a guard that passed its unit test while being wrong about the world — so the
same failure shape in the reconcile wiring is the thing most likely still hiding. Second-most likely:
G1's proposed fix routes external steps through `_check_step_resolvable`, whose plugin-cache
dependence I read but did not exercise; if its "unresolvable" verdict is noisier than assumed, the

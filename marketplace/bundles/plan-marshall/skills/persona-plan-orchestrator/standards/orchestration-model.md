...
- **The report makes it safe to run unattended.** `compact` names every mutation (`regenerated[]`) AND every abstention (`abstained[]` — each narrative section left verbatim) AND every invariant verdict (`invariants[]`), each count carrying its population. A silent compaction is indistinguishable from a lossy one — this epic's whole theme — so the report, not the mutation, is the deliverable that makes the stage safe.

...
```text
Call the `read` tool with `{ filePath: "standards/java-input-validation.md" }` before continuing.
```
...
```text
Call the `read` tool with `{ filePath: "standards/java-security-patterns.md" }` before continuing.
```

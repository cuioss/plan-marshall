# PLAN-PR-025: A refusal is recorded as a refusal, and the contract says so

epic: review-apparatus
workstream: WS-01

> ⛔⛔ **RETIRED 2026-08-29 — SUPERSEDED BY THE SPLIT. DO NOT EMIT THIS SPEC.**
>
> This spec reached **ten** deliverables and was unemittable whole. It was split at emit along the seam
> the operator settled on 2026-08-24 — **recording vs recovering** — into:
>
> | Successor | Deliverables | Subject |
> |---|---|---|
> | [`PLAN-PR-025A-a-refusal-is-recorded-as-a-refusal-the-record.md`](PLAN-PR-025A-a-refusal-is-recorded-as-a-refusal-the-record.md) | D0–D6 | A refusal is *detected, classified and described* correctly. |
> | [`PLAN-PR-025B-arm-the-refusal-recovery-that-has-never-run.md`](PLAN-PR-025B-arm-the-refusal-recovery-that-has-never-run.md) | D7–D9 | An *awaitable* refusal is *acted on*: config, attempt budget, jitter. |
>
> ⭐ **This file is KEPT, not deleted** — it is the audit record of why the split happened and of every
> amendment that accumulated here before it. Every deliverable body was copied into its successor
> **byte-for-byte**; nothing was reworded in transit, and the partition was verified clause-by-clause.
> ⛔ **Sequence 025A → 025B, never concurrent** — 025B depends on 025A's D1.
> ⚠ **Amend the SUCCESSORS, never this file.** An edit here reaches no run.

> Staged plan spec, ingested 2026-08-23 from the cloud-lane corpus (``PLAN-PR-025` (this directory)`,
...
Give the refusal producers the cause and cap their consumer already branches on, and make the contract describe what the code does.
...
Every load-bearing Problem-section claim reproduces at HEAD, including all four asserted absences
(`refusal_pattern_drift`, `trigger_semantics`, `head_sha_verified` in `automatic-review/SKILL.md`, and
the participation-site population). One sub-claim is contradicted (row 22 below).

⛔ **Correct before handing this spec to a run:**

1. **D5 names the wrong invocation site.** The `review_completeness check` call lives under
   `automatic-review/SKILL.md` § "Step-done participation guard (D3)" (`:642`); § "Producer: FIND"
   (`:589`) is the `github_pr fetch_findings` site, which has no `--declined-bots` and none possible.
2. **D6's Done-when carve-out is too broad to falsify.** As written, every survivor outside
   `bot-participation-contract.md` falls under "passages the table does not name at all" and can be
   discharged by reporting it — including the sites D6's own body lists as must-rewrite. Narrow the
   carve-out to contract-document passages.
3. **D2's provenance seam would be a THIRD copy.** `github_re_review._ReReviewStrategy._refusal_record`
   (`:325-337`) already re-implements the two-layer split open. Consolidate, or state that duplication
   is accepted and why.
4. **The `github_pr.py` non-overlap claim is wrong at one call site.** `_is_refusal_notice` has two
   callers: `:1040` (the filing pre-filter this plan means) and **`:950`, inside the participation loop
   `:941-975` that PLAN-PR-024 rewrites end-to-end**. Name which one D2 uses.
5. **Advance-disclosure sub-claim contradicted:** the honest disclaimer in `create-pr.md` is four lines
   below in the SAME blockquote (`:229-233`), not "several paragraphs away".
6. **README check 3 FAILS** at `:586`: *"…which plan `500` owns"* is an ownership verb in a sentence
   naming a contract passage. Replace with a bare pointer to the split table.
7. **Fold in, from the ingestion audit:** `010 G14`'s wiring test and `010 G10`'s second cold-read
   question into D5; `010 G4`'s three test-prose passages in
   `test_pr_agent_contentless_guide_interaction.py` into D6.

⚠ **Scope: 7 deliverables — OVER the epic's guard, across 19 files. Split before emitting.**
Recommended: (a) D0+D1+D2 the refusal producers and detection layer; (b) D3+D4 the consumer/operator
surfaces; (c) D5+D6 decline accounting and the prose sweep. Only (a) and (c) touch
`bot-participation-contract.md`, so only those need serialising against PLAN-PR-024.

## Dependencies and Sequencing

- **Depends on: PLAN-PR-024** — run it first. D6 must describe the predicate and the ledger name that
  plan leaves behind.
- **MUST NOT run concurrently with PLAN-PR-024** (adjacent bullets in one contract list; the same
  participation loop; the same `result` dict literal `:1245-1306`).
- **MUST NOT run concurrently with PLAN-PR-029** — `github_pr.py` + `test_github_pr.py`.
- ⛔ **Ownership collision to settle before either is emitted:** `030 G7` (which flags take pairs, at
  three invocation sites) is claimed by this plan's D3 AND by PLAN-PR-027 D5. This plan's own
  `Discharges` line names only `030 G5 / G8 / 120 G5`.
- ✅ The `deficit`-synopsis handshake with PLAN-PR-026 is in good shape — both sides agree in writing
  on who edits and who guards.
- Overlaps with: PLAN-PR-024, PLAN-PR-026 and PLAN-PR-031 (`review_completeness.py`,
  `test_structural_refusal.py`), PLAN-PR-027 (`030 G7`), PLAN-PR-030 (`automatic-review/SKILL.md`).
...
`../cloud-runs/{plan-dir}/gaps.md` and its sibling `verification.md`. This plan restates
...
⛔⛔ **PRE-EMIT AMENDMENT — 2026-08-25, from the PLAN-PR-034 landing ([`../landings/PLAN-PR-034.md`](../landings/PLAN-PR-034.md)).
D2 IS WRITTEN AGAINST SUPERSEDED CODE. RE-SCOPE BEFORE EMIT.**
PLAN-PR-034 shipped as #1344 (`dfabe3d8e`) and **restructured `_is_refusal_notice` and both its call
sites**, adding a third recognition arm (`_github_pr.py:297-380`) alongside the two this deliverable
names. It also touched `github_re_review.py` (+162), `review_completeness.py` (+201), and
`bot_registry.py` — all four files are D2's own surface. ⭐ **The premise is NOT refuted**: the bare
`bool` and the swallowed layer provenance were real, and PR-034 did *not* add the provenance field —
it explicitly declined to pre-empt this plan, extending `github_re_review`'s existing `layer` split
instead. ⛔ But the function D2 opens now has a different shape, a third layer, and a shipped-inert
threshold constant, so **the deliverable's text describes a two-layer function that no longer exists.**
Re-read the merged source and re-scope D2's wording before this spec is emitted. This is the **tenth**
owed pre-emit amendment (the nine in [`../cloud-wave-audit.md`](../cloud-wave-audit.md) § 9 predate it).
...
✅ **OWNERSHIP SETTLED 2026-08-25 — `030 G7` is NOT this deliverable's; it belongs to PLAN-PR-027 D5.**
The epic carried a recorded ownership collision on G7 between this deliverable and PR-027 D5. It is
**withdrawn**, derived from the two `Discharges` lines: the line above names G5, G8 and `120` G5 and
has never named G7, while PR-027 D5's names `030-G7` explicitly. Exactly one deliverable claims it.
⚠ **What IS real is a surface ADJACENCY, and it becomes a preservation obligation here.** Both
deliverables edit the `review_completeness — check` and `— deficit` blocks in
`automatic-review/SKILL.md` § Canonical invocations: PR-027 D5 adds a **form column / note** naming
which list flags take `bot_kind:value` pairs and which take bare tokens; this deliverable performs the
broader rewrite that makes the flag surface describe its own parser. ⛔ **If PR-027 D5 has already
landed when this runs, its form statement MUST survive the rewrite** — re-read those two blocks before
editing and carry the form information forward. Dropping it silently would re-open a gap another plan
already closed, and the loss would be invisible because both plans legitimately touch the same lines.
At `N = 1` the two never run concurrently, so this is ordering and preservation, never a collision.
...
## ⛔⛔ LIVE DEFECT FOLDED IN 2026-08-27 (PR #1356, finding `5ec6d3`) — FILED, NOT FIXED

`github_re_review`'s `head_sha_verified` **misses a SHA embedded in a commit URL**, so a bot that *did*
review is recorded as having declined — **a manufactured false decline that would BLOCK A MERGE**.

⭐ **This is the highest-severity item in this spec's surface and it is live in merged `main`.** It was
found during #1356's run and **overridden there** on that branch's own stated precondition, with the
reason recorded — so it is unfixed, and the next plan touching `github_re_review.py` inherits it.

⇒ **Belongs to D5**, which already owns "account a decline as a decline". D5's `head_sha_verified`
consumer work must **also fix the producer-side extraction**: recognise a SHA wherever the evidence
carries one, including inside a commit URL, not only as a bare token. ⛔ Note the direction — this
defect fails *toward blocking*, unlike the rest of this spec's defects which fail toward crediting. A
fix that only tightens will make it worse.

### D5 — Account a decline as a decline, and make the call-site population test say what it asserts

*Discharges:* `010` G6, `110` G7, `110` G8, `010` G12, `010` G14, `010` G10.

⛔ **Precondition: D0 reported PASS.** If D0 halted, this deliverable does not start.

⛔⛔ **PRE-EMIT AMENDMENT 2026-08-25 (amendment 4 of `../cloud-wave-audit.md` § 9) — TWO ITEMS ADDED.**

**(i) `010 G14` — the wiring test must assert ROUTING, not vocabulary.** The one executable guard on
the obtain-side wiring, `test_branch_cleanup_merge_queue_routing.py::test_rereview_consumer_honors_head_sha_verified_as_a_decline`,
asserts only that four literals appear *somewhere* in two markdown files
(`'head_sha_verified' in doc`, `'head_sha_verified: false' in doc`, `'declined' in doc`,
`'{declined_bots}' in doc`, plus one over the barrier). ⛔ **That discriminates against the pre-fix
state and nothing else**: it cannot tell a document that ROUTES the bit from one that merely MENTIONS
it, so a document could satisfy every assertion while still crediting a `head_sha_verified: false`
outcome as a completed re-review. **Assert the routing structurally**: pin that in
`branch-cleanup-rereview.md` the `head_sha_verified: false` polarity is the *antecedent of the decline
branch that accumulates `{declined_bots}`* — a structural read of the step block, never a whole-file
substring — and extend that same assertion across the consumer set `010 G6` enumerates, so a
`matched`-alone arm FAILS the test. This is the substring-presence-as-a-guard archetype the epic
already tracks; a presence check over a document is not a guard on its behaviour.

**(ii) `010 G10` — record the SECOND cold-read question verbatim.** The cold read is a plan obligation
that a prior run did not discharge: the sub-agent must read the changed text **cold, without this
plan**, and report *in its own words* (a) which commit a credit is evaluated against, **and (b) whether
a `declined` bot blocks, is disclosed, or is ignored.** ⛔ **(b) is the half that was missing** — the
prior report recorded sub-agent findings and dispositions but no verbatim reading of either question.
Run it against the current text of `bot-participation-contract.md` §§ "The currency rule", "Evidence
for a bot that edits one comment in place" and "Detecting a decline"; **record both answers verbatim,
never paraphrased**; and fold any mismatch back into the rewrite. ⚠ The vehicle is
`execution-context-{level}` via the manifest — **not** the cloud lane's pre-PR sub-agent, which is
unavailable to a `/plan-marshall` run.
...
- **The abandoned two-arm predicate and the two-source anchor.** The sites `010 G4` § Where names still describe a
  ...
  deliverable: read the table in [`../cloud-runs/README-as-exported.md`](../README.md) § "The
...
⛔⛔ **PRE-EMIT AMENDMENT 2026-08-25 (amendment 5 of `../cloud-wave-audit.md` § 9) — TWO CORRECTIONS.**

**(i) THE THREE TEST-PROSE PASSAGES ARE IN SCOPE.** `010 G4` § Where names, alongside its eight
document sites, *"Test prose repeating the noise-sidecar framing:
`test/plan-marshall/workflow-integration-github/test_pr_agent_contentless_guide_interaction.py:38-45,
396-404, 423-426`"*. Those three passages restate the abandoned two-source anchor exactly as the
document sites do, and a test's prose is read by the same later reader this deliverable exists to
serve. ⚠ Rewrite them with the documents; do not treat "it is a test file" as putting them outside a
prose sweep.

**(ii) THE DONE-WHEN CARVE-OUT WAS TOO BROAD AND IS NARROWED.** It previously excused *"passages the
README table assigns elsewhere **or does not name at all**"*. The second limb is the defect: **any**
passage the table happens not to name could be reported instead of fixed, which makes the exit
condition satisfiable by a table with gaps rather than by a corpus with no survivors — a carve-out
wide enough to swallow the sweep. **The carve-out is now limited to CONTRACT-DOCUMENT passages the
README table assigns to another owner.** A passage the table does not name at all is **in scope and
gets fixed**, not reported.

*Done when:* both survivor searches return no hit that describes the currency test, except for
contract-document passages the README table **assigns to another owner** — each of those reported rather than fixed, and each
...
## ⛔⛔ SPLIT BEFORE EMIT — the fold of 2026-08-24 made this spec unemittable whole

D7–D9 below were **folded in by operator decision on 2026-08-24** (CodeRabbit as a required default
with a six-attempt rate-window retry). That takes this spec to **ten deliverables**, well past the
~6 scope-bloat guard — which D0–D6 already exceeded at seven. ⛔ **Do not emit this spec as one plan.**

The seam is clean and is the one to cut on — **recording vs recovering**:

| Split | Deliverables | Subject |
|---|---|---|
| **025a — the record** | D0–D6 | A refusal is *detected, classified and described* correctly. Unchanged by the fold. |
| **025b — the recovery** | D7–D9 | An *awaitable* refusal is *acted on*: config, attempt budget, jitter. |

⭐ **This split COMPOSES with the one § Re-Grounding already recommends — it does not compete with it.**
That note splits the original seven into (a) D0+D1+D2 producers/detection, (b) D3+D4 consumer/operator
surfaces, (c) D5+D6 decline accounting and prose. Those three are subdivisions **of 025a**; 025b is a
fourth, disjoint from all of them. Read the two together as: 025a{a,b,c} then 025b.

⭐ **025b depends on 025a's D1**, which is what makes the fold correct rather than convenient: D1 emits
the refusal **cause** on both producers, and the recovery sequence branches on cause BEFORE
`rate_limit_class`. Without D1 the recovery cannot tell a size ceiling from a clock — the exact defect
D1 exists to close. **Sequence 025a → 025b; never concurrent** (they share `bot_registry.py`,
...
- `marketplace/bundles/plan-marshall/skills/automatic-review/SKILL.md` — D1 (the discriminator sentence stays, now true), D3 (canonical
  invocation form notes), D5 (both `matched: true` arms, the FIND-step `--declined-bots`), D6 (the
  restated predicate).
- `marketplace/bundles/plan-marshall/skills/automatic-review/standards/bot-participation-contract.md` — D2 (§ Consumers), D6 (the currency
...
- `marketplace/bundles/plan-marshall/skills/automatic-review/standards/coderabbit.md`,
  ...
  `marketplace/bundles/plan-marshall/skills/automatic-review/standards/sourcery.md` — D2 (`trigger_semantics`), D6
  (pr-agent's two false statements, its predicate restatements).
- `marketplace/bundles/plan-marshall/skills/workflow-integration-github/SKILL.md` — D1 (`rate_limited_bots[]` shape), D2 (the drift field),
  D6 (the canonical `fetch_findings` step body).
- `marketplace/bundles/plan-marshall/skills/tools-integration-ci/standards/api-contract.md` and `marketplace/bundles/plan-marshall/skills/tools-integration-ci/standards/pr-review-operations.md` — D1 (the
  `rate_limited_bots[]` field contract and its worked TOON example).
- `marketplace/bundles/plan-marshall/skills/phase-6-finalize/standards/branch-cleanup.md` — D3 (the pair-form note), D4 (`{cap}`'s
  derivation and the read instruction).
- `marketplace/bundles/plan-marshall/skills/phase-6-finalize/workflow/create-pr.md` — D6 (the advance-disclosure sentence, the blockquote
  re-wrap).
- `marketplace/bundles/plan-marshall/skills/plan-marshall/workflow/execution.md` — D4 (the loop-back continuation prompt).
...
### Added 2026-08-24 for D7–D9 (the recovery half)
...
they do not already exist. Confirm each before building.
  - verdict: corroborated | checked_at: 26645688b | by: review-apparatus/cleanup | rescoped: n/a | evidence: Re-grounded at HEAD 26645688b, method: whole-spec-file intersection against git diff --name-only 1169fb5bf..HEAD (248 paths). Fail-closed by design - the scan is over the WHOLE spec file, not a parsed Expected Surface section, because two section parsers disagreed on this corpus. Basename matching was measured last pass and DISCARDED as non-discriminating. NO running-row exclusion applied this pass: the queue has no running plan. Intersection: 12 hit(s). 12 hits including automatic-review/SKILL.md, bot-participation-contract.md, branch-cleanup.md and create-pr.md - all moved by #1356. NOT re-audited line by line this pass. The 5ec6d3 fold (head_sha_verified misses a SHA in a commit URL) is a LIVE defect confirmed unfixed in main by the #1356 run itself, so D5's premise is strengthened rather than decayed. The mandatory 025a/025b split and the PR-027 D5 preservation obligation both still stand.
...
- **`bot-participation-contract.md` is a shared file.** Do not resolve ownership from this plan:
  read the table in [`../cloud-runs/README-as-exported.md`](../README.md) § "The shared-document
...
- `../cloud-runs/120-review-barrier-deadlocks-on-a-refusing-bot/gaps.md` — G1, G2, G3,
  G4, G5, G6, G7, G10, G11.
- `../cloud-runs/010-participation-credited-from-a-superseded-commit/gaps.md` — G4, G6,
  G12, G13.
- `../cloud-runs/110-participation-derived-from-a-lossy-view/gaps.md` — G3, G4, G5, G7,
  G8.
- `../cloud-runs/030-a-workflow-doc-prescribes-a-flag-no-script-declares/gaps.md` — G5,
  G8.
...
```text
/plan-marshall task="implement .plan/local/orchestrator/review-apparatus/plans/PLAN-PR-025-a-refusal-is-recorded-as-a-refusal-and-the-contract-says-so.md"
```

...
- **Exemptions**: Suppress via the declarative suppression substrate (see [Declarative Suppression Substrate](#declarative-suppression-substrate)). Disable file-wide via the per-file frontmatter key `plugin-doctor-disable: [askuserquestion-prompt-quality]` (Granularity-3), or path-scoped via project / shipped-default config (Granularity-2 / Granularity-1).

...
4. **PR state + number** — via the CI abstraction (never direct `gh`/`glab`).
   ...
   - **`error_cause: auth_failed` / `provider_call_failed` / `malformed_response`** → the question *does a PR exist?* is **UNANSWERED**, which is not the same as answered "no". Do NOT degrade to `n/a`. Preserve the stdout error envelope as emitted — **every field it carries**, copied whole rather than against a fixed field list — per the middle clause. `error` is hard-coded to the no-PR wording on these arms, so the actual provider failure is stated in `context` **when that field is present**; it is absent on the pre-provider `auth_failed` arm, which has no provider output to report (see [`tools-integration-ci/standards/api-contract.md`](../../tools-integration-ci/standards/api-contract.md) § "`pr view` and the `error_cause` discriminator" for the per-arm field sets).

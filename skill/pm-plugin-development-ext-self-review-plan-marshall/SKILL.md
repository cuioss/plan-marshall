...
**Role**: Plan-marshall-domain implementor of the `ext-self-review-{domain}` extension point (see [`../../../plan-marshall/skills/extension-api/standards/ext-point-self-review-surfacing.md`](../../../plan-marshall/skills/extension-api/standards/ext-point-self-review-surfacing.md)). Surfaces concrete candidates from the worktree's staged diff so the LLM cognitive review pass in [`../../../plan-marshall/skills/phase-6-finalize/workflow/pre-submission-self-review.md`](../../../plan-marshall/skills/phase-6-finalize/workflow/pre-submission-self-review.md) can apply the eighteen structural-defect checks (symmetric pair, regex over-fit, wording disambiguation, duplication, contract drift, producer-without-consumer, source-of-truth drift, same-document contradiction, description-vs-body drift, unguarded boundary, stale count-prose, touched-claim re-check, ordinal-reference re-check, scan-derived-key reachability, worked-example clause mismatch, duplicate-claimable key, discard-without-report, hoisted-binding shadow) against a bounded surface, not an unbounded read of the whole diff.
...
delta_coverage:
  files_in_scope: {same file set files_in_scope reports, re-stated as this block's denominator}
  ...
  statement: {human-readable reading of this block}
  by_class[6]{content_class,files,files_with_candidates,files_without_candidates}:
    {python|skill_doc|standards_doc|markdown_other|structured_config|other},{N},{N},{N}
counts:
  by_family:
  ...
  total: N1+N2+N3+N4+N5+N8+N10+N11+N12+N13+N14+N16+N18+N19+N20+N21+N22+N23
...
hoisted_binding_shadows[N23]{file,line,shadowed}:
  {repo-relative-py-path},{line},{shadowed-import-name}
```
...
>
> **`structural_limit` is the OTHER unconditional field, and it is a different claim.** It is likewise emitted on every surface, but it names what this analysis can never evaluate — the behaviour of the code under inputs the diff does not contain — rather than which files it read. The two are never merged: widening the file set cures a scope claim and cures nothing here. Its consumer contract is the same document § "A clean verdict carries the structural limit of the analysis".
>
> **`delta_coverage` is the THIRD unconditional field, and the only one computed from the round's own RESULT.** The two above are properties of the round's inputs — which files were searched, and what the analysis class can reach. This one reports what the round actually OBSERVED over those files, partitioned by the content class of the file: how many files in scope produced at least one candidate, how many produced none, and the same split per content class. It closes a specific fail-open. A delta whose whole content is of a kind this pass surfaces nothing for re-surfaces the previous round's candidates unchanged and returns clean — and in the fields above, that round is indistinguishable from one that looked and found nothing. Here they separate: the first reports `files_with_candidates: 0`, the second does not.
>
> ⛔ **A zero in `files_with_candidates` is not a verdict of soundness, and not a proof that no detector covers the class.** A covered class legitimately surfaces nothing when there is nothing to surface. The block states only what the round can support — *this round produced no candidate over those files* — and leaves both readings open, because separating them would need a per-detector reach map that nothing in the surfacer derives; publishing one would be a stronger claim than the evidence carries and the same fail-open in the opposite direction. Both path-bearing entry shapes are attributed — the singular `file` most lists carry, and the `; `-joined `files` the cross-file lists (`source_of_truth`) carry instead — so a candidate naming several declaring paths credits every one of them. `candidates_unattributed` counts what names neither, so an entry that surfaced something is never absorbed into the "produced nothing" population. Every declared class is emitted, seeded to zero, for the reason `counts.by_family` seeds its families: a missing key reads as "not measured", a zero reads as "measured, none found". ⛔ That reading holds only for a class whose own `files > 0`. Seeding emits a class the round saw NO file of as well, and its `files_with_candidates: 0` is then a zero over an empty population — an absence of measurement, not a measurement of absence. Read `files` first, exactly as the block-level counts are read against `files_in_scope`.

> The `total` count covers the eighteen line-level heuristics (`regexes`, `user_facing_strings`, `markdown_sections`, `symmetric_pairs`, `flag_guard_pairs`, `keep_markers`, `producer_consumer`, `source_of_truth`, `same_document_consistency`, `description_vs_body`, `unguarded_boundaries`, `touched_claims`, `ordinal_references`, `scan_derived_keys`, `worked_example_pairs`, `duplicate_claimable_keys`, `discard_without_report`, `hoisted_binding_shadows`) only. `contract_sources`, `schema_bearing_files`, `count_prose`, and `advertised_form_help_strings` are review-anchor categories with their own counts; they are not summed into `total` — `contract_sources` and `schema_bearing_files` because each modified file contributes at most one entry whose payload is references rather than candidates, `count_prose` because it anchors a sibling-SKILL.md re-check rather than flagging an added line, and `advertised_form_help_strings` because it anchors a contract-drift sub-check rather than flagging a standalone line-level defect. `ordinal_references` IS summed into `total` because it flags a specific added line (the ordinal cross-reference whose referenced list the same change touched), `scan_derived_keys` IS summed for the same reason (it flags the scan loop's own line), `worked_example_pairs` IS summed for the same reason (it flags the GOOD marker's own line), `duplicate_claimable_keys` IS summed because it flags the insertion site, and `discard_without_report` IS summed because it flags the discard branch, and `hoisted_binding_shadows` IS summed because it flags the rebinding line — all line-level heuristics. `protected_identifiers` is a derived index over `keep_markers` entries with `kind: keep_protected` — it does not contribute to `total` either.

### Stop/stall matched controls

The fields above are ALSO the matched controls that separate an honest stop from a stall — the deterministic surfacing the pre-submission-self-review stop path consumes. A stop is honest only against stated controls; a clean claim without them is a stall signature, never a stop:

- **Stop** — a full-surface round (`surface_scope: full`, non-empty `files_in_scope`) whose `counts` block and `delta_coverage` support the recorded clean verdict (nothing-to-check over `total == 0`, examined-clean over examined candidates, zero-observation over `files_with_candidates == 0`), with `scope_statement` quoting the scope the verdict was drawn against. The gate consumes exactly these fields (see `pre-submission-self-review.md` § Step 4 Branch A confirmation checks).
- **Stop (empty-scope)** — a full-surface round with `files_in_scope == 0`: the footprint was empty, so there was nothing to search. This is a separate stop shape, not a degenerate case of the three above — each of those reasons over a searched file set (an empty total, examined candidates, a zero-candidate observation), while this one reasons over no files at all. Record it under its own name so a reader can tell "searched and clean" from "nothing to search".
- **Stall** — any clean claim missing its controls: no `scope_statement` published, no `counts` block, a delta-narrowed scope presented as a whole-surface verdict, or a zero-observation round recorded as examined-clean. Each is the shape a runner produces when it halted without examining what it claims is clean.

No new detector backs this section: the controls are the unconditional fields the Output schema already emits on every surface. Their job here is naming — a gate that checks "stop or stall" needs the control vocabulary stated in one place rather than re-derived per call site.
...
14. **Stale count-prose** — for each modified file nested inside a skill directory (the nearest ancestor containing `SKILL.md`), every contract source of that skill directory is scanned for count-prose: a digit OR an English number word (`one`..`twenty`) immediately adjacent to one of the closed set of cardinality nouns `operation`, `field`, `step`, `rule`, `command`, `check`. The contract sources are resolved through the SAME `_collect_skill_contract_sources` resolver the contract-source detector (rule 6) uses — the skill's `SKILL.md` PLUS every `standards/*.md` under it — so a stale count living in a `standards/*.md` doc is surfaced, not only one in `SKILL.md`. One conceptual input (the docs carrying this skill's contract), one resolver: the two detectors' file sets agree by construction, not by coincidence. The list is a **review anchor** (excluded from `total`), not a line-level defect flag — it surfaces every count phrase that may have gone stale because a sibling file in the directory changed. Each entry carries `file` (the contract-source path), `line` (the matched line, 1-based), and `text` (the truncated matched line), deduplicated per `(file, line)`. The cognitive review's check 11 re-counts the referent and surfaces a drift when the prose number no longer matches.
...
21. **Discard path with no report path** — an added-to `.py` function that owns a suppression report channel AND drops an item on a BARE `if`-guarded `continue`/`break` without recording it. The channel is a `'notes': notes` dict-literal emission whose key string and value identifier are the SAME suppression-report noun (`notes` / `reasons` / `warnings` / `skipped` / `dropped` / `suppressed` / `diagnostics`) AND that name is an assigned local variable in the function. A discard branch is BARE when its whole body is the single `continue`/`break` (the inline `if cond: continue` form counts); a branch that records the drop first — appends to the channel, routes the item to a sibling disposition list, logs it — is not bare and surfaces nothing. This is the mechanical form of an anti-vacuity rule the project already states in prose: suppression must be reported, never silent. Each entry carries `file`, `line` (the branch opener), `channel`, and `discard` (`continue` / `break`).
     ...
     **False-positive posture:** two narrowings hold the noise down. The channel must be an ASSIGNED local — prose that merely SHOWS `'notes': notes` (this skill's own docstrings do) registers no channel — and the discard must be BARE, so a multi-disposition dispatch loop that records every drop somewhere is not the defect. A bare `continue` in a function with NO report channel is out of scope: an ordinary loop skip is not a vacuity. The pinned real defect is the pre-fix `merge_resolver_edges()` (finding 3e04a8): three `continue` branches drop candidates without appending to `notes`, so a resolver whose every candidate the merge discarded reports `status: ok`, zero edges, and empty notes — a vacuous confident zero, inside the plan whose stated purpose is anti-vacuity.

22. **Hoisted-binding shadows** — added `.py` lines that rebind a name the file's post-image imports at top level. The detector resolves each file's hoisted (import-level) names from the post-image AST (`import x` / `from x import y`, honoring `as` aliases, star-imports excluded, non-top-level imports never counted as hoisted) and surfaces one candidate per added line matching the rebinding pattern. Each entry carries `file`, `line` (the rebinding site), and `shadowed` (the hidden import name). The cognitive review's check 18 reads the binding site and the import in context to decide whether the shadow hides a binding later code still expects. Adjudicates nothing here — whether the shadow is intentional is the reviewer's judgement, never this detector's verdict.

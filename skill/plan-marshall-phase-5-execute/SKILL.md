...
2. **Raise `question`** surfacing the `infeasibility_reason` and offering exactly three gate-level options:
...
2.5. Run the freshness check:
   ...
   Substitute the placeholders with the corresponding fields from the script's TOON output. **Both non-`fresh` branches carry `reason`**, and the `stale` branch additionally carries `observed_status` whenever the offending ledger row carried a readable status — append it to the message when present, and substitute `-` when it is not. `stale` reasons are `worktree_mutated`, `build_error`, `build_timeout`, `build_killed`, `build_indeterminate`, `notation_unrelated`, `notation_absent` — the last two additionally carry `expected_notations` and `candidate_notations`, and carry NO `observed_status` (every candidate row was `success`, so reporting it would say nothing); `undecidable` reasons are `no_registry` (the ledger file is absent or empty) or `head_unresolvable` (the working-tree sha cannot be computed), and the `head_unresolvable` sub-case omits `worktree_sha` and `ledger_path`. Substitute `-` for any field absent in the returned TOON. Do NOT call `manage-status transition` and do NOT auto-continue to finalize.

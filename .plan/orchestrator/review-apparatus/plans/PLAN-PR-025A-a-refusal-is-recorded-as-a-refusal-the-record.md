# PLAN-PR-025A: A refusal is recorded as a refusal, and the contract says so (the record)

epic: review-apparatus
workstream: WS-01

> Staged plan spec — one shippable unit of work, ready for `/plan-marshall` hand-off.
> Originally ingested 2026-08-23 from the cloud-lane corpus (staged by PR #1308); the body below
> `## Problem` is the audit's text, PRESERVED VERBATIM.
>
...
Give the refusal producers the cause and cap their consumer already branches on, and make the contract
...
## Scope and the split guard

**Seven deliverables (D0–D6), and D0 is a GATE rather than a work item** — it derives three populations,
publishes them, and can HALT the plan before any other deliverable starts. The six work deliverables
D1–D6 sit **at** the ~6 scope-bloat guard, not over it.

⛔ **Proceeding at the guard is a RECORDED decision, not an oversight.** The § Re-Grounding note above
recommends a further (a) D0+D1+D2 / (b) D3+D4 / (c) D5+D6 subdivision. That subdivision is **retained as
an ordering obligation INSIDE this plan, not as three separate plans**, because every one of D1–D5
consumes D0's three derived populations: splitting them across plans would either re-derive the same
populations three times — reproducing the hand-maintained-population defect this plan exists to close —
or make two sub-plans depend on a third's run report, a cross-plan coupling this epic does not use.
⭐ **Execute in the order (a) D0+D1+D2, then (b) D3+D4, then (c) D5+D6.** Only (a) and (c) touch
`bot-participation-contract.md`.

## Dependencies and Sequencing

- **Depends on: PLAN-PR-024** — ✅ **SATISFIED**: PR-024 shipped as #1349. D6 must describe the
  predicate and the ledger name that plan left behind — read the merged code, not this spec.
- ⛔ **MUST NOT run concurrently with PLAN-PR-025B**, which depends on this spec's D1. Sequence
  025A → 025B.
- ⛔ **MUST NOT run concurrently with PLAN-PR-029** — `github_pr.py` + `test_github_pr.py`.
- ✅ **`030 G7` OWNERSHIP SETTLED** — it belongs to PLAN-PR-027 D5, not to this spec's D3. PR-027
  shipped as #1356, so **its form statement is already in the tree**: D3's rewrite MUST carry it
  forward (see the preservation obligation in D3).
- ✅ The `deficit`-synopsis handshake with PLAN-PR-026 is in good shape — both sides agree in writing
  on who edits and who guards.
- Overlaps with: PLAN-PR-026 and PLAN-PR-031 (`review_completeness.py`, `test_structural_refusal.py`),
  PLAN-PR-030 (`automatic-review/SKILL.md`), PLAN-PR-040 (`github_pr.py`, `test_github_pr.py`).
- ✅ **No live-plan collision** as of 2026-08-29, derived via `corpus cross-check` against the three
  live sibling-epic plans. Re-derive before launch — that result expires as soon as a live plan lands.
...
```text
/plan-marshall task="implement .plan/local/orchestrator/review-apparatus/plans/PLAN-PR-025A-a-refusal-is-recorded-as-a-refusal-the-record.md"
```

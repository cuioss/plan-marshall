...
|---|---|---|---|
| A1 | False positives | **No gap was refuted.** Every gap and every non-CONFIRMED verdict was re-checked at its cited `path:line`: G1/G2 (the two boolean stubs at `test_freshness_notation_crosscheck.py:166` and `:563`) read verbatim as quoted and are unchanged since `aeab5ab`; G4/G5 quoted verbatim from `manage-solution-outline.py:947-949` and `SKILL.md:245`/`:242`; G6 mutation-reproduced; G10's two standards quotes verbatim from `outline-workflow-detail.md:315` and `phase-3-outline/SKILL.md:467`; G11's fail-open read at `:184-203`/`:248-249`; G12's `None` return confirmed at the gate. Five citations were imprecise rather than wrong. | Tightened: G4 `:946-950` → `:944` with the claim at `:947-949`; G9 `350/plan.md:101-120` → `:101-118`; G12 `extension_base.py:565-596` → `:566` plus the gate at `:646-647` and the deferral docstring at `:589-595`; this document's `file_ops.py:1029`/`:1051` → `:1030`/`:1052` (decorator vs `def`); `_invariants.py:429-447` → `:435`. |
| A2 | False negatives | **One real hole.** The audit reported two surviving stale documentation surfaces (G4, G5); a mechanical sweep of every `*.md`/`*.py` under `marketplace/` for the retired pre-materialization framing returns **four more**, all explaining an unresolvable footprint by "the worktree is not yet materialised" — false for a `disabled` plan, which is the exact wrongness F5 recorded and fixed in the reason string. One of them, `manage-config/SKILL.md:511`, is **in a file this run edited**: `aeab5ab` changes exactly two lines of it (`:548`, `:1429`) and leaves this third statement of the same claim contradicting the correction nine hundred lines below. Separately, G6 is worse than stated: the F35 defect is caught by **no** test in its bundle, not merely by one that stopped discriminating. Every guard the plan shipped was then checked for two-branch reachability; the only unreachable branch is the one already recorded as G11. | Added **G13** (medium, `bundle-docs`) naming all four sites with verbatim quotes, the published sweep population, and the two justified exclusions. Added a new § Documentation-surface sweep to this document so G4/G5/G13 trace to a row here rather than only to the summary. Extended G6's evidence with the 496-passed whole-suite result. |
| A3 | Vacuous evidence | **The audit's mutation sweep reproduces exactly.** M1–M4 were re-run from a fresh byte snapshot: M1 red with `assert 2 == 0` (the pre-fix drift count the report names), M2 green, M3 red on `test_bucket_comparison_is_case_insensitive`, M4 red on the two named cases. Six further mutations (M5–M10) were run against guards the audit had only *read*: the F34 guard removal, the F24 coercion, the fail-closed raise, the `all`→`any` documentation predicate, and the `documentation_only` early return — **all red**, so none of those guards is decorative. The audit's `(True, path)` mechanics demonstration was re-executed and reproduces byte-for-byte (`worktree_state = True`, `has_worktree = False`, `worktree_path = /home/user/plan-marshall`). | Rewrote § Test adequacy's mutation table with M5–M10, an explicit note distinguishing re-runs from new mutations, and a paragraph recording the branch-reachability audit. Recorded the second snapshot directory. |
| A4 | Counts and quotes | **No numeric defect found.** Every figure in both documents was re-derived at check time: 44 changed files and +2455/−244 (`git show aeab5ab --shortstat`), 11 commits with the trailer (GitHub API), exactly six `has_worktree`/`worktree_state` consumers at the six cited lines, 25 corpus files across 13 skills + `test/_shared`, nine production skills and 23 test modules in the diff, 21084 tests collected whole-tree, 21/5/17/496 per-suite counts, and the three-path Step 8 bridge with its `R100` rename. `6f7f9c7` = #1221 (2026-08-13, ancestor confirmed) and `e2b6665` = #1279 (19:29, 36 min before the 20:05 merge). Every quoted line checked verbatim against its attributed file. | None needed. |
| A5 | Actionability | **One *Done when* is unexecutable.** G1's `grep -rn "_query_worktree_path" test/ -A2 \| grep -E "\((True\|False),"` also matches `worktree_query_result(True, …)` and `patch_query_worktree_path(True, …)`, so it returns 20+ hits *after* a correct fix and can never go clean — verified by applying the fix and re-running it. All other *Done when*s are observable; G6's ("re-run the mutation and it goes red") is the strongest. | Replaced G1/G2's criterion with `grep -rnE "lambda [^:]*: *\((True\|False) *," test/ --include=*.py`, verified as a positive control (returns exactly `:166` and `:563` today, nothing after the fix). Recorded that the proposed fix was applied end-to-end and left the module at 17 passed. |
| A6 | Severity and topic | **Four severities under-called; no topic wrong.** G1/G2 were `low` on a runtime-latency argument, but the calibration's medium band is "an incomplete deliverable" and these two sites are the entire reason D5's population rule is undischarged — latency bounds risk, not completeness. G10 was `low` while an entire normative clause ("a missing bucket comment is a Q-Gate finding") has no implementation anywhere. G12 was `low` while its own *Done when* is literally unmet. Every `Topic` matches its owning surface, including G3 grouped under `tests` with the G1/G2 fix it depends on. | G1, G2, G10, G12 raised `low` → `medium`, each with the calibration reasoning stated in the entry and the reason it stops short of `high`. |
| A7 | Coverage | **All six deliverables covered**, plus out-of-scope compliance (all three exclusions re-checked against the diff), report accuracy, and all five residue items — each residue citation verified (`350/actual-state.md:130` R2, `350/report-01.md:47`, `_invariants.py:435`). One structural hole: **G4 and G5 traced to nothing in this document** — they appear only in the opening summary, with no evidence section behind them. D1's undelivered `module` clause carries no gap; re-checked and accepted, since D0's contract is to drop what was not confirmed defective at HEAD and no `module`-deriving code exists to be wrong. | Added § Documentation-surface sweep, which now carries rows 1–6 with the gap each maps to. |
| A8 | Internal consistency | **Two defects.** G12's "Why it matters" called the behaviour "a measurement that misreports by construction" — that phrasing invokes the calibration's *high* band and is untrue of the shipped code, which reports an accurate `unknown` that every consumer fails toward inclusion on. And § Report accuracy item 6 was labelled UNVERIFIABLE when the artifact record settles it: `15f1988` is not the last pre-merge commit (three commits follow it, and `817d959` says so), so arming happened after 19:45 and the disclosure necessarily stated 2-of-3. The overall verdict follows from the rows, and every gap now traces both ways. | Rewrote G12's rationale to name the incomplete deliverable rather than a misreport, keeping the correction visible. Item 6 re-graded from UNVERIFIABLE to resolved-consistent with the reasoning, and removed from § What I could not check. |

**Residual doubt:** the likeliest further find is more of the G13 class in surfaces the phrase sweep
cannot reach — a stale rationale that describes the `pending`/`disabled` conflation without using the
words "not yet materialised", the way G4 and G5 do. Rows 1–2 of the sweep table were found by reading,
not by matching, and nothing guarantees a third reading would find no more. A second, weaker doubt: the
pre-fix red-before-green table remains unverifiable (the branch commits are absent from a
squash-merged clone), so the ten mutations substitute for it rather than confirm it — they establish
that the tests discriminate the defects *today*, not that they were red *then*.

**Verdict on the audit:** SOUND AFTER CORRECTION — every gap it raised is real and every number it
states re-derives, but it under-counted the stale-documentation class by four sites (one inside a file
the run edited), shipped one *Done when* that can never go clean, and under-called four severities.

...
Named agent that executes the Automated Review step of the finalize phase. The narrow tool allowlist (`Read, Bash, Skill`) plus the foundational-practices skill load ensure the step carries its enforcement context directly instead of relying on a general-purpose subagent's prompt-based restatement.

...
>
> ⚠ **Known already-closed ids cited by this plan (1):** `130/G6`.
...
> It names `report-01.md` under `170`. Those reports were ingested
...
Make every agent- and operator-facing document this plan touches state what its subject actually is.
The request narrative is § Problem (what diverged and why) together with § Goal (the end state), both
below. They are not restated here. *(Section added at the 2026-09-11 `cleanup` as a pointer, so that
the ambiguity class is discharged without a second copy.)*
...
- `marketplace/bundles/plan-marshall/skills/tools-integration-ci/scripts/ci.py` *(added 2026-08-23, DB — the fix site; re-ground first, the lead is unverified)*
...
**DA — a finalize verdict derived from the GLOBAL lessons corpus can be falsified by an out-of-session
actor, and was, within nine minutes.** *(folded 2026-08-23 from PR #1332 / L4)*
The lessons corpus is global and main-anchored and any agent may mutate it at any time. A finalize step
that reads it, reaches a verdict, and records that verdict has no guarantee the corpus still supports it.
Observed: at 08:21-08:22 `lessons-housekeeping` classified **10 lessons as retains**, one decision line
each, and recorded `outcome: done`; at 08:31-08:32 **a separate terminal-report handoff, outside that
plan's session, retired all 10** as `superseded`; at 11:23 the step's iteration-2 re-derivation detected
it and flipped the verdict from *10 keep* to *0 lessons to classify*.
⭐ **The detection is exemplary and worth preserving as the model**: it corroborated the zero two
independent ways (list enumeration + a direct corpus read) and named it a **scanned-and-empty** zero
rather than a bare `0` — exactly the discriminator discipline this epic keeps re-deriving.
⛔ **The hazard is that the re-derivation only ran because that plan happened to loop back. Had it not,
the finalize record would have carried a verdict false within nine minutes of being written**, showing
`outcome: done` and a `display_detail` naming 10 retained lessons that no longer existed. A second
consequence: the Step 5a dedup gate then ran against a corpus of size 0, so nothing could classify
`merge_into` / `already_closed` — the gate was structurally unable to do its job.
**Remedy, option 1 preferred (cheaper, composes with shipped machinery):** have `lessons-housekeeping`
record a **corpus content hash** beside its verdict and have the finalize handshake re-check it before the
phase closes; a changed hash invalidates and re-fires. ⭐ *That is the same shape as the `verdict_inputs`
currency check, applied to a STORE rather than to paths* — see PLAN-TRUTH-097 DB, and keep the two
consistent. Option 2 is a plan-scoped claim over `manage-locks` so an out-of-session actor blocks.
⚠ **Generalizes beyond lessons:** any finalize step whose verdict derives from a global store rather than
the plan's own tree — the architecture inventory, the recipe registry, `marshal.json`.
⚠ **We are implicated:** the retiring "terminal-report handoff" was almost certainly this epic's own
ingestion of the #1330 report, which deleted that corpus after transfer. **A cross-run interaction, not a
rogue actor** — which is exactly why a hash check beats a policy.

**DB — `ci pr view` reports an unreachable API as an authentication failure.** *(folded 2026-08-23 from PR #1332 / § 11.7 — UNVERIFIED LEAD, observed once)*
With GitHub unreachable: `ci pr view` returned `error: Not authenticated. Run 'gh auth login' first.`
while `gh auth status` reported `X Timeout trying to log in …` **alongside `- Active account: true`**.
The account was active and the credential intact; only the network was down.
⛔ **The advice is actively harmful** — it sends the operator to re-login during an outage and risks a
needless credential rotation.
⭐ **Same conflation that PR #1332's own Fix 4 removed** from `verify_system_auth` / `verify_tool` (which
had derived "is this authenticated?" from an aggregate exit code, so `gh auth status` exiting 1 for ANY
broken account read as a false negative). That fix addressed the **exit-code** side; this is the identical
conflation surfacing on the **error-message** side, in a different layer (`ci.py` / provider ops rather
than `_providers_core.py`).
**Remedy:** distinguish *"no active authenticated account"* from *"could not reach the API to ask"*, and
report the latter as unreachable. ⭐ The discriminator is already in the data the wrapper reads —
`Active account: true` rides the same output as the timeout — **it is being discarded.**
⚠ **UNVERIFIED by us**: reproducing it requires a network outage. Recorded as a lead with its evidence,
not as a confirmed defect. Re-ground before implementing.
...
## Claim Labels
- verdict: unverifiable | checked_at: 7845a4b9a | by: truthful-signals/cleanup | rescoped: n/a | evidence: Re-grounded at HEAD 7845a4b9a AFTER PLAN-TRUTH-109 landed as #1369, which touched manage-findings/SKILL.md - the caveat recorded in landings/PLAN-TRUTH-109.md is hereby DISCHARGED. 100/G1 STILL HOLDS: SKILL.md Types: lists 12 while FINDING_TYPES defines 14, and the two absent are still arch-constraint and pr-comment-overflow - #1369 did NOT close it. Section remains table-form so the address stays section scope; the other 27 asserted gaps were NOT enumerated this pass, so the section as a whole is still an unreached population.
...
## ⭐ FOLDED 2026-09-11 — cross-repo lessons drain (1 Token-Sheriff item): a consumer CONFIGURED the phantom knob and paid for it

`lessons-handling-26-09-04-01-028` part B: Token-Sheriff PLAN-12's phase-4 composer dropped
`pre-submission-self-review` with `scope_gated_finalize_dropped` (`scope_estimate=surgical`) although the
step's `step_params` carried **`drop_review_on_scope_gate: false`**, and although the operator had chosen
the `standard` posture precisely because it adds that step.

⇒ **This is the `190/G1` gap in this spec, observed from the consumer side.** OBSERVED at `356973d80`:
`drop_review_on_scope_gate` occurs nowhere under `marketplace/bundles/` — so the sender's "precedence"
diagnosis (a per-step param failed to outrank the scope gate) is **REFUTED**: nothing reads the param at
all. The consumer set it because `doc/user/parallelism-and-locking.adoc` documents it. ⭐ **That is the
strongest possible evidence for this spec's premise**: a false doc sentence was acted on, and it silently
reduced an operator-selected posture to the one the operator had not chosen.

⚠ The sender's second limb — **consent**: a scope gate that removes the step an operator's posture choice
existed for should SURFACE that downgrade rather than record it only as a manifest drop reason — is a
composer behaviour, not a documentation fact, and is out of this spec's scope. Recorded here as a lead for
whichever plan owns the phase-4 composer's drop reporting; it widens no surface of this spec.

Adds no file surface: the documented-knob row is already declared for D-190/G1.

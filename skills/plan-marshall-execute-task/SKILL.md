---
name: plan-marshall-execute-task
...
1. **Understand Context**: Read affected files (`step.target`) if they exist. Use `architecture files --module {module}` to enumerate the module's components and `architecture which-module --path P` / `architecture find --pattern P` for module-spanning lookups; fall back to `Grep` for content-level searches inside known files and `Glob` for sub-module path patterns or when the architecture verb returns elision. Apply domain knowledge from loaded skills. **Opt-in LSP lookup:** when a language server is configured and reachable for the task's language — `plan-marshall:lsp-client`'s `preflight` returns `state: ready` — you MAY locate symbols by coordinate with its `lookup` verb (`definition` / `references` / `document-symbol` / `workspace-symbol`) instead of reading whole files; a `provider_count: 1` result with an empty `locations[]` is a real "found nothing", distinct from a `degraded` (no-server) return. When `preflight` returns `not_configured` or `unreachable`, use the `architecture` / `Grep` / `Glob` / `Read` ladder above — behaviour is byte-identical to a project that never configured a server. See [`../lsp-client/SKILL.md`](../lsp-client/SKILL.md) § Canonical invocations. *How completely the in-radius items are read and how deeply their relations are traced is the task's declared* thoroughness *over its declared* scope *— see the two-dial coverage contract (thoroughness ladder T1–T5, scope ladder, grade-to-the-floor, coupling constraint `reject thoroughness ≥ T4 ∧ scope < component`) in [`persona-plan-marshall-agent/standards/thoroughness.md`](../persona-plan-marshall-agent/standards/thoroughness.md).*
...
3. **Implement Changes**: For each step — create new files with `Write`, modify existing files with `Edit`. Apply domain patterns and maintain existing code style. **Opt-in LSP rename:** for a multi-file symbol rename with a configured, reachable server, `plan-marshall:lsp-client`'s `edit` verb produces a `WorkspaceEdit`, captures its footprint from the edit, applies it, and re-runs diagnostics — a worsened diagnostic set fails the step and rolls the change back. ⛔ **A `failed` return does not by itself mean the edit was rejected — read the `reason`.** `diagnostics_worsened` is the rejection: the parser found new errors, they are listed in `new_diagnostics[]`, and you investigate them rather than forcing the edit. `diagnostics_unavailable` is the opposite — the server produced **no verdict**, so nothing is known about the edit and there are no diagnostics to investigate; re-apply the change through `Edit` and judge it on the Step 5 build instead. Your tree is unmodified either way, and `phase` says by which route: `before` means the baseline verdict was missing and nothing was ever written (so `rolled_back: false` here means *nothing to roll back*), `after` means the edit was applied and restored. `phase: before` is what **every** call returns against a server that answers only pull-style diagnostic requests. `unsupported_resource_operation` and `apply_failed` mean the verb refused or could not complete the write. The refusal happens before any file is touched, so nothing on disk changed; `apply_failed` restores every file already written and reports the outcome in `rolled_back` — ⛔ read that flag **together with** `restore_error`, never alone, because the one state that leaves the tree partly edited is `rolled_back: false` *with* a `restore_error` — and that field is derived by re-reading the files, so a write the filesystem refused outright (a read-only path, EPERM) reports no `restore_error`, because it damaged nothing. A `degraded` return is "unavailable" (fall back to `Edit`). This does not replace Step 5 verification — `lsp-client`'s `diagnose` is a pre-build signal that **supplements, never replaces**, the quality gate.

...
2. **Plan + phase** — from the worktree cwd (`.plan/local/worktrees/<id>`) or the `$PLAN_ID` env variable, reading `current_phase` from the main checkout's `status.json`. Shown as `{icon} pm:{phase}[:{short_description}]`, where the `:{short_description}` segment is appended only when a `short_description` value is present in `status.json`.

...
- **Pin resolution has exactly three arms, and `.in_use` is not one of them — but the pin is a DIFFERENT mechanism from the keep-set above, owned by a different skill.** The six-rule union above is the `marshall-steward` sweep's *keep* decision (`cache_retention.py`); the pin is `tools-script-executor`'s *marking-exclusion* decision, computed by `generate_executor._retention_pinned_versions`. **The pin is not the keep-set**, and their two arities are not two readings of one rule. That function returns exactly three arms — the selector-resolved newest-on-disk dir, `system.provisioned_version`, and the manifest-named version — which mirror three of the **four** non-count arms of the keep union (newest-on-disk, provisioned, manifest-named, and the executing dir). The fourth, the version dir the sweep is itself executing from, is deliberately **not** mirrored: self-deletion is the sweep's own hazard, not the marker-writer's. On neither mechanism does an `.in_use` marker participate in plugin-cache pin or retention resolution — an `.in_use` file is repair residue, not a contract.

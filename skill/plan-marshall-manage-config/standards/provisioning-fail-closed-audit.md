...
| `_detect_multi_version_pollution` | (b) | **JUSTIFY, re-justified against the shared selector.** Returns `[]` on an unresolvable base_path — it makes no pollution claim it cannot substantiate — and excludes `.orphaned_at`-marked dirs from the live count. The prior justification silently rested on the marker being a currency signal; under saturation (every dir marked, zero live) the detector became **vacuous**, unable to return `>1` at all. It also claimed as a standing invariant that "the newest-on-disk dir is never marked": that claim is **refuted** — the pin set is evaluated at mark time only, nothing clears a marker when pruning later promotes an already-marked dir to newest-on-disk, so the pin is a *write-time postcondition* of `_mark_superseded_version_dirs`, not a property that holds over time. The invariant is now upheld **structurally on the read side** instead: `marketplace_bundles.select_live_version_dir` ignores a mark on the retention-pinned dir outright, so every bundle with a version dir on disk contributes at least one live dir and the `>1` comparison stays meaningful regardless of what a past pass marked. |

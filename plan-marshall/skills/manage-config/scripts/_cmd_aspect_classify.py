...
implementation`` — the conservative default. This mirrors
``change-type-heuristic``'s heuristic-first / conservative-default contract.

Scope: this verb classifies request INTENT and nothing else. It has no say in
...
The classifier reads only the request narrative, which is structurally blind to
the footprint, so it can neither decide nor influence build necessity.
...
def cmd_aspect_classify(args) -> dict[str, Any]:
    ...
    resolved ``aspect``. Below the threshold the conservative
    ``implementation`` fallback applies.
    """
    ...
    # non-implementation aspect; otherwise the conservative implementation
    # fallback applies.

...
In both cases the dispatched leaf does NOT fire `question` itself — it returns this envelope and the inline orchestrator (phase-6-finalize SKILL.md Step 3 item 7a) owns the prompt.
...
- `action`: `defer` when policy is `defer` (orchestrator skips the merge directly); `ask` when policy is `ask` (orchestrator fires `question` with `prompt_options[]`). The `rate_window_timeout` variant always uses `action: ask`.

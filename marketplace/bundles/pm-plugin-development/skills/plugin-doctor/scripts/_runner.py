...
class RuleRunner:
    ...
    def run_analyze_marketplace_rules(self, *, active_rules: frozenset[str]) -> list[dict]:
        ...
        # ``run_quality_gate``, for the same reason recorded for
        # ``documented_verb_set_drift`` below: that gate derives status as
        # ``'fail' if all_issues else 'pass'``, so it is severity-blind and any
        # rule with a non-zero standing finding count turns the tree red. This
        # rule's corpus is every ``AskUserQuestion:`` invocation block in the
        # marketplace, so it belongs on the authoring-time analyze surface.

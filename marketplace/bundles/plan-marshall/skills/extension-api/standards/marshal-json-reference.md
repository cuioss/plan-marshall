...
| `plan.phase-5-execute.verification_steps` (keyed-map serial form) | `extension_discovery.find_implementors` over `ext-point-verify-step` (built-in `canonical_verify.md`, expanded per-canonical) + project `verify-step-*` skills | phase-4-plan, phase-5-execute | [ext-point-verify-step.md](ext-point-verify-step.md) |
...
| Verify-step registry + descriptions (formerly `BUILT_IN_VERIFY_STEPS` / `BUILT_IN_VERIFY_STEP_DESCRIPTIONS` in `_config_defaults.py`) | (removed) | **adopted** | Verify-step membership, order, and per-step description are declared in the parameterized `canonical_verify.md` step doc's frontmatter (`canonicals:` enumerates the canonical set) and discovered via `extension_discovery.find_implementors` ([`ext-point-verify-step.md`](ext-point-verify-step.md)); the seed and `_discover_all_verify_steps()` both expand each implementor's `canonicals` list into `default:verify:{canonical}` ids. The centralized constant list and description map were removed — there is no parallel registry to migrate. (Verify steps remain param-less today, so the value stays `{}`; the per-step `configurable` contract becomes worthwhile only once a verify step declares owned params.) |
| Finalize-step registry + descriptions (formerly `BUILT_IN_FINALIZE_STEPS` / `OPTIONAL_BUNDLE_FINALIZE_STEPS` + `*_DESCRIPTIONS`) | (removed) | **adopted** | Finalize-step membership, order, default-seed inclusion, preset membership, and per-step description are all declared in each step doc's frontmatter and discovered via `extension_discovery.find_implementors` ([`ext-point-finalize-step.md`](ext-point-finalize-step.md)). The centralized constant lists and description maps were removed — there is no parallel registry to migrate. |
...
**Rollout direction:** the finalize-step AND verify-step description maps have been **adopted** — membership, order, and per-step descriptions for both pipelines are now declared in each step doc's frontmatter and discovered via `extension_discovery.find_implementors` (no centralized registry remains for either). The remaining **adopt-later** surfaces (`cost_size_token_table`, `per_deliverable_build`) become contract candidates only if/when a step owner emerges for them. Every other cluster is **not-a-fit** by construction — phase-level gates, project/system config, build-queue/architecture data, and pipeline-internal constants are not step-owned params.

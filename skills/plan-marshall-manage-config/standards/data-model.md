...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `auto_emit` | bool | `false` | Governs the epic orchestrator's post-landing queue-fill emit (the `plan-orchestrator` `next` verb, and the proactive emit in `analyze`). When `false` (default, the safe posture), the emit is stage-and-wait: the orchestrator produces the copy-paste block and records each plan's `launched` transition only on operator-confirmed launch. When `true`, the emit auto-fills toward `parallelization_scope` — the `launched` transition for every selected candidate in the emitted `N − R` block is auto-recorded, under the unchanged disjointness + prep-readiness + "only if sensible" guards. **`auto_emit` automates the *emit* (marking each emitted plan `launched`), NEVER the *start*: the operator-confirmed `launched → running` transition stays operator-owned under both knob values — the emit≠running invariant is absolute.** A shortfall (no qualifying candidate) emits nothing and logs the blocking reason under both values; `auto_emit=true` never emits a colliding, blocked, or unprepared plan to fill a slot. See [`persona-plan-orchestrator/standards/orchestration-model.md` § Parallelization by Surface Disjointness](../../persona-plan-orchestrator/standards/orchestration-model.md#parallelization-by-surface-disjointness). |
...
- **Technical domains**: Reference a `bundle` and declare `workflow_skill_extensions` (outline, triage, marker-detect)
- **Profiles**: Loaded at runtime from `extension.py`, not stored in marshal.json
- **Domain inclusion (optional)**: A `skill_domains.{domain}` entry MAY carry two inclusion keys — `always_on` (bool, default absent ⇒ `false`) and `file_globs` (list[str], seeded from the owning domain extension's `provides_file_globs()`; absent when that accessor returns empty). Their full semantics — which passes read them, and in which direction each pass may move the set — live in [skill-domains.md § Domain Inclusion](skill-domains.md) and are deliberately not restated here. The defaults are asymmetric: `always_on` is genuinely absent by default (no seed), while `file_globs` is seeded at the `convert_extension_to_domain_config` seam whenever the key is absent, so an already-configured project is backfilled on its next `configure`; an operator value set via `set-inclusion` wins over the seed. Neither key is written into `get_default_config()`, so `sync-defaults` neither adds nor wipes them. Both are validated by `validate_domain_inclusion` (`always_on` must be a bool; `file_globs` must be a list of str) and preserved across a `skill-domains configure` reconfigure. The seeded globs contribute no `build.map` route.
...
**What this is NOT — the axis this model is most often mis-remembered on.** The bullet below states the mechanism positively and names the belief it forecloses. It is stated because no inventoried file asserts the mistaken model, so a reader arriving here should not go hunting for a wrong sentence to correct. That is a first-party finding rather than an assumption — a complete content sweep of the crawled inventory found no instance of the belief in any inventoried file, with nothing unreadable, nothing elided, and no result truncated, so the negative is trustworthy under the complete-coverage rule rather than merely empty. The scope word is load-bearing: the crawled inventory is strictly smaller than the repository — it excludes `.claude/**`, `.github/**`, and every `.gitignore`d path — so the sweep speaks for the inventoried files and not for those trees. See [`client-api.md`](../../manage-architecture/standards/client-api.md) § search for the coverage contract.

- **There is no marker-age expiry rule, and marker presence or absence is not a keep-oracle.** `cache_retention.py` never reads `.orphaned_at` at all — not even its existence — and its keep-set is exactly the union stated above: newest-`N` (default 5) ∪ younger than `D` days (default 3) ∪ newest-on-disk ∪ `system.provisioned_version` ∪ manifest-named ∪ the version dir the sweep is itself executing from. Absence of a marker is not a keep-reason and presence of one is not a delete-reason; in particular there is no seven-day `.orphaned_at` expiry.

No version-selection or retention code under this repository consults the `.orphaned_at` marker. The `marshall-steward` `cache_retention sweep` prunes by the union above, and both the shared `select_live_version_dir` selector and the executor's embedded runtime resolver pick the numerically-newest *eligible* version dir, ignoring the marker (which has a foreign co-producer this repository neither owns nor versions) — see [`tools-script-executor` SKILL.md](../../tools-script-executor/SKILL.md) § "Version-aware bundle-path resolution".
...
**`sync-defaults` materialises an explicit `lane` on every RESOLVABLE lane-less finalize step.** After the deep-merge, `manage-config sync-defaults` fills a `lane` value into every `plan.phase-6-finalize.steps` entry that carries none. A step already carrying an explicit `lane` (`off` / `ask` / a resolved tier) is left untouched, so the pass is idempotent. The fill is decided by TWO inputs together — the row's **provenance** (freshly deep-merged from the defaults, or already present in the config) and its **element class** — never by provenance alone:

| Provenance | Element class | Fill |
|------------|---------------|------|
| freshly-merged default row | resolvable, NOT immune to `off` | `lane: off` (opt-in) |
| freshly-merged default row | resolvable, immune to `off` (`core` / `derived-state`) | its **effective lane** — never `off` |
| freshly-merged default row | unresolvable | `lane: off` (an unknown class is not a shielded one) |
| pre-existing step | unresolvable | left lane-less and NOT reported |
| pre-existing step | resolvable | its **effective lane** |

The **effective lane** is the value the composer would apply with no override (declared `lane.tier` ▸ class default: `core` / `derived-state` → `minimal`, `adversarial` / `prunable` → `standard`) — a semantic no-op that surfaces the implicit default as an explicit value.

Two rows in that matrix are the ones a reader is most likely to get wrong, and each is a consequence of the same immunity rule. A weakening `off` on a `core` / `derived-state` floor element is **immune** at compose time — ignored, with the element kept at its class-default tier — so filling a freshly-merged floor row with `off` would leave the config advertising a disabled step the composer runs on every plan, a row whose stored value and actual behaviour disagree from the moment it is written; the floor row therefore takes the effective-lane fill instead. An **unresolvable** class is *not* immune — the class is unknown, not shielded — which is why a freshly-merged unresolvable row still takes the `off` opt-in, while a **pre-existing** unresolvable one is left untouched and unreported: there, materialising a value the resolver cannot derive would not be a no-op. The same immunity rule is also what makes the whole pass safe: materialising the finalize set to a fully-explicit shape can never silently drop a correctness-floor step, because a hand-written `off` on such a step is inert.

**The four finalize ceremony gates ride the `lane` channel, not a run-at-all knob.** `qgate` / `self_review` / `simplify` / `security_audit` are each governed by their owning step's per-element `steps.<step>.lane` override — read from the merged plan-local-over-marshal source, so a plan-scoped declaration governs a ceremony gate exactly as a project-wide one does — and resolved by the manifest ceremony transform (`off→never`, `minimal→always`, every other value `→auto`). There is no flat `qgate` sibling and no `simplify` / `self_review` run-at-all param. The owning steps are `default:pre-push-quality-gate` (qgate), `default:pre-submission-self-review` (self_review), `default:finalize-step-simplify` (simplify), and `default:finalize-step-security-audit` (security_audit). The `default:pre-submission-self-review` step is config-less.

**The two adversarial infra elements seed `lane: ask`.** `plan-marshall:automatic-review` and `default:sonar-roundtrip` seed a `lane: ask` override so `marshall-steward` always prompts about them at setup / update-config and persists a resolved `off`/`standard`/`full`; a genuinely-unresolved `ask` whose provider is absent is dropped at compose time by the drop-when-no-provider safety net.

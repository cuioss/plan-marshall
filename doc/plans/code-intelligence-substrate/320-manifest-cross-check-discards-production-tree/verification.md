...
- **Checks run (mutation):** adding `or path.startswith('.claude/')` to `classify_path`'s runtime-state rung — the pre-fix rule, reintroduced — turns **three** guards red, including one per site: `TestProjectLocalTreeSurvivesFilter::test_multi_file_project_local_footprint_is_not_filtered` (manifest site) and `::test_routing_check_sees_project_local_production` (routing site). File restored from a byte snapshot; `git status --porcelain` clean for it.
...
- **Checks run (mutation):** `candidates = [Path.cwd() / raw]` turns `test_documented_relative_form_matches_the_absolute_form` and `test_manifest_check_also_resolves_the_relative_form` red. Restored from snapshot.
...
No vacuous or tautological guard was found. Baseline: the four affected test files run **214 passed** in 23s on the current tree. Every mutation above was applied to a byte snapshot in `$TMPDIR/verify-320-mutsweep/`, run, and written back by hand; `git status --porcelain` is clean for all three mutated files.
...
| Two more private copies of canonical-verify prefix knowledge, plus D3's third local normalizer | **Open** | `manage-config/scripts/_cmd_quality_phases.py:68` `_CANONICAL_VERIFY_PREFIXES = ('default:verify:', 'verify:')`; `tools-marketplace-inventory/scripts/_dep_detection.py:169` `CANONICAL_COMMAND_PREFIXES = ('default:verify:', 'verify:')`; `check-manifest-consistency.py:83` `_CANONICAL_VERIFY_PREFIX = 'verify:'`. All three agree behaviourally today (G11) |
| The delivered head `ad14fee` was never re-reviewed | **Open, and true** | PR #1288's reviews are at heads `8cd95a7` (sourcery refusal) and `622c052` (coderabbit); no bot review exists at `ad14fee` |

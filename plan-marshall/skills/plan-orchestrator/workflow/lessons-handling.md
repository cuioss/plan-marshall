...
3. **INTEGRATE each applicable lesson locally.** Three branches, and ⛔ **every applicable lesson takes exactly one of them** — an applicable lesson that matches none has no integration path at all, while step 4 below gates its removal on an integration that would then never happen, stranding it in the remote repo forever.
   ...
   **(a) Fold into an existing Step 3 cluster** — when one covers the lesson's subject. ⛔ **Do NOT re-run Step 4 for that cluster.** Step 4 already ran for it in this same sweep and already wrote ONE `finding` message to its destination epic, and the inbox channel is append-only — so a second Step 4 pass APPENDS a SECOND message for what is one disposition of one cluster. Nothing lets the destination's `analyze` verb recognise the two as the same cluster: neither this doc nor [`inbox-envelope.md`](../standards/inbox-envelope.md) defines a cluster-id key or any message-level idempotence. The cluster then presents twice, and the Output contract's per-cluster counting has two messages where it expects one.
   ...
   ⛔ **This branch presumes Step 4 actually filed a message for that cluster — verify it did before correcting anything.** Step 4 item 1's destination-phase guard can have skipped the `inbox write` entirely (the destination epic was closed, unreadable, or had no active owner), in which case there is no filed message to amend or supersede. Check via `orchestrator inbox list --slug {destination_epic}` for a `messages[]` row matching the cluster: when none exists, the cluster was never persisted — record it exactly as Step 4's own no-active-owner/closed-epic carve-out does (no `clusters_routed` contribution, the remote lesson's integration is NOT persisted, and Step 5 item 4's removal gate correctly keeps its remote file in place). Only when a filed message IS found does the correction below apply.
   ...
   - **Supersede it** — the route when `amend` refuses the correction, because the message is no longer live and queued: the destination's drain has already archived it, or the verb returns `not_live` / `not_amendable`. ⛔ **Re-run Step 4 item 1's destination-phase guard before filing the successor.** Time has passed since the cluster's original Step 4 pass, so the destination epic can have closed in the interim; the successor write is a NEW `inbox write` and is subject to the same guard as any other. When the destination now reads closed or unreadable, do NOT file the successor and do NOT supersede — treat the cluster exactly as Step 4's own no-active-owner/closed-epic carve-out does (nothing persisted for this correction; the ORIGINAL message stays superseded-pending or unresolved, per whichever `amend` refusal reached this branch, and the remote lesson's file stays put per Step 5 item 4). Only when the guard passes does the successor write below proceed. File the updated cluster as a fresh `finding` message using Step 4 item 3's `write` invocation unchanged, capture the successor's bare filename, then retire the original in its favour:

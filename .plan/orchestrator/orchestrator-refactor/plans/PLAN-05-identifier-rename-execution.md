...
- HYPOTHESIS — `epic-surface-partition` is cross-bundle (`pm-plugin-development`) and its
  ...
  - verdict: corroborated | checked_at: 9588b30b317d0312ede90f1982122aa3145ea871 | by: orchestrator-refactor/cleanup | rescoped: n/a | evidence: Cross-bundle and intact: all four tools-epic-surface-partition artifacts tracked; epic-surface-partition.py:132 still calls get_store_dir('orchestrator', epic, allow_archived=True). Caveat re-confirmed and now worse: supporting citation truthful-signals/epic.md:2160-2166 is stale, AND PLAN-06's own 2026-09-23 correction has itself drifted -- D-SWEEP-b now lives at :2114-2120 (PLAN-06 idx5 says :2102-2108) and D-SWEEP-a at :2100-2112 (PLAN-06 idx4 says :2088-2100).
- HYPOTHESIS — an accepted-alias period is unnecessary because every caller is in-repo;
  ...
  - verdict: corroborated | checked_at: 9588b30b317d0312ede90f1982122aa3145ea871 | by: orchestrator-refactor/cleanup | rescoped: n/a | evidence: Checklist resolves exactly where cited: outline-workflow-detail.md:809 header, :815-822 four-condition table, :824-829 decision table. ADR-023(a):134-145 already took the clean break. Condition 2 re-derived live: architecture search --content '--slug' --literal --category source returns count:2/file_count:2 -- only orchestrator.py and platform_runtime.py:497 declare it, both in-repo; epic-surface-partition already on --epic. ADR-024 sets the matching no-shim precedent.
- Verify-first clause: ADR-007 records that the deleted-symbol / renamed-identifier
  ...
  - verdict: corroborated | checked_at: 9588b30b317d0312ede90f1982122aa3145ea871 | by: orchestrator-refactor/cleanup | rescoped: n/a | evidence: Every ADR-007 citation resolves, small shifts. :43/:47 deleted-symbol/deleted-path classes with no dedicated whole-surface detector; :49-50 a rename is a deletion plus addition, stale refs are the deleted-symbol case; :165-168 a future detector derives from source_fingerprint.list_tracked_files/hash_objects; :176 MUST NOT parse unified-diff text. ADR-023:269-274 independently repeats the constraint. D7's sweep remains this plan's own proof of completeness.
...
```text
/plan-marshall task="implement .plan/orchestrator/orchestrator-refactor/plans/PLAN-05-identifier-rename-execution.md"
```
...
edits NO file under `.plan/orchestrator/` other than its own `inbox/{sender}-{seq}` message — the orchestrator owns

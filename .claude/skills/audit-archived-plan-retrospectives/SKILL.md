...
description: Audit archived plans across twenty-one retrospective checks — execution-manifest correctness, quality-verification findings, metrics anomalies, cross-plan recurring patterns, token-efficiency trend, scope-estimate accuracy, track-selection-accuracy (actual planning track vs counterfactual correct track), PR-merge velocity, task-count efficiency, global-log analysis, token-economics, quality-chain, sequence-and-build-minimality, input-integrity corpus completeness, task-graph redundancy, architecture-lookup-ratio (information-lookup vs build-lookup), preference-pattern-detector (recurring user gate-dispositions → architecture hints), dispatch-topology (leaf/dispatch-topology invariant), finalize-flow-conformance (post-#849 deterministic ci_verify mechanics), merge-window-accounting (widened merge-mutex admission window), and cross-check-synthesis facet-completeness — file lessons through the three-gate policy, and dormate reviewed plans
user-invocable: true
mode: workflow
...
| `--check NAME` | optional | Run a single check instead of all. Valid names: `execution-context-manifest`, `quality-verification-report`, `metrics`, `recurring-pattern-detector`, `token-efficiency-trend`, `scope-estimate-accuracy`, `track-selection-accuracy`, `pr-merge-velocity`, `task-count-efficiency`, `global-log-analysis`, `token-economics`, `quality-chain`, `sequence-and-build-minimality`, `input-integrity`, `task-graph-redundancy`, `architecture-lookup-ratio`, `preference-pattern-detector`, `dispatch-topology`, `finalize-flow-conformance`, `merge-window-accounting`, `cross-check-synthesis`. Default: run every check. When `--check cross-check-synthesis` is selected, the script still computes the upstream checks it consumes (without emitting their blocks) so the synthesis can fire. |
...
(`merge_contention` — read against `checks/merge-window-accounting.md`), or a
...
`merge_window_ci_rerun` — read each against its qualifying caveat in

...
- **Narrow the claim** to what is asserted:
  `test_at_least_one_graph_edge_is_stamped_by_a_resolver_not_declared`, with a docstring stating
  the existential claim explicitly *and why universal would be wrong here* (some edges
  legitimately are declared). This is what shipped (`a8695473d`).
...
Adjacent to the standing rule that *N passing checks of a pure function is one assertion
repeated N times*: both are cases where the surface strength of a test overstates the

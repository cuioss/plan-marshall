...
class IntegrationContext:
    ...
    def validate_project(self, project: ProjectFixture) -> bool:
        ...
        if not project.exists(self.git_dir):
            self.skipped.append(f'{project.name}: Not found at {project.absolute_path(self.git_dir)}')
            return False
        return True
...
def assert_no_null_values(
    data: dict | list, path: str = '', allowed_null_suffixes: list[str] | None = None
) -> list[str]:
    ...
    for module in modules:
        ...
        for dep in module.get('dependencies', []):
            ...
            if len(parts) == 3:
                ...
                errors.append(
                    f"{name}: dependency '{dep}' should have format 'group:artifact:scope' or 'group:artifact:version:scope'"
                )
                continue
            if scope not in valid_dep_scopes:
                errors.append(f"{name}: dependency '{dep}' has invalid scope '{scope}'")
...
def assert_gradle_module_structure(modules: list[dict]) -> list[str]:
    ...
    for module in modules:
        ...
        if not (descriptor.endswith('build.gradle') or descriptor.endswith('build.gradle.kts')):
            errors.append(
                f"{name}: descriptor should end with 'build.gradle' or 'build.gradle.kts', got '{descriptor}'"
            )
...
def assert_has_root_aggregator(
    modules: list[dict], project_root: Path | None = None, root_descriptors: list[str] | None = None
) -> list[str]:
    ...
    if project_root and root_descriptors:
        has_root_descriptor = any((project_root / desc).exists() for desc in root_descriptors)

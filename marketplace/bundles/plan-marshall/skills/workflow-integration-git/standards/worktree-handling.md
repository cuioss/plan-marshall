...
0. **Move-back precondition (script-enforced).** The plan directory MUST already have been moved back to the main checkout (via `integrate_into_main`) before the worktree may be removed — until then the worktree holds the sole authoritative plan-state copy (ADR-002 move model), and removing it would destroy that copy. This is not a prose-only invariant: `worktree-remove` itself refuses with `error: plan_dir_not_moved_back` while the plan state has not landed back. The predicate is **main-anchored, not cwd-relative** — it resolves through `marketplace_paths.resolve_main_anchored_path`, the single sanctioned main-anchored resolver, asking for the live `plans/{plan_id}/status.json` that `integrate_into_main` lands and scanning `archived-plans/` for the `{YYYY-MM-DD}-{plan_id}/status.json` that `manage-status archive` writes. Anchoring the probe to main rather than to the caller's own tree is what makes the guard answer about the tree it protects: a probe that walked up from cwd would let a caller standing inside the worktree satisfy the guard with the very plan-state copy the removal is about to destroy. Using *that* resolver rather than a raw `main_checkout_root()` walk is what makes guard and producer agree: it is the same call `integrate_into_main` computes its destination with, and it honours `PLAN_BASE_DIR` / `set_base_dir()`, which a git-common-dir walk does not — under an active override the two name different trees, and a git-derived probe would refuse a plan whose state genuinely landed. (`main_checkout_root()` is still the right resolver for the verb's `git -C` target, which must name a real git checkout.) It fails **closed** — an unresolvable main anchor is never evidence that the move-back happened. The refusal is NOT overridable by `--force` (`--force` keeps its dirty-tree meaning only) — destroying the sole authoritative plan-state copy has no legitimate fast path; an abandonment flow moves or deletes the plan dir first, then removes the worktree.
1. **Remove the worktree.** The local branch cannot be deleted while it is still checked out in a worktree, which is what fixes this step's position in the order. The caller must also be standing outside the worktree, and that condition is enforced by us rather than by git: `git worktree remove` will delete a worktree that is a shell's current working directory — with or without `--force` — so `worktree-remove` applies its own containment test between `Path.cwd()` and the resolved removal target and refuses with `error: cwd_inside_removal_target` for the target itself or any directory beneath it. That refusal is independent of the move-back guard above (two separate defences, not one shared predicate), is likewise NOT overridable by `--force`, and is remedied by changing directory out of the worktree and re-running. Any uncommitted state in the worktree at this point is a fail-loud condition, and `--force` is never the salvage: forcing *instead of* salvaging destroys work git was never holding a copy of. Salvage first — the user may still want to recover it — and only then remove with `--force` deliberately. That deliberate second step is the one refusal the flag addresses at all (`worktree_remove_failed`; the two script-owned refusals above are not overridable by it), and it is the same sequence stated in the `worktree-remove` error-handling row in [`../SKILL.md`](../SKILL.md).

...
* **The pre-commit freshness gate** (`manage-tasks pre-commit-verify-freshness`) matches change-ledger rows on `status == "success"` rather than on `exit_code == 0`, and a row lacking `status` never matches — the gate fails closed to `stale` rather than admitting a row it cannot read. Its notation cross-check is the same rule applied one level up, to the *evidence* rather than the *status*: a matching row is compared against the build notations the project's architecture resolves, and the answer is kept three-valued. A refutation (the set resolved, and this row is not in it) fails closed; an inability to resolve the set passes but is recorded as `notation_cross_check: unverified`, never folded into `corroborated`, so "passed uncross-checked" stays distinguishable from "passed cross-checked". The same rule governs the gate's own status vocabulary: the route on which no build was owed — and no ledger row is read at all — returns its own status member rather than sharing the token that asserts a build observed the tree, so a consumer branching on the token cannot admit an unexamined tree. See [`../../manage-tasks/SKILL.md`](../../manage-tasks/SKILL.md) § "Pre-Commit Verify Freshness" for the member set. Rules (b) and (e).

...
- `work/metrics.toon` — the **persisted aggregate**: `totals_tokens`, `totals_worked_ms`, `totals_wall_ms`, each with its `{field}_population_count` companion, plus the shared `totals_population_denominator` and `totals_tokens_spans_populations`. **Read the totals from the record.** They are written by `manage-metrics generate` and are the same figures the report renders — see [`manage-metrics/standards/data-format.md`](../../manage-metrics/standards/data-format.md) § "The Persisted Aggregate". Re-summing the phase breakdown here would make this aspect a second producer of a figure the store already holds, free to pick a different population than the renderer did.
  ...
  ⚠ **When the `totals_*` keys are ABSENT, they were invalidated — do not re-derive them silently.** Only `generate` computes the aggregate, and every other *write* of the store drops the `totals_*` family, so the keys are present exactly when they still sum the rows beside them. (`phase-boundary` is not an exception to watch for: it regenerates as its own last step, so the verb leaves them present.) Absence means a later write moved the rows: re-run `generate`, or report the totals as unavailable. It never means "sum the phase breakdown yourself" — that is the second-producer defect this aspect is not permitted to commit.
...
Three truthfulness rules ride with it. All three are applied by the source block
...
- **Suspect-zero.** The `build_time` block reports `suspect_count` — builds whose
  ...
  and a number was published at all, the total is a **FLOOR**; say so rather than
...
- **`killed` is SEPARATE from `error`.** The block's `killed` count is an
  ...
  a red build.
- **Absent is not zero — and the PRODUCER now says so.** `build_time` emits the
  literal `unavailable` in place of `total_build_seconds` whenever no build row
  contributed a duration (`summed_rows: 0`), so this aspect **surfaces that
  sentinel verbatim** into `totals.total_build_seconds`. ⛔ Do NOT derive it here
  from `build_count == 0`: that test was the consumer-side rule this producer
  change replaced, and it disagrees with the producer on the suspect-only case
  (`build_count > 0`, `summed_rows: 0`) — where the old test would print a `0`
  the producer refused to emit. A `0` asserts a measurement nobody made and
  averages into every cross-plan roll-up as though the plan had built instantly.

  Quote the sentinel with the population that explains it. `build_time` publishes
  `ledger_present`, `ledger_readable` and `ledger_rows_scanned` beside the total,
  so an absent ledger, an unreadable one, and one that was read and held no build
  row for this plan are three distinguishable facts — the sentinel itself is
  identical in all three and names none of them.
...
totals:
  ...
  tokens: N
  total_build_seconds: N|unavailable
                           # READ VERBATIM from log_analysis.build_time (a FLOOR when
                           # suspect_count > 0). The producer emits the literal
                           # `unavailable` whenever summed_rows == 0; surface it, never
                           # re-derive it and never substitute a 0.
  files_modified: N
  tasks_completed: N
  deliverable_count: N

...
Call the `view_file` tool with `{ AbsolutePath: "standards/trusted-domains.md" }` before continuing.
...
**Error handling for missing/invalid files**: Ask the user via `ask_question` with options to create defaults, skip the file, or abort.
...
Call the `view_file` tool with `{ AbsolutePath: "standards/domain-security-assessment.md" }` before continuing.

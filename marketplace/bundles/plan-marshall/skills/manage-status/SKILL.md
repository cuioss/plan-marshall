...
**Phase closure and the unexaminable record.** Archive closes every phase recorded `in_progress`, leaves every `pending` phase alone, and writes `current_phase: complete` once the phase structure was read IN FULL and nothing remains open — see [status-lifecycle.md](standards/status-lifecycle.md) § "Phase-closure post-condition". `phase_closure` publishes which of those two paths ran on **every successful archive response**, in the same shape the `census` cohort row uses for its two enumerated verdicts: `complete`, or `partial` with a `phase_closure_reason` naming the shortfall. A refusal carries no `phase_closure` field at all — the no-reason unexaminable path below returns `status: error` before the phase-close write, so there is no closure to report. A record whose `phases` could not be read in full keeps the `current_phase` it already had rather than being written `complete` — an unknown open-phase set is not an empty one.

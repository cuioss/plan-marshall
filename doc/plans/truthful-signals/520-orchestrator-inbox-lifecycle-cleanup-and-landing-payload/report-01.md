...
| `pm-plugin-development:plugin-architecture` | `SKILL.md` / bundle structure — `plugin.json`, one `SKILL.md` (`plan-orchestrator`), and standards/workflow docs across six skills |
...
⚠ **The fourth figure, `16287`, is NOT cleanly re-derivable, and was left standing with its population rather than replaced.** A re-run over the same commit's extracted tree with `uv run python -m pytest test/plan-marshall/ -o addopts="" -n auto` produced **16281 passed, 10 failed, 1 skipped** — 16292 collected against the report line's 16291. The two runs use the **same test path** (`get_test_path('plan-marshall')` returns `test/plan-marshall`, which is what the re-run swept), so "different population" is NOT the reason and is not claimed. ⛔ Two mechanisms this note previously asserted are false and are withdrawn: `./pw module-tests` applies **no** coverage flag (coverage is a separate `cmd_module_tests` sibling, and `addopts` carries no `--cov`), and clearing `addopts` does **not** move the collected total. What actually differs is the xdist distribution: `./pw module-tests` passes `--dist=loadgroup`, which pins `xdist_group`-marked tests to one worker, while the re-run took the default `load`, which scatters them. Grouping being silently ignored is exactly what makes the **passed** count order-dependent — the same flakiness the report line itself records as producing 3 non-passes there and 10 here. The residual one-item collected delta has **no cause assigned**: 16291 is inferred as `16287 + 1 + 2 + 1`, and a pytest *error* can be a setup/teardown error on an item already counted, so that sum is not guaranteed to equal the collected count. Naming a cause for it would be the manufactured provenance this correction exists to remove. Substituting 16281 would report a number produced under a different distribution as a correction to this one, which is the manufactured-precision defect this epic exists to close. What was corrected is the figure's **provenance**: both of its sites now name the command and the base (`51d1c9bc`) and say it is a sample of an order-dependent quantity rather than a stable one. Recorded as a **survivor** in § Findings.

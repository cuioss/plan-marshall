...
> Staged plan spec. Surfaced 2026-07-22 by the operator's audit of the PLAN-29 (#969)
> platform-agnostic waiting standard: could *consistent usage* of the waiting seam ("using the
> background primitive for a CI-wait") be verified **from the logs**? It cannot — the mechanism the
> seam selects leaves no runtime trace. Source is clean (no general body names `Monitor`); the gap is
> **observability**, not a leak. **Migrated from plan-optimization 2026-07-22** (successor epic; plan
> id kept).
>
> **Grounded at `main` @ `dfc4ac15c` (2026-07-22).** Evidence captured at ground truth:
...
**Disjointness:** does NOT touch `manage-architecture` (PLAN-43), finalize steps (PLAN-44),
phase-1-init (PLAN-41), or plan-optimization's running `_markers_search.py` (PLAN-23) /
`manage-execution-manifest.py` (PLAN-35). The title-surface seam (PLAN-05/26/30) is shipped and D2
only *reads* its best-effort contract as a shape precedent, so no collision with landed work.

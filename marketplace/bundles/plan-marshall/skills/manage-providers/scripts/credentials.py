...
def main() -> int:
    ...
    # verify
    verify_parser = subparsers.add_parser('verify', help='Test credential connectivity', allow_abbrev=False)
    ...
    dap_parser = subparsers.add_parser(
        'discover-and-persist',
    ...
    # find-by-category
    fbc_parser = subparsers.add_parser('find-by-category', help='Find providers by category', allow_abbrev=False)
    fbc_parser.add_argument('--category', required=True, help='Provider category (e.g., ci, version-control)')

    # list
    list_parser = subparsers.add_parser('list', help='List configured skills (no secrets)', allow_abbrev=False)
    list_parser.add_argument(
        '--scope', choices=['global', 'project', 'all'], default='all', help='Credential scope filter (default: all)'
    )

    # remove
    remove_parser = subparsers.add_parser('remove', help='Remove credential and metadata', allow_abbrev=False)

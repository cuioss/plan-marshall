...
`.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/570-cloud-plan-lane-contract-proposals.md`. It was ingested into this ledger on
2026-08-22 as `PLAN-CIS-055` and it will be EMITTED INTO THE PLAN-MARSHALL LIFECYCLE, not into the
cloud lane.
...
NORMALIZED + RE-SCOPED 2026-08-24 (operator-directed, orchestrator spec-repair pass). The ingest left
the body verbatim and asserted that its lane references were "satisfied by the corresponding
plan-marshall phase artifacts". That assertion was NEVER TESTED and, for `doc/plans/` paths, it was
FALSE: the same ingest deleted `.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/` from git (163 deletions,
PR #1331). The table below is NORMATIVE for everything below it.
...
| `.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/{plan}/…` cited as EVIDENCE | the archived copy at `.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/{plan}/…`. ⛔ READ-ONLY — that tree is git-ignored, so it is a local historical record and can never be a deliverable. |
| `doc/plans/cloud-bridge.md` § Path 3, "may already have been collected and deleted" | RETIRED. The wave IS collected. |
| "author / stage a successor plan file under `doc/plans/`" | ⛔ NOT this plan's job. File an inbox message to the epic (`orchestrator inbox write --kind finding`); the ORCHESTRATOR stages plans. |

⛔⛔ **THIS SPEC NEEDED MORE THAN A TRANSLATION — READ THIS BEFORE THE BODY.** Every one of its
original outputs was a cloud-lane artifact written into the deleted tree, so it was the one wave spec
that was NOT emittable as written. It has been re-scoped rather than retired, because its SUBSTANCE
survives intact: **proposals P1–P8 target `.claude/skills/cloud-plan-lane/SKILL.md` and
`doc/plans/cloud-bridge.md`, and BOTH files are alive and git-tracked** (verified 2026-08-24). Three
changes, and they govern over any contrary sentence below:

1. **`proposals.md` (D1) now lands at the tracked path `doc/plans/cloud-lane-contract-proposals.md`** —
   beside the `cloud-bridge.md` it proposes changes to, so it is discoverable next to its subject.
   The plan still PROPOSES and does not apply: § Expected Surface's "not touched, expected to appear
   nowhere in the diff" list keeps `.claude/skills/**` out of the diff, and that is deliberate.
2. **`record-corrections.md` (D2–D4) is no longer a repository deliverable.** Its targets are the
   source plans' `report-01.md`, which now live ONLY in the git-ignored local archive. Corrections to
   a local record are the ORCHESTRATOR's to hold: file them as an inbox `finding` message
   (`orchestrator inbox write --slug code-intelligence-substrate --kind finding`) and reproduce them
   in the PR body. ⛔ Do NOT write into `cloud-runs/` — it is an immutable archive of what the runs
   actually said, and editing it destroys the evidence the corrections are ABOUT.
3. **`{this-plan}/plan.md` and `{this-plan}/report-NN.md` are struck entirely** — plan-directory
   lifecycle and run reports are the cloud lane's shape, supplied here by the plan lifecycle itself.

⭐ **D0(b)'s census gets easier, not harder.** It already defines a `collected` disposition for a
missing target directory. The targets are no longer missing — they are readable at
`cloud-runs/{plan}/`, so census rows resolve to `present` or `already corrected` against that archive.
⛔ Do not report `collected` merely because `doc/plans/…` is gone; look in the archive first.
-->

<!-- INGEST FACTS (from the 2026-08-22 landed-corpus audit)
gap_coverage: 32 gaps, 0 high
deliverables: 5
workstream: WS-07
...
⚠ **RE-EXPRESSED 2026-08-24** — see the re-scope block in the provenance header, which governs. The
original list put every output inside a cloud-lane plan directory under `doc/plans/`; that tree is
deleted. This run writes exactly ONE repository file. The containment gate stays mechanical — it is
now "one tracked file, and nothing else in the diff", which is a stronger claim than the original.

- `doc/plans/cloud-lane-contract-proposals.md` — **new; D1**, the proposal register. The single
  repository deliverable of this plan.

**Written OUTSIDE the repository** (so it appears in no diff, and the containment gate above still
holds): the D2–D4 record corrections, filed as an epic inbox `finding` message via
`orchestrator inbox write`, and reproduced in the PR body under one clearly-headed section. ⛔ The
corrections are NOT written into `cloud-runs/` — that archive is the evidence they are about.
...
## Claim Labels
- verdict: contradicted | checked_at: 91a07aaa4 | by: code-intelligence-substrate/analyze | rescoped: no | evidence: REFUTED AT HEAD, and not merely unreadable. This spec's premises rest on the doc/plans/ tree, which DOES NOT EXIST at 91a07aaa4 - ls doc/plans/ returns No such file or directory, so doc/plans/README.md, doc/plans/cloud-bridge.md and doc/plans/cloud-lane-contract-proposals.md, three of this spec's four declared Expected Surface entries, are gone. That refutes the HYPOTHESIS claims about the fourteen source plan directories and the quoted defective sentences, plus every OBSERVED claim whose confirm/refute artifact is cloud-bridge.md. ONE ARM SURVIVES: the asserted-absence claim was re-run as the spec demands and HOLDS - grep -c prerequisite over .claude/skills/cloud-plan-lane/SKILL.md returns 0 and that file does exist. rescoped=no because this pass did NOT re-scope it; it must be re-scoped against the surviving lane skill alone, or retired, before it is emittable. Deliberately left BLOCKING - the gate doing its job on a spec whose world moved.
...
| Each of the fourteen source plan directories still exists with the documents this plan names | HYPOTHESIS | a listing of `.plan/local/orchestrator/code-intelligence-substrate/cloud-runs/`; **D0 settles it per target** and a `collected` state is a recorded outcome, not a failure |

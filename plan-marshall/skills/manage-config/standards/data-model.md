...
- **Domain inclusion (optional)**: A `skill_domains.{domain}` entry MAY carry two additive inclusion keys — `always_on` (bool, default absent ⇒ `false`) and `file_globs` (list[str], seeded from the owning domain extension's `provides_file_globs()`; absent when that accessor returns empty) — that union the domain into a plan's `references.domains` set unconditionally (`always_on`) or on a file-glob match against the plan's affected files (`file_globs`). The defaults are asymmetric: `always_on` is genuinely absent by default (no seed), while `file_globs` is seeded at the `convert_extension_to_domain_config` seam whenever the key is absent, so an already-configured project is backfilled on its next `configure`; an operator value set via `set-inclusion` wins over the seed. Neither key is written into `get_default_config()`, so `sync-defaults` neither adds nor wipes them. Both are validated by `validate_domain_inclusion` (`always_on` must be a bool; `file_globs` must be a list of str) and preserved across a `skill-domains configure` reconfigure. The seeded globs are domain-detection knowledge only and contribute no `build.map` route. See [skill-domains.md § Domain Inclusion](skill-domains.md).

...
Plan Marshall is distributed via a `dist-claude` orphan branch (snapshot, tracks `main`) and immutable `claude/v{x.y.z}` dist tags (pinned releases). See link:doc/distribution.md[doc/distribution.md] for the full install/update/versioning contract.
...
Refresh later with `/plugin marketplace update plan-marshall`.
...
Then install additional bundles (e.g. `/plugin install pm-dev-java@plan-marshall`) per Claude Code's plugin tooling.
...
|`plan-marshall`
|Core infrastructure, permissions, script execution, and 5-phase planning workflow with task execution

# PLAN-PR-024: Every participation credit is anchored to the commit being merged, and an unanchorable credit says so

epic: review-apparatus
workstream: WS-01

> Staged plan spec, ingested 2026-08-23 from the cloud-lane corpus (``PLAN-PR-024` (this directory)`,
...
Anchor every participation credit to the commit being merged, and make an unanchorable credit say so.
...
Every load-bearing Problem-section claim reproduces at HEAD; the citations are unusually accurate.
The headline defect was traced end-to-end at HEAD and behaves exactly as described.

⛔ **Correct before handing this spec to a run:**

1. **D0 cites a section that does not exist.** `workflow-integration-github/SKILL.md` § "the
   comment-path matcher" returns zero hits. The real section is § `github_ops pr wait-for-comments`
   (`:348`, movement arm `:360`), and the verb is `github_ops pr …`, not `github_pr`.
2. **D0's second mandatory classification has no stated destination.** The `_is_self_authored_response`
   answer says only "record it as the third comment-identity in force" — no file. If a run writes it
   into `bot-participation-contract.md` it creates a passage the shared-document table does not name.
3. **A third shared contract surface exists and no split governs it**:
   `workflow-integration-github/SKILL.md` § `github_pr fetch_findings` is written by this plan (D3, D4)
   AND by PLAN-PR-025 (D2, D6). Add it to § Notes' shared-surface list.
4. **Line drift, fixable:** `:701-702` -> `:701,703`; `~1113-1118` -> `:1114-1120`;
   `~1266-1270` -> `:1267-1271`.

⚠ **Scope:** 6 deliverables, at the epic's guard. Recommended split — (a) D0+D1+D2+D3, the currency
predicate and its signal, one loop; (b) D4+D5, the dedup identity and the rename, which the plan itself
already orders last as independent.

## Dependencies and Sequencing

- **Runs FIRST of the participation trio.** PLAN-PR-025 D6 must describe the predicate the code
  implements, and this plan changes it; PLAN-PR-025 must then describe the ledger under the name this
  plan's D5 gives it. Running this first removes both conditional branches from that plan.
- **MUST NOT run concurrently with PLAN-PR-025** — three adjacent bullets of one list in
  `bot-participation-contract.md` § "Evidence for a bot that edits one comment in place" (`:230-237`)
  belong to the two plans, inside a single git hunk.
- **MUST NOT run concurrently with PLAN-PR-029** — `github_pr.py` and `test_github_pr.py` are claimed
  by both, code plus its pinning tests.
- Overlaps with: PLAN-PR-025 (`bot-participation-contract.md`, `github_pr.py`,
  `workflow-integration-github/SKILL.md`), PLAN-PR-030 (§ Consumers table), PLAN-PR-026 and
  PLAN-PR-028 (`branch-cleanup.md`).
- Adjacent to: PLAN-PR-031 D4, which cold-READS two sections this plan rewrites. Run this first so the
  cold read grades the landed text.
...
`../cloud-runs/010-participation-credited-from-a-superseded-commit/gaps.md` and
`../cloud-runs/110-participation-derived-from-a-lossy-view/gaps.md` — which carry the
...
3. **D2 — Make every arm of the currency predicate fail closed on degenerate input** — discharges
   ...
   - **Ledger reader**: ⛔ **AMENDED 2026-08-25 (pre-emit amendment 6 of `cloud-wave-audit.md` § 9) —
     the previous wording prescribed the NON-FIX `010 G8` explicitly names.** It said to treat a row
     whose `reviewed_commit_sha` is missing or empty as *no usable record* and drop it from the map.
     G8's Task rules that out in as many words: *"Dropping such a row from the map is NOT a fix: an
     absent key takes the first-observation arm, which returns `bool(merge_candidate_sha)`
     (`github_pr.py:701-703`) and so credits the bot at any resolvable advanced HEAD — the very
     fail-open path this gap names."* Dropping the row only reaches a safe answer if this
     deliverable's first bullet also succeeds, and that bullet's DECIDED fallback is to keep today's
     credit-on-a-resolvable-SHA behaviour when the commit timestamp cannot be read — so on the failed
     read the two changes compose back into the fail-open path.
     **Do instead — one of the two G8 sanctions, not the drop:** carry an **invalid legacy record**
     state through `_recorded_currency_records` — a third state distinct from both "no record" and "a
     usable record" — and make the predicate return `False` for it; **or** version the artifact and
     migrate pre-upgrade rows to a real `(sha, updated_at)` pair before the participation loop
     evaluates them. This is the pre-upgrade key-only row the earlier artifact wrote (visible in
     `git show 50f67ed2`), and there is no schema-version guard today.
   ...
   *Done when:* three tests pass and each is recorded as failing against the pre-change code — (a) a
   ...
   `reviewed_commit_sha` is written; (c) ⛔ **AMENDED to `010 G8`'s own wording — a ledger carrying
   only pre-upgrade key-only rows resolves the bot to `participated_stale` at an advanced resolvable
   HEAD, NOT to `participated`, pinned by a test that fails against the current reader.** The former
   clause ("produces the same verdict as an empty ledger") is retired: it certified the drop-the-row
   non-fix, because an empty ledger is precisely what takes the fail-open first-observation arm. The
   test must assert the RESOLVED STATE, never an equivalence between two inputs.
...
An asserted **absence** is verified exactly as an asserted presence, and is the higher-risk half.
  - verdict: corroborated | checked_at: f6d058b4b | by: review-apparatus/cleanup | rescoped: n/a | evidence: Re-grounded at HEAD f6d058b4b (advance from 26f2f417b = PRs #1332, #1335, #1334) by DERIVATION, not re-audit: 22 paths moved, intersected against the WHOLE spec file rather than a parsed Expected Surface section. Two section-parsers disagreed on which specs intersect, so the whole-file scan was used as the fail-closed superset. Intersects .plan/marshal.json (sole change: a url field removed from the GitHub provider entry) and automatic-review/standards/pr-agent.md (this session #1334: an added ignore_patterns entry plus rewritten participation_evidence prose). Neither touches the currency-subject/subtraction passage D1 names. The 2026-08-23 audit verdict stands: confirmed, 4 spec defects to correct before a run, split 2-way recommended.

## Verification

Beyond each deliverable's *Done when*:

- **Build gate.** This plan changes Python, so run the repository build and report its result.
  ⛔ Resolve the command via `architecture resolve` and run the returned executable — **never
  hard-code `./pw`** (CLAUDE.md § Workflow Discipline). Canonical form:
  `python3 .plan/execute-script.py plan-marshall:build-pyproject:pyproject_build run --command-args "verify"`,
  with a Bash timeout of at least 600000ms. Do not treat a wrapper exit code as the verdict — read the
  returned TOON `status` / `errors[]`, because the wrapper exits 0 even on failure.
...
- **Sequencing against plan `510`.** `510-a-refusal-is-recorded-as-a-refusal-and-the-contract-says-so`
  ...
  **`bot-participation-contract.md` is shared.** Do not resolve ownership from this plan: read
  the table in [`../cloud-runs/README-as-exported.md`](../README.md) § "The shared-document
...
```text
/plan-marshall task="implement .plan/local/orchestrator/review-apparatus/plans/PLAN-PR-024-participation-credit-anchored-to-the-merge-candidate.md"
```

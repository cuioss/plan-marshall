...
```text
Call the `read` tool with `{ filePath: "standards/weld-testing-autowired.md" }` before continuing.
```
...
```text
Call the `read` tool with `{ filePath: "templates/cdi-test-class.java.tmpl" }` before continuing.
```

# PLAN-PR-029: The comment pipeline loses findings and repeats replies
...
> shared target surface; D1–D5 are carried there as D1–D5 and D0 folds into that plan's merged D0 gate.
> ⛔ **This file is NOT dead and is NOT deleted**: it remains the AUTHORITATIVE TEXT of every
> deliverable body — including the cloud-wave audit text preserved verbatim below and the six
> § Re-Grounding corrections, which bind there unchanged — and `PLAN-PR-059` points here rather than
> retyping it.

epic: review-apparatus
workstream: WS-03

> Staged plan spec, ingested 2026-08-23 from the cloud-lane corpus (``PLAN-PR-029` (this directory)`,
...
Stop the noise pre-filter destroying real findings, and stop a delivered reply being re-sent and counted untransmitted.
...
Every load-bearing Problem-section claim reproduces at HEAD, several at exact line. The `ignore.low`
lead (12 entries, exactly 4 unanchored) is exact. Both spine defects are live and unambiguous, and
this is the plan holding the wave's clearest correctness bugs.

⛔ **Correct before handing this spec to a run:**

1. **A declared Expected-Surface path DOES NOT EXIST:**
   `test/plan-marshall/manage-findings/test_findings_store.py`. D3's clear-on-change tests belong in
   `test_findings_store_resolve.py`.
2. **D4's Done-when quotes the landed report inexactly** — the report says "**the** block", the plan
   searches for "**this** block". A literal check returns a FALSE GREEN on the correction.
3. **D4's `Prompt for AI Agents` population omits `sourcery.md`** (`:182-188`), which carries the same
   trust-boundary section. It already states STRIP, so the clause passes on substance — but the run
   will hit an undeclared file.
4. **D0(a)'s lead of "two invocation blocks" is materially low** — at least **four** provider-scoped
   surfaces exist outside the GitHub catalogue. D5 item 3's enumeration will be larger than anticipated.
5. **A claim label is wrong as written:** the STRIP-rule absence sweep returns **6 marketplace hits**,
   not zero. The substantive absence (no test, no analyzer) holds; the label would read as refuted by
   the first person to re-run it.
6. ⛔⛔ **Fold in `070 G1`'s THIRD requirement — this is the wave's one substantive coverage hole.**
   D2 as written enacts exactly the half `070 G1` warns against: moving the marker earlier closes the
   duplicate-reply hole and opens a second one, where the `already responded` guard skips the finding
   on every later round and a thread whose resolve mutation failed **stays unresolved forever**. D2's
   Done-when then CERTIFIES that outcome. Add: persist the resolve state separately (a
   `resolve_pending` field or equivalent) and give the verb a resolve-only branch that re-attempts
   `RESOLVE_THREAD_MUTATION` **without** re-sending the reply; reword the Done-when so
   "an already-responded skip on the second pass" applies to the REPLY, not to the resolve.
   Verified absent from the whole staged series.

⚠ **Scope: 6 deliverables, at the guard. Split NOT recommended** — D0 gates D3 and D5, and D1/D2 share
`github_pr.py`. If forced, cut D4 (documentation-only, disjoint file set) into its own plan.

## Dependencies and Sequencing

- **MUST NOT run concurrently with PLAN-PR-024 or PLAN-PR-025** — `github_pr.py` and
  `test_github_pr.py` are claimed by all three: code plus its pinning tests, a harder collision than
  the documentation contention the shared-document table flags.
- May run concurrently with PLAN-PR-028 — disjoint files, disjoint symbols.
- Overlaps with: PLAN-PR-031 (`coderabbit.md`), PLAN-PR-030 (`gitlab_pr.py`).
- Adjacent to: PLAN-PR-034, which fixes the case where NEITHER refusal layer fires — a different
  failure mode from this plan's noise pre-filter, on the same function family.
...
## ⭐⭐ FOLDED 2026-09-13 — the classifier's unit is the BODY, its evidence is the OPENING LINE

Transferred from `truthful-signals` (`-056` item 1), first-party on PLAN-TRUTH-139 / PR #1479. A second
pre-filter with the same shape as D1's, on a different field: `review_body_summary_patterns` strips
CodeRabbit's `Actionable comments posted: N` boilerplate, and it is **right about the boilerplate and
wrong about the payload** — it classifies the WHOLE body by its opening line, so a `review_body` whose
substantive content sits *beneath* the status line is recorded as **meta**.

⛔ **Measured cost on one PR**: two such bodies carried real claims and both were classified meta —
`f7de42`, a **Major** and the most consequential single finding of its round (a corrupt
`build-queue.json` erased by a READ, after which the next admission over-admits past the cap), and
`bde483`, two accepted nitpicks. ⇒ `actionable_count` read **15 when the substantive output was 17**,
and `escapes_total` (15) excluded both: **the round's most consequential finding is in neither the
actionable count nor the gate-escape set.** The honest denominator is 17 — 16 fixed, 1
verified-and-declined, 0 wrong.

⭐ **Root cause, stated exactly**: *a pattern designed to recognise a PREFIX is being used to decide the
disposition of the WHOLE document.* Remedy: **match the status line and STRIP it**, then classify the
remainder — a body with no remaining substantive content is meta; a body with remaining content is
actionable and that content is the finding. ⛔ This is D1's own defect on the `review_body` field, so
the fix lands with D1 or not at all: correcting one pre-filter while the sibling keeps the same
premise leaves the pipeline lossy by the same mechanism.
...
5. ⛔⛔ **PRE-EMIT AMENDMENT 2026-08-25 (amendment 1 of `../cloud-wave-audit.md` § 9) — PERSIST THE
   RESOLVE STATE AND ADD A RESOLVE-ONLY RETRY BRANCH. Without this item D2 enacts the exact half
   `070 G1` warns against.** Items 1–4 move the marker earlier and *report* the resolve failure, but a
   report is not a retry path. `070 G1`'s Task is explicit that the split has a third requirement:

   > ⛔ Moving the marker earlier closes the duplicate-reply hole but opens a second one: with the
   > finding marked responded, the `already responded` guard skips it on every later round, so a
   > thread whose `RESOLVE_THREAD_MUTATION` failed stays unresolved forever. … So the split MUST also
   > persist the resolve state separately from the responded marker — a `resolved`/`resolve_pending`
   > field on the finding, or an equivalent — and give the verb a **resolve-only branch** that
   > re-attempts `RESOLVE_THREAD_MUTATION` for a transmitted-but-unresolved finding **WITHOUT
   > re-sending the reply**.

   So: persist resolve state on the finding **separately from the responded marker** (the two are
   different facts and one field cannot carry both), and give `cmd_post_responses` a branch that, for
   a finding marked responded with resolve still pending, re-attempts ONLY the resolve mutation.
   Apply the identical change to the GitLab twin. `resolved_on_provider: false` in the RETURN (item 2)
   is the report; this item is the durable state and the recovery path.

*Discharges:* 070-G1, 070-G7, 070-G12.

*Done when:* ⛔ **AMENDED 2026-08-25 — the former clause CERTIFIED the defect.** It required "exactly
one `THREAD_REPLY_MUTATION` call in total plus **an already-responded skip on the second pass**". An
already-responded skip on the second pass is precisely the permanently-unresolved thread `070 G1`
names: the test would have passed *because* the resolve was never retried, pinning the new hole as the
expected behaviour. Replaced with the resolve-only-retry assertion below — the second pass must NOT
skip; it must re-attempt the resolve and nothing else.

a test stubs a succeeding `THREAD_REPLY_MUTATION` and a failing
`RESOLVE_THREAD_MUTATION`, runs `cmd_post_responses` twice, and asserts **exactly one**
`THREAD_REPLY_MUTATION` call in total AND **a second `RESOLVE_THREAD_MUTATION` attempt on the second
pass** — the reply is never re-sent and the unresolved thread IS retried; a third pass with the
resolve stubbed to succeed clears the pending state and the finding then skips; a second
...
`marketplace/bundles/` states STRIP, or (in `cuioss-review-bot.md`) states that no such block is emitted; and
...
- `marketplace/bundles/plan-marshall/skills/workflow-integration-gitlab/SKILL.md` and
  `marketplace/bundles/plan-marshall/skills/workflow-integration-gitlab/standards/comment-patterns.json` — the `detail` claims (D4), the skip-reason row (D3).
- `marketplace/bundles/plan-marshall/skills/workflow-integration-sonar/scripts/sonar.py` and
  `marketplace/bundles/plan-marshall/skills/workflow-integration-sonar/SKILL.md`, `marketplace/bundles/plan-marshall/skills/workflow-integration-sonar/standards/sonar-rules.json` — the marker docstring and over-claim (D5), the `detail`
...
- `marketplace/bundles/plan-marshall/skills/manage-findings/standards/jsonl-format.md` and `marketplace/bundles/plan-marshall/skills/manage-findings/SKILL.md`
  — the marker's field documentation and § resolve (D3).
- `marketplace/bundles/plan-marshall/skills/automatic-review/standards/coderabbit.md` and `marketplace/bundles/plan-marshall/skills/automatic-review/SKILL.md` —
...
- `test/plan-marshall/workflow-integration-github/test_github_pr.py`,
  ...
  `test/plan-marshall/manage-findings/test_findings_store_resolve.py` — the tests each deliverable
  names. ⭐ Corrected 2026-08-24: the spec previously declared `test_findings_store.py`, which does not
...
- `../cloud-runs/070-post-responses-retransmits-already-sent-replies/gaps.md` and its
  sibling `verification.md` — gaps G1 through G13.
- `../cloud-runs/100-coderabbit-ai-agent-block-strip-vs-extract/gaps.md` and its sibling

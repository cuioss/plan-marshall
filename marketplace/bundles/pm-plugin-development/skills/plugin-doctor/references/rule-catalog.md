...
**Activation**: Analyze-surfaced only. Registered in `doctor-marketplace.py::cmd_analyze` (via the runner's marketplace-wide pass), **NOT** in `cmd_quality_gate` — it surfaces findings without failing the build, mirroring the analyze-surfaced agentfile-hygiene backstop rules. The rule is registered NON-gating initially so the currently-deferred unreachable dialogues (phase-2-refine Step 11's in-envelope confidence-loop clarification, the edge phase-1-init init-flow gates, the `execute-task` per-task gates) are visibly surfaced as findings rather than silently passing green, without exploding the migration scope or breaking the build. Promotion to `quality-gate` (build-failing) is a follow-up once those deferred call sites have been migrated to the orchestrator-owned pattern.

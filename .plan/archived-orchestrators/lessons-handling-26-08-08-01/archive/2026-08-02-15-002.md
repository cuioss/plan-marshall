...
Applies to every precedence, override, fallback, or dedup stage — anywhere one source silently
wins over another. Recognition signature: two adjacent surfaces both report success with
mutually incompatible quantities, and no third surface reconciles them. A disagreement that no
artifact records is a disagreement no test can catch.

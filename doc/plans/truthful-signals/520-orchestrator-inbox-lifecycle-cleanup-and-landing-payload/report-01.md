...
| R2-2 | `cleanup.md` Step 8 lead-in **mandated the ordering its own rationale warns against**: it said do the migration first, then gave as the reason a hazard that only arises if you do the relocation second. The document's physical block order is the opposite of the mandate too. Same defect family — list order opposite to mandated order — that cold read 2 caught inside this very step, reintroduced by the fix for it. | **fixed** — relocation first, then migration, with the correct reason |
...
| R2-5 | The **S1 provenance note asserted an invented mechanism, at both its sites**: `./pw module-tests` applies no coverage flag, and clearing `addopts` demonstrably does not move the collected total (17710 either way). The two runs sweep the same path. The load-bearing half of S1's offered bound was fabricated — the same defect class round 1's finding 3 caught. | **fixed** — rewritten on the verified mechanism (`--dist=loadgroup` vs default `load`, read from `build.py`), with the collected delta carrying no assigned cause |
...
Round 3's most useful output is not a finding. Across three rounds the defect **count** fell (19 → 14 → 10 → 12, counting round 1's table by the one-finding-per-instance rule this section opens with, which makes it 19 rather than 18) while the defect **class** stayed perfectly stable: *each round's own corrections carried a fresh unverified mechanism.* Round 1's was the `--default` tier claim. **Round 2 authored two** — the O(queue) cost bound and the `loadgroup` causal sentence — and the "*non-binding* max" parenthetical whose complement R3-3 found missing was also round 2's (`git log -S` places all three at `c077d177`). ⚠ **An earlier version of this paragraph attributed the latter two to round 3; that was wrong, and V4-6 caught it.** Round 3's own corrections are the contents of `7e4c1734`, and round 4 found three fresh unverified mechanisms in them — a false universal about relocation ordering, an exhaustive-sounding effort enumeration missing the scalar shorthand, and an inference re-asserted as fact. So the generator is **four rounds for four**, measured on each round's own commit.

**The response is structural rather than another correction.** S1's causal sentence has been *removed* rather than rewritten a fourth time: the note now states what differs and explicitly declines to say which difference caused what. A claim that is not made cannot be the next round's finding. Where a mechanism IS asserted in this round's fixes, it was executed first — the binding-`max` divergence was run before being written down, not after.

**Three pre-existing defects surfaced and deliberately NOT fixed** (out of this plan's scope, recorded as residue): `cleanup.md`'s `## Output` block declares no field for a deferred relocation or for a migration having fired; `applied[]`'s `{spec,finding_class,…}` shape cannot carry a relocated `epic.md` **section**, which has neither; and `build.py` cites a `real_marshal_json` xdist group that exists nowhere in the tree (verified — it appears only in two of its own docstrings). All three predate this branch.
...
| V4-6 | § "The generator, named" **mis-attributed both of round 3's cited mechanisms to round 3**. `git log -S` places both at `c077d177` — they are round 2's. The section's headline analytical claim was unsupported by its own evidence. | **fixed** — attribution corrected, and the count restated as four-for-four measured on each round's own commit |
...
### The generator, restated after round 4

Round 3's structural response **failed**, and the failure is instructive. Declining to name a *cause* for S1 did not stop the generator; it moved it. Round 3 instead asserted a **negative** mechanism (`cleanup.md`: "neither order can corrupt the other"), an **exhaustive enumeration** (`orchestration-model.md`: "three configurations"), and a **bare figure** (`16291`, hedge deleted). All three are the same class: *a clause whose truth was not established before it was written.* Withholding a positive causal claim while asserting a negative one, or a completeness one, is not withholding a claim.

The rule this round applies is therefore wider than round 3's: **an assertion is not just a stated cause — it is any clause that could be false, including a negation, a completeness claim, and a figure carried without its derivation.** Where round 4's fixes assert anything, it was executed first (the scalar-shorthand divergence was run, not reasoned); where nothing could be established, nothing is now said in either direction.
...
**Third pass, against the shipped text.** It took the intended reading on **all four** scenarios put to it, including the per-branch zone placement that R3-2 had just corrected — so that fix holds. It found three further defects, all mine:

1. **The ordering rationale was false — and it was the R2-2 fix.** I had written that relocation-after-migration "could sweep content the migration has just placed there into `settled.md`". Relocation moves only sections whose subject is **closed**; a queue annotation the migration has just written is live by construction, so it could not be swept regardless of order. **This is the generator firing a fourth consecutive time**, and the response is again structural: the invented cause is removed rather than replaced. The order stands, stated as a determinism choice, with no dependency claimed.

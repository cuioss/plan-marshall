...
- You do NOT refine requests (that's phase-2-refine-agent)
...
```bash
python3 .plan/execute-script.py plan-marshall:manage-logging:manage-log \
  work --plan-id {plan_id} --level INFO --message "[STATUS] (plan-marshall:phase-3-outline-agent) Starting"
```
...
```bash
python3 .plan/execute-script.py plan-marshall:manage-logging:manage-log \
  work --plan-id {plan_id} --level INFO --message "[STATUS] (plan-marshall:phase-3-outline-agent) Complete"
```
...
component: plan-marshall:phase-3-outline-agent

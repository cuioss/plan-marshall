...
**Verifying the markdown-components row.** The row fires wherever a deliverable touches markdown components, and consumer projects do carry them — under the target's project-local skill root (`.claude/skills/` on Claude; resolved via the platform-runtime `layout skill-roots` op) and the Claude `.claude/agents/` tree (a separate component root with no layout op). What verifies them differs, and the row names no single answer on purpose:

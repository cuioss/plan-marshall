# PLAN-PR-031: Every claim in the epic's own run reports resolves against the tree
...
> shared target surface; D1–D6 are carried there as D5–D10 and D0 folds into that plan's merged D0
> gate. ⛔ **This file is NOT dead and is NOT deleted**: it remains the AUTHORITATIVE TEXT of every
> deliverable body — including the cloud-lane audit text preserved verbatim below — and `PLAN-PR-060`
> points here rather than retyping it.

epic: review-apparatus
workstream: WS-03

> Staged plan spec, ingested 2026-08-23 from the cloud-lane corpus (``PLAN-PR-031` (this directory)`,
...
Make every claim in the epic own run reports resolve against the tree.

## Re-Grounding — verified against `e8324d241`, 2026-08-23

**Verdict: partially-confirmed.** This spec had never been verified before ingestion; the source audit wrote it
...
✅ **The gap citations are sound.** All 23 in-scope gap ids exist across 11 directories, and every
subject is described correctly — re-counted independently. *The plan about false citations contains no
false citation.*

⛔ **But four of its claims about the TREE are falsified, which is the same defect in the other
direction. Correct before handing this spec to a run:**

1. **D4 item 5 is FALSIFIED.** It asserts exactly one commit touches
   `marketplace/targets/pr_agent/target.py` and derives a two-bucket charter partition. There are
   **two** (`f5493b43` #1130 and `66b686bf` #1313), so the partition has **three** buckets. A stale
   count about to be written into a contract, by the plan whose subject is stale counts. (The plan does
   instruct re-derivation, so a compliant run catches it — but the body, the claim-label row and every
   derived figure are falsified as written.)
2. **D5 item 3 quotes a string that is not in the contract.** "⛔ This is a disclosure requirement, and
   it is NOT a block" does not exist. The current wording (`:1733-1735`, `:1743-1745`) is deliberately
   WEAKER — condition 6 carves CodeRabbit out — so forcing the quote would re-introduce a flat claim
   the contract has qualified.
3. **D5 item 4's premise is already satisfied.** The lane already enumerates three publish surfaces
   (`cloud-plan-lane/SKILL.md:1291`, table `:1296-1300` naming *Review summary bodies*) and carries a
   worked example of exactly the `060` failure. Reframe to the real residual: **why did a run miss a
   surface its own contract mandates?**
4. **D3 item 6 mis-describes one of its two targets.** `060`'s § Residue asserts the OPPOSITE of what
   the plan says (it explicitly labels the argument "argued, not mutation-measured"), so the Done-when
   is vacuously satisfiable and the instruction would rewrite a correct passage.
5. **D1 item 3's "five live defects" is wrong** — three code bugs (G1-G3), four counting the missing
   test. Restate by id, or the correction of a false count lands a false count.
6. **README check 1 FAILS.** Zero occurrences of `per the README table`, and no mention of the shared-
   document table at all, while D4 item 1 cold-reads two sections that table assigns to PLAN-PR-024 —
   a case the table names explicitly. (§ "Detecting a decline", also read there, is named by no row:
   report it, do not choose.)
7. **§ Notes under-enumerates its own epic** — it names three behavioural siblings where there are
   seven, omitting PLAN-PR-027, the actual owner of the very gap it hands off two sentences later.
8. **Minor stale citations:** a truncated quote (`120` row A), a wrong grep term, a "single word" that
   is not, a shared roster module labelled a test file.

⚠ **Scope: 5 deliverables + gate behind 36 sub-items across 14 files. Split along the
DISCHARGE-MEDIUM line**, which is also the review-posture line: D0-D3 are in-place record corrections
under the archived corpus (pure prose, no bundle files, no Python); D4-D5 write bundle standards and
produce contract proposals, which this plan's own § Verification says must be reviewed like code. Two
audiences in one PR is the objection its own § Out of scope raises. No gap id spans the two halves.

⭐ **D5 shrinks on ingestion.** Its `cloud-plan-lane` re-anchoring is now PLAN-PR-032's, which
re-anchors AND applies. Drop D5 items 1-3 from this spec.

## Dependencies and Sequencing

- **Run AFTER PLAN-PR-030** — both edit `090-…/report-01.md`, and that plan's contract fixes should
  land before this one appends to `bot-participation-contract.md`.
- **Run AFTER PLAN-PR-024** — D4 item 1 cold-reads two sections that plan rewrites; running after is
  what makes the cold read grade the landed text rather than the superseded text.
- May run concurrently with PLAN-PR-026, -027, -028, -029: its only code-adjacent surface is a
  transient mutation it reverts, and its contract writes append to a section none of them touch.
- ✅ **The mutation re-derivation is live**: `review_completeness.py` and `test_structural_refusal.py`
  are byte-identical at HEAD to plan 120's landing commit `9e9e9880`, so D2 item 5's drift escape hatch
  is INERT — "cannot be re-derived" is not an available outcome.
- **Fold in from the ingestion audit:** `120 G2`'s and `130 G4`'s report halves, which no plan claims.
- Overlaps with: PLAN-PR-030 (`090-…/report-01.md`), PLAN-PR-029 (`coderabbit.md`), PLAN-PR-025 and
  PLAN-PR-026 (`review_completeness.py`, `test_structural_refusal.py`).
...
⛔⛔ **FOLDED IN 2026-08-25 — a MEASURED instance of this plan's exact subject, from inbox
`plugin-doctor-detector-coverage-residue-001.md` finding 2.** A re-fired `automatic-review` leaves its
second round's comments UN-INGESTED, and `finalize-step-review-retrospective` then reports a confident
verdict over the incomplete store.

On PR #1343 `automatic-review` fired twice: round 1 at HEAD `981a3bc9` ingested 15 pr-comment findings,
all stamped `reviewed_commit_sha: 981a3bc9`; round 2 at merged HEAD `98f8bc27` produced 12 new comments
(`wait-for-comments` `new_count: 12`, baseline 12 → final 24). **Only round 1's comments reached the
finding store.** PR-comment ingestion is internal to `automatic-review`'s workflow body and no standalone
verb backfills it (`manage-findings ingest` is finding-to-lesson promotion and returned `skipped`).

⭐ **The consequence is precisely this plan's thesis, and it lands in the FLATTERING direction.**
`finalize-step-review-retrospective` declares `order: 990` *specifically* so it runs after the merge gate
has finished filling the store — its own SKILL.md says that ordered earlier it "would compare reviewers
over a store the gate had not finished filling and report a confident verdict about evidence that did not
exist yet." A re-fire defeats that guard **from the other side**: the step runs late exactly as designed,
over a store missing the later round. And round 2 is where CodeRabbit found the merge blocker — so the
comparison understates the reviewer precisely where it performed best.

**Remedy (either, or both as defence in depth):** re-ingest on every `automatic-review` firing, OR have
`review-retrospective` assert `max(reviewed_commit_sha) == merged HEAD` and **publish the gap** when it
does not. The second is this plan's idiom: a verdict that cannot state its own coverage is the defect.
...
1. **Enumerate the corpus.** List `../cloud-runs/` and record every plan **directory**
...
4. **HALT conditions.** Stop the run and report it **blocked** if any of these holds:
   - `../cloud-runs/` cannot be listed, or carries no plan directory at all;
...
**130 G15**, plus the REPORT HALVES of **120 G2** and **130 G4**.

⛔⛔ **PRE-EMIT AMENDMENT 2026-08-25 (amendment 7 of `../cloud-wave-audit.md` § 9) — TWO FURTHER ITEMS,
and both are the REPORT half only.** `120 G2` and `130 G4` each carry a code half owned elsewhere and
a record half that belongs here, and the record half is this plan's exact subject: a written account
that states a reason which was never the reason.

- **`120 G2` — correct the F29 row's stated reason.** The gap is a latent `group(1) is None` crash in
  `_extract_rate_limit_eta`; ⛔ **the code fix is NOT this plan's** (it belongs with the refusal
  surface). What IS this plan's is G2's own second clause — *"Correct the F29 row's stated reason"* —
  because the record justifies the deferral on a ground that does not hold. Restate the reason from
  what was actually true, under D2's governing rule: a figure or reason simply **wrong about a
  commit** is restated from a re-derivation now.
- **`130 G4` — restate the selection-effect sentence in `report-01.md` § D2.** The code and contract
  halves (rewriting `_PROVENANCE`'s selection-effect clause across five sites) are owned by the delta
  surface, **not here**. G4's Task names the sixth site as this plan's: *"Restate the same sentence in
  `report-01.md` § D2 — a record correction stated in place, with no correction note and no dated
  entry, exactly as G14 and G15 prescribe for that file."* ⭐ Note the two conventions already settled
  above apply verbatim: **in place, no correction note, no dated entry** — the record reads as the
  corrected statement, not as a changelog of its own repair.

⚠ **Both items are RECORD corrections and neither licenses a code change.** If the run finds the code
half unlanded, it reports that and still corrects the record — the two are independent, and a record
that states a false reason is a defect whether or not the code beneath it has moved.
...
5. **120 G8 — a mutation row counted beside the result rather than from it.** The report's
   ...
   **7** failures "all in case (a)". The audit re-ran it and observed **9**, one of them in case (c)
   ...
   1. Derive plan 120's landing commit from git — the commit that **added**
      `../cloud-runs/120-review-barrier-deadlocks-on-a-refusing-bot/report-01.md`.
...
6. **020 G18 — the owed API-Sheriff re-review.** Plan 020 carried a check "so it cannot lapse", and
   ...
   `automatic-review/standards/cuioss-review-bot.md`, alongside the reviewer pack whose confirm/refute it is,
...
- **Plugin cache sync.** This plan edits `marketplace/bundles/`, so a sync **IS** owed — the change
  does not take effect until the cache is synced (CLAUDE.md § Plugin Cache Sync), and on-main executor
  regeneration happens at finalize via `project:finalize-step-sync-plugin-cache`. ⛔ **This reverses
  the spec's earlier text**, which claimed no sync was owed; that held only in the `doc/plans/`
  standalone lane, where `/sync-plugin-cache` reads a git-ignored build tree the lane may not touch.

### D6 — A posted review disposition is a promise, and nothing re-checks it against what landed

⭐ **Folded from `review-packs-become-published-artifacts-015.md`** (PLAN-PR-038 drain, 2026-09-04) — the
same "our own record is unverified" subject this spec owns, observed one level up: not a wrong figure in a
report, but a wrong promise on a PR thread.

Replies were posted in the *"Agreed. Will be addressed by TASK-N"* form, which **states a remedy before the
fix is designed**. On CodeRabbit thread `166827` the stated remedy was not what shipped — the reply promised
`PrAgentTarget.generate` would *reject* a non-`None` bundles filter; the shipped remedy instead removed the
allow-list parameter entirely, because `generate.py` forwards `bundles=` to every registered target and
raising would have made `--target all --bundles X` exit 2 for every other target.

⛔ **The correction was posted, and that is the right handling — the defect is that NOTHING DETECTED the
divergence.** An agent happened to notice while composing a later reply. A reviewer agreed on the basis of a
remedy that never landed, so their agreement was obtained under a false description.

*Done when:* a posted disposition that names a remedy is re-checked against the landed diff before the PR is
merged, and a divergence is reported rather than left to chance. **Matched negative control required:** a
disposition whose promised remedy DID land reports no divergence — without it the check cannot be
distinguished from one that flags everything.
...
deliver**: D1–D4 are largely record corrections, so the bulk of this plan's output is *ledger hygiene*,
not a reviewable change — only its `marketplace/` and `test/` entries reach a diff. Say so explicitly in
the run report rather than presenting a near-empty PR as the whole of the work. ⚠ Re-scoping this plan
(for example, restoring the records to `doc/plans/` first so the corrections become reviewable) is an
OPEN QUESTION for the operator, deliberately not decided here.

Records under `../cloud-runs/` (each `{NNN}-…` abbreviated to its prefix):
...
- `marketplace/bundles/plan-marshall/skills/automatic-review/standards/bot-participation-contract.md`
  — D4 items 4 and 5.
- `marketplace/bundles/plan-marshall/skills/automatic-review/standards/cuioss-review-bot.md` — D4 item 6.
...
- `marketplace/bundles/plan-marshall/skills/workflow-integration-github/scripts/github_pr.py` — D6: where a posted disposition is transmitted, and where the landed-diff re-check attaches.
- `marketplace/bundles/plan-marshall/skills/manage-findings/scripts/_findings_core.py` — D6: the disposition record the re-check compares against.
- `test/plan-marshall/workflow-integration-github/test_github_pr.py` — D6, incl. the matched negative control (a promise that DID land reports no divergence).
...
`gaps.md` / `verification.md` files under `../cloud-runs/` all quote the very identifiers
...
`../cloud-runs/` explicitly, and record with each result **both** the scope expression
...
`../cloud-runs/{NNN}-…/gaps.md`, with the supporting analysis in the sibling
...
notice on PR #1182's PR-level reviews surface — is about a GitHub surface, not about the tree. Read it
through the CI abstraction (`plan-marshall:tools-integration-ci:ci pr reviews`), ⛔ never `gh`/`glab`
directly and not through the GitHub MCP server — that was the `doc/plans/` cloud-session path, and this
plan does not run there. If that surface cannot be read,

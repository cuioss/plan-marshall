...
A SUPPLIED anchor is resolved against the local object store before anything is persisted: when `--head-at-completion` resolves to no commit (`git cat-file -e {sha}^{commit}` fails — a fabricated string, or a real object that is not a commit), the command returns `status: error`, `error: unknown_head_at_completion` and **writes nothing**. Presence without reachability is a fabricated anchor, and a fabricated anchor records a verdict nobody can locate in history — so it is refused rather than persisted, on every `done` outcome, whether or not the step declares `head_dependent: true`.
...
| `unknown_head_at_completion` | 1 | `mark-step-done`: `--outcome=done` with a `--head-at-completion` SHA that resolves to no commit in the local object store (`git cat-file -e {sha}^{commit}` fails). A fabricated anchor records a verdict nobody can locate in history, so it is refused rather than persisted — nothing is written. Resolve the worktree HEAD immediately before the call and pass the real SHA. |

...
The machinery landed and is honest about its failure modes **on the paths that were tested** — no
mode yields a zero-edge success. That honesty is partial, not total: one untested path, a server that
answers `initialize` with a JSON-RPC error, is reported under an affirmatively wrong reason (see D3
and the sentence three lines below). What it does not do is the thing the
plan's Goal names: on the one repository where the harvest is materialized, the shipped resolver
derives **zero** module edges, because not one cross-bundle reference resolves. D2's stated mechanism
(the path-attribution seam) was not used and the substitution is undisclosed. D3's fourth failure mode
is not merely narrow — a genuine server-side rejection is actively misreported as a timeout, and the
test named for catching such a collapse cannot detect one.

⚠ **The harvest's reference and suppression counts are interpreter-dependent.** Every count in this
document is stated with the environment it was measured in; the zero-edge result, the 1387 files
scanned and the 548 self-edges are invariant across both, and nothing else is. See D1 and Correctness
review item 6.
...
| D3 | Lifecycle + honest failure, one negative control per mode | "Four lifecycle failure modes, each with its own negative control and a distinct stated reason" | Four controls exist and pass; the *distinctness assertion itself is vacuous* (mutation-proven) and the property is pinned only by the per-mode controls. **A server that answers `initialize` with a JSON-RPC error — including "workspace not supported" — is reported `server-timeout: … did not respond within 10s`** | PARTIAL |
...
| D5 | Documentation: resolver, tier ladder, lifecycle rationale | "Tier 2 is `PARTLY BUILT`… Tier 0's `SUBPROCESS-FREE` becomes `BY DEFAULT`" | Confirmed verbatim in `doc/concepts/code-intelligence.adoc:19-31,42-51,80-121`. Four residual false/stale statements found (seam claim ×2, `dep_type` enumeration ×1, "every bundle declares its domain identity" ×1 — G11) | PARTIAL |
...
- **Verdict:** **PARTIAL.** The deliverable's substance landed; **four** statements across the three bullets above are wrong — two mechanism claims (the seam), one enumeration (`dep_type`), and one false universal ("every bundle declares its domain identity", G11).
...
|---|---|---|---|
| A1 | False positives | Every gap's cited `path:line` was opened and read. **All eleven gaps that existed when this attack ran are real** — G12 and G13 were filed later in this same review (by A3 and A2 respectively), so the count here is the pre-adversarial one; `gaps.md` now carries thirteen, and the two additions were each established by the mutation or measurement that filed them. The two HIGH ones were reproduced from scratch: the zero-edge outcome on four whole-tree harvests, and the JSON-RPC-rejection-reported-as-timeout probe verbatim including its reason string. Every quotation in both documents (`code-intelligence.adoc:113`, `ext-point-derivation-resolver.md:231`, `module-discovery.md:348`, `extension-architecture.adoc:16`, `_dep_detection.py:25-32`, the `extension_base` import line and its three column offsets) is verbatim from the file it is attributed to. Two citations were stale: `lsp_harvest.py:420-422` for the seam docstring (it is at `:416-418`; `:420-422` is the adjacent, accurate drop-and-note sentence), and the outer truncation flag at `:293` (it is at `:294`). One was mis-located: `EXPECTED_MANIFEST_COUNT = 11` lives in `test/marketplace/test_plugin_license.py:20`, not in `test_extension_discovery.py`. `_PRODUCTION_BUNDLES` begins at `:511`, not `:513` (`:513` is its `pm-code-intelligence` entry, which is what was claimed). | Citations corrected in G6, G8 and § Report accuracy; no gap withdrawn. |
| A2 | False negatives | Both CONFIRMED rows re-derived independently and both hold (D0 by execution; D4 by the zero-`pm_code_intelligence`-hit sweep, the `config_defaults` identity assertion, and a live `discover_plugin_modules` returning `ran: false, not-configured`). **One defect the audit missed:** `_candidate_files` skips vendor and virtualenv trees as reference *sources* while `_within` admits them as reference *targets*. Reachable half — 419 of 1 920 harvested references (31% of the `unattributable-endpoint` note) are `.venv` site-packages files. Latent half — `make_prefix_attributor`'s root-scoped fallback (`:588-591`) then attributes those to a module whose `paths.module` is `.`, a value `_pyproject_cmd_discover.py:159` really emits, producing a silent wrong edge with no note; demonstrated directly. | New Correctness review item 6; new gap **G13** (medium, `lsp/resolvers`). |
| A3 | Vacuous evidence | All three claimed mutations re-run and all three reproduce the same named red tests. Two **new** mutations went the other way. (i) Deleting `truncated = True` at `:313` leaves all 50 tests green — this converts G8's evidence from a grep into a proof, and the grep it previously cited was itself inaccurate (`out-of-workspace` does appear at `test_lsp_harvest.py:198`, in a docstring). (ii) Collapsing `REASON_SERVER_ABSENT` onto the `server-failed-to-start:` prefix leaves `test_every_failure_mode_states_a_distinct_reason` **green** — the test compares whole interpolated strings, which differ by binary name, so it cannot detect the collapse it is named for. This document's "distinctness is mutation-resistant" was therefore unsupported: the property is pinned by the four per-mode `startswith` controls, and the mutation the first pass ran proved the skip list, not D3's *Done when*. | D3 verdict text and the Test adequacy D3 row rewritten; new gap **G12** (medium, `tests`); G2's *Done when* strengthened from distinct strings to distinct prefixes, since otherwise G2 would be closed by a test that cannot fail. |
| A4 | Counts and quotes | Re-derived at the moment of checking. **Reproduced exactly:** 1387 candidate files (= `git ls-files '*.py' \| wc -l`), 249 refs at the 200-file budget, 6/6 on `build-maven/scripts` with the same six pairs, 0 module edges, 548 self-edges, 7 discovered resolvers, `lsp_harvest` on all 11 modules, 11/153/2/2/157 at `c86de8b` drifting to 154/158, 50 tests passing, `7 impls · Active`, the deferred import at `_maven_cmd_discover.py:555`. **Reproduced conditionally:** `refcount=1920`, `unattributable-endpoint: 1372`, `out-of-workspace: 12740`, `unresolved-symbol: 5321` hold only with the project `.venv/bin` first on `PATH`; without it, twice and deterministically, 1501 / 953 / 12764 / 5731. The delta is exactly the 419 `.venv` targets. **Refuted:** "≈ 118 s full crawl" — a linear extrapolation this document's own D1 probe (60.4 s) already contradicted; measured 56.0 / 70.9 / 83.8 s. **Refuted:** "Every reference pyright can resolve is intra-directory, hence intra-bundle, hence a self-edge" — 1 172 of 1 920 are not intra-directory, 878 target `test/`, and 7 of the 548 self-edges are cross-directory. The conclusion (zero edges is structural) survives on the correct ground: **zero cross-bundle references resolve**. The two mutation tails ("3 failed, 6 passed", "1 failed, 1 passed") were `-k`-scoped subsets presented without their selector and are unreproducible as written. | D0 and D1 sections rewritten with both populations and the invariants that hold across them; the extrapolation replaced by the measurement; the root-cause sentence replaced; mutation tails restated against the full 34-test population; gaps.md intro and G1 evidence updated. |
| A5 | Actionability | All thirteen entries carry a concrete `path`, a concrete change, and an observable *Done when*. No "review/consider/investigate" phrasing survives. G7's *Done when* is a literal grep — executed here, it currently returns the offending line, so it is a real gate. G3 is the only "decide and record" entry; it names both alternatives and its *Done when* is satisfied by either, which is the honest shape for a gap whose right answer is a design choice. | None needed. |
| A6 | Severity and topic | Severities re-checked against the calibration. G1 and G2 are correctly high (a *Done when* unmet in practice; a shipped reason that is affirmatively wrong). G3–G8 and G10 are correctly medium. G9 is correctly low — behaviour is right, the assertion is missing on a non-load-bearing path. **G11 was mis-rated low**: "every bundle declares its domain identity" is a false claim in shipped documentation, which the calibration places at medium, and it is the same class as G4/G5/G6. Topics all match the owning surface: the four `lsp/resolvers` entries touch the engine or its docstrings, `bundle-docs` for the two `marketplace/bundles/**/standards/` files, `documentation-surface` for the two `doc/concepts/` files, `tests` for the two test gaps, `architecture-core` for the `.plan/project-architecture/` overlay. **G12 and G13 are not covered by the sweep above** — both were filed later in this same review, after A6 ran. Their ratings were set at filing and are recorded in their own entries: G12 medium / `tests` (a test that reads as a strong invariant and asserts a weaker one), G13 medium / `lsp/resolvers` (the reachable half misreports a diagnostic count; the wrong-edge half is not reachable through the only shipped producer, which is why it is not high). Neither has been re-checked against the calibration by an independent pass. | G11 raised low → medium with the reason stated in the entry. |
| A7 | Coverage | All six deliverables have a row and a per-deliverable section; out-of-scope compliance, report accuracy and the full eight-item residue list are each covered. No deliverable is silently unmentioned. One half-deliverable is thinly covered rather than uncovered: D4's "supplies language-server-specific settings *within* [the shared surface]" — what actually landed there is documentation (the two-consumers table), not a setting. That is a defensible reading of the deliverable's fallback clause ("if that plan has not landed … define the minimum"), so CONFIRMED stands. | Noted here rather than changed. |
| A8 | Internal consistency | PARTIALLY REFUTED follows from the rows: two CONFIRMED, four PARTIAL, with one literal *Done when* ("its edges appear in the store") unmet rather than merely incomplete. Every gaps.md entry traces to a verification finding and every actionable finding now has an entry — including the two the first pass left unfiled: Correctness review item 2 (resolution by iteration order, which violates a stated Axis-D obligation) is now folded into G3's action and *Done when* rather than orphaned, and the new item 6 became G13. Correctness items 3 and 5 remain deliberately unfiled and say why. The two report-only defects (§ Correctness spot-check overstating "derived edges", § Residue presupposing edges exist) stay unfiled: the calibration puts a stale run-report claim at low, and a lane run report is a dated record of one execution rather than documentation of current state. One internal contradiction was found and fixed — the ≈118 s extrapolation against the same document's 60.4 s measurement. | G3 extended; gaps.md intro count corrected eleven → thirteen. |

**Residual doubt:** the likeliest further finding is in the direction A4 opened. The harvest's outputs
are interpreter-dependent, and **the implementation neither controls nor normalizes that dependence**
— the reference count, three of the four notes, and the wall-clock all move with which Python pyright
happens to resolve against, and there is no `pythonPath` / `workspace/configuration` handling to pin
it. (The dependence itself *is* acknowledged: this document states it in the opening warning and
again under D1, and every count above carries the environment it was measured in. What is missing is
any pinning in the shipped artefact — nothing in the code, the tests or the shipped documentation
names an interpreter or fixes one.) G13 fixes the `.venv` symptom; it does not make the harvest reproducible. A
further round would most likely find that the *whole* reference set, not just its vendor slice, is
unpinned across machines, which would make any future precision measurement (the run's own declared
residue) unrepeatable. Secondarily, `test_end_to_end_harvest_against_a_real_server` and its
materialization twin were exercised against the pyright present here and would silently become
environment-skipped elsewhere; nothing asserts the server was actually available, which is the
finding-20 shape one level up. Neither was pursued to a gap because both sit outside the deliverables
as written.

**Verdict on the audit:** SOUND AFTER CORRECTION — every gap it filed is real and both HIGH findings
reproduce independently, but its stated root cause for the headline was factually wrong, its central
counts were environment-dependent without saying so, its D0 extrapolation contradicted its own
measurement, and it credited a vacuous test as mutation-resistant; all four are now corrected and two
further gaps were added.

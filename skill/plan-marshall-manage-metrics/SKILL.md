---
name: plan-marshall-manage-metrics
...
- `--step-id` — The dispatch step key (the `record-step` `step_id`) carried on the boundary row so the reconciliation joins on the key first, with timestamp-window fallback for rows that carry none. Forward the dispatch's step key on EVERY call — an omitted flag writes an empty key, never a pairing claim, and is correct only when the dispatch genuinely has no step; the key must not contain a comma or any line separator.

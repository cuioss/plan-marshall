...
The marker itself is **existence-only**: only its presence is ever consulted, never its content, because a foreign co-producer writes the same field in a different encoding. That invariant is declared at the single read site, `marketplace_bundles._partition_version_dirs`, and is not restated here.

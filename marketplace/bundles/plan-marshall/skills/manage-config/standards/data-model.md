...
**What this is NOT — the two axes this model is most often mis-remembered on.** Both bullets below state the mechanism positively and name the belief each forecloses. They are **additive**: no sentence in this repository ever stated either mistaken model, so a reader arriving here should not go hunting for a wrong sentence to correct. That is a first-party finding rather than an assumption — a complete content sweep of the crawled inventory found no in-repo instance of either belief, with nothing unreadable, nothing elided, and no result truncated, so the negative is trustworthy under the complete-coverage rule rather than merely empty.

...
Two actors want to edit the same checkout — the developer and the agent — and stock Claude Code lets them. Plans contaminate the developer's working tree, half-finished changes from a forgotten plan show up in `git status` from yesterday, and "concurrent plans" is a euphemism for "serialised plans pretending." Plan Marshall sidesteps the contention by giving every plan its own paired resources — a dedicated *feature branch* and a dedicated *git worktree* — so plan-bound work is plan-bound from start to finish, and the shipping arc back to mainline is one well-defined transition rather than a per-plan ritual.

Two ideas carry the design: plan-bound isolation while a plan is in flight, and a clean post-execute shipping flow that turns that isolated work into landed mainline state.
...
[NOTE]
====
Because several plans can be in flight at once, the merge step is coordinated by a cooperative lock: a plan acquires the lock before merging into `main` and releases it afterward, so two plans never race to land on `main` at the same moment. The lock is cooperative — it serialises the merge boundary without blocking the rest of each plan's isolated work. The exact acquisition and release mechanism is part of the canonical worktree/merge contract linked under <<related,Related>>.
====

...
| Secret in image | Hardcoded password/token in `ENV`, `ARG` leaked via `--build-arg`, `.env` copied into image, `RUN ... && rm <secret>` false-fix (leaks in image history/metadata, or does not erase from prior layers if split across separate `RUN` instructions), secret written to a file by a `RUN` script | `oci-security` (Secret Management) |

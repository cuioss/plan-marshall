...
| Eight prose sites still describe the abandoned predicate or the two-source anchor | HYPOTHESIS — the *count* is a lead; the sites named in D6 were each read | The two survivor searches D6 specifies, run over `marketplace/bundles/plan-marshall/skills/` and `test/`. The count settles only when both searches are re-run |

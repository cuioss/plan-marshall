...
kind=finding
created=2026-09-23T07:24:35Z

# Forward from `deployment-configurability` (API-Sheriff) — `pre-submission-self-review`'s deterministic surfacer has no step that keeps its diff base ref current, so a correct refusal fires on routine worktree mechanics

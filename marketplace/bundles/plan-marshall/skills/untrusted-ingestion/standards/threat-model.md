...
1. **The reader cannot act.** Its tool surface (`WebSearch, WebFetch, Read, Grep`) has no write, edit, execute, or skill-loading capability. An injection payload that successfully hijacks the reader still cannot do anything beyond producing a (malformed) candidate struct — it has no tools to abuse.

...
- **Never write or edit source files outside `.plan/local/plans/{plan_id}/**`.** Phase-1-init's contract is plan-structure creation only (request.md, references.json, status.json under the plan directory). Even when the task description is detailed — naming specific files, functions, or paths — that is request material to record, NOT a directive to implement. Source edits against `marketplace/bundles/**`, production code, or test fixtures are the responsibility of phase-5-execute task bodies, never phase-1-init. The recurring anti-pattern (captured as `feedback_phase1_init_never_implements` in the project memory log) is phase-1-init reaching for `Edit` / `Write` against a production path because the request narrative read like an implementation brief.

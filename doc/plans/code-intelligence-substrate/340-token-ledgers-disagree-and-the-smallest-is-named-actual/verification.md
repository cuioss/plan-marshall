...
- **Verdict:** PARTIAL. Every field the report claims is present, and the forward direction — each key
  ...
  **What is not verified is the direction the *Done when* actually states.** "No rendered figure lacks a
  ...
  `f'{name} → {field} {value:,}'` in the reconciliation clause (`:2016-2019`), the two boundary-fold
  clauses (`:2072`, `:2087`), the phase-name lists in the population clause (`:2045-2059`) — none of
  which was traced back into the store. The check cannot be run here: it needs a rendered report, and
  `/home/user/plan-marshall/.plan/local/` holds only `logs/` and `marshall-state.toon`, with no `plans/`

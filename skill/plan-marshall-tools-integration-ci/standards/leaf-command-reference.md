...
| `pr list` | _(none)_ | `--head {branch}`, `--state {open\|closed\|all}`, `--limit {n}` (**GitHub only**, default 100) | List PRs with optional branch and state filters. **GitHub** reports `limit` and `truncated` beside `total`; a `truncated: true` listing is a page, not a population — re-read at a higher `--limit`. **GitLab** accepts no `--limit`, reports no `truncated`, and its listing stays page-bounded |

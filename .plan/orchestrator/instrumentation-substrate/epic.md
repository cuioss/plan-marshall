...
**Phase**: orchestrating
**Inbox (derived)**: 0 queued, 12 archived
**Parked**:
- PLAN-09 (WS-05)
...
- 2026-09-22 — **`epic_mismatch` recovery: `truthful-signals-001.md` re-filed as `truthful-signals-002.md`.**
  The 2026-09-18 message's envelope named `epic=next-level`, the epic's name before the 2026-09-21 pure
  rename; the validator rejected it against the current slug. Neither `inbox amend` nor `inbox supersede`
  applies — both refuse an already-invalid target — so the payload was re-filed verbatim via `inbox write
  --slug instrumentation-substrate` (content unchanged, only the envelope corrected) and the broken
  original archived directly. Resolves the Open Defect this recovery superseded; the re-filed message
  drains normally as a `candidate-lesson`.
- 2026-09-22 — **`truthful-signals-002` (recovered) discarded, routed back — operator-confirmed.** The
  two domain-skill lessons (`persona-security-expert` OAuth-remedy counterparty check, `pm-dev-java:
  java-core` catch-narrowing sweep) matched no staged spec's declared population: PLAN-01/02/08 are
  bound to `CLAUDE.md`'s mechanical hard-rule set, and PLAN-09 (already transferred to
  `post-run-quality`) is about the lessons *store's* mechanics, not lesson content. Alternatives
  considered and put to the operator: fold into PLAN-01/02 by widening their declared population
  (rejected — the same de-escalation-sweep-style scope growth the epic's Non-Goals gate behind WS-02),
  and stage a new plan (rejected — opens a sixth workstream for two examples). Filed into

...
If the `### Queue annotations` zone both rules move content into does not exist — likely on the same pre-marker `epic.md` Rule 1 addresses, since the zone and the markers shipped together — **create it**, as a `### Queue annotations` subsection of `## Ordered Queue`. Its position depends on which branch you are in, because the two branches differ in whether a marker exists to position against:

- **Rule 2 branch** (the pair is present): place it **immediately after the `END` marker**. Placing it after the *table* here would put it between the last data row and `END` — inside the generated region, which is the exact loss this instruction exists to prevent.
- **Rule 1 branch** (the pair is absent) **and the table carries `Notes` content**: place it **immediately after the existing table**. There is no `END` to position against at step-1 time, and step 2 then inserts `END` immediately below the last data row — between the table and the zone — so the zone ends up outside the pair.
- **Neither rule applies**: create nothing.
- **Rule 1 applies but the table carries no `Notes` content**: create nothing. Rule 1's condition is purely structural (queue present, markers absent) and says nothing about content, so it fires on a ledger with an empty `Notes` column too — and there the zone would have nothing to hold. (Rule 2 cannot reach this case: its condition already requires a hand-written line.)

The governing rule in both of those cases is the same one: an absent zone with no content to move into it is not a defect, and an empty zone added speculatively is noise. **Create the zone only when the rule that applies has something to put in it.**

Moving content into a zone that is not there is not an option, and inventing a different destination would put it back inside the region the generator owns.

**Rule 1 — insert the marker pair.** Applies **only when both hold**: `epic.md` carries a `## Ordered Queue` section, AND that section contains no `<!-- BEGIN GENERATED: ordered-queue -->` marker. When the pair is already present, Rule 1 does not apply at all. Its **two steps** run in the order below. When the table carries `Notes` content AND the `### Queue annotations` zone is absent, the zone-creation instruction above runs **before** both of them, because step 1 then needs a destination — three actions in that case. When the table carries no `Notes` content, no zone is created (see the bullets above) and step 1 moves nothing, so there are two:

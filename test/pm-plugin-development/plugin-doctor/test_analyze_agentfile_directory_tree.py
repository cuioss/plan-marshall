...
build-failing under quality-gate — registered in ``doctor-marketplace.py``'s
``cmd_quality_gate`` (via ``RuleRunner.run_quality_gate``) as well as
``cmd_analyze``.
...
class TestQualityGateRegistration:
    ...
    def test_call_present_in_both_analyze_and_quality_gate(self) -> None:
        ...
        assert 'analyze_agentfile_directory_tree(' in quality_gate_body

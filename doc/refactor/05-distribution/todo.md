...
- [ ] For each comment:
      - Real issue + sensible fix → apply, commit, push; reply via:
        `... pr prepare-body --plan-id refactor-05-distribution --for edit --slot reply-<n>` (write text), then
        `... pr reply --pr-number $PR --plan-id refactor-05-distribution --slot reply-<n>`

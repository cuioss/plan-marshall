...
**ARGUMENT_NAMING_FLAG_UNKNOWN** (severity: error): Flags `--{flag}` tokens following a notation+sub pair when `{flag}` is absent from the shared derived surface's flag set for the resolved (notation, subcommand) pair — that set is the subcommand's whole subtree unioned with the root parser's own flags, since argparse honours a root-declared flag (`--plan-id`, `--project-dir`) on every subcommand even when it renders only in the root's help. The `details.known_flags` field lists the known long flags, and `details.flag` carries the offending name. Flag sensitivity here is deliberately lower than an exact `add_argument` declaration set: the shared derivation over-approximates a node's flags (every `--long-token` anywhere in that node's `--help` output, widened further by the subtree union), so a flag declared two levels down validates a top-level invocation too. That is a real, deliberate widening — fewer findings, never a false finding — not the exact-declaration boundary this rule checked before the surface consolidation.
...
- **Exemptions**: Short flags (`-f`) are not subject to canonical-forms convention and are excluded from the scan. Flags on a script whose derived surface has no entry for the resolved (notation, subcommand) pair are skipped silently — the notation rule reports the missing script when applicable.
...
- **Anti-vacuity guard**: every finding publishes the `population_size` it examined, and when the ext-point IS defined in the tree (the standard doc exists) but the derived population is empty, the rule emits its own finding — so a clean result can never read as a vacuous pass over an unread population.
...
- **Anti-vacuity guard**: every finding publishes the `population_size` it examined, and an EMPTY script population over a non-empty bundles tree emits its own finding — so a clean result can never read as a vacuous pass over an unread population.
...
**Intent**: A component may declare the build-time `targets:` frontmatter field naming the build targets it ships to (absent ⇒ every target). The multi-target generator rejects an unknown target name, an empty declaration, and a value spanning more than one physical line; this rule surfaces each while the author is still looking at the file rather than at build time. An absent field is correct and is the state of nearly every component — it is never flagged.

**Detection**: Pure static analysis — the leading `---`-fenced frontmatter is parsed line-by-line for a TOP-LEVEL `targets:` key in either the inline-flow (`[a, b]`) or block (`- a`) form. Valid names are derived from the targets' own `register_target('{name}', …)` registrations under `marketplace/targets/*/__init__.py`, never transcribed into the analyzer. Top-level is relative to the block, not to column zero: the block is dedented by the common indent of the lines that CARRY STRUCTURE — blank lines and whole-line comments are excluded, since a single `#` at column zero would otherwise pin the common indent at zero and leave an indented block entirely unscanned. A uniformly indented block therefore still has top-level keys, while a key indented beyond its siblings stays nested. A key whose inline text is only a comment (`targets: # note`) has no inline value: the list beneath it is the value. `details.reason` distinguishes `targets_unknown` (with `details.unknown_targets` and `details.registered_targets`), `targets_empty`, `targets_ambiguous_indent` (an indented block carrying a line shallower than its own keys, which a line scan cannot resolve and which is refused rather than guessed), and three reasons for a value that is not on the key's own line alone — `targets_multiline_scalar` (a plain scalar continued across lines), `targets_quoted_scalar` (a quoted one), and `targets_block_scalar` (a `>`/`|` block scalar). Those three are one detection under three names: the shape is diagnosed after the decision to flag, so misclassifying it changes the reason and the sentence, never whether the component is reported.
...
**Recommended fix**: Correct the target name to a registered one; delete the `targets:` field entirely to ship the component to every target; or, for the two value-shape reasons, write the list explicitly as `targets: [a, b]` or a `- ` block.

...
|------|------|--------------|-------|
| **Epic** | A long-running goal too large for one plan — a roadmap, a campaign, a multi-plan feature | One `.plan/orchestrator/{slug}/` tree (`epic.md` + `status.json`) | The orchestrator |
...
Each epic lives in one git-tracked, repo-local tree under the orchestrator store:
...
**The executing plan MUST NOT create or edit any file under `.plan/orchestrator/{epic}/`** — not `status.json`, not `epic.md`, not `workstreams/`, not `plans/`, not `landings/`. The orchestrator owns every ledger write and reconciles the epic from the landed PR through the `analyze` verb. A plan has exactly two channels back to the epic: its PR, and its `inbox/` OUTBOX.
...
- **The regeneration mechanism — GENERATED markers, in place.** Every derivable surface of `epic.md` lives between a `<!-- BEGIN GENERATED: {name} -->` / `<!-- END GENERATED: {name} -->` marker pair; `compact` replaces only the content between the markers, from `status.json` and the filesystem, and leaves **every byte outside the markers untouched**. That boundary is what makes "a retraction survives a pass verbatim" a **structural** property rather than a matter of care. A block whose markers are absent from a given `epic.md` is reported (`markers_absent`) and skipped — never fabricated, because inserting markers into a hand-authored document is a structural edit the stage has no mandate to make. **The remedy for that refusal is a ONE-TIME migration the orchestrator performs itself**, stated here beside the refusal so the two are read together: on an `epic.md` predating the `ordered-queue` marker pair, the orchestrator moves any per-row `Notes` into the adjacent `### Queue annotations` zone and then inserts the marker pair around the existing table, writing through the [direct-file-write carve-out](#carve-outs) and never through the script. A **separate** rule — applying when the markers are already present AND hand-written content has accumulated between them — evacuates that content to the same zone. Both conditions are read over the whole file, the scope `_marker_indices` uses, so **at most one applies to a given block**; they are NOT complements — a block whose pair is present with nothing hand-written between the markers matches neither, which is the steady state of every ledger after its first compaction. The migration covers every block in `GENERATED_BLOCKS`, not the queue alone. Only the marker insertion is one-time (it destroys its own precondition); the evacuation rule is checked on every pass, because content can land between the markers at any time. Without it the refusal is permanent and the queue is never regenerated at all. The step's condition and sequence are [`workflow/cleanup.md`](../../plan-orchestrator/workflow/cleanup.md) § Step 8's; they are not restated here. A `regenerated` block additionally reports `replaced_body`, the pre-write between-marker text, so a first pass over an already-annotated ledger names the content it overwrote rather than reporting only a line-count delta. The stage writing the between-marker regions of `epic.md` directly is a bounded, deterministic extension of the [direct-file-write carve-out](#carve-outs): it is the orchestrator's own mechanical ledger-writing arm, and the judgement half never enters the script.

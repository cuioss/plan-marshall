...
description: |
  Named agent that performs the finalize-phase Knowledge Capture step. Loads plan-marshall:dev-general-practices into its own context, then delegates end-to-end to the authoritative standard phase-6-finalize/standards/knowledge-capture.md to surface new memories and artifacts worth preserving from the plan run.
...
- Execute ONLY the steps documented in `phase-6-finalize/standards/knowledge-capture.md`. Do not add discovery steps, invent arguments, or skip documented steps.
...
```
marketplace/bundles/plan-marshall/skills/phase-6-finalize/standards/knowledge-capture.md
```

The standard is the source of truth for the step sequence, including:
- Scanning the plan run for reusable knowledge
- Creating / updating memories via `plan-marshall:manage-memories`
...
python3 .plan/execute-script.py plan-marshall:manage-logging:manage-logging \
  work --plan-id {plan_id} --level INFO --message "[STATUS] (plan-marshall:knowledge-capture-agent) Complete"
```

## Output

Return the TOON block emitted by the `knowledge-capture.md` workflow verbatim (at minimum: `status`, `memories_created`, `memories_updated`).

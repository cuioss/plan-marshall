---
name: plan-marshall-phase-2-refine
...
2. **Persist `scope_estimate`** to `references.json` via `manage-references set --field scope_estimate --value {scope_estimate}` (one of `none | surgical | single_module | multi_module | broad`), then re-run the deterministic classification-validation gate via `manage-status classification-validate --plan-id {plan_id}` — this persist overwrites the pre-route band phase-1-init wrote, so it is the first point at which a narrow-over-large mismatch can exist. Flag-not-block; see [`refine-workflow-detail.md`](standards/refine-workflow-detail.md) § "Re-run the classification-validation gate after the overwrite".
...
| Error | Action |
|-------|--------|
| Architecture not found | Return `{status: error, message: "Run /plan-marshall-marshall-steward first"}` and abort |
| Compatibility not configured | Return `{status: error, message: "compatibility not configured. Run /plan-marshall-marshall-steward first"}` and abort |

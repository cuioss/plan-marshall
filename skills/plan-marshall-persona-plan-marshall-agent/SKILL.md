---
name: plan-marshall-persona-plan-marshall-agent
...
### Step 1: Load Core Development Rules and User-Communication Rules

**Important**: Load BOTH standards at the start of any development work. Neither is optional and neither is loaded on demand — they are the unconditional tier.

```text
Call the `view_file` tool with `{ AbsolutePath: "standards/agent-behavior-rules.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/user-communication.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/tool-usage-patterns.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/argument-naming.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/thoroughness.md" }` before continuing.
```
...
- **`execution_tier: per_task`** — the Bash call runs synchronously inside the current dispatch. Pass `timeout: bash_timeout_seconds * 1000` (milliseconds) on the Bash tool call. Never accept the host's default Bash timeout (2 minutes on the Claude target; resolved per active target through the platform-runtime `harness bash-timeout-ceiling` seam) for these commands; the host platform will silently auto-move the call to background and the dispatch will lose the synchronous-return path.
...
The floor for ad-hoc build/verify invocations outside the architecture-resolved envelope is 600000ms (10 minutes) on the Claude target (the seam-resolved `harness bash-timeout-ceiling` default; a target without that ceiling resolves its own); the architecture-resolved `bash_timeout_seconds` always supersedes the floor.
...
When issuing `python3 .plan/execute-script.py {notation} {subcmd} ...` calls, quote the subcommand and flag names verbatim from the executor mappings or the script's `--help` output. Never extrapolate plausible-sounding verbs (`add-fix-task`, `read-context`, `tail`, `--filter-status`, `--tail`) from surrounding workflow prose. Plausible names that match the workflow's narrative but not the actual argparse declaration produce silent `exit_code: 2` failures that bypass the script body and corrupt downstream behaviour. When in doubt, invoke the script with `--help` first, or grep `.plan/execute-script.py`'s embedded `SCRIPTS = { ... }` mapping.

The five canonical argparse-rejection recurrence signatures (verb-paraphrase, verb-scoped `--plan-id`/`--project-dir`, doubled bundle-prefix, router-scoped `--plan-id`/`--project-dir` placed AFTER the verb on the CI surface — the mirror of the second, prescribing the opposite move — and missing `--phase` / `--resolution`-vs-`--status`) with their concrete invented-vs-canonical examples, the accepted read-verb aliases carve-out, and the authoring contract live in [`agent-behavior-rules.md` § Never invent script subcommands — recurrence signatures](standards/agent-behavior-rules.md#never-invent-script-subcommands--recurrence-signatures). The `ARGUMENT_NAMING_*` plugin-doctor rule cluster (unconditionally active under `quality-gate`) is the structural edit-time guard against this class.
...
| coverage-gathering-contract.md | The reusable coverage-gathering contract: canonical `ask_question` gather shape, the static cell→instruction expansion table, the gather→expand→consume implementor obligation, status.json persistence (identifier + expanded instruction), Current-Implementations table |

...
- **`search --content` covers the INVENTORY, not the tree** — it searches crawled files only, so `.git`, `node_modules`, `target`, caches, gitignored paths (including `.plan/`), and dotfile trees outside the allowlist (`.claude/**`, `.github/**`) are never searched. A zero result means *"not in any inventoried file"*, never *"not in the tree"*. For an out-of-inventory path, fall back to `Glob`/`Grep`/`Read` scoped to that path; if those tools are unavailable in the current envelope, report the coverage gap instead of recording a clean negative. A `count: 0` is trustworthy only with `files_scanned > 0`, `unreadable == []`, `truncated == false`, and `elided == []`.

...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/lombok-core-annotations.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/lombok-canonical-methods.md" }` before continuing.
```

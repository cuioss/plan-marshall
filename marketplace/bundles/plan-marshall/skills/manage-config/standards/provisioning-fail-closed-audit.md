...
|------|-------|-------------|
| `cmd_system` retention `set` (lines 66-96) | (a) | **FIX (D4).** Writes `retention[field] = value` with NO whitelist check on `field`, then returns `success_exit`. A typo'd or retired retention key persists silently. This is the one concrete silent-success write the sweep found; deliverable 4 routes it through the validated-write seam / generalizes the `cmd_project set` whitelist. |
| `cmd_project set` (lines 163-225) | (a) | **JUSTIFY.** Already fails closed: rejects unknown fields (`unknown_field`), invalid `pr_strategy` / `pr_compact_max_changed_files` values (`invalid_value`), a non-`str` or empty `user_language` (`invalid_value`, which is what stops `_coerce_value` persisting a `bool` for `--value false`), and non-list/non-string `working_prefixes` (`invalid_type`, `invalid_json`) before `save_config`. This is the pattern D4 generalizes. |
| `cmd_project get` (lines 128-161) | (b) | **JUSTIFY.** Routes the caller-named field through the same `reject_unknown_provisioning_field` seam as the `set` arm above — `unknown_field` — BEFORE consulting the live `project` block, so a retired or typo'd key already persisted there is refused rather than served (the field-NAME half of the read guard; the seam ordering is what closes it, since a membership check placed after the live-block read would still admit the persisted key). An admitted field absent from the live block falls back to `DEFAULT_PROJECT`. A `user_language` read from the live block is additionally re-validated at this read boundary and refused with `invalid_value` — the read-side mirror of the `set` arm above, and the same read-boundary re-validation `pr-decision` performs below. It is load-bearing rather than belt-and-braces: `marshal.json` is operator-editable and `sync-defaults` preserves an already-present key without inspecting its value, so an unguarded non-string persists indefinitely and reaches the language rule, which resolves anything other than `auto` as a pin. The `DEFAULT_PROJECT` fallback arm carries no guard — `get_default_config` self-validates the seed. |
| `cmd_project pr-decision` (lines 227-259) | (b) | **JUSTIFY.** Re-validates the resolved knobs at the read boundary and rejects a negative `--changed-files`; a corrupt marshal.json fails loud here rather than producing a wrong verdict. |
| `cmd_system` unknown sub-noun / verb (line 98); `cmd_project` unknown verb (line 261); `cmd_plan` unknown sub-noun (line 275) | — | **JUSTIFY.** All terminate in `error_exit`, never a silent success. |
...
| `cmd_preflight` | (a)/(b) | **JUSTIFY (canonical exemplar).** Fails closed: reports `marshal_status: unknown` with a legible warning when the manifest is unresolvable (never a vacuous `fresh`); regenerates the executor (safe derived state, ADR-002) on version staleness; surfaces a structured error when regeneration fails. It no longer detects multi-version pollution or writes any marker: the executor resolves bundle script paths at run time (`select_live_version_dir` / `_resolve_notation_by_target` pick the numerically-newest eligible version dir), so a stale version dir on disk cannot shadow the current scripts and there is nothing to contain. The preflight's verdict inputs are local-only by design: it answers "is my executor consistent with my cache?", never "is my cache current?" — the cache-vs-upstream question is owned by `marshall-steward`'s `cache_freshness check`. |
| `generate_executor` | (a) | **JUSTIFY.** Runs four deterministic guards (format-version handshake, placeholder-residue, `py_compile` self-check, emitted-path provenance) and commits atomically; refuses with `status: error` (preserving the pre-existing executor byte-identical) rather than emitting a broken executor. |
| `_check_emitted_path_provenance` (Guard 4) | (a) | **JUSTIFY.** Groups every emitted path (mappings, shared-module dirs, logging dir, collected script dirs) by owning bundle and returns `status: error` naming the bundle and the conflicting version dirs when any bundle spans more than one version dir, leaving any pre-existing executor byte-identical. Extends ADR-009's fail-closed principle from status reads to a generation write: never report success for an artifact whose internal consistency was never checked. No-ops in the version-less marketplace layout, where there is no provenance to compare — an honest "not applicable", not a vacuous pass. |

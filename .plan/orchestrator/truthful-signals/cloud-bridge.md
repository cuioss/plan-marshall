...
| This epic's cloud plans (the tree IS the state) | `doc/plans/truthful-signals/` | yes |
| This epic's cloud plans | `doc/plans/truthful-signals/{cloud-plan}/` | yes |
| The run contract a cloud session follows | `.claude/skills/cloud-plan-lane/SKILL.md` | yes |
| This epic's orchestrator ledger | `.plan/local/orchestrator/truthful-signals/` | **no** |
...
- **Never** write epic state into `doc/plans/`, and never expect a cloud session to have written
  anything into `.plan/local/orchestrator/truthful-signals/`. Git is the only shared medium.

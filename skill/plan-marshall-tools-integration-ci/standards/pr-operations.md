...
| `pr merge-queue` | `enqueued` | Two reads with two roles. The **pre-condition** is a pre-enqueue probe on **both** providers, run *before* the enqueue call so an off-routed target incurs no side effect — **GitHub** reads the PR's own base-branch queue configuration, **GitLab** the project's `merge_trains_enabled` flag — and any state other than `eligible_configured` refuses. The **claim** rests on the PR's own queue membership: **GitHub** reads the base branch's `mergeQueue.entries` after the call (and, when the complete list does not carry the PR, the PR's own `mergeQueueEntry`) and returns `true` only when one of those reads shows the PR, otherwise `indeterminate` with its reason; **GitLab**'s dedicated merge-train endpoint succeeds only against a real train and returns the created car, on which the arm reports `true`. See *Merge-Queue PR* → *The membership read* below. |
...
- **`eligible_unconfigured` / `ineligible` carrying no probe error** — the probe READ the platform's setting and found no required queue or train; `safe-merge` proceeds unchanged.
- **Any probe that established nothing** — an unresolvable base branch or project scope, a missing auth scope/permission, a non-404 API error, or a malformed response — refuses, per the fail-closed paragraph below. On GitLab these surface as `unsupported`, or as an `ineligible` carrying the auth-scope remedy. The refusal follows the probe's **error**, not the discriminator alone, which is what stops an unread setting from reading as "no train to bypass".
...
| `auto_merge_armed_awaiting_checks` | The list was read to its end without the PR, and the PR's own state carries an `autoMergeRequest` but no `mergeQueueEntry`. While required checks have not yet passed, `gh pr merge --auto` only **arms** auto-merge; GitHub adds the PR to the queue once they pass, and the admission can also lag the call. An armed auto-merge is not a membership, so it is never reported as `enqueued: true`. |
| `pr_not_listed` | The list was read to its end — the last page reported `hasNextPage: false` — and does not carry the PR, and the PR's own state carries neither a `mergeQueueEntry` nor an `autoMergeRequest`, or could not be read (a failed PR read keeps this value rather than guessing). Still not a negative: the queue may already have merged or ejected it between the call and the read. |

A caller that receives `indeterminate` must not proceed as though the PR were queued: report the reason and confirm the PR's queue membership before continuing. `enqueue_observation` names the read(s) and what they saw on both GitHub values, so the verdict is auditable either way.
...
enqueue_observation: "mergeQueue(branch: main).entries read to its end (3 entries); PR not listed; pullRequest(number: 123) carries an autoMergeRequest and no mergeQueueEntry"
queue_precondition: merge_queue rule active on branch
enqueue_unobserved_reason: auto_merge_armed_awaiting_checks
```

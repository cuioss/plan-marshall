...
- Never fire `ask_question` from the dispatched leaf on a timeout escalation — return the
...
`Task` and `ask_question`. Both omissions follow from this body's own contract rather than from an
...
- **`ask`** (default — halt and ask the operator): decision-log, then return `status: escalate_ask` with `reason: re_review_timeout` and the three prompt options encoded in the TOON so the orchestrator (phase-6-finalize SKILL.md Step 3) fires the `ask_question`. The dispatched leaf does NOT fire `ask_question` itself — it returns the escalation envelope and the inline orchestrator owns the prompt:
...
size refusal was actually seen — so on every run where no reviewer refused on size, the executor
delivers a bare `--measured-diff-size`. It therefore declares `nargs='?'` with `const=''` exactly as
the list flags do, and the bare form reads as unmeasured. Without that declaration the documented call
above is an argparse rejection on the ordinary path, which routes to the **UNKNOWN verdict** below —
the one verdict whose force-done hatch is explicitly unavailable — so the happy path would deadlock the
step.
...
In all cases the dispatched leaf does NOT fire `ask_question` itself — it returns this envelope and the inline orchestrator (phase-6-finalize SKILL.md Step 3 item 7a) owns the prompt.
...
- `action`: `defer` when policy is `defer` (orchestrator skips the merge directly); `ask` when policy is `ask` (orchestrator fires `ask_question` with `prompt_options[]`). All three rate-window variants and the structural variant always use `action: ask`.
...
both documented call sites interpolate it unconditionally and the executor strips an empty-string
argument, so an unmeasured diff delivers a bare flag. Bare reads exactly as omitting it: the classifier

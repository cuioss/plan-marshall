...
The `checks wait` handler (`cmd_ci_wait`, GitHub `_github_ci.py` / GitLab `gitlab_ops.py`) waits for a whole CI run to reach a terminal conclusion. That is a **different shape** from the section-3 snapshot-diff waits: those block until a single scalar snapshot *changes* (unresolved-comment count grows, status flips), and a fixed-interval `poll_until` re-fetch is the right tool. A CI run instead has a **known-busy window** — it will be running for roughly as long as it ran last time — so polling `gh pr checks` every 30 s from second zero burns API quota during a window where the answer cannot yet be "done". `cmd_ci_wait` therefore uses a two-stage **seed-then-watch** wait instead of the fixed-interval loop:
...
**Detach behind the `await-long-running` seam.** The orchestrator does not block synchronously on the whole CI wait: it detaches the `checks wait` call behind the shared [`await-long-running`](../../plan-marshall/workflow/await-long-running.md) detach-and-notify seam (the remote-CI-wait consumer), backgrounding it and waking on the completion notification. The p50 seed + watch-verb tail described here is the wait's *internal* mechanism; the seam is how the orchestrator *consumes* it without babysitting.

This pattern is intentionally **not** added to the section-5 subcommand catalog: it is the internal mechanism of the existing `checks wait` verb, not a new `wait-for-*` subcommand. The section-2 "when to add a new subcommand" test does not apply — no new verb is minted.

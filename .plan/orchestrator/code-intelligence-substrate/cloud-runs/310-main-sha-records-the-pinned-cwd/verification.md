...
Re-verified at `d19db26` on `claude/code-intelligence-substrate-analysis-kah884` (the tree moved
past the `f97303b` in this document's header while concurrent agents committed; `_invariants.py` is
byte-identical, SHA-256 prefix `08c3dcfa4f896e3c`, 83306 bytes — the same file this audit measured).

| # | Attack | What was found | Correction applied |
|---|---|---|---|
| A1 | False positives | **No fabricated gap.** Every gap's cited `path:line` was opened and the claim held: `_invariants.py:409-410`, `:356`/`:359`, `:1856`; `invariant-check-summary.md:48`, `:24`, `:46`, `:7-18` (8 rows, 7 registry entries missing — the 7 names match); `plan-retrospective/SKILL.md:34`; `marketplace_paths.py:402-405` (stale `_main_checkout_root`, absent from `merge_lock.py` — confirmed by grep) and `:473-484`; `cwd-policy.md:79`; `audit.py:789`; `report-01.md:485-486` (quoted verbatim); `_invariants.py:274`/`:1087`/`:1879` with no `except TaskGraphInvalid` anywhere | Two supporting claims *inside* otherwise-true gaps were falsified and rewritten — see A4 |
| A2 | False negatives | **G1 reproduced first-party and found to be characterised too narrowly, but correctly rated.** A real repo + real linked worktree at `<main>/.plan/local/worktrees/p`, swept over 5 `PLAN_BASE_DIR` shapes × 2 cwds: the mislabel fires for **every non-canonical override** (a bare directory *and* `<root>/.plan`), from the worktree root as well as from a subdirectory, and across **all three** `main_*` columns — not "flat override, subdirectory, `main_sha`". The subdirectory is what converts a fail-closed refusal into a silent write, which is a different claim from the one the audit made. Separately, the hypothesis that a mislabelled `main_dirty_files` could turn layer-D into a **false hard block** was tested and **refuted** — `_check_main_dirty_drift` gate 1 (`_handshake_commands.py:335`) excludes the phase-5/6 boundaries where the hole lives. D1's populations (15 registry entries, 3 `main_*`, 4 pre-fix `_repo_root()` callers at `7612c3a^`, 10 `_run_script` sites), D3's both-directions coverage and refuse-before-persist ordering (`:445` before `:447`/`:455`), and D5's three named cases were re-derived independently and all hold | § Correctness review 1 replaced with the measured sweep table; D2's verdict re-scoped to "CONFIRMED for the production branch"; the refuted layer-D hypothesis recorded in Correctness review 3 as a checked negative; G1's mechanism, affected-column set and preamble rewritten |
| A3 | Vacuous evidence | **The audit's own non-vacuity claim was overstated by a factor of two.** § Negative results said all 19 tests but "the four" are in the red set of M1/M2/M3. Re-running all three from a self-taken snapshot: M1 = 6 red, M2 = 4 red, M3 = 2 red (each matching the report exactly), but the **union is 11, not 15** — M3's `a_commit_less_feature_branch…` is shared with M1. **Eight** tests are outside the union, not four. Each of the eight was then examined individually and found unperturbed by construction (two assert deliberately-unchanged branches, two are stubbed `None` guards, one asserts a permit all three mutations preserve, and three read surfaces outside `_invariants.py` — `test_refusal_exits_non_zero_under_strict_verify:235` stubs `cmd_verify` at `:255`) | § Test adequacy now names all 11 union members; § Negative results now states 11-of-19 and enumerates the eight with per-test reasons, and says plainly that their non-vacuity rests on the report's un-re-run M4-M11 |
| A4 | Counts and quotes | Re-derived at check time: registry **15**; `main_*` **3**; pre-fix `_repo_root()` callers **4** (`:336`, `:452`, `:456`, `:516`); `_run_script` **10** sites; tests **10 + 9 = 19** by collection; diff **11** Python files (6 prod, 5 test) from `git show --stat 7612c3a`; snapshot **83306** bytes / `08c3dcfa4f896e3c`; the sibling-shape sweep still returns **exactly one** hit (`:357`); `base_dir_override_active` still has **exactly four** readers at the named lines. **Three stated figures were wrong.** (i) `_diffs` does **not** read "every `main_*` column" — it `continue`s past `main_dirty_files` at `:493-494` (in both documents). (ii) G7's "this plan added a second consumer" of `main_checkout_root()` — the pre-existing population is **four** call sites across three modules, so this is the fifth. (iii) G9's "the four existing members" of `VERIFY_REFUSAL_ERRORS` — the set has **five** (`_cmd_lifecycle.py:46-52`), and the strict-exit tuple has three, so the two already disagree | All three corrected in place, each labelled as a correction so the earlier figure is not silently overwritten |
| A5 | Actionability | Every entry already carried a concrete path, change and observable *Done when* — no "review/consider/investigate". **But G1's action (b) was measured and is unsafe as written.** "Return `None` for a flat override" breaks the suite: `test/conftest.py:1183-1185`'s autouse `_plan_base_dir_sandbox` gives *every* test a flat `PLAN_BASE_DIR`, so the `main_*` columns empty out suite-wide — **7 red** in `test/plan-marshall/plan-marshall/` alone. Conversely action (a) was measured **safe and sufficient**: the containment check closes every silent row in the sweep and leaves 19/19 and 570/570 green. G9's `_cmd_lifecycle.py` was cited by bare basename and lives in a **different skill** (`manage-status`) from every other path in that entry — a later run would look in the wrong directory | G1's Action split into a required (a) with its measured result and an optional (b) carrying the measured 7-test breakage and the sandbox-migration precondition; Effort and Risk re-stated from measurement; G9 and the D3 row given full paths |
| A6 | Severity and topic | **G1's `medium` upheld, but for reasons the entry did not state** — so it was upheld, not merely left alone. Against the calibration: "shipped behaviour is wrong" and "a measurement misreports" are met *in the configuration*, but the high band is for defects reachable on the shipped default path, and this one is not — the sweep shows the fix complete with no override, there is **no writer** of `PLAN_BASE_DIR` under `marketplace/` and no production `set_base_dir()` caller, and the limitation is disclosed in three shipped surfaces, so no documented contract is unimplemented. The guard also **can** fire (it does, from the worktree root), which is the specific high-band limb that would otherwise apply. What is left is an incomplete deliverable with an untested branch = medium. The entry's own supporting claim — *"`PLAN_BASE_DIR` is a documented **user** override … not a test-only hook"*, inherited from `report-01.md:217-218` without re-checking — is only one-third true: `file_ops.py:374` says "tests, user override", but `bootstrap_plugin.py:33` says "(for testing)" and `cwd-policy.md:44` calls it a test-isolation hook. **G3's topic was wrong**: its owning surface is `.claude/skills/audit-archived-plan-retrospectives/`, so `architecture-core` would route it to the wrong fix plan | G1 given an explicit § Severity paragraph justifying medium and correcting the three-source citation; G3 re-topiced to `detectors/auditor` with the reason stated |
| A7 | Coverage | Complete. All five deliverables have verdicts and per-deliverable detail; out-of-scope compliance, collateral, report accuracy and the four-row residue list are all present; the plan's own Verification clauses (resolver-level assertion, bidirectional D3, pre-fix failure, D4's two numbers, `./pw verify`) are each addressed or explicitly declared unchecked. No deliverable is silently unmentioned | None needed |
| A8 | Internal consistency | Verdict CONFIRMED WITH GAPS follows from 4 CONFIRMED + 1 PARTIAL. Every gap traced back to a finding **except G7** — the `cwd-policy.md:79` universal appeared nowhere in `verification.md`, so that gap had no origin in the audit it belongs to. The A3 arithmetic error above was also an internal contradiction (§ Negative results implied a union of ≥15 while § Test adequacy's own 6/4/2 caps it at 12) | G7 given a traced origin as § Report accuracy item 5; the union arithmetic reconciled across both sections |

**Mutation discipline.** `_invariants.py` was snapshotted to
`/tmp/adv-310-main-sha-records-the-pinned-cwd-mutsweep/_invariants.py.orig` before any edit and
written back from that snapshot after each of the five mutations run here (M1, M2, M3, the G1(a)
containment variant, the G1(b) `None` variant). No `git checkout`, `git restore` or `git stash` was
used at any point. Post-restore the file hashes `08c3dcfa4f896e3c…` and `git status --porcelain` lists
no entry for it; the entries it does list belong to concurrent agents and were never opened here.

**Residual doubt:** the largest un-run surface is the report's mutation rows M4-M11 — the eight this
audit's own union does not reach — and with them the non-vacuity of the eight tests named in
§ Negative results; a further round that re-ran those from its own snapshots is where a genuinely
vacuous test would most likely surface. Second most likely: the D4 rule's *other* discriminators
(`invariant-check-summary.md:50-53` argues three alternatives all fail) were read but not
independently falsified — in particular the merge-commit inversion argument, which is the load-bearing
reason the rule rests on `captured_at` at all. Neither is reachable without either the machine-local
corpus or a full-suite budget.

**Verdict on the audit:** SOUND AFTER CORRECTION — every gap it filed is real and every deliverable
verdict survives independent re-derivation, but its non-vacuity claim was overstated two-fold, three
stated counts were wrong, G1's mechanism was drawn too narrowly and one of its two proposed fixes
would have broken the suite, and G7 entered `gaps.md` without an origin in this document.

...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Bootstrap script for plugin root detection"
    )
    ...
    resolve_parser = subparsers.add_parser(
        "resolve",
        help="Resolve a path relative to a bundle"
    )
    resolve_parser.add_argument(
        "bundle",
        help="Bundle name (e.g., 'pm-workflow')"
    )
    resolve_parser.add_argument(
        "path",
        help="Path relative to bundle root"
    )

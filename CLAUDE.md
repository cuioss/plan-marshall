...
- **No shell file operations** — Use Read/Write/Edit/Glob/Grep, never `cat`, `head`, `find`, `ls`, or bare `grep` via Bash. (One carve-out: `git grep <pattern> -- <pathspec>` is the sanctioned broad-content-sweep primitive when the `Grep` tool is not granted — see `plan-marshall:persona-plan-marshall-agent` § "Bash: No file operations".)

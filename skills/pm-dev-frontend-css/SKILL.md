...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/css-essentials.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/css-responsive.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/css-quality-tooling.md" }` before continuing.
```

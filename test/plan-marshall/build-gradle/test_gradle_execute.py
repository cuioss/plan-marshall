...
_PLAN_ID = 'gradle-execute-test-plan'
...
def test_config_declares_the_outer_floor_as_a_literal_300(tmp_path, monkeypatch):
    """Gradle declares its own outer floor explicitly, and it reaches the executor.
    ...
    """
    assert _gradle_execute_mod.GRADLE_OUTER_FLOOR_SECONDS == 300
    ...
    execute_direct(args='build', command_key='gradle:build', project_dir=str(tmp_path), plan_id=_PLAN_ID)
...
def test_config_has_no_require_wrapper_knob():
    ...
    so Gradle auto-detects the wrapper and falls back to the system binary."""
...
def test_execute_direct_absent_wrapper_resolves_system_binary(tmp_path, monkeypatch):
    """execute_direct auto-resolves to the system 'gradle' binary when running in
    an empty directory: with the gate removed, an absent gradlew falls back to the
    ...
    # Empty tmp_path has no gradlew — resolution falls back to the system 'gradle'.
    ...
    assert calls[0]['wrapper'] == 'gradle'


def test_execute_direct_resolves_present_wrapper(tmp_path, monkeypatch):
    """With a present gradlew, resolution prefers the project wrapper ./gradlew."""
    monkeypatch.setenv('PLAN_BASE_DIR', str(tmp_path))
    ...
    result = execute_direct(args='build', command_key='gradle:build', project_dir=str(tmp_path), plan_id=_PLAN_ID)

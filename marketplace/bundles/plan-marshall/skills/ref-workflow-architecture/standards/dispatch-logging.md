...
- [`../../dev-general-practices/standards/agent-behavior-rules.md`](../../dev-general-practices/standards/agent-behavior-rules.md) **lines 196 and 241** — the authoritative rule prohibiting unconstrained generic subagents inside plan-marshall phase work, and the Quick Reference decision-matrix row that directs callers to `plan-marshall:execution-context-{level}` instead. The `[DISPATCH]` emission is the mechanical observable that lets the retrospective audit verify compliance with this rule.

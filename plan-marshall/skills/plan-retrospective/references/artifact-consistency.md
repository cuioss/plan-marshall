...
The footprint is **unresolvable** only when NO resolution tier answers. The shared whole-chain resolver (`_footprint_resolver.resolve_footprint`) runs five RESOLVING tiers in order and then reports the sentinel — **live diff → realized-footprint capture (`references.realized_footprint`) → merge-commit (`references.merge_commit_sha`) → PR-landing (`references.pr_number`) → legacy `references.modified_files` key → unresolvable** — so unresolvable means no live worktree diff (the tier-1 `git` invocation failed, or archived mode skipped tier 1 and no worktree is on disk), no captured realized footprint, no merge-commit recovery, no PR-landing recovery, and no legacy key. The resolver names that population in `_footprint_resolver.RESOLVING_TIERS`; judge against that rather than against a count restated anywhere. A tier-1 diff failure reports unresolvable rather than falling through to a lower tier — the worktree resolved but the diff did not, so a lower tier would answer a different question while presenting as the same measurement. A key that is present but empty is the opposite case: a resolved, genuinely-empty footprint, which still yields a measured verdict.

**The PR-landing tier is the fallback for a capture that never happened, not a redundant fourth answer.** On the async merge-queue / squash path `default:branch-cleanup` writes neither `realized_footprint` nor `merge_commit_sha`, so tiers 2 and 3 fail *together* and the landing is resolvable only from the PR number — which is also the only selector that survives the head-branch deletion the queue performs. The tier is ordered strictly BELOW the merge-commit tier, so a recorded `merge_commit_sha` always decides the answer: this tier can only ever convert an `inconclusive` into a resolved verdict, never change one an earlier tier already gave.

⛔ **Its read is tri-state, and a provider failure is never an absent SHA.** The tier reads the landing commit through `ci pr view --pr-number N`, whose three outcomes — the PR reported merged with a SHA, the provider reported no SHA (an open PR), and the provider could not be read at all — stay distinct. All three unresolvable branches fall through to the next tier, so a provider outage degrades to the same honest `inconclusive` this document already governs; it never manufactures a resolved-but-empty footprint.

> **Note on the emitted finding text.** The `severity: warning` finding this check emits when nothing resolves enumerates the tiers the message was written against and does not name the PR-landing tier. Every clause it does state remains true, and the verdict itself (`inconclusive`) is unchanged and correct — read the tier list above, not the finding string, as the authority on which tiers were consulted.
...
- The actual drift cross-check is delegated to `plan-marshall:plan-retrospective:check-manifest-consistency`, which compares the manifest assumptions against the plan's realized footprint (resolved through the shared footprint resolver, not a diff of its own). Cross-check rules are codified in `standards/manifest-crosscheck.md`.

**The forward has a named receiving rule, and it is `declared_vs_realized_set` (rule M6)** — see [`standards/manifest-crosscheck.md` § Rule M6](../standards/manifest-crosscheck.md#rule-m6-declared-vs-realized-set-comparison-is-received). That rule reads this aspect's emitted fragment (`work/fragment-artifact-consistency.toon`), consumes the `outline_only` / `references_only` sets and the `forwarded_to_manifest` flag from the `affected_files_exact_match` block, grades a non-empty `outline_only` at `warning` and a non-empty `references_only` at `info`, and publishes both set sizes beside its verdict.

⛔ **State the receiver, never merely the intent to hand off.** The downgrade above trades this aspect's own `warn` for a promise that another aspect will report the drift, so the promise is only honoured while a rule actually reads the flag. It previously had no reader at all: the finding was downgraded here and picked up nowhere, so on every manifest-bearing plan the drift was **dropped** rather than re-routed — strictly worse than the duplicate reporting the downgrade exists to avoid. A future change that retires or renames M6 MUST either re-point this paragraph at the new receiver or restore the `warn`; leaving the downgrade with no receiver re-creates the silent drop. The structural guard is `test/plan-marshall/plan-retrospective/test_forwarded_flag_has_a_receiver.py`, which fails when any `forwarded_to_*` flag is mentioned by fewer than two `plan-retrospective` producer scripts — a mention-based proxy for a receiver existing, not proof that one actually reads the flag.

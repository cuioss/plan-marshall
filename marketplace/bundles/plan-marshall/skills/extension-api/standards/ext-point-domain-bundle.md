...
See [extension-contract.md](extension-contract.md) for the complete method contract (required `get_skill_domains()`, optional `config_defaults`, `discover_modules`, `provides_triage`, `provides_outline_skill`, `provides_recipes`, `classify_paths`, `classify_globs`, `classify_build_class`) and the minimal / build-bundle examples.

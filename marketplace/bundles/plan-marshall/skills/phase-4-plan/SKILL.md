...
- `affected_files_count` — `manage-references get --field affected_files`, count entries. This mirrors the deliverables' declared file surface, which for a survey-scope deliverable is its `Files to survey:` / `Files expected to mutate:` pair rather than an `Affected files:` list — see Step 8b § B2 for why the distinction matters to the bypass predicate.

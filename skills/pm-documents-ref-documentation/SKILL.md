---
name: pm-documents-ref-documentation
...
```text
Call the `view_file` tool on `pm-documents-ref-asciidoc/SKILL.md` before continuing.
...
```markdown
# In command file
Call the `view_file` tool on `pm-documents-ref-documentation/SKILL.md` before continuing.

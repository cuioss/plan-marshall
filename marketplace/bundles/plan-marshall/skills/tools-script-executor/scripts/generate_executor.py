...
# Repo-local shim directory name. The shim at {PLAN_DIR_NAME}/execute-script.py
# is a thin stub that exec's the real executor from the per-project global dir.
# Can be overridden via environment variable for alternative deployments.
PLAN_DIR_NAME = os.environ.get('PLAN_DIR_NAME', '.plan')

# The repo-local shim lives at ./{PLAN_DIR_NAME}/execute-script.py so existing
# call sites (CLAUDE.md, skills, docs) keep working verbatim.
SHIM_DIR = Path(PLAN_DIR_NAME)
SHIM_PATH = SHIM_DIR / 'execute-script.py'
...
# Runtime-resolved locations (per-project global directory, respects
# PLAN_BASE_DIR env var for tests).
def executor_path() -> Path:
    """Real executor path: {global_dir}/execute-script.py."""
    return _get_plan_base_dir() / 'execute-script.py'
...
def generate_executor(mappings: dict[str, str], base_path: Path, dry_run: bool = False) -> bool:
    ...
    # Write the repo-local shim at ./{PLAN_DIR_NAME}/execute-script.py so
    # every documented call site (python3 .plan/execute-script.py …) still
    # works. The shim resolves the real executor at runtime from the same
    # per-project global dir and exec's it.
    write_shim(SHIM_PATH)
    return True


SHIM_TEMPLATE = '''#!/usr/bin/env python3
"""Plan-marshall executor shim.

Generated by plan-marshall:tools-script-executor:generate_executor.
DO NOT EDIT MANUALLY — regenerate via /marshall-steward.

This shim exists so the documented call site
...
def main() -> int:
    ...
    if not executor.exists() or executor.resolve() == Path(__file__).resolve():
        sys.stderr.write(
            f'SHIM_ERROR: real executor not found at {executor}\\n'
            'Run /marshall-steward to generate it.\\n'
        )
        return 2
    os.execv(sys.executable, [sys.executable, str(executor)] + sys.argv[1:])
...
def write_shim(shim_path: Path) -> None:
    """Write the repo-local executor shim.

    The shim is a tiny stub that locates the real executor in the
    per-project global dir and exec's it with the inherited argv.
    """
...
def detect_legacy_drift() -> list[str]:
    ...
    """
    repo_plan = SHIM_DIR  # typically Path('.plan')
...
def cmd_generate(args) -> dict:
    ...
    result: dict = {
        ...
        'shim_generated': str(SHIM_PATH.resolve()),
        'logs_cleaned': logs_cleaned,
    }

    # Surface legacy drift — files under the repo-local .plan/ that used to
    # live there but moved to the per-project global directory in PR1. The
    # report is informational; cleanup is manual per the PR1 design.
    drift = detect_legacy_drift()
    if drift:
        result['legacy_drift_count'] = len(drift)
        result['legacy_drift'] = drift
        print(
            f'Legacy drift: {len(drift)} entries still present under {SHIM_DIR}/. '
            'These are no longer read and can be safely deleted: '
            + ', '.join(drift),
            file=sys.stderr,
        )

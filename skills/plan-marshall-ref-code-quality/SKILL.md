---
name: plan-marshall-ref-code-quality
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/code-organization.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/error-handling.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/documentation-principles.md" }` before continuing.
```

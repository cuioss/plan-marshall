...
`requires_prompt_fields` closes it with a both-direction conformance guard, and these two quantified directions are the ONLY declaration-vs-carriage detector scope:

- **No orphan declaration (∃-direction).** Every field in `requires_prompt_fields` MUST appear in the step's own dispatch `prompt:` body. This catches a field declared Required but left to the generic template — which carries only the five generic fields and structurally cannot send a step-specific one, the exact failure mode that motivated this field.
- **No undeclared field (∀-direction).** Every field a step's dispatch `prompt:` body carries beyond the generic contract MUST appear in `requires_prompt_fields`. This catches carriage that has drifted past the declaration.

`test/plan-marshall/phase-6-finalize/test_step_prompt_fields_contract.py` asserts both directions and no third scope, deriving the step population from `find_implementors()` and each obligation from the step's own frontmatter — so a step that declares `requires_prompt_fields` later is covered with no edit to the test.

**`instructions` is a generic-contract field, not a step-specific one.** The dispatch contract requires **exactly one** of `workflow`/`instructions`, so a step that carries `instructions` in place of `workflow` (e.g. `default:finalize-step-simplify`) is dispatching through the generic contract and declares no `requires_prompt_fields` for it. Only a field beyond the six generic-contract names (`name`, `plan_id`, `skills`, `workflow`, `instructions`, `WORKTREE`) is a step-specific field.

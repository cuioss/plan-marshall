...
**Structural enforcement supersedes the prose-only invariant.** The rule above was prose-only for its first shipping cluster, and prose-only hardening proved insufficient across repeated observations of leaves backgrounding builds despite the instruction. The enforcement is now **structural and compose-time**: the phase-5 verification tier is a resolved manifest fact, not a rule the leaf must remember. `manage-execution-manifest` `compose` resolves each `phase_5.verification_steps` entry's `execution_tier` via `architecture resolve` and stamps it into `phase_5.step_execution_tier` (a per-step record list). `phase-5-execute` consumes the stamp: only `per_task`-tier steps are in the leaf's runnable slice, and every `orchestrator`-tier step is routed to the orchestrator's `await-long-running` seam by `plan-marshall/workflow/execution.md` § "Orchestrator-tier phase-5 verification". Because the manifest routed the long build AWAY from the leaf before the leaf ever ran, a dispatched leaf **structurally never receives** an orchestrator-tier build to background-and-lose — the guard no longer depends on the leaf choosing to obey. See [`../../manage-execution-manifest/SKILL.md`](../../manage-execution-manifest/SKILL.md) § "Per-step execution_tier stamping" and [`../../manage-execution-manifest/standards/decision-rules.md`](../../manage-execution-manifest/standards/decision-rules.md) § "execution_tier Stamping" for the composer contract.

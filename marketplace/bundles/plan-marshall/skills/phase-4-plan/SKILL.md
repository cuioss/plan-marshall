...
2. **Peer plan exists for the deletion phase**: A separate plan (lesson-derived or otherwise) carries the deletion scope per the PLAN A / PLAN B split pattern, AND the current plan's deliverables are restricted to the additive surface. Verify by checking `plan_source` metadata or the deliverable narrative for an explicit reference to the follow-up plan id. When present, proceed.

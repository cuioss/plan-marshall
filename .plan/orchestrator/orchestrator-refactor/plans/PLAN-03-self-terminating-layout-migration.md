...
- ⚠ REFUTED AT HEAD (was HYPOTHESIS) — a redirect at the shared resolver is NOT sufficient on
  ...
  - verdict: contradicted | checked_at: 9588b30b317d0312ede90f1982122aa3145ea871 | by: orchestrator-refactor/cleanup | rescoped: yes | evidence: Refutation re-reproduced first-party at HEAD, unchanged. orchestrator inbox detect --source-id against the old .plan/local path returns orchestrated:false, detection:unrecognised_id. _orchestrator_inbox.py:357 _SOURCE_ID_RE still hardcodes ^.plan/orchestrator/; :376 _RETIRED_SOURCE_ID_RE only classifies the old prefix, resolving to unrecognised_id (:1278-1280, documented :1248-1256). No filesystem resolution occurs at that call site. Re-scope already absorbed into D3/D6 and the bullet text; no new re-scope owed.
- HYPOTHESIS — the expiry trigger should be release-count-based (a `marshal.json`
  ...
  (verify-at-outline).
  - verdict: unverifiable | checked_at: 9588b30b317d0312ede90f1982122aa3145ea871 | by: orchestrator-refactor/cleanup | rescoped: n/a | evidence: Both cited symbols survive -- cache_retention.py:135 resolve_knobs, :170 read_provisioned_version, :16/:159 plugin_cache_keep_versions; _config_defaults.py:122/:146/:1371-1372 the system.retention knob family -- but no expiry grammar exists anywhere at HEAD against which release-count-vs-date could be confirmed. shim-remove-when returns count:48/file_count:24/files_scanned:3131; the only reader is still _analyze_shim_marker.py (presence/well-formedness only). A design judgment with no ground truth.
- Verify-first clause: the sweep MUST NOT remove a redirect on marker-absence. Absence of a
  ...
  - verdict: corroborated | checked_at: 9588b30b317d0312ede90f1982122aa3145ea871 | by: orchestrator-refactor/cleanup | rescoped: n/a | evidence: Premise holds, still unimplemented, still a live requirement. skipped_no_tombstone in manage-lessons/scripts/manage-lessons.py(8) + manage-lessons/SKILL.md(4), pinned by test_cleanup_superseded.py(3) and test_lesson_id_resolution.py(4). shim-remove-when now spans 24 files (up from 21 last pass, spec recorded 18), _analyze_shim_marker.py remains the sole reader. D0's re-derivation gate is still load-bearing.
...
- HYPOTHESIS (added 2026-09-21) — other currently-in-flight plans across other epics whose
  ...
  - verdict: unverifiable | checked_at: 9588b30b317d0312ede90f1982122aa3145ea871 | by: orchestrator-refactor/cleanup | rescoped: n/a | evidence: Mechanism half reproduces (see idx 9) but the population remains empty. PLAN-TRUTH-144 is no longer reachable: absent from live truthful-signals corpus (46 specs), present only in archived snapshot truthful-signals-26-09-21 -- it landed. manage-status list now returns total:3, but neither live plan yields a source_id: manage-plan-documents request read for antigravity and the-comment-pipeline-on-the-way-in both return error:document_not_found. No in-flight plan survives whose source_id could be read.

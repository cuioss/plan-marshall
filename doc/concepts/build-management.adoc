...
* **The LLM / task executor** asks for a canonical name by module: `architecture resolve --command verify --module my-module` returns the **full executor-wrapped command** (e.g. `python3 .plan/execute-script.py plan-marshall:build-maven:maven run --command-args "verify -pl my-module"`); the LLM runs that returned string verbatim. The model never has to know what underlying tool the module uses, never has to construct a build-skill dispatch.
...
== Resolve once, then run the returned command

A skill that needs to run a build calls the resolver once. **The resolver returns a complete, ready-to-execute shell command — already wrapped through the script-executor proxy, already bound to the right build skill for that module's tool, already carrying the correct `--command-args` for the canonical command.** The LLM does not separately construct a build-skill dispatch; it simply runs what the resolver returned.

[source,bash]
----
# 1. Resolve the canonical command to a fully-wrapped executable
python3 .plan/execute-script.py plan-marshall:manage-architecture:architecture \
  resolve --command verify --module my-module

# Returns TOON (real example from a Python module):
#   module:     my-module
#   command:    verify
#   executable: python3 .plan/execute-script.py plan-marshall:build-pyproject:pyproject_build run --module my-module --targets verify
#
# For a Maven module the executable would be:
#   python3 .plan/execute-script.py plan-marshall:build-maven:maven run --module my-module --targets verify
# — same shape, same script-executor proxy, just routed to a different build skill.
# Hybrid modules (e.g. Maven + npm in one directory) return `executables[N]{build_system,command}:` instead.

# 2. Execute the returned `executable` string verbatim
{the executable string returned in step 1}
----

The resolver reads `.plan/project-architecture/{module}/enriched.json` (per-module, persisted) and looks up the requested canonical command in the module's `commands` map. The stored value is **the full executor invocation, already pre-composed at architecture-discover time** — the LLM never has to know which build skill (`build-maven` / `build-gradle` / `build-npm` / `build-pyproject`) owns this module, nor how to format the underlying tool's flags. The lookup is deterministic — no LLM judgement, no probabilistic match. If `verify` doesn't resolve, the module legitimately doesn't have it (e.g. pom-only aggregator modules have only `quality-gate`); the LLM gets an error TOON, not a hallucinated guess.

Behind the scenes the executor proxy routes to one of four link:../../marketplace/bundles/plan-marshall/skills/extension-api/standards/ext-point-build.md[`ext-point-build`] implementations (`build-maven`, `build-gradle`, `build-npm`, `build-pyproject`). All four expose the same `run --command-args …` subcommand and the same return TOON, so the LLM's interaction with the build is uniform regardless of which tool sits behind the wrapper.
...
exit_code           0
duration_seconds    45
log_file            .plan/temp/build-output/default/maven-2026-05-18-102315.log
command             ./mvnw -l .plan/temp/build-output/default/maven-2026-05-18-102315.log clean verify
----
...
The `command` field carries the *actual* shell invocation the build skill ran — including the `-l {log_file}` redirect that captures stdout/stderr to the named log. It is not the canonical command string; it is the real, fully-flagged invocation as executed.
...
For each module, the discovery extension also runs the canonical-command resolution rules described in link:../../marketplace/bundles/plan-marshall/skills/extension-api/standards/canonical-commands.md[`canonical-commands.md`]:
...
| link:../../marketplace/bundles/plan-marshall/skills/build-pyproject/SKILL.md[`build-pyproject`] | Pyprojectx (`./pw`) | mypy type-checking, ruff linting, pytest with Cobertura coverage, test-directory-based module discovery.
|===
...
2. architecture resolve --command verify --module {module}
   → returns executable: "python3 .plan/execute-script.py
                           plan-marshall:build-maven:maven run
                           --command-args \"verify -pl {module}\""
3. Run the returned executable verbatim via Bash.
   ⏳ … 7 minutes pass …
4. Result TOON returned by the build skill:

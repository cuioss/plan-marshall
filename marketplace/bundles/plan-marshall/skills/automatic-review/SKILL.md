...
This step fires on a **re-entry** of `plan-marshall:automatic-review` after a phase-5 loop-back: a fix commit produced during the loop-back has advanced the worktree HEAD past the `reviewed_commit_sha` stamped on the staged `pr-comment` findings, so the bot reviews on record are stale for the new tree. It is gated by the `re_review_on_loopback` config knob (default `false`) and reuses the D2 `bot_kind`-keyed re-review registry — it posts an explicit trigger comment for each participating bot in `required_bots ∪ optional_bots` (each bot's `trigger_comment` from its registry doc), since a registered bot's auto-review-on-push cannot be relied on in general — it is a per-repository property of that bot's caller workflow, not of the bot, and a bot whose review is rate-limited or refused produces no evidence however it was triggered. ⚠ For `pr-agent` specifically this is repository-dependent and must be read from the caller: in **plan-marshall** its workflow includes `synchronize`, so every pushed HEAD IS reviewed automatically and an explicit trigger comment is a redundant belt-and-braces path rather than its only one; in a repository whose caller omits `synchronize` the trigger comment remains the ONLY re-review path. See `standards/pr-agent.md` § "Signal calibration". The fresh review is then surfaced through the existing `fetch_findings` FIND below and consumed by the dispatcher-owned unified triage — this is NOT a parallel path.

...
| `cmd_consult` / `cmd_list` / `cmd_get` / `cmd_set_body` / `cmd_set_title` | A | **JUSTIFY (read paths whose zero is not authority-bearing).** Each resolves through `get_lessons_dir()` (main-anchored) and reports a lesson set or a single lesson. None drives a destructive or existence-proof decision on an empty read: `cmd_consult` returns a surfaced set the outline author judges, `cmd_list` is a listing, and the three single-lesson verbs return an explicit error rather than an empty success. That last clause no longer rests on a single error value: `cmd_get` now resolves through the three-state `resolve_lesson` seam and renders `not_found` for an absent file and the distinct `unresolvable` for one that is present and could not be read, while `cmd_set_body` / `cmd_set_title` keep their own presence check plus an explicit `malformed_lesson`. The justification is STRONGER under the split, because the failure it guards against — a record that exists being reported as one that does not — is now unrepresentable on this path rather than merely unlikely. |
...
| The `get_lessons_dir()` call sites (`add`, `remove`, `supersede`, `convert-to-plan`, `cleanup-superseded`, `retire-quiet`, …) | A, B | **JUSTIFY (main-anchored write paths, fail-closed on absence) — re-justified against the shipped two-value contract.** All resolve through the main-anchored resolver, so Direction B is closed. None reads an absence as authority: the write verbs create the store when missing, and the mutate verbs return an explicit error rather than treating an unreadable store as an empty one. Of this subject list, the TWO-value `not_found` / `unresolvable` contract applies only to the members that route through `resolve_lesson` — `remove` and `supersede` — for whom `not_found` reports an absent lesson and `unresolvable` a present-but-unreadable one. `convert-to-plan` is a member of this row's *subject* (it IS a `get_lessons_dir()` call site) but not of the two-value contract: it gates on a bare `source.exists()` and returns a single `not_found` for an absent source, never `unresolvable` — it relocates via a byte-for-byte `read_bytes`/`copyfile`/read-back-verify cycle that never decodes the content, so a present lesson always converts regardless of whether its metadata or encoding is readable, which is correct behaviour for a relocation verb rather than a gap in this one. The re-justification matters for `remove` because `remove --allow-unreadable` adds a **deletion path that runs over an unreadable read**, which the previous wording implicitly ruled out by having no such path exist. The authority for that deletion is NOT the read: it is the operator's explicit opt-in flag, absent by default, and the tombstone is written before the unlink so the record of the retirement outlives the file even on that path. An unreadable read still authorizes nothing on its own. |

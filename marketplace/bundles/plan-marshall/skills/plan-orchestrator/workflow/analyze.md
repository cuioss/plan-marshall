...
Corroboration and this verb's write-bound **drafting** work are dispatchable, gated by the [Dispatch Decision Rule](../../persona-plan-orchestrator/standards/orchestration-model.md#dispatch-decision-rule) — which owns the three tests, the S1 / S2 safety constraints, and the fall-back clause, none of them restated here. **Five** sub-steps of this verb are dispatchable, and the bullets below are a complete index of them: the one landing corroboration, the three drafting sub-steps, and the untrusted-text extraction.
...
- **Dispatchable** — corroborating a full-ship landing against **first-party** ground truth (the real diff, the real code, the plan's on-disk artifacts, CI check state) when the read burden is large. The vehicle is `execution-context-{level}`, because the corroboration needs Bash for `git` and for the read-side `plan-marshall:tools-integration-ci:ci` calls; the prompt body carries the S1 read-only instruction. The dispatch level is config-resolved via `manage-config effort resolve-target --role orchestrator.analyze --plan-id none --caller plan-marshall:persona-plan-orchestrator --workflow {the doc the leaf loads}` (the four facts are NOT optional decoration: `--workflow` is what makes the resolve seam emit the `[DISPATCH]` line and its paired decision-log record, so a bare `--role` resolve leaves this dispatch with no trail at all — see [the canonical form](../../persona-plan-orchestrator/standards/orchestration-model.md#dispatch-decision-rule)) — the `orchestrator.analyze` surface, resolved through `orchestrator.effort.analyze` → `orchestrator.effort.default` → `plan.effort` → `inherit` and clamped by `orchestrator.effort.max` (see [`effort-roles.md` § Orchestrator role group](../../plan-marshall/standards/effort-roles.md)); its `target` field is the `execution-context-{level}` variant to dispatch. Return shape: `corroborations[N]{claim,verdict,evidence}`, with `verdict` one of `corroborated` / `contradicted` / `unverifiable`. The orchestrator consumes the return as data and performs every resulting ledger write itself.
  ...
  **Also dispatchable — this verb's three write-bound drafting sub-steps.** Each one only COMPOSES what a later write will say, so each passes write-freedom under the refined test; the leaf writes nothing, and every APPLY stays inline below. All three ride the same `orchestrator.analyze` surface and the same resolve form as the corroboration above — fired once for this envelope, with `--workflow` naming the doc this leaf loads, since the resolve seam emits one dispatch record per firing — and they are dispatched as **ONE envelope that iterates internally over the drain's messages, never one envelope per message**:

  - **The Step 4 item 1 landing-report body** — drafted from the corroborated facts and returned as structured data, never written by the leaf. One envelope drains every `kind: landing` message in the same firing, so the shape is cardinal like its siblings. Return shape: `landing_reports[N]{plan,body}`.
  - **The Step 5b per-message dispositions** — one proposed disposition per message, each with its rationale. Return shape: `proposals[N]{message,disposition,rationale}`, where `disposition` is one of the closed four-member vocabulary `promote` / `fold` / `stage` / `discard`, and the leaf observes the same restriction the Step 5b table states: `promote` is available only for a `kind: candidate-lesson` message.
  - **The Step 5b Stage new-spec body** — the content of the spec a `stage` disposition would create. Return shape: `spec_drafts[M]{message,plan_slug,body}`, where `message` is the SAME message identifier the owning `proposals[]` row carries — the correlation key that ties a draft to the proposal it belongs to. Without it, two or more `stage` proposals drafted in the same firing carry no ordering, no one-draft constraint, and no shared key: `plan_slug` exists only on the draft and `message` only on the proposal, so the orchestrator cannot tell which spec body applies to which message. The key does not retreat from the one-envelope-not-per-message contract (decision.log `5337c3`) — it is what makes that contract safe.

  Each of these returns is a **proposal the orchestrator adjudicates**, never a decision it applies — the same standing [`decompose.md`](decompose.md) gives its candidate mapping.

  ⛔ **A drafted spec body MUST NOT reserve a monotonic resource by assumption** — no assumed `PLAN-NN` ordinal, no assumed ADR number, no assumed step-order slot. The draft states the SELECTION RULE ("the next free plan ordinal") and the orchestrator performs the allocation when it applies the write, because the leaf drafts against a HEAD that a sibling landing may already have moved past.
...
- **Inline-only**, each entry naming the test it fails — parsing the operator's own paste, the rule's already-in-context clause (**depth**); Step 3 granularity classification and the inbox-scan drain loop, whose enumeration and archival are deterministic script calls (**depth**); every fork-prone judgement, namely Step 5's absorb-versus-escalate classification, which is small besides, and the Step 4 item 7 / Step 5 item 4 proactive emit, which the rule reserves outright because `next` is never dispatched (**fork-freedom**); and every **APPLY** (**write-freedom**) — the Step 2b `corpus set-verdict` calls, the Step 3 item 3 `epic.md` Open Defect entry for an invalid message, the Step 3 item 4a `epic.md` Open Defect entry for a failed archival, the Step 4 item 1 `Write` of `landings/PLAN-NN.md`, the Step 4 `complete: false` branch's `epic.md` Open Defect entry for an incomplete landing, the item 2 `queue --transition` and the three item 3 `queue --set-row` stamps, the item 4 `epic.md` narrative reconciliation, the item 6 `regenerate-view` call, the Step 5 absorb-arm Watch / Open Defect entries, the Step 5b corpus, spec and queue writes, and the two-sided decision-log record every disposition owes, including Step 6's logging, the resume-anchor write, and its `regenerate-view` call. The drafting sub-steps above supply the CONTENT of several of these writes; performing them stays here.
...
2. Mark the plan shipped in its queue row, `queue/PLAN-NN.json`:
...
3. Stamp the plan's three result fields, one `--set-row` call per field, so the landing is reconciled COMPLETELY by sanctioned calls. A `shipped`/`landed` row missing `pr` or `landing` renders the `(!) missing: ...` gap marker in the START-HERE block of `queue-view.md`, regenerated at item 6, so an unstamped row is visible rather than silent:
   ...
   `queue --set-row` is THE stamping mechanism, and it is one of the two sanctioned queue write forms — single append via `queue --add-row` (which is also how `decompose` seeds a queue), and single mutate via `queue --transition` / `queue --set-row`. The boundary is stated once at [`orchestration-model.md` § The queue-write boundary](../../persona-plan-orchestrator/standards/orchestration-model.md#the-queue-write-boundary); what it means here is that each stamp read-modify-writes ONLY the located row file, `queue/PLAN-NN.json`, inside that file's own critical section, so stamping one landing never touches another plan's row and a concurrent session acting on a different plan writes a different file. There is no whole-queue write form to stamp with. Editing a ledger file by direct file access is prohibited outright (see the skill's Enforcement block).

4. Reconcile the hand-written narrative in `epic.md` against the ledger: START HERE and the Ordered Queue table are GENERATED into `queue-view.md` at item 6 (⛔ **never hand-edit `queue-view.md`, and never paste either block into `epic.md`**), so here you move only the *narrative* sections the generator does not own — retire queue items the landing folded in (with a decision naming what absorbed them), move resolved Open Defects out, retire satisfied Watches, add new defects/watches the landing surfaced, and record any per-row sequencing caveat in the `## Queue annotations` zone, keyed by plan id. A landing whose `cleanup_owed` fact is `true` surfaced branch cleanup nothing re-entering will discharge: add a Watch naming the plan, its `merge_state`, and the owed cleanup, and retire it only once that cleanup is confirmed done — `complete: true` establishes that the fact was carried, and this is where it is acted on.
...
6. Regenerate the generated view — START HERE and the Ordered Queue — from the ledger files:
   ...
   The regenerated `queue-view.md` is committed together with the queue change it renders. A `row_unreadable` or `ledger_unreadable` refusal writes nothing: resolve the named source file first, then regenerate.
...
| **Stage** | Stage a NEW spec plus its queue entry, appending the row with `queue --add-row` (see [`orchestration-model.md` § The queue-write boundary](../../persona-plan-orchestrator/standards/orchestration-model.md#the-queue-write-boundary)) — one new row file, `queue/PLAN-NN.json`, created atomically and touching no other row. |
...
START HERE renders the anchor, so regenerate the view after writing it, and commit the anchor file and `queue-view.md` together:

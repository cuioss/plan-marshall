...
* **Per-iteration only when parallel-or-different-models**: spinning up N parallel dispatches is justified when each subagent runs independently — the sole such case in the marketplace is the `enrich-module` workflow dispatched under `--phase phase-6` from `architecture-refresh` Tier-1.

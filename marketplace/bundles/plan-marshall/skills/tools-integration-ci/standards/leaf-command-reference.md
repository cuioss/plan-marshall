...
| `checks wait` | `--pr-number` | `--error-style {maven\|gradle\|npm\|generic}` | Poll CI until completion. Use Bash timeout ≥ 1800000 ms (30 min safety net — the Claude-target outer ceiling configured for remote-CI waits; the host cap is resolved per active target through the platform-runtime `harness bash-timeout-ceiling` seam) |

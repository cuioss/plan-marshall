...
| A7 | Coverage | **Complete for the deliverables.** All five have a verdict row and a detail section; out-of-scope compliance, undeclared collateral, report accuracy and the three declared residue items are each covered, and the document is honest about what it did not check. One *analytical* hole rather than a coverage hole: D1's own pairing logic was read for its `not_evaluated` guard but not for its comparison semantics, which is where G15 lives. | Archived-mode addendum added under "Not checked"; **G15 added (medium)** — `shape_violation` pairs by per-role count, and `role=verification-feedback` has both a seam-emitting producer (`plan-marshall/workflow/execution.md:287-292`) and a hand-writing one (`workflow-pr-doctor/SKILL.md:30-38`) at HEAD, so a hand-written line cancels a genuine shortfall one-for-one. |
| A8 | Internal consistency | The overall verdict did **not** follow from the rows: the summary asserted *"each satisfies its literal Done when"* while the table graded D3 PARTIAL, and D1's own detail conceded a bare `0` in the shipped output while the row said CONFIRMED. Gap↔finding tracing is otherwise sound in both directions: all twelve original gaps trace to a numbered correctness defect, a report-accuracy bullet, a test-adequacy row or a residue row, and every finding warranting action appears in `gaps.md`. | Summary rewritten to state per-deliverable outcomes; D1 → PARTIAL; overall verdict → PARTIAL. The three new gaps are traced from the D2 detail (G13), the report-accuracy bullet (G14) and the D1 detail (G15). |

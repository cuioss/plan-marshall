...
| `cmd_project set` (lines 149-211) | (a) | **JUSTIFY.** Already fails closed: rejects unknown fields (`unknown_field`), invalid `pr_strategy` / `pr_compact_max_changed_files` values (`invalid_value`), a non-`str` or empty `user_language` (`invalid_value`, which is what stops `_coerce_value` persisting a `bool` for `--value false`), and non-list/non-string `working_prefixes` (`invalid_type`, `invalid_json`) before `save_config`. This is the pattern D4 generalizes. |
| `cmd_project get` (lines 125-147) | (b) | **JUSTIFY.** Returns `field_not_found` for an unknown field rather than a vacuous value; falls back to `DEFAULT_PROJECT` only for known fields. A `user_language` read from the live block is additionally re-validated at this read boundary and refused with `invalid_value` — the read-side mirror of the `set` arm above, and the same read-boundary re-validation `pr-decision` performs below. It is load-bearing rather than belt-and-braces: `marshal.json` is operator-editable and `sync-defaults` preserves an already-present key without inspecting its value, so an unguarded non-string persists indefinitely and reaches the language rule, which resolves anything other than `auto` as a pin. The `DEFAULT_PROJECT` fallback arm carries no guard — `get_default_config` self-validates the seed. |
| `cmd_project pr-decision` (lines 213-245) | (b) | **JUSTIFY.** Re-validates the resolved knobs at the read boundary and rejects a negative `--changed-files`; a corrupt marshal.json fails loud here rather than producing a wrong verdict. |
| `cmd_system` unknown sub-noun / verb (line 98); `cmd_project` unknown verb (line 247); `cmd_plan` unknown sub-noun (line 261) | — | **JUSTIFY.** All terminate in `error_exit`, never a silent success. |

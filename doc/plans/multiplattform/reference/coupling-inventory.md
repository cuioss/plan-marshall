...
|---|---|---|
| `extension-api/scripts/configurable_contract.py` — the project-local step-doc resolver | Builds `project_root / '.claude' / 'skills'` segment-wise instead of routing through `get_project_skill_roots()` — the same shape as the implementor scan one file over, which plan `030` closed; the two were not registered together, so this half survived | — |
| `marshall-steward/scripts/bootstrap_plugin.py` — `_detect_claude_root`, `_detect_opencode_root` | Branches on the target name and composes each target's cache/skill roots inline, rather than reading them from that target's own runtime. The bootstrap runs *before* the plugin is resolvable, so the layout helpers may genuinely be unavailable here — the remedy needs establishing, not assuming, which is why this is registered rather than scoped | — |
...
| `tools-permission-doctor/scripts/permission_common.py`, `tools-permission-fix/scripts/permission_fix.py` | Bound to `claude_runtime` by direct import rather than through the runtime registry: settings load/save, both settings-path selectors, and (since plan `030`) the default-permission renderer all resolve to the Claude implementation whatever `runtime.target` says. Behaviour is correct today because only `claude` and `opencode` are registered and the OpenCode path never reaches these scripts, but it fails the [principles §6](principles.md#6-open-to-further-targets) cost bar — adding a target would mean editing these general skill scripts. The same file's settings-mapping argument is the other half: the Claude settings-file *shape* crosses into `ensure_default_permissions` as a parameter, which §1 forbids; closing that means restructuring every `permission_fix` subcommand, not one call | — |
| `tools-permission-doctor/scripts/permission_doctor.py` analysis rules + the three permission standards documents (`permission-architecture.md`, `permission-validation-standards.md`, `permission-anti-patterns.md`) | The Claude permission model as analysis subject matter (`Skill()`/`Bash()`/`Write()` grammar, anti-pattern regexes, `.claude/commands/`) — rule-pack-class knowledge, a different shape than live render/resolve residue. **`permission-architecture.md` § "Resolution Priority" additionally states the READ preference backwards** — it has `.claude/settings.json` winning, where `get_project_settings_path` has always preferred `.claude/settings.local.json`. Plan `030` found this, and left it: the file is named in that plan's own Out of scope, so correcting it there would have been an adjacent fix in another plan's surface | — |

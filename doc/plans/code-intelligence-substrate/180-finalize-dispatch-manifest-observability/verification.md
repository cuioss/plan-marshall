...
- **Found:**
  ...
  - **Incompleteness 2 — `loop_back`.** `_emit_completion_marker` is called with no inspection of `outcome`, so a `loop_back` recording (a step that explicitly has **not** settled — `SKILL.md:1100` calls it a "PRODUCTIVE non-completion") emits `[STEP] … Completed step: {step}`. That contradicts the documented principle two paragraphs later at `SKILL.md:1297` ("the item-7a `defer` branch records nothing (the step did not settle, so it owes no completion)") and `manage-status/SKILL.md:374` ("the **terminal** write emits"). The audit's `completion_count` denominator (`check-dispatch-audit.py:521`) counts these lines.
...
- **Checks run:** I re-derived the population independently — `find_implementors('plan-marshall:extension-api/standards/ext-point-finalize-step')` returns **26** step docs; `.plan/marshal.json` registers **25** finalize steps; `registered − implementors = ∅`. So the derivation genuinely covers the whole registry rather than a subset. The full file (30 tests) passes at HEAD.
- **Verdict:** CONFIRMED. One latent brittleness is recorded as a gap: the one implementor that is *not* registered (`default:emit-landing`) would turn a future self-classification sentence into a hard `assert` failure with a misleading message rather than a classification comparison.
...
1. **`_cmd_mark_step.py:209-216` (called at `:420`, `:478`) emits `[STEP] … Completed step:` for `outcome=loop_back`.** Input: any dispatched finalize step that records a loop-back (the ordinary `automatic-review` / `sonar-roundtrip` FIX path, `SKILL.md:1100`). Consequence: the work log asserts a completion for a step the dispatcher will re-fire, and the same step emits a second completion line when it finally settles; `check-dispatch-audit.py:521` counts every such line into `completion_count`, the D3 ratio denominator. The behaviour contradicts `SKILL.md:1297` and `manage-status/SKILL.md:374`, both of which describe the emission as riding a **terminal** write.
...
3. **`standards/finalize-step-simplify.md:113-127` performs a bare resolve and spawns with no emission at all.** Consequence: a finalize spawn that leaves no `[DISPATCH]` line and no resolve record — the precise "dispatch count < spawn count" condition D2 exists to eliminate.
...
5. **Second-order:** `check-dispatch-audit.py:91-95`'s corrected comment ("Every finalize `[DISPATCH]` line carries this caller **because** each finalize dispatch resolves its target with `--caller …` and the resolve seam emits") is true only of the SKILL.md sites; defects 2 and 3 are counter-examples. No behavioural consequence — the caller-scoped count still works — but the stated reason is not the whole truth.

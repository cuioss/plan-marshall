...
- **Action:** make the contradictory state **impossible to persist**, not merely visible. A warning is not sufficient on either write path: `enrich_skills_by_profile` persists the block and returns `status: success` even when `_validate_skills_by_profile_structure` warns (`_cmd_enrich.py:469-485`, recorded in `verification.md` § D2), so a validator that only flags `minimal: true` on a populated profile still leaves the populated+`minimal` block on disk and phase-4-plan still sets `task.skills = []`. Pick one of three dispositions and apply it on **both** write paths:
  1. **Reject the write** — `_validate_skills_by_profile_structure` returns an error rather than a warning for a profile carrying `minimal: true` together with any `defaults`/`optionals` entry, and `enrich_skills_by_profile` does not persist it; or
  2. **Normalize on write** — drop the `minimal` key during normalization whenever the profile carries (or gains) at least one entry, in `enrich_add_domain` (`:332-348`) *and* in `enrich_skills_by_profile`; or
  3. **Change the consumer** — `phase-4-plan/SKILL.md:314-316` tests emptiness **before** `minimal`, so a populated block always yields its entries and the `minimal` short-circuit is reachable only for a genuinely empty profile.

  Record which disposition was chosen and why; disposition 2 is the smallest change and keeps the read path unchanged, but any of the three closes the state.
- **Done when:** after the change, **no supported CLI sequence leaves a populated profile carrying `minimal: true` on disk, or (disposition 3) no such block can zero a task's skills** — and tests assert this through **both** entry points, not one:
  - through `enrich add-domain`: declaring a profile `{"defaults": [], "optionals": [], "minimal": true}` and then enriching it with a domain that supplies skills yields a persisted block with the skills and no `minimal` key (dispositions 1-2), or a task whose `skills` carries those entries (disposition 3);
  - through `enrich skills-by-profile`: writing the contradictory pair directly is refused (disposition 1) or normalized (disposition 2), asserted on the **persisted** block rather than on the returned `warnings[]` — a `status: success` with a warning and the block still on disk does **not** satisfy this criterion.
- **Effort:** S

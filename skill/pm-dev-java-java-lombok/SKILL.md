...
```text
Call the `read` tool with `{ filePath: "standards/lombok-core-annotations.md" }` before continuing.
```
...
```text
Call the `read` tool with `{ filePath: "standards/lombok-canonical-methods.md" }` before continuing.
```

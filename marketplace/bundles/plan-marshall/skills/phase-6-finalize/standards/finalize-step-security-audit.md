...
1. **The `security_class_inactive` pre-filter** — drops the step only when `affected_files_count == 0` **AND** the plan's live footprint is resolvable and genuinely empty. An UNRESOLVABLE footprint (the worktree not yet materialised) is no evidence about the change surface rather than evidence of an empty one, so it keeps the step — the same fail-toward-inclusion discipline the absent `change_type` leg follows. It is deliberately a *fail-toward-inclusion* gate: there is no `change_type` leg, because `change_type` — even reconciled to the plan's settled classification — is orthogonal to the security surface. A `bug_fix` or `feature` plan can equally touch security-sensitive code, so a security sweep gates on the change **surface**, not on the plan's change type. Only a plan with no change surface at all — nothing declared and nothing in the worktree — has nothing to audit.

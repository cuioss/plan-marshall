---
name: plan-marshall-manage-references
...
| `domains_provenance` | string | Why each domain in the set survived — or did not. Records, per domain, which inclusion leg or legs claimed it (`task` / `always_on` / `file_globs`), or `none` where no leg did, or `system_exempt` for the synthetic `system` domain — which is exempt from leg evaluation rather than claimed by a leg, so it is the one value that is not a leg name and never appears alongside one. Written by phase-3-outline's narrowing step from `manage-config domain-narrow`'s return, as the compact one-line `{domain}={legs}` rendering (`;`-separated across domains, `+`-separated within one domain's legs — e.g. `documentation=none;python=file_globs;plan-marshall-plugin-dev=always_on`). Written **alongside** `domains`, never instead of it: `domains` says which domains the plan carries, and this key says on what evidence, so a narrowed set stays distinguishable from an over-provisioned one after the fact. Written on **both** narrowing outcomes — a `narrowed: false` run records a full provenance line too — so its PRESENCE proves the narrowing pass ran and its content says what the pass found. Absent only on a plan the pass never ran on at all (a plan that predates the step, or whose outline phase did not complete), which is why gating the write on `narrowed: true` would defeat the key's purpose: "examined, nothing droppable" and "never examined" would then be indistinguishable. |

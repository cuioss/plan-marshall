...
class TestFormatCommit(unittest.TestCase):
    ...
    def test_basic_format(self):
        ...
        stdout, _, code = run_script(['format-commit', '--type', 'feat', '--subject', 'add new feature'])
    ...
    def test_all_commit_types(self):
        ...
        for commit_type in valid_types:
            stdout, _, code = run_script(['format-commit', '--type', commit_type, '--subject', 'test subject'])
    ...
    def test_validation_warning_long_subject(self):
        ...
        stdout, _, code = run_script(['format-commit', '--type', 'fix', '--subject', long_subject])
    ...
    def test_validation_error_very_long_subject(self):
        ...
        stdout, _, code = run_script(['format-commit', '--type', 'fix', '--subject', very_long_subject])
    ...
    def test_validation_warning_past_tense(self):
        ...
        stdout, _, code = run_script(['format-commit', '--type', 'fix', '--subject', 'fixed the bug'])
        self.assertEqual(code, 0)
        result = json.loads(stdout)
    ...
    def test_claude_footer_present(self):
        ...
        stdout, _, code = run_script(['format-commit', '--type', 'feat', '--subject', 'add feature'])
        self.assertEqual(code, 0)
        result = json.loads(stdout)
...
"""
        with tempfile.NamedTemporaryFile(mode='w', suffix='.diff', delete=False) as f:
            ...
            stdout, _, code = run_script(['analyze-diff', '--file', f.name])
            self.assertEqual(code, 0)
            result = json.loads(stdout)
            self.assertEqual(result['status'], 'success')
    ...
    def test_analyze_file_not_found(self):
        ...
        stdout, _, code = run_script(['analyze-diff', '--file', '/nonexistent/file.diff'])

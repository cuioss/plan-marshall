...
| 17 | Phase dispatch boundaries — **injected, never dispatched**: no `collect-fragments add` command exists for this key and none can, because its spelling is underscored while the registry header requires hyphenated keys and the `--aspect` pattern admits only `[a-z][a-z0-9-]*`. `analyze-logs` emits the block under this key in its own result (`read_dispatch_boundaries_per_phase`), and `compile-report` renders it from the bundle under the same key. | `dispatch_boundaries` | `analyze-logs` (emitted inside the log-analysis result, not registered separately) | `references/log-analysis.md` |

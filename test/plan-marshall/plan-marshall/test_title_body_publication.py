...
def _title_body_path(ctx: PlanContext) -> Path:
    """Return the title-body.txt path inside the active fixture plan dir."""
    assert ctx.plan_dir is not None
    return ctx.plan_dir / TITLE_BODY_FILENAME


def _read_title_body(ctx: PlanContext) -> str:
    """Read the title-body artifact stripped of the trailing newline."""
    return _title_body_path(ctx).read_text(encoding='utf-8').rstrip('\n')
...
def test_create_writes_single_trailing_newline():
    """atomic_write_file appends exactly one '\\n' — no double newline."""
    with PlanContext(plan_id='tb-newline') as ctx:
        _create_plan('tb-newline', 'Newline test', '1-init,2-refine')
        raw = _title_body_path(ctx).read_text(encoding='utf-8')
...
def test_set_phase_republishes_title_body():
    """cmd_set_phase must rewrite title-body.txt to reflect the new phase."""
    with PlanContext(plan_id='tb-setphase') as ctx:
...
def test_transition_republishes_title_body():
    """cmd_transition must rewrite title-body.txt to the next phase."""
    with PlanContext(plan_id='tb-transition') as ctx:
...
def test_transition_last_phase_deletes_title_body():
    ...
    with PlanContext(plan_id='tb-last') as ctx:
        ...
        assert _title_body_path(ctx).exists()
        ...
        assert not _title_body_path(ctx).exists(), (
...
def test_archive_deletes_title_body():
    ...
    with PlanContext(plan_id='tb-archive') as ctx:
        ...
        archive_root = ctx.fixture_dir / 'archived-plans'
...
def test_read_cold_bootstraps_missing_title_body():
    ...
    with PlanContext(plan_id='tb-cold') as ctx:
...
def test_read_does_not_republish_when_file_present():
    ...
    with PlanContext(plan_id='tb-noop') as ctx:
...
def test_render_without_short_description_produces_no_trailing_segment():
    """When status['short_description'] is absent or empty, the
    artifact renders as ``pm:{phase}`` with NO trailing colon segment."""
    with PlanContext(plan_id='tb-noshort') as ctx:
...
def test_partial_write_does_not_corrupt_existing_file():
    ...
    bit-identical.
    """
    with PlanContext(plan_id='tb-atomic') as ctx:
        ...
        original_mode = ctx.plan_dir.stat().st_mode
        os.chmod(ctx.plan_dir, 0o500)  # r-x------
        try:
            result = run_script(
            ...
            os.chmod(ctx.plan_dir, original_mode)

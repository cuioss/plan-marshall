...
- As a **safety net** against a missed clear (an interrupted or killed call whose clear never fires), a stale `build-busy` token is defensively cleared at the next phase transition / `set-phase`, so it cannot leak the 🔨 hammer icon across a phase boundary; the manual clear-and-push after the bracketed op above remains the primary mechanism. See `manage-status` [`status-lifecycle.md`](../../manage-status/standards/status-lifecycle.md) § Title Token for the phase-writer clear mechanics — not duplicated here.

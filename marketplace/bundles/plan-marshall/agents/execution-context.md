...
- **`Grep` / `Glob` are declared, but the harness MAY revoke them at runtime.** The `tools:` frontmatter above grants `Grep`, and the project `.claude/settings.json` denies nothing; the revocation happens at harness runtime, above both project surfaces, so no edit in this repository widens it back. What the revocation produces is an asymmetry: the `Grep` permission is withdrawn while the project's prohibition on bare Bash `grep` / `find` stays fully enforced by the file-operation hard rule and its enforcement hook — so the hard rule goes on prescribing a content-search remedy the leaf cannot reach. Navigation is unaffected: the leaf discovers through the structured architecture inventory — `architecture find --pattern P`, `architecture which-module --path P`, `architecture files --module X` (structured queries first, per CLAUDE.md) — and scans inside an already-known file with `Read`. For a broad content sweep across prose that those cannot cover, the leaf MUST NOT silently degrade to spot-checks: it runs the sweep with **`git grep`** (e.g. `git grep -n "pattern" -- marketplace`), which reaches the hard rule's own prescribed remedy through the one Bash program family that rule permits — see the carve-out in [`persona-plan-marshall-agent` § Bash: No file operations](../skills/persona-plan-marshall-agent/SKILL.md#bash-no-file-operations) and its worked form in [`tool-usage-patterns.md` § Broad content sweep](../skills/persona-plan-marshall-agent/standards/tool-usage-patterns.md#broad-content-sweep). Returning the coverage gap to the main-context orchestrator is the residual path only — for content that is not in git-tracked files, and therefore invisible to `git grep` as well. The `allowed-tools-body-drift` hook stays clean because the agent body invokes no `Grep:` / `AskUserQuestion:` directive, so the granted set and the declaration agree.

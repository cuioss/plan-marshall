...
#:
#: The ids are looked up BY PREFIX rather than zipped onto
#: ``_mod.HARNESS_NOTICE_PREFIXES`` by position, because the rows come from that
#: tuple and a parallel id list is only correct while the two orders agree —
#: nothing here would notice them diverging, and every row would be silently
#: mislabelled. Keyed on the prefix, a reordering carries each id along with its
#: own prefix, and a prefix added to the tuple without a name here raises a
#: KeyError at collection rather than arriving unlabelled.
_NOTICE_ID_BY_PREFIX = {
    'This session is being continued from a previous conversation': 'session-continued',
    'Caveat: The messages below were generated by the user while running local commands': ('local-command-caveat'),
    'Stop hook feedback:': 'stop-hook-feedback',
}

_NOTICE_IDS = [_NOTICE_ID_BY_PREFIX[prefix] for prefix in _mod.HARNESS_NOTICE_PREFIXES]
...
class TestOperatorProseSurvives:
    ...
    def test_unmatched_open_tag_does_not_swallow_operator_prose(self):
        ...
        """
        assert _mod.strip_harness_envelopes('<unclosed> keep this prose') == ('<unclosed> keep this prose')
...
class TestOperatorBearingEnvelopes:
    ...
    def test_whitespace_only_command_args_is_not_operator_signal(self):
        ...
        assert _mod.is_operator_authored(f'{STOP_HOOK_NOTICE}\n<command-args>  </command-args>') is False

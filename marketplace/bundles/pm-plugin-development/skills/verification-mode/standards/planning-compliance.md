...
| 4-plan | task-contract.md | `manage-tasks:manage-tasks list --plan-id {plan_id}` plus `manage-tasks:manage-tasks read --plan-id {plan_id} --task-number {N}` for each task |

...
* xref:skill-handling.adoc[Concepts › Skill Handling] — why `dev-agent-behavior-rules` loads Tier-1 on every dispatch.
* xref:automatic-reviews.adoc[Concepts › Automatic Reviews] — the findings-pipeline architecture Layer 3 sits on top of.
* link:../../marketplace/bundles/plan-marshall/skills/dev-agent-behavior-rules/standards/agent-behavior-rules.md[`agent-behavior-rules.md`] — the hard-rules file loaded at every dispatch.
...
* link:../../marketplace/bundles/plan-marshall/skills/plan-marshall/references/phase-handshake.md[`phase-handshake.md`] — handshake script contract, the 14-row invariant registry, the five exception classes (`MainCheckoutDirtiedDuringPlan`, `WorktreeMetadataDrift`, `PhaseStepsIncomplete`, `TaskGraphInvalid`, `BlockingFindingsPresent`), the drift envelope, and the override path.

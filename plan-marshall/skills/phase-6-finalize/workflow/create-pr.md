...
- `status: error` → the read did **not** establish that no PR exists. Branch on `error_cause`, the machine-readable discriminator the `pr view` error envelope carries; a bare `status: error` is NOT a no-PR signal, because that one envelope collapses three materially different causes and its human `error` message is hard-coded to the no-PR wording on the provider-call arm:
  ...
  - **`error_cause: auth_failed` / `provider_call_failed` / `malformed_response`** → the question *does a PR exist?* is **UNANSWERED**, which is not the same as answered "no". STOP and return an error TOON per the middle clause, preserving the stdout error envelope as emitted — **every field it carries**, copied whole rather than against a fixed field list. Where the cause is stated differs per arm, which is why the envelope is copied whole rather than mined for one field: on `provider_call_failed` and `malformed_response` the `error` field is the hard-coded no-PR wording and the actual failure is stated in `context` (the provider's stderr, and the unparsable stdout slice, respectively), while on the pre-provider `auth_failed` arm `error` carries the **provider-specific** auth failure reason and `context` is **ABSENT** — that arm returns before the provider call, so it has no provider output to report. Copying the envelope whole is what makes the instruction true on all three arms; enumerating fields would name a `context` the auth arm does not carry and would report the auth arm's real cause as the no-PR wording. See [`tools-integration-ci/standards/api-contract.md`](../../tools-integration-ci/standards/api-contract.md) § "`pr view` and the `error_cause` discriminator" for the per-arm field sets. Do NOT create: a transient failure on a branch that already has an open PR is exactly how this step opens a **duplicate** PR, and that failure and a genuine absence previously shared one envelope.
...
Every other shape — a non-zero exit, an exit-0 `status: error`, or a `status: success` whose `pr_number` is absent or empty — STOPS this step with an error TOON and returns it to the orchestrator. On the exit-0 `status: error` shape that error TOON carries the stdout error envelope verbatim — every field it carries, not a fixed subset — per the § "Exit-code convention for every script call" middle clause. Stderr is typically empty there, so the envelope is the only account of the cause, and which diagnostic fields accompany `status` and `error` depends on which arm failed (`operation` and `context` on a provider-call failure, `message` / `plan_id` / `plan_dir` plus `hint` and `hint_caveat` when the `--plan-id` did not resolve). Such a shape MUST NOT reach `Log PR creation` and MUST NOT reach `Mark Step Complete`: there is no `--outcome done` branch below that is reachable with an absent `pr_number`. The reason this requirement is explicit rather than implied by the exit-code convention is that `ci_base.output_error` prints `status: error` and still returns exit 0, so an exit-code-only reading would mark the step `done` and record a `pr_number` fact for a PR that does not exist — after which review and merge address a PR number nothing created.

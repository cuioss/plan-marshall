...
- User review gates must be respected. The forward-direction transition gates (`plan_without_asking`, `execute_without_asking`) gate phase transitions (3→4, 4→5); the two auto-continuation knobs `finalize_without_asking` and `loop_back_without_asking` are flat knobs under `plan.phase-6-finalize` (read at runtime via `manage-config plan phase-6-finalize get --field <knob>`). `plan.phase-6-finalize.finalize_without_asking` gates the forward 5→6 transition; `plan.phase-6-finalize.loop_back_without_asking` is the symmetric **reverse-direction** gate that controls whether a phase-6-finalize step's `outcome: loop_back` continues inline (capped by `phase-6-finalize.max_iterations`, default 3) or halts and prompts the user. The knob gates both granularity tiers on the same terms; which continuation runs is decided by the step's recorded `loop_back_target` — `5-execute` re-dispatches the execute pipeline against the freshly-allocated fix tasks, while `6-finalize` replays the loop-back-marked finalize step in place and enters no execute pipeline at all. Both default to `true`, so the phase-5-execute ↔ phase-6-finalize cycle runs unattended in both directions and `max_iterations` is what terminates it; set either to `false` to be asked at that boundary. See [`manage-config/SKILL.md`](../../manage-config/SKILL.md) § "Phase-Local gate_mode Gates and Automation Knobs" for the automation-knob semantics and [`phase-6-finalize/SKILL.md`](../../phase-6-finalize/SKILL.md) Step 3 § "Loop-back continuation hook" for the dispatch shape.

...
"""Tests for the pm-dev-java-cui:search-markers domain-owned marker detector.
...
_search_markers = load_script_module('pm-dev-java-cui', 'search-markers', 'search_markers.py')

search_openrewrite_markers = _search_markers.search_openrewrite_markers
cmd_search = _search_markers.cmd_search
extract_recipe_name = _search_markers.extract_recipe_name
MARKER_PATTERN = _search_markers.MARKER_PATTERN
DEFAULT_SKIP_PATTERNS = _search_markers.DEFAULT_SKIP_PATTERNS
AUTO_SUPPRESS_RECIPES = _search_markers.AUTO_SUPPRESS_RECIPES
...
def _read_fixture_marker() -> str:
    ...
    Scans with a plain delimiter walk rather than with MARKER_PATTERN, so the
    expectations stay independent of the module under test.
    """
...
class TestMarkerPattern:
    ...
    def test_no_match_on_plain_todo(self):
        assert list(MARKER_PATTERN.finditer('// TODO: normal todo comment')) == []

    def test_no_match_on_empty_line(self):
        assert list(MARKER_PATTERN.finditer('')) == []
...
class TestSearchJavaFiles:
    ...
    def test_marker_details_are_correct(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (Path(tmp) / 'Foo.java').write_text(f'package com.example;\n{raw}\n')
            result = search_openrewrite_markers(tmp)
...
class TestSearchSkipPatterns:
    ...
    def test_skips_build_directory(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            assert search_openrewrite_markers(tmp)['data']['total_markers'] == 0
    ...
    def test_custom_skip_patterns(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            assert search_openrewrite_markers(tmp)['data']['total_markers'] == 1
            # Custom pattern skips 'generated'
            assert search_openrewrite_markers(tmp, skip_patterns=('generated',))['data']['total_markers'] == 0
...
class TestRecipeCategorization:
    ...
    def test_auto_suppress_recipe_categorized(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            marker = search_openrewrite_markers(tmp)['data']['markers'][0]
...
class TestRecipeSummary:
    ...
    def test_recipe_summary_counts(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary = search_openrewrite_markers(tmp)['data']['recipe_summary']
            assert summary['RecipeAlpha'] == 2
            assert summary['RecipeBeta'] == 1

    def test_empty_recipe_summary(self):
        with tempfile.TemporaryDirectory() as tmp:
            assert search_openrewrite_markers(tmp)['data']['recipe_summary'] == {}
...
class TestSearchExitCode:
    """Tests for the cmd_search gate exit-code contract.
    ...
    def _run(source_dir: str) -> int:
        args = SimpleNamespace(source_dir=source_dir, extensions='.java', skip_patterns=None, format='json')
        return int(cmd_search(args))

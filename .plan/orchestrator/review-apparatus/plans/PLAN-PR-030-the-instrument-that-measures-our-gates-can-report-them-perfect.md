# PLAN-PR-030: The instruments that measure our gates report only what they measured
...
> shared target surface; D1–D6 are carried there as D1–D6 and D0 folds into that plan's merged D0
...
> Staged plan spec, ingested 2026-08-23 from the cloud-lane corpus (``PLAN-PR-030` (this directory)`,
...
Make the instruments that measure our own gates report only what they measured.
...
Most load-bearing claims reproduce at HEAD, including the blocker (`structural_share: 100.0` at
`1/1` on a shrunk roster) and every asserted absence. Three claims carry stale detail and two
deliverables target the wrong artifact.

⛔ **Correct before handing this spec to a run:**

1. **D4/G12 NAMES THE WRONG FILE.** The dispatched-envelope schema is in
   `pre-submission-self-review.md` § "Dispatched-envelope output" (`:330-336`), which has no
   `structural_limit` field. `ext-point-self-review-surfacing.md` § Output Schema is the SURFACER's
   schema and **already carries the field** (`:64`, `:65`, `:80`). A run following the text literally
   adds a duplicate field to the wrong schema and leaves the gap open.
2. **D4/G13's real surface is ~25 files; four are declared.** The marker goes on every Stage-1
   candidate: 19 bundled step docs + 6 project-local. `extension-api/standards/ext-point-finalize-step.md`
   § "Implementor Frontmatter" is the documented source of truth for step frontmatter and would need a
   row; it is mentioned nowhere in the spec. The plan's own collateral-change check would fire on ~22
   files.
3. **D6 edits another plan's directory with no disagreement report.** PLAN-PR-031 anticipates the same
   situation and reports it; this plan is silent.
4. **Stale detail:** "four sites restate the guarantee absolutely" — three state it, the fourth only
   cross-references. `resolve_bot_kind` is three defs above `assess_delta`, not two. `cmd_reconcile`
   and `cmd_assess` do not guard the identical expression.
5. **Fold in, from the ingestion audit:** `130 G4`'s sixth site — `130-…/report-01.md` § D2's
   selection-effect restatement — fell between this plan and PLAN-PR-031 silently.

⚠ **Scope: 6 deliverables at the guard, but the real measure is surface — three unrelated instruments
across four bundles, two project-local skills and five test suites. SPLIT INTO THREE:**
(a) the delta — D1+D2+D3; (b) the gate verdicts — D4, which alone justifies a plan because its G13 arm
adds a frontmatter field to every registered finalize step, a marketplace-wide contract change;
(c) the count-prose detector — D5 + D6's `090` corrections.

## Dependencies and Sequencing

- **MUST NOT run concurrently with PLAN-PR-024, PLAN-PR-025 or PLAN-PR-026.** The § Consumers
  contention in `bot-participation-contract.md` is **three-way, not two-way**: PLAN-PR-024 and
  PLAN-PR-025 each ADD a row to the table body; this plan REWRITES the `review_gate_delta assess` row
  (`:663`) inside it. Two insertions plus one in-place edit in one markdown table conflict on the same
  hunk. PLAN-PR-026 additionally shares `finalize-step-review-retrospective/SKILL.md` with it, a file
  no split governs.
- **Run BEFORE PLAN-PR-031** — both edit `090-…/report-01.md` (different sections, one file), and this
  plan's contract fixes should land before that plan appends to the same document.
- ✅ If split as recommended, **(b) and (c) are free of all of this** and can run immediately,
  concurrently with anything. Only (a) carries the contention.
- Overlaps with: PLAN-PR-029 (`gitlab_pr.py`), PLAN-PR-027 (`automatic-review/SKILL.md`),
  PLAN-PR-028 (`phase-6-finalize/SKILL.md`).
...
## ⛔⛔ FOLDED IN 2026-08-27 (PR #1356) — a VACUOUS GREEN inside this epic's own instrument

From inbox `-001` and `-006`. **This is the plan's thesis, observed in the instrument itself.**

Three consecutive self-review rounds each found **one** member of the same class and reported
`cohort_size: 1`. That number is **a property of the surfacer's candidate list published as a property
of the tree** — the exact substitution this plan exists to stop. One directed class sweep then found
**four**, including `verification-feedback.md`'s pr-state producer collapsing every `ci pr view`
non-success into *"no PR exists"* and returning a clean *"nothing to triage"*.

⇒ **Deliverable:** derive `cohort_size` from a **class-closure sweep**, never from the candidate list,
and make the published number carry the population it was computed over. A `cohort_size` the surfacer
cannot see past is a floor, and must render as one.

⭐ **Companion measurement from the same PR (`-006`): 12 of 19 bot findings were ONE archetype the
in-run self-review had already passed.** So the miss is not a coverage accident — the surfacer's blind
spot is systematic and an external reviewer walked straight into it. That is the strongest available
argument that this plan's fix is worth its cost, and it is first-party.

## ⭐⭐ FOLDED IN 2026-08-27 — the self-review loop's three invisible bounds (6 lessons)

From inbox `lessons-handling-26-08-26-01-001.md`. Its suggested fold target was `PLAN-PR-018`, which
is **retired**, so the cluster lands here: this plan already owns `pre-submission-self-review.md` and
`ext-self-review-plan-marshall`. Every lesson named is already in the global corpus — this fold turns
them into scope, and allocates no new corpus entry.

**The failure mode: the loop's termination predicate, its candidate coverage, and its finding
persistence each bound what it can find, and each bound is invisible from its output.**

| Lesson | Bound |
|---|---|
| `2026-08-25-15-001` | ⭐ **Termination on "found nothing new" cannot distinguish convergence from oscillation.** A self-seeding loop never produces an empty round — every round's fix authors the next round's finding — so the predicate can only terminate by exhausting the ceiling. Observed 4/4/2 over 3 rounds with **6 of 10 findings authored by the previous round's own fix**; stopped by a human, after the ceiling was raised 3→8 |
| `2026-08-08-19-006` | The delta round HAS shipped (round N+1 reads the previous round's `head_at_completion`); the **intra-round** re-scan has not, so a fix's residue still costs a round. Second residual: a delta round's class-closure sweep reaches only the delta surface |
| `2026-08-08-19-005` | A finding raised **at** the ceiling has no iteration left to review its fix. ⛔ Ceiling-iteration findings are systematically less likely to be fixed, and **that bias is invisible in the "N iterations, M findings, all resolved" summary** — which is this plan's exact thesis |
| `2026-08-23-22-001` | The surfacer emits `user_facing_strings` with `context: docstring` only — **assertion messages and comments are in no sub-list at all**. ⛔ Consequence: a delta whose entire content is uncovered prose yields a candidate set **identical to the previous round's** (round 6's surface was byte-identical to round 5's) |
| `2026-08-24-20-001` | Findings returned but not persisted to `qgate-6-finalize.jsonl`, so the store under-reports what the step found |
| `2026-08-26-05-006` | Seed the candidate surface from the plan's **own target defect class** — two instances in one run of a plan reproducing the class it was chartered to close, both found only by a general 61-candidate sweep |

⛔⛔ **The one remedy the evidence predicts will FAIL — carried verbatim from the source:** *"Do NOT
convert this into a checklist item for authors."* The stated mechanism is that **writing the argument
for a defect class is what makes you blind to committing it** — the author holds the rule as stated
rather than as applied, and the sharpest instance sits closest to the prose that forbids it, which is
exactly where a reviewer stops looking. An instruction to "remember your own rule" is the remedy the
evidence rules out. ⭐ This epic has now seen that shape four times (PLAN-PR-034's `classify_bot`, and
twice inside one run above), so treat it as established, not as a hypothesis.

⚠ **Scope:** six lessons is itself past this plan's comfortable size. Evaluate a split at outline —
termination/ceiling (the first three) versus candidate coverage (the last three) is the natural cut.

## ⭐ FOLDED 2026-09-13 — `review_commitments` reports clear over an EMPTY population (`5cbc17`)

Transferred from `truthful-signals` (`-055`), first-party on PLAN-TRUTH-103 / PR #1475. The
vacuous-guard archetype on the review-commitments path: **a clean verdict that can be produced by
having nothing to check**, with no population published beside it.

⛔ This is D4's rule (*every gate verdict distinguishes checked, degraded, not-reached and
never-performed*) meeting a NAMED instrument — `review_commitments.py`, already on this spec's declared
surface — so it is absorbed into D4 rather than added as a deliverable. **`clear` over zero
commitments must be `not-reached`, never `clear`**, and the verdict must carry the population it was
computed over. ⚠ The archetype's own recurrence note applies: this epic has repeatedly introduced a
vacuous guard *inside the fix for one*, so D4's control must fail when the population is empty.
...
this runs — `../cloud-runs/130-review-bots-catch-what-in-house-gates-cannot/gaps.md` and
`../cloud-runs/090-feed-pr-findings-back-into-local-review/gaps.md`, with the supporting
...
6. **D6 — Record the open measurement-semantics proposals, and correct two records that restate rather
   ...
   - **Report correction — the detector-registry enumeration (090/G5).** The 090 plan's claim-labels
     ...
     check to `../cloud-runs/090-feed-pr-findings-back-into-local-review/report-01.md`
...
- `marketplace/bundles/plan-marshall/skills/phase-6-finalize/standards/pre-push-quality-gate.md`,
  `marketplace/bundles/plan-marshall/skills/phase-6-finalize/standards/ci-verify.md`, `marketplace/bundles/plan-marshall/skills/phase-6-finalize/workflow/sonar-roundtrip.md`,
...
- `marketplace/bundles/pm-plugin-development/skills/ext-self-review-plan-marshall/scripts/_self_review_patterns.py`
  and `marketplace/bundles/pm-plugin-development/skills/ext-self-review-plan-marshall/SKILL.md` — D5's reach-axis record and `## Tests` index row.
...
- `test/plan-marshall/automatic-review/test_review_gate_delta.py`,
  `test/plan-marshall/automatic-review/test_counting_rule_parity.py`, `test/plan-marshall/automatic-review/test_bot_participation_contract.py`,
...
## ⭐⭐ FOLDED IN 2026-08-31 — TWO new self-review blind spots, both observed live on PR #1368

Drained from PLAN-PR-025A's landing. Both are candidate classes for the surfacer, and both are
cases where **the run produced its own counter-evidence and the self-review pass did not read it.**

**1. A guard's own diagnostic, fired during the run that is changing that guard.** The
`refusal_pattern_drift` guard PLAN-PR-025A introduced emitted a live FALSE POSITIVE into that
same run's logs (`refusal_pattern_drift[1]: sourcery,registry_refusal_patterns`) — its predicate
was bidirectional (`len(_layers) == 1`) for a unidirectional condition (structural ∧ ¬registry).
Ten rounds of self-review passed over it; **CodeRabbit found it** (inline `86d508`), and it was
fixed in-run by TASK-014.
⇒ **New candidate class:** when a diagnostic fires during a run modifying that diagnostic's
subject area, surface the emission as a first-class candidate. And for every emitted diagnostic,
state its scenario in one sentence and confirm the predicate is not broader than it.

**2. Diff-derived candidates cannot see a doc line the change made stale WITHOUT touching it.**
Convergence took **eight rounds** because the surfacer's candidate set is derived from the diff,
so a passage rendered false by a change elsewhere is structurally invisible to it. ⛔ This is the
**population** limb of this spec's own thesis: a surfacer that can only see what the diff touched
reports a property of its candidate list, not of the tree — the same `cohort_size 1` failure this
spec already carries, in a second form.

⚠ Both were recorded by the plan itself, not re-derived by this orchestrator. The `86d508`
attribution and the eight-round count are the run's own report; corroborate at PR #1368 before
pricing either.

### ⭐⭐ A THIRD instance, and it is the MEASURED version of this spec's thesis (folded 2026-08-31)

Transferred from `truthful-signals-041`. ⚠ **RELAYED, not re-derived here.**

**Five internal self-review passes reported the diff clean. An external reviewer then found 31.3%.**
(`findings-read-absent-plan-dir-returns-clean-zero-001`, `phase-6-finalize`.)

⛔ **This is not another anecdote — it is the same claim with a denominator.** This spec argues the
instrument that measures our gates can report them perfect; here it reported perfect five times over
a diff carrying nearly a third more findings than it saw. Together with the two instances above
(a guard's own false positive surviving ten rounds; diff-derived candidates blind to untouched-but-
stale lines) that is **three independent shapes of the same blind spot**, so the defect is
systematic rather than incidental.

⭐ Corroborated in the same window by the wait-vs-override pair folded onto PLAN-PR-026: the plan
that WAITED recovered 5 findings its 5 internal passes had missed, one of them in the code it had
just added. **Two independent measurements, same direction.**

## Claim Labels

- OBSERVED (added 2026-08-31, RELAYED from `truthful-signals-041`, NOT re-derived): five internal
  self-review passes reported clean; an external reviewer subsequently found 31.3%. Confirm/refute
  at that plan's run report and PR.
...
1. **Build gate.** The diff touches `*.py`, so the Python gate applies. Resolve the command via
   ...
   Read the returned TOON `status` / `errors[]`, not the wrapper exit code.

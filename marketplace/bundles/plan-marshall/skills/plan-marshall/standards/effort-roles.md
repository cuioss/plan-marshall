...
The `orchestrator` role group is the epic-orchestration counterpart of the five phase groups. Unlike them it does NOT resolve under `plan.<phase>` — it resolves under the top-level `orchestrator.effort` block (a sibling of `plan`; see [`extension-api/standards/marshal-json-reference.md` § Orchestrator Configuration](../../extension-api/standards/marshal-json-reference.md)). The group covers the three dispatch surfaces the `plan-orchestrator` fires — `analyze` (landing ground-truth corroboration plus that verb's write-bound drafting dispatches), `decompose` (context research and candidate mapping plus its spec-body draft), and `reader` (the `analyze` untrusted-text ingestion) — each addressed as `orchestrator.{surface}`. The complete index of the dispatches these surfaces back is [`orchestration-model.md` § Dispatch Decision Rule](../../persona-plan-orchestrator/standards/orchestration-model.md#dispatch-decision-rule), Effort-dimension bullet.
...
| Group | Surfaces (sub-keys) | Resolution root |
|-------|---------------------|-----------------|
| `orchestrator` | `analyze`, `decompose`, `reader` (+ `default` slot, `max` ceiling) | `orchestrator.effort` (sibling of `plan`, NOT `plan.<phase>`) |

Per-surface resolution order (surface ∈ {`analyze`, `decompose`, `reader`}):
...
The `max` key is a ceiling that clamps the resolved surface level on the ordinal ladder — it does NOT replace resolution; an unset `max` is a no-op. With `orchestrator.effort` unset everywhere, every surface falls through to `plan.effort` (then `inherit`) — identical to today's `effort resolve-target --default` behaviour.

**This project pins all three surfaces explicitly** — `analyze` and `decompose` at `level-5`, `reader` at `level-3` — each written through `manage-config effort set --scope orchestrator.{surface}`. That is this repository's own configuration, not a new default: the resolution order and the `max` clamp above remain the general contract, and a project that pins nothing still falls through to `plan.effort` exactly as described. The consequence of pinning every surface is that **no orchestrator dispatch level is decided by `plan.effort`** — moving the plan-wide fallback no longer moves the orchestrator tier. `effort read` reports which of the two is in force through its `source` field: `orchestrator.effort.{surface}` for a pinned surface, `plan.effort` for an unpinned one. No `max` is written, because nothing needs clamping once every surface is explicit.

Dispatch sites resolve via the two-flag or dotted `--role` form — e.g. the dotted sub-key `--role orchestrator.analyze` on a `manage-config effort resolve-target` call. That role fragment is deliberately NOT written here as a whole invocation: a full-looking command line missing `--workflow` is a copy source for exactly the trail-less dispatch the flag exists to prevent. ⛔ **A resolve that BACKS a real dispatch also carries `--plan-id`, `--caller` and `--workflow`** — `--workflow` is the flag the resolve seam keys its `[DISPATCH]` line and paired decision-log record on, so the abbreviated form above is a level QUERY and leaves no dispatch trail. The canonical dispatch form is [`orchestration-model.md` § Dispatch Decision Rule](../../persona-plan-orchestrator/standards/orchestration-model.md#dispatch-decision-rule). The **`reader` surface** is special: it resolves a LEVEL like the others, but the dispatch site composes the read-only `execution-context-reader-{level}` variant from that resolved level (untrusted-text ingestion runs under the reader variant), so the reader site reads the level via `manage-config effort read --role orchestrator.reader` and composes the variant name itself rather than calling `resolve-target`.
...
| orchestrator analyze dispatches (landing corroboration; landing-report, disposition and spec-body drafts) | plan-orchestrator (analyze) | `orchestrator.effort.analyze` → `orchestrator.effort.default` → `plan.effort` → `inherit`, then `max` clamp |
| orchestrator decompose dispatches (context research and candidate mapping; spec-body draft) | plan-orchestrator (decompose) | `orchestrator.effort.decompose` → `orchestrator.effort.default` → `plan.effort` → `inherit`, then `max` clamp |

...
**There is ONE sanctioned main-anchored resolver utility, used by a bounded set of five cross-session consumers.** `resolve_main_anchored_path` in `script-shared/marketplace_paths.py` is the single deliberate exception to cwd-relative resolution: it resolves a subpath under the MAIN checkout's `.plan/local` regardless of caller cwd (test override first, then `git rev-parse --git-common-dir`). It is consumed by exactly five genuinely-shared cross-session corpora, each reached through that one utility:
...
## The machine-global home root — a distinct third anchor tier

The five consumers above are all **per-repo** cross-session state: they are shared across the *worktrees of one repository* and resolve to *that repository's* main checkout. A separate, distinct tier exists for **machine-wide** state shared across *every checkout on the host*: the machine-global home root, `home_root()` in `script-shared/marketplace_paths.py` (`~/.plan-marshall` by default, overridable via the `PLAN_MARSHALL_HOME` env var). It does not depend on git resolution and is not repo-scoped.

Its bounded membership is: the credential store (`~/.plan-marshall/credentials/`) and the build-slot queue (`~/.plan-marshall/build-queue.json`), with a slot reserved for a future `marshalld` orchestration daemon. **`build-queue.json` moved here from the per-repo main-anchored set** because build-session coordination spans every checkout on the host, not just one repository — two sessions in *different* repos must contend for the same slot budget, which a per-repo main-anchored path cannot express. Because the queue now records holders from multiple checkouts, each entry is stamped with the originating checkout's `project_root` (via the public `main_checkout_root()`) so its liveness is judged against the repo it came from.

This tier is distinct from BOTH `resolve_main_anchored_path` (the per-repo ADR-002 exception, above) AND `get_store_dir` in `tools-file-ops/file_ops.py` (the parameterized store-root mechanism whose `plans` / `orchestrator` stores are both per-repo). Per-repo state (run-config, lessons, merge state, plans) deliberately stays per-repo; only genuinely host-wide state uses the home-root tier. The full rationale is recorded in ADR-008.
...
- **Stays main-shared (never moves)**: the genuinely-shared per-repo cross-session corpora — `.plan/local/merge.lock`, `.plan/local/run-configuration.json`, `.plan/local/lessons-learned`, `.plan/local/merge-queue.json`, `.plan/local/orchestrator/`, and `.plan/local/plans/NO_PLAN/build-results/` — each resolved to the main checkout via the single `resolve_main_anchored_path` utility regardless of any worktree's pinned cwd. Serialising concurrent finalizes and merges, accumulating adaptive timeouts, recording lessons, tracking epic orchestration, and collecting the output of builds that belong to no plan are all inherently cross-session concerns. (Only the `NO_PLAN` sentinel's build results are main-anchored; a REAL plan's `build-results/` sits inside its plan directory and therefore moves with it.) `archived-plans/` is also main-resident but is touched only at finalize after cwd returns to main, so the uniform cwd rule suffices — it needs no resolver. (The build-slot queue is NO longer here: it moved to the machine-global home-root tier at `~/.plan-marshall/build-queue.json` because it coordinates across every checkout on the host, not just one repository — see "The machine-global home root" above.)
...
- **Cross-session coordination is main-scoped (per-repo)**: the shared per-repo corpora (merge lock, run-config, lessons, merge queue, orchestrator store, plan-less build results) are the deliberate exception, reached through the ONE `resolve_main_anchored_path` utility; they always resolve to main because per-repo cross-session coordination is inherently a main-checkout concern. Machine-wide state (credentials, the build-slot queue) instead uses the home-root tier. Keeping each to one small, explicitly-enumerated mechanism prevents the codebase from regrowing pervasive ad-hoc git-common-dir resolution.

## Assertion

`file_ops.get_base_dir()` MUST resolve via the single uniform cwd walk-up (`set_base_dir()` → `PLAN_BASE_DIR` → nearest ancestor containing `.plan/local`) and MUST NOT reintroduce a sideways main-anchored resolver for PLAN-SCOPED state. The ONE sanctioned main-anchored resolver is `resolve_main_anchored_path`, used by exactly six per-repo cross-session consumers (`merge.lock`, `run-configuration.json`, `lessons-learned`, `merge-queue.json`, `orchestrator`, `plans/NO_PLAN/build-results`); it is the explicitly-enumerated exception, not a general escape hatch. New **per-repo** cross-session shared state MUST route through that utility rather than copying git-common-dir resolution; new **machine-wide** (cross-repo) state MUST instead be anchored under the machine-global home-root tier (`home_root()`), NOT `resolve_main_anchored_path` — routing host-wide state through the per-repo utility would wrongly bind it to one repository's checkout. Any change that regresses the uniform cwd rule for plan-scoped state, or regrows a pervasive main-anchored resolver outside the one utility, breaks the boundary invariant for every plan-scoped script and must be rejected in review.

...
**Verifying the markdown-components row.** The row fires wherever a deliverable touches markdown components, and consumer projects do carry them — under the target's project-local skill and agent roots (`.claude/skills/` and `.claude/agents/` on Claude; resolved via the platform-runtime `layout skill-roots` op). What verifies them differs, and the row names no single answer on purpose:

...
def test_full_sweep_emits_exploration_share_block(tmp_path):
    ...
    assert f'check: exploration-share\nstatus: success\nfixed_since: {audit.CHECK_ERA["exploration-share"]}' in output
    assert output.index('check: exploration-share') < output.index('check: cross-check-synthesis')
...
def test_surgical_overpay_coupling_fires_on_miss_and_big_spend():
    ...
    all_results = {
        'lane-lever-effectiveness': {'rows': [{'plan_id': 'p-over', 'flags': 'checkpoint_over'}]},
        'token-economics': {'rows': [{'plan_id': 'p-over', 'flags': ['big_spend_tiny_footprint(2Mtok)']}]},
    }
...
def test_surgical_overpay_coupling_unfired_when_facets_disjoint():
    ...
    all_results = {
        'lane-lever-effectiveness': {'rows': [{'plan_id': 'p-over', 'flags': 'checkpoint_over'}]},
        'token-economics': {'rows': [{'plan_id': 'other', 'flags': ['big_spend_tiny_footprint(2Mtok)']}]},
    }
    result = audit.cross_check_synthesis(all_results)
    by = {r['coupling']: r for r in result['rows']}

...
- transcript-less target policy covering Antigravity
...
accounting distinguishes concurrent plans' writes and reclaims a stale merge
mutex. Daemon, generator, and capture-footprint failure modes carry diagnosable
...
- merge-mutex reclamation past hold budget
...
Finalize steps expose a verdict_inputs surface so currency compares instead of
invalidating, and no phase-5 path reads an absent verdict as passing. The four
dispatch context-load flags are passed at every call site. Closure claims carry
checkable arithmetic, and loop-back overrides re-arm the machinery they bypass.
- verdict_inputs surface for step currency
...
The realized footprint is captured at branch-cleanup before the worktree is
removed, diffed against the upstream base with a recorded creation SHA. Twin
...
- realized-footprint capture at branch-cleanup
...
file. Handshake failures carry diagnostics, anchors resolve to real commits,
...
- logged decision on transcript-less branch
- session_id on transcript-less dispatch template
- handshake diagnostics on empty-stderr failure
- handshake tolerance for legitimately decreased counts
- commit-existence validation for completion anchors
...
Findings-bearing rounds stamp returned_with_findings, VERIFY emission is restored
...
carries a receipt, metrics rows re-close on loop-back, and CI infrastructure noise
classifies as noise rather than defect.
- returned_with_findings stamping on loop-back
...
- 5-execute metrics re-close on finalize loop-back

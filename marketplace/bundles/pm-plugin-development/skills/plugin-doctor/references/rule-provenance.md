...
| Rule ID | Class | Emitter | Source |
|---------|-------|---------|--------|
| `fail-closed-gate-read` | safety | `_analyze_fail_closed_gate_reads.py` | The fail-closed gate-verb hardening plan — a read-only gate/boundary verb (a `cmd_*` / `*-status` / `assert-*` / `verify` / `*-validate` / `check_*` / `load_*` handler that forms a verdict by reading a file) that reads with only a prior `.exists()` guard can still raise `OSError` (permission denied, the path resolving to a directory, a mid-read deletion race). An uncaught `OSError` crashes the verdict path — strictly worse than a structured error, since the caller cannot distinguish "the invariant could not be evaluated" from a hard process death and may silently advance past an unverified gate. The originating corrective fix (the `boundary-status` OSError wrap) already shipped; this rule generalizes it as forward enforcement. The read must be enclosed in a `try` catching `OSError`. Whitelist: the analyzer's own file and `tools-file-ops:file_ops.py` (the canonical fail-closed read). |
| `redundant-contract-typed-isinstance` | structural | `_analyze_fail_closed_gate_reads.py` | The fail-closed gate-verb hardening plan — the inverse anti-pattern of `fail-closed-gate-read`. An `isinstance(param, Cls)` guard on a parameter already annotated with that concrete contract type (e.g. `metadata: dict[str, Any]` then `isinstance(metadata, dict)`) is defensive theatre: the annotation is the contract, the guard can never be false in correct code, and it misleads the next reader. The companion redundant-isinstance removal already shipped at the corrective-fix site; this rule generalizes it. Only concrete builtin contract types are eligible; `Any` / union / `Optional` annotations and untrusted-input ingestion boundaries are out of scope. |

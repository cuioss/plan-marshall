...
handshakes[2]{phase,captured_at,override,override_reason,main_sha,main_dirty,main_dirty_files,main_dirty_exempted,worktree_sha,worktree_dirty,references_valid,task_state_hash,qgate_open_count,config_hash,unfinished_tasks_count,phase_steps_complete,pending_findings_by_type,pending_findings_blocking_count,pr_title_present}:
...
| `main_dirty_exempted` | always | the other half of the same partition — sorted list of dirty `.plan/` paths the exemption DROPPED as **untracked** plan state ([resolved main-anchored](#main-scoped-resolution)) | `'informational_only'` (never blocking, at any boundary) | a filtered zero in `main_dirty_files` that hides an exempted population — it distinguishes a genuinely clean main checkout from one whose every leak was exempted |
...
**`main_dirty_exempted` rationale**: this is the project's only `'informational_only'` invariant, and the classification is a **mandatory** registration rather than an optional relaxation. The column records the population `main_dirty_files` declined to count — untracked `.plan/` bookkeeping — which every capture rewrites by construction, so its value differs at essentially every boundary. Since `is_invariant_blocking_at_phase()` fails safe to `blocking_at_every_boundary` for an *unmapped* invariant, leaving the entry out would not leave the column neutral: it would make a by-construction-changing set refuse every transition. The column also stays out of `summarize-invariants._CORE_INVARIANTS`, so historical rows captured before it existed are not reported as a `missing invariant` error.
...
3. Add the column name to `HANDSHAKE_FIELDS` in `_handshake_store.py` — and, when the capture returns a **list**, to `HANDSHAKE_LIST_FIELDS` in that same module as well, or the column round-trips as a string and its baseline interpretation breaks
4. Add an `INVARIANT_BLOCKING_SCOPE` entry unless `blocking_at_every_boundary` is genuinely wanted — the map is consulted with that value as the fail-safe default, so an omitted entry is a *strict* registration, not a neutral one
5. Add a drift test case

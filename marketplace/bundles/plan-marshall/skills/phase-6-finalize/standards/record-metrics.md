...
Omit `--session-id` when the transcript-gated resolver dispatched finalize without one (transcript-less target, absent identity). `enrich` then takes the enrich-skip branch: it skips enrichment, stamps the population-carrying gap flag, and succeeds unenriched — the final report carries no transcript-sourced session tokens and the gap flag marks the total as unenriched, never measured. On a transcript-capable target `--session-id` stays required and an absent identity keeps the missing-identity error.

An **inline** 6-finalize step (one that runs in the orchestrator's own context rather than a dispatched subagent) produces no `<usage>` envelope and no accumulator contribution — there is no per-step `<usage>` source to read. Its cost is nonetheless captured by `enrich`'s phase-window attribution: `enrich` sums the parent-window `message.usage` (input/output/cache_read/cache_creation) into the `6-finalize` row. `enrich` records that inline contribution as a distinct `inline_main_context_tokens` field on ANY phase whose window carries a non-zero `input + output + cache_creation` sum — `cache_read` is excluded to match the dispatched-`<usage>` total definition, so a phase whose window carries cache reads alone gets no figure from `enrich` and is completed by `generate` as a measured `0` — and stamps `total_tokens_population` on the row to name which population `total_tokens` measures there. On the mixed signature (this phase's usual shape: dispatched steps AND inline finalize steps both ran) the dispatched `total_tokens` is left byte-identical (explicit-wins) and the row reads `mixed`. On a phase that dispatched nothing at all, the same inline sum is ALSO folded into `total_tokens` and the row reads `inline`. See `manage-metrics/standards/data-format.md` § "Inline Main-Context Attribution" for the three signatures, the field schema, and the consumer rule (`inline_main_context_tokens` and a dispatched `total_tokens` are different populations and are never summed). Inline step cost is therefore attributed via this phase-window path, NOT via a per-step `<usage>` (which does not exist for an inline step).
...
`generate` reconciles **all six canonical phases** explicitly rather than silently summing whichever boundary rows happen to be present: it folds each phase's durable `work/metrics-accumulator-{phase}.toon` into the phase row when no boundary closed it, and it stamps an `end_time`-presence check (`any_phase_missing_end_time` + `phases_missing_end_time`) computed over the canonical-six baseline. This is what lets an interrupted / loop-back / never-reached `6-finalize` close surface its accrued tokens and its gap instead of producing a complete-looking under-count.

**Read the check for what it is.** It asks one question per canonical phase — does the row carry an `end_time`? — and nothing more. It is not a completeness verdict and not an internal-consistency check: a phase absent from `phases_missing_end_time` carries its boundary marker, which says nothing about whether the figures on that row are complete or mutually consistent. The reconciliation behavior and the field schema are owned by `manage-metrics` — see `marketplace/bundles/plan-marshall/skills/manage-metrics/standards/data-format.md` § "`end_time` Presence Across the Canonical Phases" for the field schema, the predicate, what the check does NOT assert, and the floor-not-truth semantics; this step consumes the returned values, it does not re-derive them.
...
## Surface Missing end_time Boundary Markers

`generate` returns the `end_time`-presence check (see `## Generate Final Metrics Report` above). When `any_phase_missing_end_time` is `true`, at least one canonical phase was never closed by its boundary — the canonical case is a `6-finalize` whose terminal `record-metrics` close never folded its durable accumulator in (interrupt / loop-back / never-reached). Surface the gap as a `[WARN]` work-log entry naming those phases so the dropped attribution is visible in the audit trail instead of being silently archived:

```bash
python3 .plan/execute-script.py plan-marshall:manage-logging:manage-logging \
  work --plan-id {plan_id} --level WARNING --message "[WARN] (plan-marshall:phase-6-finalize:record-metrics) Phases missing an end_time boundary marker: {phases_missing_end_time}. Their totals were never closed in, so the aggregate is a floor."
```

Emit this line ONLY when the captured `any_phase_missing_end_time` is `true`. When it is `false` (every canonical phase carries the marker), skip it — there is no gap to surface. Surfacing the gap NEVER blocks archive: the `[WARN]` line is informational, and the step proceeds to `## Log Artifact` and on to `default:archive-plan` regardless.
...
An `any_phase_missing_end_time: true` result from a **successful** `generate` is NOT an error condition — it is the expected first-class signal that a canonical phase boundary was dropped, not a script failure. The gap is recorded as the `any_phase_missing_end_time` flag (persisted in `metrics.toon` and surfaced in this step's output contract) plus the `[WARN]` work-log line from `## Surface Missing end_time Boundary Markers`, never as a silent omission. The never-block-archive guarantee holds uniformly across all three outcomes: a non-success status logs `[ERROR]` and continues; a successful generate reporting a missing marker logs `[WARN]` and continues; a generate reporting every marker present logs neither. Archive always proceeds.

---
name: pm-dev-frontend-jest-testing
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/test-fundamentals.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/dom-component-testing.md" }` before continuing.
```
...
```text
Call the `view_file` tool with `{ AbsolutePath: "standards/mocking-async.md" }` before continuing.
```

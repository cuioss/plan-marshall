...
- **Rule 1 branch** (the pair is absent): place it **immediately after the existing table**. There is no `END` to position against at step-1 time, and step 2 then inserts `END` immediately below the last data row — between the table and the zone — so the zone ends up outside the pair.
...
**Rule 1 — insert the marker pair.** Applies **only when both hold**: `epic.md` carries a `## Ordered Queue` section, AND that section contains no `<!-- BEGIN GENERATED: ordered-queue -->` marker. When the pair is already present, Rule 1 does not apply at all. Its **two steps** run in the order below — and when the `### Queue annotations` zone is absent, the zone-creation instruction above runs **before** both of them, since step 1 needs a destination. Three actions in that case, not two:

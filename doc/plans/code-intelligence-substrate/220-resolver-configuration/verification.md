...
Re-derived against `a90adeb`, which is byte-identical to the audited `62e3807` across `marketplace/`,
`test/`, `.gitignore` and `doc/{user,concepts,adr}` — so no finding below is attributable to tree drift.

| # | Attack | What was found | Correction applied |
|---|---|---|---|
| A1 | False positives | Walked every gap's `path:line`. G1's sites (`_cmd_client_handlers.py:231-234`, `_cmd_client_query.py:966-1031`, `:1219-1222`, `:1238-1240`) are exact. G2 (`_cmd_client_render.py:97-101`, `:87`), G4 (`run-config-standard.md:807-859`), G5 (`report-01.md:157`), G7 (`:378-381`), G8 (`menu-configuration.md:89-108`, `:110-129`, `:24`), G9 (`extension_api.py:167` vs `run_config.py:872`, `menu-derivation-resolvers.md:51,60-61`), G10 (`_derivation_merge.py:105-108`) all check out. **Two stale citations:** G3's `client-api.md:105` is the *pre-fix* table location — the third-state row is at `:111` today (six lines, not the "±1–2" this document claimed); G7's `test_feasibility_underivable_guard.py:58` is `:56`. No gap was found not to exist. | G3 re-pointed to `:111` and given the full ADR-014 filename; G7 re-pointed to `:56` and its patch-target evidence enumerated (`test_graph_resolver_provenance.py:28,138,144,318,621`); the "±1–2 lines" claim in § Report accuracy rewritten to name which citations are exact and which one is not, and why it mattered. |
| A2 | False negatives | Read the shipped mechanism independently rather than the audit's reasoning: the gate (`_partition_configured_resolvers` → `_derive_edges` → `get_module_graph`, one caller each), all four `resolver_count` sites, `count_dispatched`, the roster, the footer, `cmd_derivation_resolver_set/get/list/remove`. Every fail-open branch resolves to *active*, in all three readers — confirmed, not merely re-read. **Two real misses.** (i) `test_derivation_resolver_configuration.py:249-269` **asserts the G1 defect as correct** (`assert edges['status'] == 'not_derivable'`) and its docstring states the generalisation G1 refutes; neither document mentioned it, and G1's *Risk if fixed* named the wrong tests. (ii) `cmd_path` (`_cmd_client_handlers.py:579-582`) and `cmd_impact` (`:638-641`) carry the same false parenthetical G3 targets — "there were no edges to walk" / "nothing could have depended on the module" — and were outside G3's site list, which its own *Done when* grep would not have caught. | G1's *Risk if fixed* rewritten to lead with the pinning test and to say it must be re-scoped, not deleted; § Test adequacy's "coverage hole" paragraph rewritten to name it. G3 widened from six sites to eight and its *Done when* extended to the two docstrings. |
| A3 | Vacuous evidence | Re-ran **both** claimed mutations from a fresh byte snapshot in `$TMPDIR/adv-220-mutsweep/`, restoring by `cp` (never `git checkout`/`restore`/`stash` — other agents hold unstaged work here) and verifying by `md5sum` + `git status --porcelain`. Mutation 1 (`DERIVATION_RESOLVER_ENABLED_DEFAULT: True → False`) → **8 failed, 11 passed**, first failure `test_unconfigured_project_still_derives_edges`. Mutation 2 (delete the per-entry `try/except` in `cmd_derivation_resolver_list`) → **1 failed, 21 passed**, `test_list_survives_a_raising_entry_read`. Both readings identical to the audit's. The D1 round-trip evidence is non-vacuous by construction: `test_round_trip_persists_to_the_machine_local_store` re-reads the file (`read_run_config(get_run_config_path())`) between write and assert. | Mutation paragraph annotated with the independent re-run; no substantive change needed. |
| A4 | Counts and quotes | Re-derived every number at the moment of checking. **Confirmed:** 20/14/6 `.py` files; 47/45/1 tracked `.claude` paths; 7 `DerivationResolverBase` subclasses and 8 `derivation_file_patterns` implementations (7 concrete + the ABC default); 14/22/19/14 per-file test counts; 50 passed and 19 passed; 900 passed for `manage-architecture + extension-api`; 1 failed / 47 passed for `test_argparse_surface.py` alone; the round-1 table's 14 rows against a stated 13, with rounds 2/3/4 at 11/10/10 all re-deriving. **Two count defects:** the `len(resolvers)` sweep claimed "the only remaining occurrences are the two roster `count` fields" — there are **four**, two of them documentation lines that negate the invariant; and the dispatch-site grep was presented as returning two hits when it returns 14, of which two are calls. **One quote defect:** both documents quoted the round-1 row as `3 self-caught`, dropping `while fixing`. **One inherited number:** G3's `:105` was copied from `report-01.md` R3-6 rather than re-derived (see A1). | Both sweep sentences rewritten with the real figures and the correct reading; the round-1 quote made verbatim in both documents; § Test adequacy rewritten to distinguish the three *added* files (55 tests) from the three-file run that gave 50, so the two figures no longer read as a contradiction. G4's "five omitted sections" was **checked and upheld** — `language_servers` / `derivation_resolvers` / `display_timezone` from the standard's own Schema block, plus `build` and `ci` from `manage-run-config/SKILL.md:47-58` — and the enumeration written into the entry. |
| A5 | Actionability | G1–G7, G9, G10 are executable by a run that has read neither the plan nor this audit. **G8 was not:** its *Done when* ("the stated option count matches the non-navigation options; no page carries a fifth element") is **already true today** — `:24` says 12, the four pages carry exactly 12 — so as written the entry asks a later run to satisfy a condition that needs no work, while its *Action* ("when the next Configuration entry lands…") is conditioned on an event that may never come. | G8 rewritten into a change that is executable now — record the cap and the four edits the next entry requires, in `menu-configuration.md` beside the Page 4 block — with the entry stating plainly that the restructuring itself must not be pre-built. |
| A6 | Severity and topic | Calibration re-applied to all ten. G1 high (shipped behaviour wrong, a consumer branches on it) — upheld. G2 low, G3 medium, G4 medium, G5 low, G7 low, G9 low, G10 low — upheld. Topics upheld: G1/G2/G10 `architecture-core` (the seam and its renderer), G3/G4 `documentation-surface`, G5 `documentation-surface` (report prose), G6/G7 `tests`, G8 `bundle-docs`, G9 `lsp/resolvers`. **One re-severity:** G6 was `low` on the calibration's "a stale claim confined to the run report" — but the identical rationale is in shipped source at `test_graph_family_bundle_project.py:353-357`, so it is "a false claim in shipped documentation", which is medium. | G6 raised low → medium, kind `report-defect` → `doc-defect`, and its *Where* / *Action* / *Done when* widened to both sites. |
| A7 | Coverage | All five deliverables carry a verdict; out-of-scope, collateral, report accuracy and the four declared residue items are each covered. Nothing in `plan.md` is silently unmentioned. The one coverage hole is inside a covered section, not a missing one (see A2). | None needed beyond A2's widening. |
| A8 | Internal consistency | The overall verdict follows from the rows. But **three `gaps.md` entries had no counterpart in this document** — G2 (footer wording), G9 (`configured` divergence) and G10 (`_derivation_merge.py` docstring) appear nowhere in § Correctness review, whose "One defect found" heading positively excluded them; G8's trace was a residue row that did not name it. The reverse direction is clean: every finding here that warrants action reaches `gaps.md`. | § Correctness review's heading changed to "One behavioural defect found, plus three smaller surviving inconsistencies" and G2/G9/G10 written up there with their evidence; the residue-1 row now names G8; the opening summary updated to state four surviving findings rather than one. |

**Residual doubt:** the largest unprobed surface is the one both rounds declared out of reach — the
menu as *executed by an agent*. `menu-derivation-resolvers.md` is a markdown workflow with no harness,
so "the menu lists a discovered resolver" (D1's *Done when*) is verified through the roster verb it
calls, not through the rendering an operator would see; a further round with an agent-in-the-loop
harness would most likely find wording in Steps 2–4 that the roster's field names no longer support —
G9 is one instance of exactly that class, found by reading rather than by executing. Second, the
report's `./pw verify` figures remain unverified by construction (full-suite runs are excluded), so a
build-gate misstatement would survive both rounds. Third, residue 3 stays *unconfirmed* rather than
disproved: four clean multi-directory invocations do not enumerate the combination space, and the
module-level `extension_discovery` binding that would cause it is still present in two legacy files.

**Verdict on the audit:** SOUND AFTER CORRECTION — its central finding (G1) reproduced, its mutation
evidence re-ran red at identical readings, and every gap it filed exists; what it missed was the test
that pins G1 as correct behaviour, three of its own findings never written into `verification.md`, two
overstated sweep counts, and one gap whose *Done when* was already satisfied.

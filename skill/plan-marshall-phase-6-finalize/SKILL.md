...
- **SETTLE (local HEAD-changing, `order < 11`)** — `finalize-step-sync-baseline` (3, rebase→HEAD), `pre-push-quality-gate` (5, gate), `project:finalize-step-plugin-doctor` / `pre-submission-self-review` / `finalize-step-simplify` / `finalize-step-security-audit` (9, fixes→HEAD), and `architecture-refresh` (10, derived-state — sorts LAST in the settle band so the descriptor refresh captures the code-mutating settle edits, including security-audit's hardening). Each mutating step's edits are committed on the feature branch by the dispatcher's commit instrumentation (Step 3 item 5f) before the barrier runs.

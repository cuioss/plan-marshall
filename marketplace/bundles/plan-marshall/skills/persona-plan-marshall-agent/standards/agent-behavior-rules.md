...
- **Content lookup across modules** — "Which FILES contain the string `foo`?" → `architecture search --content --pattern foo` (add `--literal` for a fixed string, `--ignore-case` to fold case — the two compose). Hits carry `module` / `category` / `path` / `match_count`; the response carries no line bodies, and `count: 0` always rides with the coverage fields so a negative is never bare. Read the coverage fields before believing a zero: a negative is trustworthy only under the canonical complete-coverage conjunction. An unreadable file or an elided inventory bucket is a file that might hold the match and was never scanned, which is a coverage gap to report, not an absence to record. See [`manage-architecture/standards/client-api.md`](../../manage-architecture/standards/client-api.md) § search → "Complete-coverage rule" for the field list and the full contract.
...
>
> **Architecture-first:** the flag is a string inside file bodies, so this is a CONTENT question — `architecture search --content --literal --pattern='--audit-plan-id'` returns the module-attributed files that actually contain it, each with a `match_count`, plus the coverage fields that make a zero result trustworthy when the complete-coverage rule holds. Scoped, deterministic, fast. (Had the question been "which PATHS look like `*audit-plan-id*`", `architecture find --pattern '*audit-plan-id*'` would be the verb — `find` never opens a file.)
>
...
**Recurrence signatures (self-audit checklist):** Before issuing any `manage-*` call, run the call against these five canonical argparse-rejection signatures observed across phase-6-finalize workflow steps and other plan phases. Each entry names the wrong shape, a concrete invented-vs-canonical example from the lesson catalogue, and the correct shape per the live argparse declaration:
...
2. **Top-level `--plan-id` / `--project-dir` where the flag is verb-scoped** — placing `--plan-id` or `--project-dir` immediately after the notation on `manage-architecture` / `manage-config`, where those flags are declared on the subcommand (or named `--audit-plan-id`), not at the top level. Example: `manage-architecture --plan-id X resolve …` → canonical `manage-architecture resolve --command quality-gate --audit-plan-id X`. Signature 4 is the mirror of this one and prescribes the OPPOSITE move on a different surface — read it before generalizing this rule to any script.
...
4. **Router-scoped `--plan-id` / `--project-dir` written AFTER the verb on the CI surface** — the **mirror** of signature 2, and the two prescribe OPPOSITE moves, so read both before acting. On `tools-integration-ci:ci` these two flags are *router*-scoped: `ci.py` consumes them before the provider parser is built, so they belong BEFORE the first positional. Written after the verb, argparse rejects them as `unrecognized arguments: --plan-id …` — a message that reads as "this flag does not exist" while the flag is right there in the wrong position. Example: `tools-integration-ci:ci pr view --pr-number 7 --plan-id X` → canonical `tools-integration-ci:ci --plan-id X pr view --pr-number 7`. Signature 2 moves the flag the other way, ahead-of-verb → after-verb, because on `manage-architecture` / `manage-config` the same flag NAMES are declared on the subcommand. The surface decides, not the flag name: check where the script declares it before moving anything. The rejection message names the fix on this surface — it prints the caller's own invocation with the flag moved.
5. **Missing required `--phase`, and `--resolution` vs `--status` confusion** — omitting the mandatory `--phase` on phase-scoped finding verbs, or substituting a plausible-but-wrong flag name. Examples: `manage-findings qgate list --plan-id X` → canonical `manage-findings qgate list --plan-id X --phase {phase}` (`--phase` is required); `--status resolved` → canonical `--resolution {value}` where the verb declares `--resolution`, not `--status`.

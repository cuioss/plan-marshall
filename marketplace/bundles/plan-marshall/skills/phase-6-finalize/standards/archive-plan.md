...
The gate reads two deterministic signals — the `foreign` column on `manage-solution-outline list-deliverables` intersected with the declares-a-change predicate (`declares_change` in `_plan_parsing.py`), which is the population it iterates, and the `ci pr landing-state` verb (the per-repository done-ness discriminator) — not artifact prose. A foreign path declared `read`, including a marker-less `Files to survey` bullet, names a file the deliverable only consults, so no pull request could ever clear it and it stays out of the population; a foreign path with no intent marker still enters it.

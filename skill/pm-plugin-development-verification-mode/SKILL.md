...
```text
Call the `skill` tool with `{ name: "pm-plugin-development-verification-mode" }` before continuing.
```
...
```text
Call the `skill` tool with `{ name: "pm-plugin-development-verification-mode" }` before continuing.
Call the `skill` tool with `{ name: "plan-marshall-phase-3-outline" }` before continuing.
```
...
```text
Call the `skill` tool with `{ name: "plan-marshall-plan-retrospective" }` before continuing.
```

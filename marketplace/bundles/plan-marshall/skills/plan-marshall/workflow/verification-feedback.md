...
**Idempotency is an explicit per-finding marker, not a property of terminality.** Terminality (`_RESPONDABLE_RESOLUTIONS`) is the RESPOND loop's *selection* criterion, not an exclusion criterion: a finding is respondable precisely *because* its resolution is terminal, so a terminal finding stays eligible on every pass and would re-transmit forever without a guard. The guard is therefore a `responded` marker stamped on each finding in the same unit of work that transmits its reply; a later pass skips any finding already carrying it. The **GitHub** (`github_pr`) and **Sonar** (`sonar`) respond verbs implement this marker, so a loop-back re-entry re-transmits **nothing** for a disposition already sent and their `count_responded` reports only the dispositions transmitted **that round**, never a standing re-count of every terminal finding. The **GitLab** (`gitlab_pr`) respond verb does not carry the marker, so its `count_responded` re-counts terminal findings on each pass. A disposition that genuinely CHANGED between rounds is transmitted again on the marker-carrying verbs: `manage-findings resolve` clears the marker whenever it changes a finding's resolution or reply body, so the guard is keyed on `(finding, disposition)`, never a blanket suppression.

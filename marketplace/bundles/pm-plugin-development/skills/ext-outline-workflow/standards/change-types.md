...
**Track-agnostic classification (applies to every change type below):** When building deliverables, classify each deliverable's `Affected files` against the [Human-Gated Config Classification](../SKILL.md#human-gated-config-classification) predicate. A deliverable that touches a human-gated settings/permission surface is human-gated and MUST split the unattended authoring from the human-gated activation step (or carry a `**Human-gated activation:**` note). This fires regardless of change type or track — do NOT restate the predicate or required action here; apply the SKILL.md section.

...
The other direction is the **9 rows this table marks `Conditional` or `Optional`** (`mutates_source`, `head_dependent`, `verdict_inputs`, `advances_main_via_rebase`, `records_facts`, `post_run_review`, `requires_prompt_fields`, `reads`, `destroys`). None is on the record, and none is an unread declaration, but the reader is not uniformly a script: most of the nine have a live Python consumer that reads the fact from the step doc's own frontmatter directly, via `extension_discovery._read_frontmatter_fields`. `advances_main_via_rebase` is the exception — its declared consumer is the phase-6-finalize dispatcher's own re-resolution procedure (`SKILL.md` § "Post-rebase step-doc re-resolution contract"), which the orchestrating agent follows directly rather than a script reading the field. The record deliberately carries the seeding/preset consumers' fields, not every declared key — see [Hook API](#hook-api).

...
OpenRewrite marker syntax that `_markers_search.py`'s `MARKER_PATTERN` detects.
Its marker text is checked in **verbatim and unmodified** from the upstream
recipe project's own test resources — it is never hand-written, and no assertion
in this repository's test suite may re-derive a marker literal from memory.

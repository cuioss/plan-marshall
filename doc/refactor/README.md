...
| 07 | Target extensibility (optimise for further targets) | Generalise the seams so target #3 is cheap: target-opaque `install-hook`, target-neutral ABC contracts, consolidated registration, the `targets:` frontmatter mechanism, and finishing data-driven body transforms (the registered-idiom class already ships as `mapping.json::body_idiom_rewrites` + a fail-closed shared applier; the `Skill:`-directive and `/slash` rewrites are still emitter code) | [07-target-extensibility.md](07-target-extensibility.md) |

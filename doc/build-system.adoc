...
2. Five **variants** (`execution-context-{low|medium|high|xhigh|xxhigh}.md`), each with `model:` and `effort:` baked into the frontmatter from the level → primitive mapping in [`model-levels.md`](../marketplace/bundles/plan-marshall/skills/plan-marshall/standards/model-levels.md).

`plugin.json` registers all six entries for the canonical so Claude Code's agent registry sees every variant.

After the refactor, the marketplace ships exactly one role-eligible canonical agent: `plan-marshall:execution-context`. Every plan-marshall `Task:` invocation dispatches one of its six variants. The workflow body the agent runs is supplied at dispatch time via the `workflow` field in the prompt body, per [`ext-point-execution-context-workflow`](../marketplace/bundles/plan-marshall/skills/extension-api/standards/ext-point-execution-context-workflow.md).
...
Each `Task:` dispatch carries a fixed overhead (skill-load preamble, Worktree Header echo, return-TOON marshalling). The granularity heuristics in [`dev-general-practices`](../marketplace/bundles/plan-marshall/skills/dev-general-practices) govern when a step belongs in a dispatch envelope vs. inline in the orchestrator's context:
...
* **Per-iteration only when parallel-or-different-models**: spinning up N parallel dispatches is justified when each subagent runs independently (the only such case in the post-refactor surface is `cross.manage-architecture-enrich-module`).

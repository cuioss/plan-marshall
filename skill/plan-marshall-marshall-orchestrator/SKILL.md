---
name: plan-marshall-marshall-orchestrator
...
```text
/plan-marshall-marshall-orchestrator                          # No verb — defaults to status
/plan-marshall-marshall-orchestrator init slug={slug}         # Scaffold a new epic
/plan-marshall-marshall-orchestrator decompose slug={slug}    # Decompose the epic into workstreams and plan specs
/plan-marshall-marshall-orchestrator status slug={slug}       # Report queue and plan states
/plan-marshall-marshall-orchestrator next slug={slug}         # Emit the next ready-to-run /plan-marshall-plan-marshall command
/plan-marshall-marshall-orchestrator analyze slug={slug}      # Analyze a landing or mid-flight observation; drains the epic inbox when invoked without a paste
/plan-marshall-marshall-orchestrator resume slug={slug}       # Re-anchor a fresh session from the persisted tree
/plan-marshall-marshall-orchestrator close slug={slug}        # Freeze the epic into history.md
/plan-marshall-marshall-orchestrator archive slug={slug}      # Relocate a closed epic to archived-orchestrators/
/plan-marshall-marshall-orchestrator lessons                  # Lessons-handling mode (dated-slug epic)
```
...
```text
Call the `skill` tool with `{ name: "plan-marshall-persona-marshall-orchestrator" }` before continuing.
```

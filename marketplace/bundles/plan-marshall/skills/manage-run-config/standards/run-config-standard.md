...
**Key Insight**: the host platform's Bash tool has a **default 120-second timeout** (Claude target; the default is resolved per active target through the platform-runtime `harness bash-timeout-ceiling` seam). Long-running builds need two timeout layers:

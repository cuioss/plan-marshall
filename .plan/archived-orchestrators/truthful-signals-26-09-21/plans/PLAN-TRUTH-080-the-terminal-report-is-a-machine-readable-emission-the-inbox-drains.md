> ⛔ **SHIPPED — this spec is a RECORD, not a brief.** Executed in the standalone cloud lane as
> `302-the-terminal-report-is-a-machine-readable-emission-the-inbox-drains` and landed. Do not emit it. The run artifacts — `report-01.md`,
> `verification.md`, `gaps.md` — are at `cloud-runs/302-the-terminal-report-is-a-machine-readable-emission-the-inbox-drains/`; the landing analysis is at
> `landings/PLAN-TRUTH-080.md`. Installed here at the 2026-08-22 ingestion so the queue↔spec
> bidirectional invariant holds over every row.
